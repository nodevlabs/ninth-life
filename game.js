...
(() => {
  ...
  const DEVOTION_MILESTONES = {
    Autumn: [
      ...
      { at: 80, name: "Ironwood", desc: "Injuries heal at start of each round", fx: { fastHeal: true } },
  ...
  const TRAIT_DETAIL = {
    ...
    Scrapper: "Tough and battle-hardened. Adds 3 bonus, or 5 bonus if they are hardened. Battle makes them stronger. Auto-heals injuries at the start of each round. Discard to gain +1 Morale.",
  ...
  const TRAITS = [
    ...
    { name: "Scrapper", icon: "\u{1F94A}", desc: "+3 bonus, +5 if scarred. Auto-heals injuries. Discard: +1 Morale", tier: "rare" },
    ...
    { name: "Nocturnal", icon: "\u{1F319}", desc: "+2 bonus per Morale level. Grows every round", tier: "legendary" },
  ...
  const POWER_COMBOS = [
    { name: "Twins", bonus: { c: 10, m: 1 }, standalone: { c: 12, m: 1 }, ex: "2 same Power", hidden: true, echo: "matched pair" },
    { name: "Two Pair", bonus: { c: 20, m: 2 }, standalone: { c: 18, m: 1 }, ex: "2+2 same Power", hidden: true, echo: "double matched" },
    { name: "Prowl", bonus: { c: 15, m: 1 }, standalone: { c: 15, m: 1 }, ex: "3 consecutive Power", hidden: true, echo: "in step" },
    // ~23%
    { name: "Triplets", bonus: { c: 50, m: 4 }, standalone: { c: 40, m: 2 }, ex: "3 same Power", hidden: true, echo: "equals" },
    // ~10%
    { name: "Full House", bonus: { c: 100, m: 7 }, standalone: { c: 70, m: 4 }, ex: "3+2 same Power", hidden: true, echo: "a perfect den" },
    { name: "Stalk", bonus: { c: 50, m: 3 }, standalone: { c: 45, m: 2 }, ex: "4 consecutive Power", hidden: true, echo: "rising" },
    // ~7%
    { name: "Mirrors", bonus: { c: 180, m: 12 }, standalone: { c: 120, m: 7 }, ex: "4 same Power", hidden: true, echo: "matched" },
    // ~0.6%
    { name: "Nine Lives", bonus: { c: 120, m: 8 }, standalone: { c: 90, m: 5 }, ex: "5 consecutive Power", hidden: true, echo: "the last hand" },
    // ~1%
    { name: "Quintuplets", bonus: { c: 350, m: 20 }, standalone: { c: 220, m: 12 }, ex: "5 same Power", hidden: true, echo: "impossible odds" }
    // near 0%
  ];
  const WILD_CARDS = [
    { id: "wc_moonfire", name: "Moonfire", icon: "\u{1F319}", desc: "Next play scores twice", cost: 8, flavor: "The second colony scored one hand so perfectly the dark paused. This is that hand.", type: "moonfire" },
    { id: "wc_straywind", name: "Stray Wind", icon: "\u{1F32C}\uFE0F", desc: "Next play: all cats count as same season", cost: 5, flavor: "The wind doesn't care what season you were born in.", type: "straywind" },
    { id: "wc_bloodpact", name: "Blood Pact", icon: "\u{1FA78}", desc: "Next play: all cats copy strongest trait", cost: 6, flavor: "One trait. Five bodies. The colony becomes a single thought.", type: "bloodpact" }
  ];
  const FAMS = [
  ...
  function getNerve(lvl) {
    if (lvl == null || isNaN(lvl) || lvl < 0) return NERVE[0];
    if (lvl <= NERVE_MAX) return NERVE[Math.floor(lvl)] || NERVE[NERVE_MAX];
  ...
  const SKILL_BRANCHES = {
    ...
    memory: { name: "THE MEMORY", icon: "\u2726", color: "#c084fc", glow: "#c084fc33", desc: "We survive by remembering what the others forgot.", philosophy: "Meta. Knowledge. Legacy.", lore: "The Seventh Colony chose to remember everything. Names, faces, patterns, losses. The Forgetting came for them specifically. There is no record of what they knew.", dedicateLine: "The colony chooses to remember. The dark erases. You write it back.", masteryLine: "The Seventh Colony left no record. You are their record now.", apexLore: "Mythic traits aren't random. They're memories so deep they've become physical. When a cat is born with one, something from before the nine colonies is trying to get out." }
  };
  const UPGRADES = [
    ...
    { id: "u_dev_summer", branch: "flame", name: "Summer's Echo", icon: "\u2600\uFE0F", desc: "Start runs with Summer devotion tier 1 active", cost: 85, b: { devMemory: "Summer" }, max: 1, tier: 3, flavor: "The Second Colony burned so bright the dark still flinches at Summer cats." },
    ...
    { id: "u_b", branch: "memory", name: "Blood Memory", icon: "\u{1FA78}", desc: "Starter inherits a Hearth cat's trait", cost: 55, b: { bloodMemory: 1 }, max: 1, tier: 2, flavor: "Memory in the blood. Deeper than names." },
    ...
    { id: "u_dev_any", branch: "memory", name: "Season's Memory", icon: "\u{1F300}", desc: "Start runs with Autumn OR Winter devotion tier 1 (alternates)", cost: 85, b: { devMemoryAlt: true }, max: 1, tier: 3, flavor: "The Seventh Colony remembered all four seasons. Two survived in the record." },
    ...
    { id: "u_mythic", branch: "memory", name: "Mythic Bloodline", icon: "\u{1F31F}", desc: "50% chance of Legendary trait per draft wave", cost: 160, b: { mythicChance: 1 }, max: 1, tier: 5, flavor: "The Seventh Colony's lost knowledge. Remembered into flesh." }
  ];
  const MILESTONES = [
  ...
  const EPITHETS = {
    ...
    longWatch: {
      key: "longWatch",
      test: (c) => c._longWatchEligible && (c.stats?.tp || 0) >= 10 && !c.epithet,
      titles: ["of the Long Watch", "the Steadfast", "who Never Left"],
      bonus: { mult: 2, chips: 10 },
      desc: "+2 bonus, +10 power",
      flavor: "Thirty days by the fire. They earned every ember."
    }
  };
  ...
  function epithetToastMsg(cat2) {
    ...
    if (cat2._hearthChild && cat2._hearthParents) {
      return `${fn}, child of ${cat2._hearthParents}, earned: "${cat2.epithet}". The bloodline carries forward.`;
    }
  ...
  function getDeathMemorial(cat2, ante) {
    ...
    if (cat2._hearthChild && cat2._hearthParents) {
      return `${fn}, child of ${cat2._hearthParents}. The Hearth sent them into the dark. The dark sent them back.`;
    }
    if (ep) {
    ...
    if (tp >= 12) return `${fn}. The elder. ${tp} hands. ${bs.toLocaleString()} at their peak. The colony followed them. Now the colony has to lead itself.`;
  ...
  const CAT_REACTIONS = {
    ...
    bigHand: (fn, trait) => trait === "Scrapper" ? [`${fn} bared their teeth. Not at the dark. At the number. Like they could fight it higher.`] : trait === "Feral" ? [`${fn} howled. The colony joined. The dark pretended not to hear.`] : trait === "Guardian" ? [`${fn} checked every cat after. Like scoring that hard was dangerous.`] : [`${fn} stood still for a long time after that hand. Processing something the rest of them felt.`],
    partnerDied: (fn, trait) => trait === "Devoted" ? [`${fn} stopped eating. Nobody made them start. They just did, three days later. Quieter.`] : [`${fn} sleeps where they used to. Closer to the wall now. Facing away.`],
    bossClear: (fn, scarred) => scarred ? [`${fn} touched their scar after the boss fell. Not pain. Recognition.`] : [`${fn} was the last one to stop watching the place where the boss had been.`],
    elder: (fn) => [`${fn} is the elder now. Every decision passes through their eyes first.`, `${fn} has played more hands than anyone. The colony defers without being asked.`],
    benched: (fn, trait) => trait === "Feral" ? [`${fn} paced the perimeter. Every round they don't play, the pacing gets faster.`] : trait === "Scrapper" ? [`${fn} shredded a root while waiting. The root didn't deserve it.`] : [`${fn} watched from the bench. Said nothing. The silence was louder than scoring.`]
  };
  const DEN_GROWTH = [
  ...
  const ACHIEVEMENTS = [
    ...
    // Tier 4. Myth (hard: 30✦)
    { id: "ninth_dawn", name: "The Remembering", desc: "Break the cycle", icon: "\u{1F305}", dust: 30, check: (s) => s.ninthDawnCleared === true, reward: "Secret epigraph pool" },
  ...
  const COLONY_EVENTS = [
    ...
    // ——— ATMOSPHERIC EVENTS (no meaningful choice, pure narrative + small fx) ———
    {
      id: "atm_vigil",
      title: "The Vigil",
      icon: "\u{1F56F}\uFE0F",
      ...
      textFn: (targets) => {
        const c = targets[0];
        const cn = c ? c.name.split(" ")[0] : "One of them";
        return `${cn} sat at the entrance all night. Not watching for danger. Watching for dawn. When it didn't come, they sat there anyway. The others slept better knowing someone was awake. That's not nothing. That might be everything.`;
      },
      choices: [
        { label: "Let them watch.", desc: "+1 Morale.", fx: { fervor: 1 } }
      ]
    },
    {
      id: "atm_naming",
      ...
      tag: "grief",
      textFn: (_, ctx) => {
        const fn = (ctx.fallenNames || [])[0] || "the one who fell";
        return `The colony started calling ${fn}'s favorite sleeping spot by their name. No one decided this. It just happened. \"I'll be over by ${fn}'s place.\" \"Put the food near ${fn}'s spot.\" The name outlived the cat. That's what names are for.`;
      },
      choices: [
        { label: "The name stays.", desc: "+1 Morale.", fx: { fervor: 1 } }
      ]
    },
    {
      id: "atm_old_song",
      ...
      tag: "quiet",
      textFn: (_, ctx) => {
        const elders = (ctx.all || []).filter(c => (c.stats?.tp || 0) >= 5);
        const en = elders.length > 0 ? elders[0].name.split(" ")[0] : "The oldest";
        return `${en} started humming. Not a melody anyone taught them. Something from before. The kittens stopped and listened. Even the scarred ones stopped. Whatever it was, it lived in the colony before any of them did. And it would live there after.`;
      },
      choices: [
        { label: "Listen.", desc: "+2 Morale. Elders +1 Power.", fx: { fervor: 2, elderPower: true } }
      ]
    },
    {
      id: "atm_counting",
      ...
      textFn: (targets, ctx) => {
        const c = targets[0];
        const cn = c ? c.name.split(" ")[0] : "Someone";
        const n = ctx.colony || 12;
        return `${cn} counted everyone tonight. Twice. The number was the same both times: ${n}. They still looked worried. The Third Colony's leader counted too. Every morning, every evening. The difference is ${cn} doesn't know that story yet.`;
      },
      choices: [
        { label: "Same number. That's enough.", desc: "No effect. The moment is the reward.", fx: {} }
      ]
    },
    {
      id: "atm_gift",
      ...
      minNight: 2,
      tag: "quiet",
      textFn: () => "A dead mouse, placed carefully at the entrance. Not from anyone in the colony. From something outside. Something that saw the fire and didn't run. You'll never know what it was. But it left this. And leaving something for strangers in the dark is either madness or the beginning of something.",
      choices: [
        { label: "Accept it.", desc: "+3 Rations.", fx: { gold: 3 } }
      ]
    },
    {
      id: "atm_dream",
      ...
      minNight: 4,
      tag: "quiet",
      condFn: (_, ctx) => (ctx.all || []).some(c => c.bondedTo),
      textFn: (_, ctx) => {
        const bonded = (ctx.all || []).filter(c => c.bondedTo);
        const c1 = bonded[0];
        const c2 = bonded.length > 1 ? bonded[1] : null;
        const n1 = c1 ? c1.name.split(" ")[0] : "Two of them";
        const n2 = c2 ? c2.name.split(" ")[0] : "their partner";
        return `${n1} and ${n2} woke at the same time. Same breath, same jolt, same wide eyes. They won't say what they dreamed. They don't have to. Whatever it was, they faced it together, even asleep. Especially asleep.`;
      },
      choices: [
        { label: "Together.", desc: "Bonded pair +1 Power each.", fx: { bondedPower: 1 } }
      ]
    },
    // --- VIRMIRE EVENTS (player-chosen consequence) ---
    {
      id: "narrow_path",
      ...
      tag: "sacrifice",
      once: true,
      condFn: (_, ctx) => (ctx.colony || 0) >= 10,
      textFn: (_, ctx) => `The path ahead narrows. Rock on both sides, pressing in. Room for ${(ctx.colony || 12) - 2} to pass. Two have to stay behind and find another way around. They'll make it. Probably. The question is who you trust to survive alone.`,
      choices: [
        { label: "The weakest stay. They'll slow us down.", desc: "2 lowest Power cats benched next round. They return hardened.", fx: { narrowWeak: true } },
        { label: "The strongest stay. They can handle it.", desc: "2 highest Power cats benched next round. They return with stories.", fx: { narrowStrong: true } },
        { label: "We go together or not at all.", desc: "-2 hands next round. Everyone stays.", fx: { tempHands: -2 } }
      ]
    },
    {
      id: "the_offering_v2",
      ...
      tag: "sacrifice",
      once: true,
      condFn: (_, ctx) => (ctx.colony || 0) >= 8 && (ctx.all || []).some(c => c.scarred),
      textFn: (_, ctx) => {
        const scarred = (ctx.all || []).filter(c => c.scarred);
        const sn = scarred[0] ? scarred[0].name.split(" ")[0] : "The scarred one";
        return `The dark speaks. Not in words. In pressure. In the way the air thins around ${sn}. It wants something. A scar for a scar. One of yours for safe passage. The colony looks at you. Not at ${sn}. At you.`;
      },
      choices: [
        { label: "Give it what it wants.", desc: "Random scarred cat loses 2 Power. Colony safe this night.", fx: { offeringScar: true, eventDenSafe: true } },
        { label: "We don't pay. Not to the dark.", desc: "+3 Morale. The dark remembers defiance.", fx: { fervor: 3 } }
      ]
    },
  ...
  const EXPANDED_BOSSES = [
    {
      ...
      defeatFn: (ctx) => ctx.clutch ? "Held together by a thread. I know what threads do." : ctx.deathless ? "Not a single grudge took one of them? That's not a colony. That's a miracle." : ctx.fallen > 2 ? "You healed some. Not all. The wounds that don't show are the ones I planted." : "You healed what I infected. Impressive. Temporary.",
      lore: "The colony that ate itself.",
      mechanic: { id: "grudge_double", name: "Internal Wounds", desc: "Grudge penalties doubled", icon: "\u{1FA78}" }
    },
    {
      id: "eclipse",
      name: "The Eclipse",
    ...
    },
    {
      ...
      defeatFn: (ctx) => ctx.clutch ? "By that much? Of course by that much. It's always by that much." : ctx.deathless ? `All of them. Every single one. We lost three on Night 4. You lost none. Don't take that for granted.` : ctx.fallen > 2 ? `${ctx.fallen} names. We lost the same number. The math doesn't care about the story. But I do.` : "All of them. You kept all of them. We... couldn't.",
      lore: "One hand short. That's the whole story.",
      mechanic: { id: "almost", name: "So Close", desc: "Target +15%. But your best play scores double", icon: "\u{1F525}" }
    }
  ];
  const BOSS_TRAITS = [
    ...
    { id: "bleeding", name: "Bleeding", icon: "\u{1FA78}", desc: "Target shrinks 2% per play", flavor: "It weakens, but will you last?", fx: { bleeding: true } },
    ...
    { id: "fading", name: "Fading", icon: "\u{1F56F}\uFE0F", desc: "Target grows 5% per play remaining", flavor: "It grows stronger as time runs out.", fx: { fading: true } },
  ...
  const HEAT_FLAVOR = [
    "",
    "The cycle noticed you. It's adapting.",
    "They're sending you to break what eight colonies couldn't.",
    "The dark is fighting harder. That means the cycle is weakening.",
    "Everything you've built is a weapon against the pattern.",
    "Ninth life. Last light. Break the cycle or there won't be a tenth."
  ];
  const HEAT_RELICS = [
    null,
    // index 0 unused
  ...
  const THE_REMEMBERING = {
    ...
    icon: "\u{1F305}",
    taunt: "I am the cycle. Eight times the dark closed. Eight times a colony fell. I am the pattern. And you are trying to break me.",
    tauntFn: (ctx) => {
      const runs = ctx.totalRuns || 1;
      const td = ctx.totalDeaths || 0;
      const hc = ctx.hearthCats || 0;
      const ep = ctx.epithets || 0;
      const pf = ctx.prevFallen || [];
      if (td > 15 && pf.length > 0) return `${td} names fed to the dark across ${runs} colonies. ${pf.slice(0, 3).map(f => f.name.split(" ")[0]).join(". ")}. I remember them even if you don't. That's the cycle. You sacrifice, the dark takes, and nothing changes. Unless you change it now.`;
      if (td === 0 && runs > 3) return `Not a single name lost across ${runs} colonies. The cycle doesn't know what to do with that. Neither do I.`;
      if (hc >= 15) return `${hc} names in the Hearth. ${hc} cracks in the dark. Each one weakens the cycle. But the cycle has survived eight colonies. Can ${hc} names do what eight colonies couldn't?`;
      if (runs >= 15) return `Colony ${runs}. The First Colony got one attempt. You got ${runs}. The cycle has never been tested this many times. It's starting to show.`;
      if (ep >= 5) return "They have titles now. The cycle doesn't recognize titles. The dark has never had to fight names that fought back.";
      return "I am the First Colony's hunger and the Eighth Colony's last hand. I am every way a colony has ever died. Break me and the dark loses its playbook.";
    },
    defeat: "The cycle breaks. Not because you were strong. Because you remembered every colony that came before and refused to let the pattern hold.",
    defeatFn: (ctx) => ctx.clutch ? "By that much. The eighth colony missed by one hand. You made it by one hand. The difference between a cycle and a breakthrough is a single moment of defiance." : "Eight colonies. Eight patterns. All broken. The dark doesn't know what comes next. Neither do you. But for the first time, something comes next.",
    lore: "The last question is not 'did you survive.' It is 'did you break the cycle.'"
  };
  ...
  const EPIGRAPHS = [
    ...
    "The cycle has run eight times. Each time the dark wins, it gets easier. Each time you remember, it gets harder for the dark.",
    "Dawn isn't a place. It's a threshold. Cross it enough times and it stays open.",
    "The Hearth doesn't just remember. It accumulates. Every name is a crack in the dark.",
    "Nine lives. Not because cats are lucky. Because that's how many attempts the world gets before the dark becomes permanent.",
    "The eighth colony was one hand short. You're carrying their hand now.",
    "Break the pattern. That's not a strategy. That's the entire point."
  ];
  function getEpigraph(meta) {
    ...
    if (w >= 1 && h >= 3 && cats >= 9 && BK.every((b) => (meta.stats.disc || []).some((d) => d.startsWith(b))) && (meta.achv || []).length >= 3) {
      if (meta.ninthDawnCleared) return "The cycle breaks. Nine colonies. Eight fell to feed the dark its patterns. The ninth refused. The dark has no playbook for what comes next.";
  ...
  const DEN_QUIET = [
    () => "Nothing happened. That's the rarest thing in the dark.",
    () => "The fire crackled. The cats slept. Nobody died. Write that down.",
    () => "A quiet night. They exist. You just have to survive long enough to see one.",
    () => "Everyone accounted for. Everyone breathing. The simplest victory.",
    () => "The dark pressed against the den walls. The den held. That's all.",
    () => "Silence. Not the bad kind. The kind that means everyone is still here.",
    () => "One of the kittens dreamed. You could tell by the way their paws twitched. Nobody woke them.",
    () => "The oldest cat groomed the youngest. No one asked them to. The colony is learning to be gentle.",
    () => "Rain outside. Or something like rain. Inside, dry. Inside, warm. Inside, together.",
    () => "You counted them. Same number as yesterday. In the dark, that's a miracle."
  ];
  const LORE_FRAGMENTS = [
    ...
    "The floor was worn smooth in a circle. Pacing. Someone paced here for a long time.",
    "A fishing line, carefully coiled. The hook was bent into a shape that wasn't a hook. It was a letter.",
    "Bones, arranged in a constellation. The same one you can see from the entrance on clear nights.",
    "Two nests, side by side, exactly the same size. One was slept in. One was kept for someone who never came.",
    "A scratch on the ceiling. Someone stood on someone else's shoulders to make it. It says 'HERE.'",
    "You found a counting system. Marks grouped in fives. It stops at forty-three.",
    "Someone planted seeds in a crack. They grew. Nothing eats them. Even the dark has things it won't touch.",
    "An old collar, too small for any cat in your colony. A kitten's collar. The name is worn away.",
    "The stones here are warm. Not from the sun. From something underneath. The First Colony built over it. They didn't know.",
    "You hear dripping. Following it leads to clean water. Following it further leads to a wall of names.",
    "A toy. Rough, handmade. A ball of yarn wound around a stone. Someone made this for someone small.",
    "Nothing grew here. But something tried. You can see the marks where roots pushed against stone and gave up.",
    "Nothing on the walls. No tallies, no names, no scratches. The walls are too clean. Someone erased them.",
    "Nothing in the den but warmth. Old warmth. The kind that means a body was here recently. Centuries ago.",
    "Scratched deeper than the rest, under all the other marks: 'THE CYCLE CAN BE BROKEN.' No signature.",
    "Eight fires went out. Eight times the dark closed. Someone carved a ninth tally. It's still being carved.",
    "You found a diagram. Eight circles, each crossed out. A ninth circle, open. Inside it: a question mark.",
    "The walls here are thinner. Not eroded. Weakened. Something on the other side has been pushing back.",
    "A pattern in the stone. The same pattern the bosses follow. Someone mapped it. Someone was looking for the flaw.",
    "Dawn light, preserved in crystal. Impossible. The First Colony trapped it before they fell. It still glows."
  ];
  const COLONY_RECORDS = [
    "THE FIRST COLONY lasted forty-three days. They starved when the river shifted. The Hunger didn't come for them. It was already there, waiting in their bones. The youngest was named Thistle. She was three seasons old.",
    "THE SECOND COLONY built on ground that wasn't theirs. The Territory had been watching since day one. They thought the walls would hold. The walls held. The ground didn't. It opened beneath them on the twenty-ninth night.",
    "THE THIRD COLONY was led by a cat called Moth. She couldn't stop counting heads. Every morning, every evening. When the count went from twelve to eleven, she didn't tell anyone. She just counted louder. The Mother remembers her.",
    "THE FOURTH COLONY was the largest. Twenty-eight cats at their peak. The Swarm came on a night when the watch was changing. Nobody's fault. Everybody's loss. The line broke at the south wall. It was over in minutes.",
    "THE FIFTH COLONY turned on itself. Nobody remembers why. The grudges multiplied faster than the kittens. By the end, cats who'd been bonded were fighting over fish bones. The Fraying didn't have to do anything. It just watched.",
    "THE SIXTH COLONY stopped. Not died. Stopped. One by one, they sat down and didn't get up. The Eclipse didn't kill them. It convinced them that rest was the same as peace. The fire went out. Nobody relit it.",
    "THE SEVENTH COLONY has no record. No names, no stories, no marks on any wall. The Forgetting took everything. If you're reading this, they existed. That's more than the dark left them.",
    "THE EIGHTH COLONY beat four bosses. They were one hand short on the fifth night. One hand. The Ember that Remains was there at the end. Not as an enemy. As a witness. It still talks about them.",
    "Nine fires have burned in the dark. Eight went out. You are sitting beside the ninth. The cats around you don't know about the other eight. They don't need to. You do.",
    "The bosses aren't creatures. They're patterns. The Hunger is how the first colony died. The Territory is how the second colony died. Every boss is a memory the dark can't let go of. You're fighting ghosts.",
    "Stardust isn't metaphorical. When a colony falls, something remains. A residue. Light that memory casts. The Hearth burns on it. Every upgrade you buy is powered by the ones who came before.",
    "There will not be a tenth colony. This is the system's last allocation. Nine attempts to maintain light in a space that has decided it's done with light. The dark isn't evil. It's just finished. You are the argument against finished."
  ];
  const WHISPER_OVERFLOW = {
    ...
    boss: [
      ...
      "The cycle cracks. One more pattern the dark can't rely on.",
      "Eight colonies fed that pattern. You just starved it."
    ]
  };
  const WHISPER_SHOP = [
  ...
  function resolveDen(denCats, hasMatchmaker, denSafe, heatDenFight, ctx = {}) {
    ...
    const MAX_COLONY = 25 + (ctx.colonyCap || 0);
    const MAX_BIRTHS = 2 + (ctx.breedBoost >= 0.15 ? 1 : 0) + (ctx.colonyCap > 0 ? 1 : 0);
        ...
        if (!noBreed && !breedBlocked && roll < bCh) {
          ...
          if (loser.injured) {
            if (hasPhoenix) {
              ...
              if (ctx.denSafety && Math.random() < 0.5) {
                loser.power = Math.max(1, loser.power - 2);
                loser.scarred = true;
                results.push({ type: "fight", c1: cats[i], c2: cats[j], loser, dmg: 2, wasInjured: true, saved: true });
              } else {
                deaths++;
  ...
  function narrativeReason(c, cats, mechanicalReasons) {
    ...
    if (catHas(c, "Alpha") && c.power >= Math.max(...cats.map((x) => x.power))) return `${n} leads from the front. The highest Power in the group.`;
  ...
  function calcScore(cats, fams, fLvl, cfx = {}, ctx = {}) {
    let scoreCats = [...cats];
    if (ctx.wildCard === "straywind" && scoreCats.length > 0) {
      const dominant = scoreCats.reduce((m, c) => { m[c.breed] = (m[c.breed] || 0) + 1; return m; }, {});
      const topBreed = Object.entries(dominant).sort((a, b) => b[1] - a[1])[0][0];
      scoreCats = scoreCats.map((c) => ({ ...c, breed: topBreed, _wildSeason: true }));
    }
    if (ctx.wildCard === "bloodpact" && scoreCats.length > 0) {
      const strongest = scoreCats.reduce((best, c) => (!best || (c.trait && c.trait.name !== "Plain" && (c.trait.tier === "mythic" ? 4 : c.trait.tier === "legendary" ? 3 : c.trait.tier === "rare" ? 2 : 1) > (best.trait && best.trait.name !== "Plain" ? (best.trait.tier === "mythic" ? 4 : best.trait.tier === "legendary" ? 3 : best.trait.tier === "rare" ? 2 : 1) : 0)) ? c : best), null);
      if (strongest?.trait && strongest.trait.name !== "Plain") {
        scoreCats = scoreCats.map((c) => ({ ...c, trait: strongest.trait, _wildTrait: true }));
      }
    }
    cats = scoreCats;
    if (ctx.bossMechanic === "weakest_benched" && cats.length > 1) {
      const weakest = cats.reduce((w, c) => !w || c.power < w.power ? c : w, null);
      if (weakest) cats = cats.map((c) => c.id === weakest.id ? { ...c, _bossHalved: true, power: Math.max(1, Math.ceil(c.power / 2)) } : c);
    }
    if (ctx.bossMechanic === "season_exile" && cats.length > 0) {
      const seasons = {};
      cats.forEach((c) => { seasons[c.breed] = (seasons[c.breed] || 0) + c.power; });
      const strongest = Object.entries(seasons).sort((a, b) => b[1] - a[1])[0]?.[0];
      if (strongest) cats = cats.map((c) => c.breed === strongest ? { ...c, _exiled: true, power: Math.max(1, Math.ceil(c.power / 2)) } : c);
    }
    if (ctx.bossMechanic === "forced_full") {
      ctx._noFocus = true;
      ctx._noBench = true;
    }
    ...
    if (btFxAll.length > 0) {
      btFxAll.forEach((bt) => {
        ...
        if (bt.fx.enragedMult) bd.push({ label: `\u{1F525} Enraged: all cats +${bt.fx.enragedMult}B`, chips: 0, mult: 0, type: "boss_trait" });
      });
    }
    ...
    if (devFx.chipScale > 0) {
      ...
      chips += bonus;
      bd.push({ label: `\u2600\uFE0F Wildfire +${bonus}P`, chips: bonus, mult: 0, type: "devotion" });
    }
    if (devFx.powerBoost) {
      let pbTotal = 0;
      cats.forEach((c) => {
      ...
      if (pbTotal > 0) bd.push({ label: `\u{1F451} Devotion Power +${pbTotal}P`, chips: pbTotal, mult: 0, type: "devotion" });
    }
    ...
    toS.forEach((c, si) => {
      ...
      if (c.epithetKey && !c._re) {
        ...
        if (epDef?.bonus) {
          ...
          } else if (epDef.bonus.mult) reasons.push(`"${c.epithet}" +${epDef.bonus.mult}B`);
          if (epDef.bonus.clutchMult && !ctx.beatingPace) reasons.push(`"${c.epithet}" clutch +${epDef.bonus.clutchMult}B`);
          if (epDef.bonus.soloMult) {
            const myB = getCatBreeds(c);
            const al = !cats.some((x) => x.id !== c.id && getCatBreeds(x).some((b) => myB.includes(b)));
            if (al) reasons.push(`"${c.epithet}" solo +${epDef.bonus.soloMult}B`);
          }
    ...
    }
    {
      ...
      if (ctx.bossMechanic !== "bond_curse") bondedPairs.forEach(([a, b], pi) => {
    ...
    if (!cfx.silence) {
      ...
      fams.forEach((f) => {
        if (f.htBonus && f.htBonus[htName]) {
          ...
          if (hb.mult) {
            mult += hb.mult;
            bd.push({ label: `${f.icon} ${f.name}: ${htName} +${hb.mult}B`, chips: 0, mult: hb.mult, type: "fam" });
          }
        }
      });
    }
    const focusScale = [0, 1.5, 1.3, 1.2, 1.1];
    if (cats.length >= 1 && cats.length <= 4 && !ctx._noFocus) {
    ...
    if (ctx.lastStand) {
      ...
      mult += lsBonus;
      bd.push({ label: `\u{1F525} LAST STAND +${lsBonus}B`, chips: 0, mult: lsBonus, type: "laststand", allCats: true });
    }
    ...
    if (ctx.wildCard === "moonfire") {
      finalTotal *= 2;
      bdEnd.push({ label: "\u{1F319} Moonfire: scores twice", chips: 0, mult: 0, xMult: 2, type: "wild", allCats: true });
    }
    if (ctx.prestige > 0) {
  ...
  function _CC({ cat: _cat, sel, onClick, sm, cw: _cw, dis, hl, fog, chemHint, denMode, onTraitClick, traitFog }) {
    ...
    return /* @__PURE__ */ React.createElement(
      ...
      /* @__PURE__ */ React.createElement("div", { style: {
        ...
        "img",
        {
          ...
          style: {
            ...
            filter: cat2.injured ? "saturate(0.3) brightness(0.5)" : traitFog ? "saturate(0.4) brightness(0.7)" : cat2.scarred ? "contrast(1.15) brightness(0.9)" : "none",
      ...
      /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", bottom: 0, left: 0, right: 0, zIndex: 4 } }, /* @__PURE__ */ React.createElement("div", { style: { position: "relative", height: sm ? 14 : 18 } }, /* @__PURE__ */ React.createElement("div", { style: {
        ...
        lineHeight: 1.2,
        whiteSpace: "nowrap"
      } }, fn, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, fontWeight: 700, marginLeft: 2, color: cat2.sex === "M" ? "#60a5fa" : "#f472b6" } }, cat2.sex === "M" ? "\u2642" : "\u2640")), !sm && cat2.epithet && /* @__PURE__ */ React.createElement("div", { style: { background: "#0d1117", textAlign: "center", padding: "0 4px 2px", overflow: "hidden" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 8, color: "#fbbf24aa", fontStyle: "italic", whiteSpace: "nowrap", letterSpacing: 0.5, lineHeight: 1 } }, cat2.epithet))), sm && /* @__PURE__ */ React.createElement("div", { style: { background: "#0d1117", padding: "1px 4px 3px", textAlign: "center", height: 12 } }, /* @__PURE__ */ React.createElement("div", { style: {
        fontSize: 7,
        fontWeight: 700,
        color: neon + "aa",
        letterSpacing: 0,
  ...
  function NinthLife() {
    ...
    const [bestBossHand, setBestBossHand] = useState(0);
    ...
    const [sWild, setSWild] = useState(null);
    const [activeWild, setActiveWild] = useState(null);
    ...
    const [denGuide, setDenGuide] = useState(null);
    const [denBirthPause, setDenBirthPause] = useState(false);
    const [colonyPortrait, setColonyPortrait] = useState(false);
    ...
    const [deathChoiceQueue, setDeathChoiceQueue] = useState([]);
    ...
    const [dedicateModal, setDedicateModal] = useState(null);
    const [demoStep, setDemoStep] = useState(0);
    const epigraphRef = useRef(null);
    const [dailyPlayers, setDailyPlayers] = useState(null);
    const dailyStreak2 = React.useMemo(() => { try { const d = JSON.parse(localStorage.getItem("nl_daily") || "{}"); return d.streak || 0; } catch { return 0; } }, [ph]);
    ...
    const BOSSES = [
      {
        ...
        tauntFn: (ctx) => {
          ...
          if ((ctx.totalRuns || 0) >= 10 && ctx.gold < 5) return `Colony ${(ctx.totalRuns || 0) + 1}. You bring more mouths each time. I bring the same empty plate.`;
          if ((ctx.totalRuns || 0) >= 8) return "I've been counting your Rations since you sat down. You haven't noticed. That's how I work.";
        ...
        mechanic: { id: "weakest_benched", name: "Starvation", desc: "Weakest cat scores half", icon: "\u{1F9B4}" }
      },
      {
        id: "territory",
        name: "The Territory",
        ...
        mechanic: { id: "season_exile", name: "Claimed Ground", desc: "One season scores half Power", icon: "\u{1F3F4}" }
      },
      {
        id: "mother",
        name: "The Mother",
        ...
        tauntFn: (ctx) => {
          ...
          if ((ctx.totalRuns || 0) >= 10 && (ctx.hearthCats || 0) > 5) return `You've saved ${ctx.hearthCats || 0} to the Hearth. She would have saved them all. That was the problem.`;
          if ((ctx.totalRuns || 0) >= 8) return "Do you count them? Every morning, every evening? She did. The count went from twelve to eleven on Night 3. She didn't tell anyone.";
        ...
        mechanic: { id: "bond_curse", name: "Grief Mirror", desc: "Bonded pairs lose their bonus", icon: "\u{1F494}" }
      },
      {
        id: "swarm",
        name: "The Swarm",
        ...
        mechanic: { id: "forced_full", name: "Outnumbered", desc: "Too many to focus. No bench safety", icon: "\u{1F41C}" }
      },
      {
        id: "forgetting",
        name: "The Forgetting",
        ...
        tauntFn: (ctx) => {
          ...
          if ((ctx.totalRuns || 0) >= 10) return `Colony ${(ctx.totalRuns || 0) + 1}. Will you remember the cats from colony 1? Colony 2? The names blur. That's me.`;
          if ((ctx.totalRuns || 0) >= 8) return "The Seventh Colony had a name for their leader. I took it. I took all of them. I'll take yours.";
        ...
        mechanic: { id: "trait_fog", name: "Erasure", desc: "Trait icons hidden during play", icon: "\u{1F32B}" }
      }
    ];
    const wins = meta?.stats?.w || 0;
    const FULL_BOSS_POOL = [...BOSSES, ...wins >= 3 || (meta?.heat || 0) >= 3 ? EXPANDED_BOSSES : []];
    const prophecy = React.useMemo(() => { if (!meta || !meta.stats.r) return null; const bIdx = Math.min((meta.stats.r || 0) % BOSSES.length, BOSSES.length - 1); const nb = BOSSES[bIdx]; const night = Math.min(5, Math.floor(Math.random() * 3) + 2); const cat = meta.cats.length > 0 ? meta.cats[Math.floor(Math.random() * meta.cats.length)] : null; const showWild = Math.random() < 0.5; return { boss: nb, night, cat, showWild }; }, [meta?.stats?.r, ph]);
    ...
    function getMB() {
      ...
      Object.entries(meta.ups || {}).forEach(([id, cnt]) => {
        ...
        if (u) Object.entries(u.b).forEach(([k, v]) => {
          if (typeof v === "string" || typeof v === "boolean") b[k] = v;
          else b[k] = (b[k] || 0) + v * cnt;
        });
      });
      if (meta.stats.disc) {
    ...
    const eTgt = () => {
      ...
      if (blind === 2 && boss?.mechanic?.id === "almost") t = Math.round(t * 1.15);
      return t;
    };
    ...
    function startGame() {
      ...
      try {
        ...
        if (lastPlay !== today && meta) {
          ...
          const hearthRestBonus = Math.min(10, Math.max(2, (meta?.cats?.length || 0)));
          setMeta((m) => {
            const nm = { ...m, dust: (m.dust || 0) + hearthRestBonus };
            saveS(nm);
            return nm;
          });
          setTimeout(() => toast("\u{1F3E0}", `Your Hearth rested. +${hearthRestBonus}\u2726 stardust from ${Math.max(0, meta?.cats?.length || 0)} cats.`, "#c084fc", 4e3), 1e3);
        }
      } catch (e) {
      }
      _cid = 0;
      ...
      if (isFirstRun && !isDailyRun) {
        for (const s of BK) { strayBreeds.push(s); strayBreeds.push(s); strayBreeds.push(s); }
        while (strayBreeds.length < strayCount) strayBreeds.push(pk(BK));
        strayBreeds.length = strayCount;
      } else {
      ...
      if (mb.mythicChance > 0 && Math.random() < 0.5 && !allDraft.some((c) => (c.trait || PLAIN).tier === "legendary")) {
      ...
      setDevotion(() => {
        ...
        const init = {};
        if (mb.devMemory) {
          const ms = DEVOTION_MILESTONES[mb.devMemory];
          if (ms && ms.length > 0) init[mb.devMemory] = ms[0].at;
        }
        if (mb.devMemoryAlt) {
          const alt = (meta?.stats?.r || 0) % 2 === 0 ? "Autumn" : "Winter";
          const ms = DEVOTION_MILESTONES[alt];
          if (ms && ms.length > 0) init[alt] = ms[0].at;
        }
        return init;
      });
      setNightMod(null);
    ...
    function advanceFromScoring() {
      ...
      try {
        if (autoPlay) {
          if (!autoPlay._secondHand && isFirstRun && ante === 1 && blind === 0) {
            const firstScore = sRes?.total || 0;
            const willClear = (rScore + (sRes?.total || 0)) >= eTgt();
            setAutoPlay(null);
            if (willClear) {
              setGuide({ step: 2, msg: "" });
            } else {
              setTimeout(() => {
                if (ph !== "playing" || hand.length < 3) { setGuide({ step: 2, msg: "" }); return; }
                const bc2 = {};
                hand.forEach((c) => { bc2[c.breed] = (bc2[c.breed] || 0) + 1; });
                const worst = Object.entries(bc2).sort((a, b) => a[1] - b[1]);
                const mixedIds = [];
                worst.forEach(([breed]) => {
                  const cat = hand.find((c) => c.breed === breed && !mixedIds.includes(c.id));
                  if (cat && mixedIds.length < 3) mixedIds.push(cat.id);
                });
                if (mixedIds.length >= 2) {
                  const idxs2 = mixedIds.map((id) => hand.findIndex((c) => c.id === id)).filter((i) => i >= 0);
                  setAutoPlay({ step: -3, idxs: idxs2, _secondHand: true, _firstScore: firstScore });
                } else {
                  setGuide({ step: 2, msg: "" });
                }
              }, 800);
            ...
            const firstScore = autoPlay._firstScore || 0;
            const secondScore = sRes?.total || 0;
            setAutoPlay(null);
            if (firstScore > 0 && secondScore > 0 && firstScore > secondScore * 1.5) {
              setTimeout(() => toast("\u{1F4A1}", `See the difference? Matched seasons scored ${Math.round(firstScore / secondScore)}x more. Your turn.`, "#fbbf24", 5e3), 600);
            }
            setGuide({ step: 2, msg: "" });
          }
        }
        const result = sRes;
        let emberBonus = 0;
        if (blind === 2 && boss?.mechanic?.id === "almost" && result.total > bestBossHand) {
          emberBonus = result.total;
          setBestBossHand(result.total);
          result.bd.push({ label: `\u{1F525} SO CLOSE: \xD72`, chips: 0, mult: 0, xMult: 2, type: "wild", allCats: true });
        }
        const cats = scoringCats;
        const tgt2 = eTgt();
        const ns = rScore + result.total + emberBonus;
        if (emberBonus > 0) setTimeout(() => toast("\u{1F525}", `SO CLOSE: best play doubled! +${emberBonus.toLocaleString()}`, "#fbbf24", 3e3), 800);
        setRScore(ns);
        ...
        cats.forEach((c) => {
          ...
          if (result.total > c.stats.bs) {
            ...
            if (c.stats.tp >= 3 && result.total > wasPB * 1.3) {
              ...
              if (c._hearthChild && c._hearthParents) {
                setTimeout(() => toast("\u{1F525}", `${fn}, child of ${c._hearthParents}. ${result.total.toLocaleString()} in a single hand. The bloodline proves itself.`, "#fbbf24", 4e3), 1200);
              }
            }
          }
          if (wasKitten) {
        ...
        if (ns >= tgt2) {
          ...
          if (isBossClear && ante >= MX) {
            ...
            const scarredCat = cats.find((c) => c.scarred);
            if (scarredCat) {
              const fn = scarredCat.name.split(" ")[0];
              setTimeout(() => toast("\u2694\uFE0F", pk(CAT_REACTIONS.bossClear(fn, true)), BREEDS[scarredCat.breed]?.color || "#fbbf24", 3e3), 2500);
            }
            setTimeout(() => {
              setClutch(false);
            ...
            if (eligible.length > 0) {
              ...
              if (isBloodied) {
                ...
                if (isFirstRun && !seen.injury) {
                  ...
                  setTimeout(() => toast("\u{1F4A1}", "Injured cats score half. Injuries are permanent unless healed by traits, upgrades, or events.", "#ef4444", 6e3), 2e3);
                }
                if (victim._newEpithet) {
                  delete victim._newEpithet;
                  setTimeout(() => {
    ...
    function playH() {
      ...
      cats.forEach((c) => {
        if (c._mateDied && !c._griefAcknowledged) {
          ...
          const mateName = mateCat?.name?.split(" ")[0] || mn || "their partner";
      ...
      const result = calcScore(cats, fams, ferv, cfx, { gold, deckSize: allC.length, discSize: disc.length, handSize: hs(), beatingPace, bossTraitFx: activeBT, scarMult: getMB().scarMult || 0, grudgeWisdom: getMB().grudgeWisdom || 0, hasMastery: !!getMB().xp, bondBoost: getMB().bondBoost || 0, comboBoost: getMB().comboBoost || 0, wildfireBonus: getMB().wildfireBonus || 0, doubleBench: getMB().doubleBench || 0, kindredMult: tempMods.kindredMult || 0, weatherSeason: weather?.season || null, nightModFx: nightMod?.fx || {}, lastHandIds, lastHandLost, lastHandType, htLevels, devotion, bench: hand.filter((c) => !cats.find((x) => x.id === c.id)), lastStand: isLastStand, wildCard: activeWild?.type || null, bossMechanic: blind === 2 && boss?.mechanic?.id || null, prestige: meta?.prestige || 0 });
      if (activeWild) { setActiveWild(null); setWildUsed(true); }
      ...
      if (stRef.current) clearTimeout(stRef.current);
      const aft = [];
      const pbCats = cats.filter((c) => result.total > c.stats.bs);
      if (pbCats.length > 2) {
        aft.push({ icon: "\u{1F3C6}", text: `${pbCats.length} cats set a new personal best: ${result.total.toLocaleString()}`, color: "#fbbf24" });
      } else {
        pbCats.forEach((c) => {
          aft.push({ icon: "\u{1F3C6}", text: `${c.name.split(" ")[0]} PB: ${result.total.toLocaleString()}`, color: "#fbbf24" });
        });
      }
      const bondedInHand = cats.filter((c) => c.bondedTo && cats.find((x) => x.id === c.bondedTo));
      if (bondedInHand.length >= 2) {
        const a2 = bondedInHand[0], b2 = bondedInHand[1];
        const an = a2.name.split(" ")[0], bn = b2.name.split(" ")[0];
        const bht = (a2._bondHandsTogether || 0) + 1;
        [setHand, setDraw, setDisc].forEach((s) => {
          s((arr) => arr.map((x) => x.id === a2.id || x.id === b2.id ? { ...x, _bondHandsTogether: (x._bondHandsTogether || 0) + 1 } : x));
        });
        const bondMsg = bht === 1 ? `${an} and ${bn} play side by side. Closer than before.` : bht === 3 ? `${an} reaches for ${bn} first. Always.` : bht === 5 ? `They don't need to look at each other anymore. They just know.` : bht === 8 ? `${an} and ${bn}. The colony can't imagine one without the other.` : `${an} & ${bn}: Together`;
        aft.push({ icon: "\u{1F495}", text: bondMsg, color: "#f472b6" });
      }
      const weakest = cats.reduce((w, c) => !w || c.power < w.power ? c : w, null);
      if (weakest && weakest.power <= 3 && result.total > eTgt() * 0.5 && cats.length >= 3) {
        const wn = weakest.name.split(" ")[0];
        aft.push({ icon: "\u{1F43E}", text: pk([`${wn} scored alongside cats twice their size. Nobody carried them. They kept up.`, `P${weakest.power}. Nobody expected ${wn} to matter. ${wn} expected it.`, `${wn}. The smallest one. Not the weakest. There's a difference.`]), color: BREEDS[weakest.breed]?.color || "#888" });
      }
      if (rScore < tgt * 0.3 && rScore + result.total >= tgt) {
        aft.push({ icon: "\u{1F525}", text: pk(["From nothing. They were losing. And then they weren't.", "The dark had this round. Until it didn't.", "Comeback. The kind they'll tell stories about."]), color: "#fef08a" });
      }
      const kittenInHand = cats.find((c) => c._bornNight && c._bornNight >= ante - 1);
      if (kittenInHand && result.total > 2000) {
        const kn = kittenInHand.name.split(" ")[0];
        aft.push({ icon: "\u{1F431}", text: pk([`${kn} was born in the dark. Now they score in it.`, `Born Night ${kittenInHand._bornNight}. Already proving something.`, `The kitten is keeping up. More than keeping up.`]), color: "#c084fc" });
      }
    ...
    function recruitCat() {
      ...
      toast("\u{1F4E3}", cost === 0 ? `${rb} ${rn} joins. Free.` : `${rb} ${rn} joins the colony. (${cost}\u{1F41F})`, cost > 0 ? "#fbbf24" : "#4ade80");
      Audio.cardSelect(); Haptic.light();
    }
    ...
    function endRun(won, finalScore) {
      try {
        ...
        if (isDailyRun) {
          ...
          if (meta) {
            const dd = getDailyData();
            const dustHarvest = Math.floor(fScore / 1e4);
            const streak = dd.streak || 1;
            const knownLore = meta.dailyLore || [];
            const newRecord = COLONY_RECORDS.find((r2) => !knownLore.includes(r2));
            let streakReward = null;
            if (streak === 14 && !meta._streak14Claimed) {
              streakReward = { type: "hearth_power", label: "14-day streak! A Hearth cat gains +1 Power permanently." };
            } else if (streak === 30 && !meta._streak30Claimed) {
              streakReward = { type: "epithet_unlock", label: "30-day streak! Epithet 'of the Long Watch' unlocked." };
            }
            const nm = {
              ...meta,
              stats: { ...meta.stats, r: (meta.stats.r || 0) + 1 },
              dust: (meta.dust || 0) + dustHarvest,
              dailyLore: newRecord ? [...knownLore, newRecord] : knownLore,
              _streak14Claimed: meta._streak14Claimed || (streak >= 14),
              _streak30Claimed: meta._streak30Claimed || (streak >= 30),
              _longWatchUnlocked: meta._longWatchUnlocked || (streak >= 30),
            ...
            setMeta(nm);
            saveS(nm);
            if (dustHarvest > 0) setTimeout(() => toast("\u2726", `Daily harvest: +${dustHarvest}\u2726 stardust (${fScore.toLocaleString()} scored)`, "#c084fc", 3500), 1500);
            if (newRecord) setTimeout(() => toast("\u{1F4DC}", newRecord, "#d4a54a", 8e3), 3500);
            if (streakReward) setTimeout(() => toast("\u{1F451}", streakReward.label, "#fbbf24", 5e3), 5e3);
          }
          return;
        }
    ...
    function nextBlind() {
      ...
      const upgFastHeal = getMB().fastHeal || 0;
      const healCat = (c) => {
        if (!c.injured) return c;
        const fastHeal = catHas(c, "Scrapper") || devFastHeal || upgFastHeal;
        if (fastHeal) return { ...c, injured: false, injuryTimer: 0 };
        return c;
      };
    ...
    function genShop() {
      ...
      setDen([]);
      setRerollCount(0);
      if (ante >= 2 && !wildUsed && !sWild && !activeWild && (!seen.wildCard || Math.random() < 0.4)) {
    ...
    function endNight() {
      ...
      results.forEach((r) => {
        if (r.type === "breed") {
          r.baby._bornNight = ante; if (!seen.firstKitten) { setSeen((s) => ({ ...s, firstKitten: true })); setTimeout(() => toast("\u{1F431}", `${r.baby.name.split(" ")[0]} is born! Your first kitten.`, "#4ade80", 5e3), 1200); }
      ...
      results.forEach((r) => {
        ...
        if (r.type === "death") {
          ...
          const vn = r.victim.epithet ? r.victim.name.split(" ")[0] + " " + r.victim.epithet : r.victim.name.split(" ")[0];
          ...
          if (isDailyRun) {
            ...
            toast("\u{1F480}", `${vn} is gone. +${dustGain}\u2726`, "#ef4444", 3e3);
          } else {
            setDeathChoiceQueue((q) => [...q, { victim: r.victim, vn, carrier, dustGain: (r.victim.stats?.tp || 0) >= 5 ? 2 : 1 }]);
          }
      ...
      setDenRes(results); if (results.some((r) => r.type === "breed")) { setDenBirthPause(true); setTimeout(() => setDenBirthPause(false), 2000); }
      setDen([]);
      setPh("denResults");
      if (meta) {
    ...
    function chooseEvent(idx) {
      ...
      if (fx.narrowWeak || fx.narrowStrong) {
        const allNow = [...hand, ...draw, ...disc];
        const sorted = [...allNow].sort((a, b) => fx.narrowWeak ? a.power - b.power : b.power - a.power);
      ...
      if (fx.offeringScar) {
        const scarred = [...hand, ...draw, ...disc].filter((c) => c.scarred);
        const target = pk(scarred);
        if (target) {
          [setHand, setDraw, setDisc].forEach((s) => {
            s((arr) => arr.map((x) => x.id === target.id ? { ...x, power: Math.max(1, x.power - 2), story: [...(x.story || []).slice(-3), "Gave a scar to the dark. The price of safe passage."] } : x));
          });
          toast("\u{1F56F}\uFE0F", `${target.name.split(" ")[0]} paid the price. The dark accepted.`, "#ef4444", 3e3);
        }
      }
    ...
    async function buyUpg(u) {
      ...
      const tierReqMap = { 1: 0, 2: 1, 3: 3, 4: 5, 5: 7 };
      if (branchLvl < tierReqMap[u.tier]) return;
      ...
      if (u.tier >= 4 && !primary) {
        setDedicateModal(u.branch);
        return;
      }
      if (u.cost >= 100 && !u._confirmed) {
        u._confirmed = true;
        toast("\u2726", `Buy ${u.name} for ${u.cost}\u2726? Tap again to confirm.`, SKILL_BRANCHES[u.branch]?.color || "#c084fc", 3e3);
        setTimeout(() => { u._confirmed = false; }, 4e3);
        return;
      }
      u._confirmed = false;
      const um = { ...meta, dust: (meta.dust || 0) - u.cost, ups: { ...meta.ups, [u.id]: cur + 1 } };
      setMeta(um);
      await saveS(um);
      const B = SKILL_BRANCHES[u.branch];
      if (u.tier >= 5) {
        Haptic.heavy();
        toast(B?.icon || "\u2726", u.name + " \u2605 " + (B?.apexLore || u.desc), B?.color || "#c084fc", 7e3);
      } else {
        Haptic.medium();
        toast(B?.icon || "\u2726", u.flavor || u.name + " acquired.", B?.color || "#c084fc", 3e3);
      }
      const tierUps2 = UPGRADES.filter((x) => x.branch === u.branch && x.tier === u.tier);
      const tierDone = tierUps2.every((x) => (um.ups[x.id] || 0) >= x.max);
      const TIER_N = { flame: { 1: "SPARK", 2: "BLAZE", 3: "INFERNO", 4: "FIRESTORM", 5: "NINTH FIRE" }, shelter: { 1: "ROOTS", 2: "WALLS", 3: "HAVEN", 4: "FORTRESS", 5: "SANCTUARY" }, memory: { 1: "GLIMPSE", 2: "ECHO", 3: "VISION", 4: "ORACLE", 5: "OMNISCIENCE" } };
      if (tierDone) setTimeout(() => toast("\u2713", TIER_N[u.branch]?.[u.tier] + " complete!", B?.color || "#4ade80", 2500), 1500);
      const branchDone = UPGRADES.filter((x) => x.branch === u.branch).every((x) => (um.ups[x.id] || 0) >= x.max);
      if (branchDone) setTimeout(() => toast(B?.icon || "\u2726", B.masteryLine || B.name + " MASTERED.", B?.color || "#4ade80", 6e3), 2500);
    }
    ...
    *{box-sizing:border-box;-webkit-tap-highlight-color:transparent}
    ...
    if (ph === "naming" && namingCat) {
      if (meta && meta.stats.r === 1 && !seen.namingFrame) {
        ...
        setTimeout(() => toast("\u{1F56F}\uFE0F", "Names have power here. The cycle has run eight times. Names are how you break it.", "#fbbf24", 5e3), 600);
      }
    ...
    if (ph === "nightCard" && nightCard) {
      ...
      return /* @__PURE__ */ React.createElement("div", { onClick: ncAdvance, style: { ...W, cursor: "pointer" } }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: {
        ...
        return /* @__PURE__ */ React.createElement(React.Fragment, null, showWhisper && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf2466", fontStyle: "italic", textAlign: "center", maxWidth: 300, lineHeight: 1.5, animation: "fadeIn 2s ease-out .5s both", marginTop: 8 } }, wLine), narLine && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#c084fc55", fontStyle: "italic", textAlign: "center", maxWidth: 320, lineHeight: 1.6, animation: "fadeIn 2.5s ease-out 1s both", marginTop: showWhisper ? 4 : 8, padding: "6px 12px", borderRadius: 8, background: "#c084fc06", border: "1px solid #c084fc11" } }, narLine), (() => {
          ...
          if (overPlayed.length === 1) return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff44", fontStyle: "italic", textAlign: "center", maxWidth: 300, lineHeight: 1.5, animation: "fadeIn 3s ease-out 1.5s both", marginTop: 4 } }, "The same group again. ", overPlayed[0].name.split(" ")[0], " carries every play. The others watch.");
    ...
    if (ph === "colonyFormed" && colonyData) {
      ...
      const enterNight = () => {
        try {
          ...
          if (mb.devMemory) {
            const ms = DEVOTION_MILESTONES[mb.devMemory];
            if (ms && ms[0]) setTimeout(() => toast(BREEDS[mb.devMemory]?.icon || "\u2726", `${ms[0].name}: ${ms[0].desc}. The cycle remembers what you've learned.`, BREEDS[mb.devMemory]?.color || "#fbbf24", 4000), 800);
          }
          if (mb.devMemoryAlt) {
            const alt = (meta?.stats?.r || 0) % 2 === 0 ? "Autumn" : "Winter";
            const ms = DEVOTION_MILESTONES[alt];
            if (ms && ms[0]) setTimeout(() => toast(BREEDS[alt]?.icon || "\u2726", `${ms[0].name}: ${ms[0].desc}. A season's echo carries forward.`, BREEDS[alt]?.color || "#fbbf24", 4000), 800);
          }
        } catch (e) {
      ...
      if (!chosen || chosen.length === 0) return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: mob ? "flex-start" : "center", minHeight: "100vh", zIndex: 1, gap: mob ? 10 : 14, padding: mob ? "30px 16px 20px" : 20, overflowY: "auto", maxHeight: "100vh" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: "#fbbf24" } }, "Colony formed."), /* @__PURE__ */ React.createElement("button", { onClick: enterNight, style: { ...BTN("linear-gradient(135deg,#fbbf24,#f59e0b)", "#0a0a1a"), padding: "14px 44px", fontSize: 17, letterSpacing: 4 } }, "Enter the Night")));
      ;
      ...
      })), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#999999aa", fontStyle: "italic", textAlign: "center", lineHeight: 1.6, maxWidth: 320, marginTop: 12, animation: "fadeIn 1.2s ease-out 1.5s both", padding: "8px 14px", borderRadius: 10, background: "#ffffff04", border: "1px solid #ffffff08" } }, "The dark runs in cycles. Eight colonies fell to the pattern. You are the ninth. Break it.")) : /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#999999aa", letterSpacing: 6, animation: "epicReveal 1.2s ease-out forwards" } }, "SURVIVE ", MX, " NIGHTS"), meta && meta.stats.r > 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#777777bb", letterSpacing: 3 } }, (() => {
      ...
      })()), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#888", letterSpacing: 4, animation: "fadeIn .6s ease-out .2s both" } }, "THE COLONY"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 15, color: "#d97706bb", fontStyle: "italic", textAlign: "center", lineHeight: 1.6, maxWidth: 360, animation: "fadeIn .8s ease-out .15s both" } }, colonyThesis), bloodMemMsg && /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 16px", borderRadius: 8, background: "linear-gradient(135deg,#7a665208,#ef444408)", border: "1px solid #c084fc22", animation: "fadeIn 1s ease-out .3s both", textAlign: "center", maxWidth: 380 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#c084fc", fontStyle: "italic", lineHeight: 1.6 } }, "\u{1FA78} ", /* @__PURE__ */ React.createElement("b", null, bloodMemMsg.heir.split(" ")[0]), " carries something old. ", bloodMemMsg.trait.icon, " ", /* @__PURE__ */ React.createElement("span", { style: { color: "#e8e6e3" } }, bloodMemMsg.trait.name), ". inherited from ", /* @__PURE__ */ React.createElement("span", { style: { color: "#fbbf24" } }, bloodMemMsg.ancestor), " of the Hearth.", bloodMemMsg.scarred && /* @__PURE__ */ React.createElement("span", { style: { color: "#d97706" } }, " The scar came with it."))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 20, justifyContent: "center", animation: "fadeIn 1s ease-out .25s both" } }, /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 32, fontWeight: 900, color: "#fbbf24", letterSpacing: 2, animation: "comboBurst .6s ease-out .35s both" } }, 14), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#666", letterSpacing: 2 } }, "SOULS"), false), /* @__PURE__ */ React.createElement("div", { style: { width: 1, height: 40, background: "#ffffff0a" } }), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 32, fontWeight: 900, color: "#ef4444", letterSpacing: 2, animation: "comboBurst .6s ease-out .45s both" } }, MX), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#666", letterSpacing: 2 } }, "NIGHTS"))), /* @__PURE__ */ React.createElement("div", { style: { width: "100%", marginTop: 4 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#4ade80", letterSpacing: 2, marginBottom: 6, textAlign: "center" } }, "CHOSEN. YOUR EDGE"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10, justifyContent: "center", animation: "fadeIn .6s ease-out .6s both" } }, chosen.map((c) => /* @__PURE__ */ React.createElement("div", { key: c.id, style: { textAlign: "center", cursor: "pointer" }, onClick: () => setTraitTip(c) }, /* @__PURE__ */ React.createElement(CC, { cat: c, hl: true }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: BREEDS[c.breed]?.color, marginTop: 2, fontWeight: 700 } }, c.name.split(" ")[0]), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888" } }, c.trait.name !== "Plain" ? c.trait.icon + " " + c.trait.name : "plain"))))), false && /* @__PURE__ */ React.createElement("div", { style: { width: "100%", marginTop: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888", letterSpacing: 2, marginBottom: 6, textAlign: "center" } }, "THE ONES WHO WERE ALREADY HERE"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, justifyContent: "center", animation: "fadeIn 1s ease-out .9s both" } }, BK.map((b) => {
    ...
    if (ph === "title") {
      ...
      if (showUpgrades) availTabs.push("✦ skill tree");
      ...
      return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement(Dust, null), /* @__PURE__ */ React.createElement("img", { src: "https://raw.githubusercontent.com/greatgamesgonewild/ninth-life/main/hero.png", alt: "", style: { position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 800, opacity: 0.42, pointerEvents: "none", zIndex: 0, maskImage: "linear-gradient(to top, rgba(0,0,0,0.6) 0%, rgba(0,0,0,0.3) 40%, transparent 80%)", WebkitMaskImage: "linear-gradient(to top, rgba(0,0,0,0.6) 0%, rgba(0,0,0,0.3) 40%, transparent 80%)" }, onError: (e) => { if (!e.target._retry) { e.target._retry = true; e.target.src = "https://greatgamesgonewild.github.io/ninth-life/hero.png"; } else { e.target.style.display = "none"; } } }), /* @__PURE__ */ React.createElement("style", null, CSS), (() => {
      ...
      })(), hasRun && meta && !mob && (meta.codex || []).length > 0 && /* @__PURE__ */ React.createElement("div", { onClick: () => { const cx = meta.codex || []; const tot = BK.length * (TRAITS.length + 1); const pct = Math.round(cx.length / tot * 100); const frags = (meta.loreFragments || []).length; const byBreed = BK.map((b) => ({ b, n: cx.filter((k) => k.startsWith(b)).length })); toast("\u{1F4D6}", `Codex: ${cx.length}/${tot} (${pct}%). ` + byBreed.map((x) => `${BREEDS[x.b].icon}${x.n}`).join(" ") + (frags > 0 ? ` \xB7 ${frags} lore fragments` : ""), "#c084fc", 5e3); }, style: { display: "flex", alignItems: "center", gap: 6, cursor: "pointer", animation: "fadeIn 2.5s ease-out" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#c084fc88", letterSpacing: 2 } }, "\u{1F4D6} CODEX ", (meta.codex || []).length, "/", BK.length * (TRAITS.length + 1), (meta.loreFragments || []).length > 0 ? ` \xB7 ${(meta.loreFragments || []).length}\u{1F4DC}` : ""), /* @__PURE__ */ React.createElement("div", { style: { width: 40, height: 3, background: "#1a1a2e", borderRadius: 2, overflow: "hidden" } }, /* @__PURE__ */ React.createElement("div", { style: { height: "100%", width: `${(meta.codex || []).length / (BK.length * (TRAITS.length + 1)) * 100}%`, background: "#c084fc", borderRadius: 2 } }))), hasRun && meta && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 2 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 12, alignItems: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { color: "#c084fc", fontSize: 13, fontWeight: 700 } }, (() => { const pb = meta?.primaryBranch; const PB = pb ? SKILL_BRANCHES[pb] : null; return PB ? PB.icon + " " : "\u2726 "; })(), sd, " Stardust"), sd > 0 && !showUpgrades && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#c084fc88" } }, "earned from the Hearth"), sd > 0 && showUpgrades && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#c084fc88", cursor: "pointer" }, onClick: () => setTab("✦ skill tree") }, "spend on upgrades \u25B8"), meta.cats.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#c084fcbb" } }, "\u{1F3E0} +", calcTotalHearthDust(meta.cats, getMB().dustBonus || 0, getHeatFx(meta?.heat).dustMult || 1).total, "/run"), /* @__PURE__ */ React.createElement("span", { onClick: () => setTab("\u{1F3E0} hearth"), style: { fontSize: 10, color: "#fbbf24aa", cursor: "pointer", padding: "1px 6px", borderRadius: 4, background: "#fbbf2408", border: "1px solid #fbbf2418" } }, meta.cats.length, " cats \u25B8"))), showHeat && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 4 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 2, alignItems: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#ef4444" } }, "Heat ", meta.heat), meta.heat > 0 && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#4ade80" } }, "+", meta.heat * 25, "% hearth")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 3, alignItems: "center" } }, Array.from({ length: (meta.stats.mh || meta.heat || 1) + 1 }).map((_, h) => {
        ...
        return availTabs.map((t) => {
          ...
          const hasGlow = isUpg && canAffordUpgrade && safeTab !== "skill tree";
        ...
        return /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 4, marginTop: 2 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => {
          Audio.init();
          startDailyRun();
        }, disabled: played, style: { ...BTN(played ? "#1a1a2e" : "linear-gradient(135deg,#67e8f9,#3b82f6)", played ? "#67e8f988" : "#0a0a1a"), padding: "8px 28px", fontSize: 12, letterSpacing: 3, textTransform: "uppercase", border: played ? "1px solid #67e8f933" : "none", opacity: played ? 0.6 : 1 } }, played ? "Daily Complete" : "\u2600\uFE0F Daily Colony"), played && todayScore > 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#67e8f9aa" } }, "Today: ", todayScore.toLocaleString(), " \xB7 +", Math.floor(todayScore / 1e4), "\u2726", streak > 1 ? ` \xB7 ${streak} day streak` : ""), !played && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: "#67e8f966" } }, "Score \u2192 stardust. Lore unlocks. One attempt.", allTimeBest > 0 ? ` Best: ${allTimeBest.toLocaleString()}` : "", streak > 1 ? ` \xB7 ${streak} day streak` : ""), (meta?.dailyLore || []).length > 0 && (meta?.dailyLore || []).length < COLONY_RECORDS.length && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 8, color: "#d4a54a44" } }, "\u{1F4DC} ", (meta.dailyLore || []).length, "/", COLONY_RECORDS.length, " colony records"), played && /* @__PURE__ */ React.createElement("button", { onClick: () => {
...
"${r.flavor}"` }, r.icon, " ", r.name) : null;
        ...
        const branchMax = (br) => UPGRADES.filter((u) => u.branch === br).reduce((s, u) => s + u.max, 0);
        const tierReq = { 1: 0, 2: 1, 3: 3, 4: 5, 5: 7 };
        return /* @__PURE__ */ React.createElement(React.Fragment, null,
          /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", marginBottom: 4 } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff33", letterSpacing: 4, marginBottom: 2 } }, "THE HEARTH PATH"),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: meta.primaryBranch ? SKILL_BRANCHES[meta.primaryBranch].color + "44" : "#ffffff15", fontStyle: "italic", marginBottom: 4, lineHeight: 1.4, maxWidth: 280, textAlign: "center" } }, meta.primaryBranch ? SKILL_BRANCHES[meta.primaryBranch].lore : totalLevels > 0 ? "Every colony chose how it would survive. Yours chooses now." : "Eight colonies tried. Eight fell. The ninth path is yours to walk."),
            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "center", gap: 4, alignItems: "baseline" } },
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 18, fontWeight: 900, color: "#c084fc" } }, "\u2726 ", meta.dust || 0),
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#c084fc66" } }, "stardust")),
            totalLevels === 0 && (meta.dust || 0) === 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff22", marginTop: 6, lineHeight: 1.5, maxWidth: 280 } }, "Win runs and save cats to the Hearth to earn stardust. Play daily runs for bonus harvest."),
            totalLevels === 0 && (meta.dust || 0) > 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#c084fc66", marginTop: 6, lineHeight: 1.5, maxWidth: 280 } }, "Choose your path. Each branch deepens as you invest. Deeper tiers unlock with more upgrades in that branch."),
            (() => {
              const ab = [];
              if (mb.hands > 0) ab.push({ l: "+" + mb.hands + "H", c: "#ef4444" });
              if (mb.gold > 0) ab.push({ l: "+" + mb.gold + "\u{1F41F}", c: "#fbbf24" });
              if (mb.fervor > 0) ab.push({ l: "+" + mb.fervor + "N", c: "#f59e0b" });
              if (mb.freeRecruits > 0) ab.push({ l: "+" + mb.freeRecruits + "R", c: "#c084fc" });
              if (mb.shelter > 0) ab.push({ l: "+" + mb.shelter + "Den", c: "#4ade80" });
              if (mb.scarMult > 0) ab.push({ l: "Scar\xD7", c: "#ef4444" });
              if (mb.bondBoost > 0) ab.push({ l: "Bond+", c: "#f472b6" });
              ...
              if (mb.eternalHearth > 0) ab.push({ l: "Inherit", c: "#4ade80" });
            ...
            totalLevels > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "center", gap: 12, marginTop: 6, alignItems: "center" } },
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 9, color: "#ffffff22" } }, totalLevels, " owned"),
              /* @__PURE__ */ React.createElement("span", { onClick: () => {
                const refund = UPGRADES.reduce((s, u) => s + (meta.ups?.[u.id] || 0) * Math.floor(u.cost * 0.5), 0);
                if (refund <= 0) return;
                if (confirm("The colony forgets what it learned. The stardust scatters.\n\nRespec all upgrades? You receive " + refund + "\u2726 (50% refund).")) {
                  const nm = { ...meta, dust: (meta.dust || 0) + refund, ups: {}, primaryBranch: null };
                  setMeta(nm); saveS(nm);
                  toast("\u2726", "The path dissolves. " + refund + "\u2726 return to the fire. Choose again.", "#c084fc", 4e3);
                  Haptic.medium();
                }
              }, style: { fontSize: 9, color: "#ef444466", cursor: "pointer" } }, "respec"), (meta.dust || 0) >= 200 && (meta.prestige || 0) < 3 && /* @__PURE__ */ React.createElement("span", { onClick: () => { if (confirm("Sacrifice 200\u2726 for a permanent +5% score bonus? Currently " + (meta.prestige || 0) + "/3.")) { setMeta((m) => { const nm = { ...m, dust: (m.dust || 0) - 200, prestige: Math.min(3, (m.prestige || 0) + 1) }; saveS(nm); return nm; }); toast("\u2726", "Prestige " + ((meta.prestige || 0) + 1) + "/3! Permanent +5% score.", "#fef08a", 4e3); } }, style: { fontSize: 9, color: "#fef08a66", cursor: "pointer" } }, "prestige +5%"), (meta.prestige || 0) > 0 && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 9, color: "#fef08a44" } }, "\u2726 +" + (meta.prestige || 0) * 5 + "% prestige"))
          ),
          branches.map((br) => {
            const B = SKILL_BRANCHES[br];
            const bLvl = branchLevels(br);
            const bMax = branchMax(br);
            const tiers = [1, 2, 3, 4, 5];
            const allMaxed = bLvl >= bMax;
            const hasAffordable = UPGRADES.filter((u) => u.branch === br).some((u) => {
              const o = meta.ups?.[u.id] || 0;
              return o < u.max && bLvl >= tierReq[u.tier] && (meta.dust || 0) >= u.cost;
            });
            const TIER_NAMES = {
              flame: { 1: "SPARK", 2: "BLAZE", 3: "INFERNO", 4: "FIRESTORM", 5: "NINTH FIRE" },
              shelter: { 1: "ROOTS", 2: "WALLS", 3: "HAVEN", 4: "FORTRESS", 5: "SANCTUARY" },
              memory: { 1: "GLIMPSE", 2: "ECHO", 3: "VISION", 4: "ORACLE", 5: "OMNISCIENCE" }
            };
            const pct = bMax > 0 ? Math.round(bLvl / bMax * 100) : 0;
            return /* @__PURE__ */ React.createElement("div", { key: br, style: { marginBottom: 2 } },
              ...
              openBranch !== br && !allMaxed && (() => {
                const primary = meta.primaryBranch || null;
                const next = UPGRADES.filter((u) => u.branch === br && (meta.ups?.[u.id] || 0) < u.max).sort((a, b) => a.tier - b.tier || a.cost - b.cost)[0];
                if (!next) return null;
                const brLvl = branchLevels(br);
                const tReq = { 1: 0, 2: 1, 3: 3, 4: 5, 5: 7 };
                const locked = brLvl < tReq[next.tier];
                const dedBlocked = next.tier >= 4 && primary && primary !== br;
                const can = !locked && !dedBlocked && (meta.dust || 0) >= next.cost;
                return /* @__PURE__ */ React.createElement("div", { style: { padding: "3px 12px 6px", fontSize: 9, color: dedBlocked ? "#ef444444" : can ? B.color + "aa" : "#ffffff22" } }, dedBlocked ? "\u{1F512} Dedicated elsewhere" : can ? "\u25B8 " + next.name + " (\u2726" + next.cost + ")" : locked ? "\u25B8 " + next.name + " (locked)" : "\u25B8 " + next.name + " (\u2726" + next.cost + " \u2022 need " + ((meta.dust || 0) < next.cost ? next.cost - (meta.dust || 0) + "\u2726" : "unlock") + ")");
              })(),
              (openBranch === br) && /* @__PURE__ */ React.createElement("div", { style: { padding: "4px 0 8px 0", borderRadius: "0 0 10px 10px", background: B.color + "04", borderLeft: `1.5px solid ${B.color}22`, borderRight: `1.5px solid ${B.color}22`, borderBottom: `1.5px solid ${B.color}22`, animation: "fadeIn .2s ease-out" } },
                /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: B.color + "66", fontStyle: "italic", textAlign: "center", padding: "4px 12px 6px", lineHeight: 1.4 } }, B.desc),
                tiers.map((t) => {
...
Saved from Night ${c.fromAnte || "?"}`, style: {
      ...
      })), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#999999aa", textAlign: "center", marginTop: 8, fontStyle: "italic" } }, "The cycle breaks. Nine colonies. Eight fell to feed the dark its patterns. The ninth refused. The dark has no playbook for what comes next."), (meta.dailyLore || []).length > 0 && /* @__PURE__ */ React.createElement("div", { style: { width: "100%", marginTop: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#d4a54a88", letterSpacing: 3, textAlign: "center", marginBottom: 6, fontWeight: 700 } }, "\u{1F4DC} THE RECORD OF NINE (", (meta.dailyLore || []).length, "/", COLONY_RECORDS.length, ")"), (meta.dailyLore || []).map((rec, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { fontSize: 10, color: "#d4a54a88", fontStyle: "italic", lineHeight: 1.6, padding: "6px 10px", borderRadius: 6, background: "#d4a54a06", border: "1px solid #d4a54a11", marginBottom: 3 } }, rec)), (meta.dailyLore || []).length < COLONY_RECORDS.length && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: "#ffffff22", textAlign: "center", marginTop: 4 } }, "Complete daily runs to discover more records. ", COLONY_RECORDS.length - (meta.dailyLore || []).length, " remaining.")))), safeTab === "bestiary" && meta && /* @__PURE__ */ React.createElement("div", { style: { width: "100%", display: "flex", flexDirection: "column", gap: 8, animation: "fadeIn .3s ease-out" } }, (() => {
        const br = meta.stats.bossRecord || {};
    ...
    if (ph === "event" && colEvent) {
      ...
      const EVT_BG_BASE = "https://raw.githubusercontent.com/greatgamesgonewild/ninth-life/main/events/";
      const t = evt.title || "";
      const evtBgKey = t.includes("Storm") ? "storm" : t.includes("Fire") || t.includes("Feast") || t.includes("Cache") || t.includes("Ration") || t.includes("Debt") ? "feast" : t.includes("Scar") || t.includes("Elder") || t.includes("Historian") || t.includes("Blood") || t.includes("Hardened") ? "scarkeeper" : t.includes("Shrine") || t.includes("Sacred") || t.includes("Moon") || t.includes("Quiet") || t.includes("Colony's") || t.includes("Name") || t.includes("Wall") ? "shrine" : t.includes("Wager") || t.includes("Challenge") || t.includes("Pact") || t.includes("Gambl") ? "wager" : t.includes("Stranger") || t.includes("Wander") || t.includes("Stray") || t.includes("Sickness") || t.includes("Lifeboat") ? "wanderer" : t.includes("Wind") || t.includes("Frost") || t.includes("Growth") || t.includes("Spring") || t.includes("Summer") || t.includes("Autumn") || t.includes("Winter") ? "shrine" : "feast";
      const evtBgUrl = EVT_BG_BASE + evtBgKey + ".png";
      return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("img", { src: evtBgUrl, alt: "", style: { position: "fixed", top: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 800, height: "100vh", objectFit: "cover", objectPosition: "center 30%", opacity: 0.32, pointerEvents: "none", zIndex: 0, maskImage: "radial-gradient(ellipse at 50% 30%, rgba(0,0,0,0.5) 0%, transparent 70%)", WebkitMaskImage: "radial-gradient(ellipse at 50% 30%, rgba(0,0,0,0.5) 0%, transparent 70%)" }, onError: (e) => { e.target.style.display = "none"; } }), /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", top: "12%", left: "50%", transform: "translateX(-50%)", width: 350, height: 350, borderRadius: "50%", background: `radial-gradient(circle,${evtGlow}06,transparent 70%)`, pointerEvents: "none", zIndex: 0 } }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: mob ? "flex-start" : "center", minHeight: "100vh", zIndex: 1, gap: 0, padding: mob ? "40px 16px 20px" : 20, maxWidth: 480, overflowY: "auto", maxHeight: "100vh" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff55", letterSpacing: 8, marginBottom: 16, animation: "fadeIn 1.2s ease-out", textTransform: "uppercase" } }, isDailyRun ? "\u2600\uFE0F DAILY \xB7 " : "", "Night ", ante, " ", "\xB7", " ", ["Dusk", "Midnight", "After the Boss"][blind]), /* @__PURE__ */ React.createElement("div", { style: { fontSize: mob ? 44 : 64, marginBottom: mob ? 4 : 8, animation: "fadeIn .6s ease-out", filter: `drop-shadow(0 0 40px ${evtGlow}22)` } }, evt.icon), /* @__PURE__ */ React.createElement("h2", { style: { fontSize: mob ? 16 : 20, color: "#e8e6e3", letterSpacing: mob ? 4 : 6, margin: "0 0 6px 0", fontWeight: 600, animation: "fadeIn .8s ease-out" } }, evt.title), evt.tag && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: evtGlow + "66", letterSpacing: 4, padding: "3px 12px", borderRadius: 12, border: `1px solid ${evtGlow}15`, marginBottom: 16, animation: "fadeIn 1s ease-out", textTransform: "uppercase" } }, evt.tag), /* @__PURE__ */ React.createElement("div", { style: { fontSize: mob ? 13 : 15, color: "#c8c2bbdd", textAlign: "center", lineHeight: mob ? 1.7 : 2.1, maxWidth: mob ? 320 : 360, fontStyle: "italic", animation: "fadeIn 1.2s ease-out", marginBottom: 16, padding: "0 8px" } }, evtText), meta && meta.cats.length > 0 && tgts.length > 0 && (() => {
        ...
        return /* @__PURE__ */ React.createElement("button", { key: i, onClick: () => canAfford && chooseEvent(i), style: {
          padding: mob ? "12px 16px" : "16px 22px",
    ...
    if (ph === "overflow" && oData) {
      ...
      return /* @__PURE__ */ React.createElement("div", { style: { ...W, animation: clutch ? "flash .6s ease-out" : "none" } }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), clutch && /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", inset: 0, zIndex: 50, background: "radial-gradient(circle,#fbbf2433,transparent 70%)", pointerEvents: "none", animation: "flash 1.5s ease-out" } }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", zIndex: 1, gap: isQuickClear ? 6 : 10, padding: 20 } }, !isQuickClear && clutch && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 22, fontWeight: 900, color: "#fbbf24", letterSpacing: 8, textShadow: "0 0 30px #fbbf24cc", animation: "clutchBurst .8s ease-out", marginBottom: 4 } }, "CLUTCH"), !isQuickClear && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: blind >= 2 ? "#4ade80" : "#fbbf24", letterSpacing: 3 } }, blind >= 2 ? clutch ? "Survived. Barely." : "Survived." : "Cleared."), isFirstRun && ante === 1 && blind === 0 && !seen.shop && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf24aa", textAlign: "center", maxWidth: 300, lineHeight: 1.6, animation: "fadeIn 1s ease-out" } }, "Round 1 of 3 done. Midnight next, then the Boss.", /* @__PURE__ */ React.createElement("br", null), "\u{1F41F} = rations. Spend them to get stronger."), clearLine && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "#ffffffbb", fontStyle: "italic", textAlign: "center", maxWidth: 320, lineHeight: 1.6, animation: "fadeIn 1.2s ease-out", textShadow: "0 0 15px #ffffff11" } }, clearLine), blind >= 2 && boss && (() => {
        ...
        const contextLine = (() => {
          ...
          const mvpCat = allC.reduce((best, c) => (c.stats?.bs || 0) > (best?.stats?.bs || 0) ? c : best, allC[0]);
          const scarCats = allC.filter(c => c.scarred);
          const bondedCats = allC.filter(c => c.bondedTo);
          if (mvpCat && (mvpCat.stats?.tp || 0) >= 8) return `${mvpCat.name.split(" ")[0]} scored ${(mvpCat.stats.bs || 0).toLocaleString()} at their peak. The colony leans on that.`;
          if (scarCats.length >= 3) return `${scarCats.length} scarred. ${scarCats.length} still fighting. The dark miscounted.`;
          if (bondedCats.length >= 4) return `${Math.floor(bondedCats.length / 2)} bonded pairs. The colony doesn't fight for survival anymore. It fights for each other.`;
          if (fallen.length === 0 && ante >= 3) return `Night ${ante}. Not a single name lost. The Third Colony's leader would weep.`;
          return null;
        })();
        const line = contextLine || (moodLine ? moodLine : seed < 3 && whisperPool ? whisperPool[seed % whisperPool.length] : overflowLines[((ante - 1) * 3 + blind) % overflowLines.length]);
    ...
    if (ph === "defeat" && defeatData) {
      ...
      const mvpName = defMvp ? defMvp.name + (defMvp.epithet ? " " + defMvp.epithet : "") : "someone";
      const shareText = `\u{1F431} Ninth Life
${mvpName} led ${allC.length + fallen.length} cats into the dark.${fallen.length > 0 ? `
${fallen.length} didn't come back.` : `
All of them made it.`}
...
\u2192 https://greatgamesgonewild.github.io/ninth-life/`;
      return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: mob ? "flex-start" : "center", minHeight: "100vh", zIndex: 1, gap: mob ? 8 : 12, padding: mob ? "30px 16px 20px" : 20, maxWidth: 500, overflowY: "auto", maxHeight: "100vh" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf24aa", letterSpacing: 4, animation: "fadeIn .8s ease-out" } }, "COLONY #", (meta?.stats?.r || 0) + 1, " \xB7 NIGHT ", ante), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 36, fontWeight: 900, color: "#e8e6e3", letterSpacing: 2, animation: "fadeIn 1s ease-out" } }, defeatData.score.toLocaleString()), defeatData.target > 0 && /* @__PURE__ */ React.createElement("div", { style: { width: Math.min(280, vw - 60), height: 6, background: "#1a1a2e", borderRadius: 3, overflow: "hidden", border: "1px solid #ffffff08", animation: "fadeIn 1.2s ease-out" } }, /* @__PURE__ */ React.createElement("div", { style: { height: "100%", width: `${Math.min(100, defeatData.score / defeatData.target * 100)}%`, borderRadius: 3, background: defeatData.score >= defeatData.target * 0.9 ? "linear-gradient(90deg,#fbbf24,#ef4444)" : "linear-gradient(90deg,#ef4444,#ef444488)", transition: "width 1s ease-out" } })), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "center", animation: "fadeIn 1.2s ease-out" } }, /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", padding: "4px 10px", borderRadius: 6, background: "#ffffff04", border: "1px solid #ffffff08", minWidth: 60 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 900, color: "#3b82f6" } }, handsPlayed), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 8, color: "#666", letterSpacing: 1 } }, "HANDS")), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", padding: "4px 10px", borderRadius: 6, background: "#ffffff04", border: "1px solid #ffffff08", minWidth: 60 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 900, color: "#c084fc" } }, bestHandScore.toLocaleString()), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 8, color: "#666", letterSpacing: 1 } }, "BEST HAND")), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", padding: "4px 10px", borderRadius: 6, background: "#ffffff04", border: "1px solid #ffffff08", minWidth: 60 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 900, color: getNerve(rMaxF).color } }, getNerve(rMaxF).name), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 8, color: "#666", letterSpacing: 1 } }, "MORALE")), fallen.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", padding: "4px 10px", borderRadius: 6, background: "#ffffff04", border: "1px solid #ef444422", minWidth: 60 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 900, color: "#ef4444" } }, fallen.length), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 8, color: "#666", letterSpacing: 1 } }, "LOST"))), /* @__PURE__ */ React.createElement("div", { style: { width: 40, height: 1, background: "#ef444433", margin: "2px 0" } }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "#ef4444aa", fontStyle: "italic", textAlign: "center", maxWidth: 340, lineHeight: 1.7, animation: "fadeIn 1.8s ease-out" } }, defSentence), deficit > 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: deficit < defeatData.target * 0.15 ? "#fbbf2488" : "#ef444466", animation: "fadeIn 2s ease-out" } }, deficit < defeatData.target * 0.15 ? `Only ${deficit.toLocaleString()} short. That's one good hand.` : `${deficit.toLocaleString()} short of ${defeatData.target.toLocaleString()}`), runLog.length > 2 && /* @__PURE__ */ React.createElement("details", { style: { animation: "fadeIn 2.5s ease-out", maxWidth: 340, width: "100%" } }, /* @__PURE__ */ React.createElement("summary", { style: { fontSize: 10, color: "#666", cursor: "pointer", textAlign: "center", letterSpacing: 2 } }, "THE CHRONICLE"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 3, marginTop: 6, padding: "6px 10px", borderRadius: 8, background: "#ffffff04", border: "1px solid #ffffff08" } }, (() => {
        ...
        const tips = [];
        if (fams.length === 0) tips.push("Wards boost every play. Buy one early from the Market.");
        ...
        if (bestHt === "Stray" || bestHt === "Kin") tips.push("Colony (4 same season) scores 3\xD7 more than Kin. Draft one season.");
        if (fallen.length >= 3) tips.push("Camp builds Morale. Events and traits can heal injuries. Try camping before the boss.");
    ...
    if (ph === "gameOver" || ph === "victory") {
      ...
      return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("img", { src: SPLASH_BASE + (won ? "victory" : "defeat") + ".png", alt: "", style: { position: "fixed", top: won ? "auto" : 0, bottom: won ? 0 : "auto", left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 800, opacity: won ? 0.15 : 0.1, pointerEvents: "none", zIndex: 0, maskImage: won ? "linear-gradient(to top, rgba(0,0,0,0.5) 0%, rgba(0,0,0,0.2) 50%, transparent 80%)" : "linear-gradient(to bottom, rgba(0,0,0,0.4) 0%, rgba(0,0,0,0.15) 40%, transparent 70%)", WebkitMaskImage: won ? "linear-gradient(to top, rgba(0,0,0,0.5) 0%, rgba(0,0,0,0.2) 50%, transparent 80%)" : "linear-gradient(to bottom, rgba(0,0,0,0.4) 0%, rgba(0,0,0,0.15) 40%, transparent 70%)", animation: "fadeIn 2s ease-out" }, onError: (e) => { e.target.style.display = "none"; } }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: mob ? "flex-start" : "center", minHeight: "100vh", zIndex: 1, gap: mob ? 8 : 12, padding: mob ? "30px 16px 20px" : 20, maxWidth: 550, overflowY: "auto", maxHeight: "100vh" } }, vStep === 0 && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 48, animation: "comboBurst .8s ease-out" } }, won ? isNinthDawn ? "\u{1F305}" : "\u{1F451}" : "\u{1F480}"), /* @__PURE__ */ React.createElement("h2", { style: { fontSize: won && isNinthDawn ? 48 : 32, letterSpacing: won && isNinthDawn ? 16 : 8, margin: 0, color: won ? void 0 : "#ef4444", background: won ? "linear-gradient(180deg,#fef08a,#fbbf24)" : void 0, WebkitBackgroundClip: won ? "text" : void 0, WebkitTextFillColor: won ? "transparent" : void 0, animation: "comboBurst .6s ease-out", fontWeight: 700, textTransform: "uppercase", filter: won ? "drop-shadow(0 0 20px #fbbf2433)" : "none" } }, won ? isNinthDawn ? "BROKEN" : "THEY MADE IT" : "THE DARK WON"), won && !isNinthDawn && (() => {
        ...
        return /* @__PURE__ */ React.createElement("div", { style: { width: "100%", textAlign: "center" } }, isFirstHearthSave && picked.length === 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#c084fcbb", fontStyle: "italic", lineHeight: 1.6, marginBottom: 10, maxWidth: 360, margin: "0 auto 10px", padding: "10px 14px", borderRadius: 10, background: "#c084fc06", border: "1px solid #c084fc15" } }, "The Hearth is a small fire. It can only hold two names. Only those who bonded or raised kittens earned the right to stay. The dark takes the rest. Their children will begin the next colony."), !isFirstHearthSave && picked.length === 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#c084fcbb", fontStyle: "italic", lineHeight: 1.6, marginBottom: 10, maxWidth: 360, margin: "0 auto 10px", padding: "10px 14px", borderRadius: 10, background: "#c084fc06", border: "1px solid #c084fc15" } }, "The fire holds two. Only bonded or parent cats can stay. The dark takes the rest at dawn. Two is all the fire can hold."), picked.length === 1 && (() => {
          const p = picked[0];
      ...
      } }, /* @__PURE__ */ React.createElement("span", { style: { color: "#666", minWidth: 35 } }, "N", e.ante, ".", e.blind + 1), e.type === "trait" && /* @__PURE__ */ React.createElement("span", null, "\u2728 ", e.data.cat, " gained ", e.data.trait), e.type === "draft" && /* @__PURE__ */ React.createElement("span", null, "Drafted: ", e.data.picked, " (", e.data.rejects, " turned away)"), e.type === "hand" && /* @__PURE__ */ React.createElement("span", null, e.data.cats, " played ", e.data.type, " for ", e.data.score.toLocaleString()), e.type === "breed" && /* @__PURE__ */ React.createElement("span", null, e.data.baby, " was born to ", e.data.parents), e.type === "fight" && /* @__PURE__ */ React.createElement("span", null, e.data.loser, " was hardened in a fight"), e.type === "death" && /* @__PURE__ */ React.createElement("span", null, e.data.victim, " died"), e.type === "night" && /* @__PURE__ */ React.createElement("span", null, "Night ", e.data.to, " begins"), e.type === "phoenix" && /* @__PURE__ */ React.createElement("span", null, e.data.risen, " rose from ashes!"), e.type === "bond" && /* @__PURE__ */ React.createElement("span", null, e.data.c1, " bonded with ", e.data.c2), e.type === "training" && /* @__PURE__ */ React.createElement("span", null, e.data.c1, " and ", e.data.c2, " sparred"), e.type === "grudge" && /* @__PURE__ */ React.createElement("span", null, "\u26A1 Grudge: ", e.data.c1, " vs ", e.data.c2), e.type === "reconcile" && /* @__PURE__ */ React.createElement("span", null, "\u{1F54A}\uFE0F Peace: ", e.data.c1, "+", e.data.c2, e.data.bonded ? " \u{1F495}" : ""), e.type === "mentor" && /* @__PURE__ */ React.createElement("span", null, e.data.elder, " mentored ", e.data.young), e.type === "found" && /* @__PURE__ */ React.createElement("span", null, e.data.cat, " found +", e.data.gold, "\u{1F41F}"), e.type === "growth" && /* @__PURE__ */ React.createElement("span", null, e.data.cat, " grew stronger"), e.type === "wanderer" && /* @__PURE__ */ React.createElement("span", null, e.data.cat, " joined the colony"), e.type === "reward" && /* @__PURE__ */ React.createElement("span", null, "Claimed: ", e.data.name)))))), hearthPair === null && !colonyPortrait && /* @__PURE__ */ React.createElement("button", { onClick: () => setColonyPortrait(true), style: { ...BTN("linear-gradient(135deg,#fbbf24,#f59e0b)", "#0a0a1a"), padding: "10px 36px", fontSize: 15 } }, "Face the Cost"), colonyPortrait && (() => {
        const hearthNames2 = new Set((meta?.cats || []).map((c) => c.name));
        const leftBehind = allC.filter((c) => !hearthNames2.has(c.name));
        const saved = allC.filter((c) => hearthNames2.has(c.name));
        const allGone = [...leftBehind.map((c) => ({ name: c.name.split(" ")[0], breed: c.breed, epithet: c.epithet, type: "taken" })), ...fallen.map((f) => ({ name: f.name.split(" ")[0], breed: f.breed, epithet: f.epithet, type: "fallen" }))];
        const closingLine = saved.length >= 2 ? `Dawn was never safety. It was a window. ${saved.map((c) => c.name.split(" ")[0]).join(" and ")} reached the fire. The rest held the dark back long enough. That's how the cycle breaks. Not all at once. Two names at a time.` : saved.length === 1 ? `${saved[0].name.split(" ")[0]} reached the fire alone. Everyone else bought the time. One more name in the Hearth. One more crack in the cycle.` : `No one reached the fire. The cycle holds. But the dark had to fight harder this time. That counts.`;
        const copyTxt = `\u{1F431} Ninth Life \xB7 Colony #${meta?.stats?.r || "?"}\nReached the Hearth: ${saved.map((c) => c.name + (c.epithet ? " " + c.epithet : "")).join(", ") || "none"}\nTaken by the dark: ${allGone.map((c) => c.name).join(", ")}\n\u2192 https://greatgamesgonewild.github.io/ninth-life/`;
        return /* @__PURE__ */ React.createElement("div", { style: { width: "100%", maxWidth: 420, padding: "20px", borderRadius: 14, background: "linear-gradient(180deg, #0d0d1a, #000000)", border: "1.5px solid #ef444433", animation: "fadeIn 1.5s ease-out", textAlign: "center" } },
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ef444488", letterSpacing: 6, marginBottom: 12, fontWeight: 700 } }, "THE DARK CLOSES"),
          saved.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 16, padding: "12px 16px", borderRadius: 10, background: "linear-gradient(145deg,#fbbf2410,#fbbf2408)", border: "1.5px solid #fbbf2444", boxShadow: "0 0 30px #fbbf2418" } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: "#fbbf24", letterSpacing: 4, marginBottom: 6 } }, "\u{1F525} REACHED THE FIRE"),
            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10, justifyContent: "center" } }, saved.map((c) => /* @__PURE__ */ React.createElement("div", { key: c.id, style: { textAlign: "center", animation: "float 3s ease-in-out infinite" } },
              /* @__PURE__ */ React.createElement(CC, { cat: c, sm: true, hl: true }),
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: "#fbbf24", fontWeight: 700, marginTop: 2 } }, c.name.split(" ")[0]),
              c.epithet && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 8, color: "#fbbf2488" } }, c.epithet))))),
          allGone.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 12 } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: "#ef444466", letterSpacing: 3, marginBottom: 8 } }, "TAKEN BY THE DARK"),
            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 2, flexWrap: "wrap", justifyContent: "center" } }, allGone.map((c, i) => /* @__PURE__ */ React.createElement("span", { key: i, style: { fontSize: 10, color: c.type === "fallen" ? "#ef444466" : (BREEDS[c.breed]?.color || "#888") + "55", padding: "2px 5px", animation: `fadeIn ${1 + i * 0.12}s ease-out`, fontStyle: c.type === "fallen" ? "italic" : "normal" } }, c.name, c.epithet ? " " + c.epithet : "")))),
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#ffffff55", fontStyle: "italic", lineHeight: 1.8, marginBottom: 14, maxWidth: 340, margin: "0 auto 14px" } }, closingLine),
          /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, justifyContent: "center" } },
            /* @__PURE__ */ React.createElement("button", { onClick: () => { try { navigator.clipboard?.writeText(copyTxt); toast("\u{1F4CB}", "Farewell copied.", "#fbbf24", 2000); } catch(e) {} }, style: { ...BTN("#1a1a2e", "#666"), padding: "8px 14px", fontSize: 10, border: "1px solid #ffffff0a" } }, "Copy"),
            /* @__PURE__ */ React.createElement("button", { onClick: () => setPh("title"), style: { ...BTN("#1a1a2e", "#ef4444"), padding: "10px 28px", fontSize: 13, fontWeight: 700, border: "1px solid #ef444444", letterSpacing: 2 } }, "Let Go")));
      })(), hearthPair === null && (() => {
        const nightGrid = (() => {
          const sq = { "Autumn": "\u{1F7EB}", "Winter": "\u{1F7E6}", "Spring": "\u{1F7E9}", "Summer": "\u{1F7E7}" };
          const cts = {};
        ...
        const mvpFull = mvp ? mvp.name + (mvp.epithet ? " " + mvp.epithet : "") : "the colony";
        const vicShare = `\u{1F431} Ninth Life
${mvpFull} led ${allC.length + fallen.length} cats through ${ante} nights of dark.${fallen.length === 0 ? `
Every single one survived.` : `
${fallen.length} didn't make it. The rest carry their names.`}
${nightGrid} \xB7 ${(meta?.heat || 0) > 0 ? "Heat " + meta.heat + " \xB7 " : ""}${totalScored.toLocaleString()} scored
\u2192 https://greatgamesgonewild.github.io/ninth-life/`;
    ...
    if (ph === "denSelect") {
      ...
      return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start", minHeight: "100vh", zIndex: 1, gap: 10, padding: mob2 ? "16px 12px" : "20px", maxWidth: 600, paddingTop: mob2 ? 40 : 60, overflowY: "auto", maxHeight: "100vh" } }, /* @__PURE__ */ React.createElement("h2", { style: { fontSize: mob2 ? 18 : 20, color: campMode ? "#fbbf24" : "#c084fc", letterSpacing: 4, margin: 0 } }, campMode ? "\u{1F3D5} CAMP" : "THE DEN"), campMode && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#fbbf24", fontWeight: 700 } }, gold, " \u{1F41F}"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: mob2 ? 11 : 12, color: "#ffffff55", fontStyle: "italic", textAlign: "center", maxWidth: 320, lineHeight: 1.6 } }, campMode ? "Pick 2 cats for the watch. The rest sleep by the fire." : nightText), campMode ? /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 14px", borderRadius: 10, background: "#fbbf2408", border: "1px solid #fbbf2422", fontSize: 12, color: "#fbbf24cc", lineHeight: 1.7, textAlign: "center", maxWidth: 380, animation: "fadeIn .6s ease-out" } }, /* @__PURE__ */ React.createElement("b", null, "Pick 2 cats for the watch."), " Unbonded \u2642+\u2640 may bond. Grudged pairs may reconcile. Others gain +1 Power.") : !seen.den && /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 14px", borderRadius: 10, background: "#c084fc08", border: "1px solid #c084fc22", fontSize: 12, color: "#c084fccc", lineHeight: 1.7, textAlign: "center", maxWidth: 380, animation: "fadeIn .6s ease-out" } }, /* @__PURE__ */ React.createElement("b", null, "YOUR FIRST DEN"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#c084fccc", lineHeight: 1.6 } }, "Shelter a ", /* @__PURE__ */ React.createElement("span", { style: { color: "#60a5fa" } }, "\u2642 male"), " + ", /* @__PURE__ */ React.createElement("span", { style: { color: "#f472b6" } }, "\u2640 female"), " to breed a kitten."), (() => { const ac = [...hand, ...draw, ...disc].filter((c) => !c.injured); const sm = ac.filter((c) => c.sex === "M").sort((a, b) => b.power - a.power)[0]; const sf = ac.filter((c) => c.sex === "F").sort((a, b) => b.power - a.power)[0]; if (!denGuide && sm && sf) setTimeout(() => setDenGuide({ m: sm.id, f: sf.id }), 200); return sm && sf ? /* @__PURE__ */ React.createElement("div", { style: { marginTop: 4, padding: "6px 10px", borderRadius: 8, background: "#4ade800a", border: "1px solid #4ade8022", animation: "breathe 2s ease-in-out infinite" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: "#4ade80" } }, "\u{1F449} Tap ", /* @__PURE__ */ React.createElement("b", null, sm.name.split(" ")[0]), " \u2642 and ", /* @__PURE__ */ React.createElement("b", null, sf.name.split(" ")[0]), " \u2640 below")) : null; })(), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 4, fontSize: 10, color: "#ffffff44" } }, "Everyone else enters the wilds: grow, train, or fight."), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 4 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => { setSeen((s) => ({ ...s, den: true })); setDenGuide(null); }, style: { fontSize: 10, background: "none", border: "1px solid #c084fc33", borderRadius: 4, color: "#c084fc", cursor: "pointer", padding: "3px 10px" } }, "I know how this works"))), campMode ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { width: "100%", maxWidth: 480 } }, den.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, justifyContent: "center", alignItems: "center", marginBottom: 8, padding: "8px 12px", borderRadius: 10, background: "#fbbf2408", border: "1px solid #fbbf2422" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fbbf24", letterSpacing: 2, fontWeight: 700 } }, "\u{1F525} THE WATCH"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4 } }, den.map((c) => /* @__PURE__ */ React.createElement("div", { key: c.id, onClick: () => toggleDen(c), style: { cursor: "pointer", textAlign: "center" } }, /* @__PURE__ */ React.createElement(CC, { cat: c, sm: true, cw: denCardW, sel: true }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: BREEDS[c.breed]?.color || "#fbbf24", marginTop: 1, fontWeight: 700, lineHeight: 1 } }, c.name.split(" ")[0], " ", /* @__PURE__ */ React.createElement("span", { style: { color: c.sex === "M" ? "#60a5fa" : "#f472b6" } }, c.sex === "M" ? "\u2642" : "\u2640")))))), den.length < 2 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fbbf2466", textAlign: "center", marginBottom: 6 } }, den.length === 0 ? "Tap 2 cats below" : "Pick 1 more"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", justifyContent: "center" } }, (() => {
        ...
        return sorted.map((c) => {
          ...
          const isGuided = denGuide && (c.id === denGuide.m || c.id === denGuide.f);
          return /* @__PURE__ */ React.createElement("div", { key: c.id, onClick: den.length < MAX_ISOLATE ? () => { toggleDen(c); if (isGuided && den.length === 0) setTimeout(() => toast("\u{1F6E1}", "Now tap the other one to complete the pair!", "#4ade80", 2500), 300); if (isGuided && den.length === 1) { setSeen((s) => ({ ...s, den: true })); setDenGuide(null); setTimeout(() => toast("\u{1F495}", "Sheltered! They'll breed a kitten tonight.", "#f472b6", 3e3), 500); } } : void 0, style: { cursor: den.length < MAX_ISOLATE ? "pointer" : "default", opacity: den.length >= MAX_ISOLATE ? 0.6 : dimmed ? 0.35 : 1, position: "relative", textAlign: "center", animation: isGuided ? "guidePulse 1.5s ease-in-out infinite" : "none", boxShadow: isGuided ? "0 0 12px #4ade8044" : "none", borderRadius: 8 } }, /* @__PURE__ */ React.createElement(CC, { cat: c, sm: true, cw: denCardW, denMode: true, onTraitClick: (ct) => setTraitTip(ct) }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 8, color: BREEDS[c.breed]?.color || "#888", marginTop: 1, maxWidth: denCardW + 8, lineHeight: 1, whiteSpace: "nowrap" } }, c.name.split(" ")[0], " ", /* @__PURE__ */ React.createElement("span", { style: { color: c.sex === "M" ? "#60a5fa" : "#f472b6", fontWeight: 700 } }, c.sex === "M" ? "\u2642" : "\u2640")));
        });
    ...
    if (ph === "denResults" && denRes) {
      const mob2 = vw < 500;
      const n = (c) => c.epithet ? c.name.split(" ")[0] + " " + c.epithet : c.name.split(" ")[0];
      ...
      const hero = beats.length > 0 && beats[0].pri <= 1 ? beats[0] : null;
      const majors = hero ? beats.filter((b) => b.pri <= 1).slice(1) : beats.filter((b) => b.pri <= 1);
      const minors = beats.filter((b) => b.pri >= 2);
      return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start", minHeight: "100vh", zIndex: 1, gap: 10, padding: mob2 ? "16px 12px" : "20px", maxWidth: 550, overflowY: "auto", paddingTop: mob2 ? 30 : 40, maxHeight: "100vh" } }, /* @__PURE__ */ React.createElement("h2", { style: { fontSize: mob2 ? 16 : 18, color: "#c084fc", letterSpacing: 4, margin: 0, animation: "fadeIn .6s ease-out" } }, "WHAT HAPPENED IN THE DARK"), denRes.length === 0 && /* @__PURE__ */ React.createElement("div", { style: { color: "#666", fontSize: 13, fontStyle: "italic", textAlign: "center", lineHeight: 1.6, maxWidth: 340 } }, cpk(flavorCache, "denQuiet", DEN_QUIET, (fn) => fn())), hero && /* @__PURE__ */ React.createElement("div", { style: { width: "100%", animation: "fadeIn 1s ease-out", transform: "scale(1.02)", marginBottom: 6 } }, hero.el), majors.map((b, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { width: "100%", animation: `slideInLeft .4s ease-out ${i * 0.4}s both` } }, b.el)), minors.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 1, width: "100%", maxWidth: 420, padding: "8px 12px", borderRadius: 10, background: "#ffffff04", border: "1px solid #ffffff08", animation: `slideInLeft .4s ease-out ${majors.length * 0.4 + 0.2}s both` } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#c084fc88", letterSpacing: 4, textAlign: "center", fontWeight: 700, marginBottom: 4 } }, "DEN REPORT"), minors.map((b, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { animation: `slideInLeft .3s ease-out ${majors.length * 0.6 + 0.5 + i * 0.15}s both` } }, b.el))), /* @__PURE__ */ React.createElement("button", { onClick: denBirthPause ? void 0 : () => {
        const babies = [];
      ...
      }, style: { ...BTN(denBirthPause ? "#333" : "linear-gradient(135deg,#fbbf24,#f59e0b)", denBirthPause ? "#666" : "#0a0a1a"), marginTop: 8, padding: "10px 32px", fontSize: 14, opacity: denBirthPause ? 0.4 : 1, cursor: denBirthPause ? "default" : "pointer" } }, denBirthPause ? "..." : "The Sun Comes Up")));
    }
    if (ph === "shop") {
      const mb = getMB();
      ...
      })()), sWild && !activeWild && /* @__PURE__ */ React.createElement("div", { style: { width: "100%", animation: "fadeIn .5s ease-out", padding: "8px 6px" } },
        /* @__PURE__  */ React.createElement("div", { style: { fontSize: 10, color: "#fef08a88", letterSpacing: 3, marginBottom: 6, textAlign: "center" } }, "\u2726 RARE OFFERING"),
        /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "center" } },
          /* @__PURE__ */ React.createElement("button", { onClick: () => {
            if (gold < sWild.cost) return;
            setGold((g) => g - sWild.cost);
            setActiveWild(sWild);
            setSWild(null);
            Haptic.heavy();
            toast(sWild.icon, `${sWild.name} acquired. Activate it before playing a hand.`, "#fef08a", 4e3);
          }, disabled: gold < sWild.cost, style: { padding: "10px 16px", borderRadius: 10, background: gold >= sWild.cost ? "linear-gradient(135deg, #fef08a11, #fef08a06)" : "#1a1a2e", border: `1.5px solid ${gold >= sWild.cost ? "#fef08a44" : "#ffffff11"}`, cursor: gold >= sWild.cost ? "pointer" : "default", maxWidth: 280, width: "100%", textAlign: "center", opacity: gold >= sWild.cost ? 1 : 0.4 } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 24, marginBottom: 2 } }, sWild.icon),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 800, color: "#fef08a", letterSpacing: 2 } }, sWild.name),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fef08acc" } }, sWild.desc),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fef08a66", fontStyle: "italic", marginTop: 4, lineHeight: 1.4 } }, sWild.flavor),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: gold >= sWild.cost ? "#fef08a" : "#ef4444", fontWeight: 700, marginTop: 6 } }, sWild.cost, " \u{1F41F}")))),
      (shopTab === "upgrades" || shopTab === "scrolls" || shopTab === "wards") && /* @__PURE__ */ React.createElement("div", { style: { width: "100%", animation: "fadeIn .3s ease-out", background: "#c084fc04", borderRadius: 10, padding: "8px 6px" } }, !seen.shop2 && /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 14px", borderRadius: 8, background: "#fbbf2408", border: "1px solid #fbbf2422", marginBottom: 10, animation: "fadeIn .6s ease-out" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf24cc", lineHeight: 1.6 } }, /* @__PURE__ */ React.createElement("b", null, "\u{1F6E1}\uFE0F Wards"), " boost your score every play.", /* @__PURE__ */ React.createElement("br", null), /* @__PURE__ */ React.createElement("b", null, "\u{1F4DC} Scrolls"), " level up hand types for bigger bases.", /* @__PURE__ */ React.createElement("br", null), "Both shape your strategy."), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6, textAlign: "center" } }, /* @__PURE__ */ React.createElement("button", { onClick: () => setSeen((s) => ({ ...s, shop2: true })), style: { fontSize: 10, background: "#fbbf24", border: "none", borderRadius: 4, color: "#0a0a1a", cursor: "pointer", padding: "3px 12px", fontWeight: 700 } }, "Got it"))), fams.length === 0 && sFams.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 14px", borderRadius: 8, background: "#4ade8008", border: "1px solid #4ade8033", marginBottom: 8, animation: "breathe 2s ease-in-out infinite" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#4ade80", fontWeight: 700 } }, "\u{1F4A1} Buy a ward! They boost your score every play.")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#c084fcbb", letterSpacing: 2, marginBottom: 6 } }, "\u{1F6E1}\uFE0F WARDS FOR SALE"), sFams.length > 0 ? /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 } }, sFams.map((f, i) => {
        const wp = famPrice(f);
    ...
    return /* @__PURE__ */ React.createElement("div", { style: { ...W, animation: fFlash === "down" ? "shake .3s ease" : "none" } }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), isNL && /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", inset: 0, border: "2px solid #fef08a44", pointerEvents: "none", zIndex: 50, animation: "glow 2s ease-in-out infinite" } }), reshuf && /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", top: "40%", left: "50%", transform: "translate(-50%,-50%)", zIndex: 200, animation: "flash .8s ease-out forwards", fontSize: 13, color: "#fbbf24", letterSpacing: 4, fontWeight: 700, whiteSpace: "nowrap" } }, "\u267B RESHUFFLE"), toasts.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", top: mob ? "auto" : 12, bottom: mob ? 80 : "auto", left: mob ? "50%" : "auto", right: mob ? "auto" : 12, transform: mob ? "translateX(-50%)" : "none", zIndex: 250, display: "flex", flexDirection: "column", gap: 6, pointerEvents: "none", maxWidth: mob ? 320 : 280, alignItems: mob ? "center" : "flex-end" } }, toasts.slice(0, mob ? 1 : 3).map((t) => /* @__PURE__ */ React.createElement("div", { key: t.id, style: { display: "flex", gap: 8, alignItems: "center", padding: t.big ? "12px 18px" : "8px 14px", borderRadius: t.big ? 10 : 8, background: "#1a1a2eee", border: `1.5px solid ${t.color}${t.big ? "66" : "44"}`, boxShadow: `0 4px 16px #00000066,0 0 ${t.big ? 16 : 8}px ${t.color}${t.big ? "44" : "22"}`, animation: `${t.neg ? "slideInLeft" : "slideInRight"} .3s ease-out` }, className: t.neg ? "toast-enter-neg" : "toast-enter" }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: t.big ? 22 : 16, flexShrink: 0 } }, t.icon), /* @__PURE__ */ React.createElement("span", { style: { fontSize: t.big ? 14 : 12, color: t.color, fontWeight: t.big ? 700 : 600, lineHeight: 1.3 } }, t.text)))), autoPlay && ph === "playing" && (() => {
            ...
            if (ci >= autoPlay.idxs.length) {
              ...
              autoRef.current = setTimeout(() => {
                ...
                if (cats.length >= 1) {
                  ...
                  if (result.total > eTgt() * 5 && cats.length > 0) {
                    const mvpC = cats.sort((a, b) => (b.power || 0) - (a.power || 0))[0];
                    const tn = (mvpC.trait || PLAIN).name;
                    aft.push({ icon: "\u{1F525}", text: pk(CAT_REACTIONS.bigHand(mvpC.name.split(" ")[0], tn)), color: "#fef08a" });
                  }
                  cats.forEach((c) => {
                    const tp = (c.stats?.tp || 0) + 1;
                    if (tp === 8) aft.push({ icon: "\u{1F451}", text: `${c.name.split(" ")[0]} has seen every night. The colony follows their lead now.`, color: "#fbbf24" });
                    else if (tp === 12) aft.push({ icon: "\u{1F451}", text: pk(CAT_REACTIONS.elder(c.name.split(" ")[0])), color: "#fbbf24" });
                  });
      ...
      if (guide.step === 0) {
        ...
        msg = postAuto ? `Your turn! Score ${tgt.toLocaleString()} total. ${hLeft} hands left.` : need > 0 ? hLeft === 1 ? "\u26A0 LAST HAND" : "Need " + need.toLocaleString() + " more" : "Cleared! \u{1F389}";
    ...
    })(), /* @__PURE__ */ React.createElement("div", { style: { width: "100%", maxWidth: 700, padding: mob ? "6px 12px" : "8px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", zIndex: 1, borderBottom: `1px solid ${isBoss ? "#ef444422" : "#ffffff0a"}` } }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#888", letterSpacing: 2 } }, "NIGHT ", ante, "/", MX, !mob && /* @__PURE__ */ React.createElement("span", { style: { color: "#666" } }, " Round ", blind + 1, "/3"), !mob && meta?.primaryBranch && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 9, color: SKILL_BRANCHES[meta.primaryBranch]?.color + "66", marginLeft: 4 } }, SKILL_BRANCHES[meta.primaryBranch]?.icon), !mob && /* @__PURE__ */ React.createElement("span", { style: { color: "#ffffff22", marginLeft: 6, fontSize: 9, letterSpacing: 3 } }, NIGHT_PLACES[Math.min(ante - 1, NIGHT_PLACES.length - 1)])), /* @__PURE__ */ React.createElement("div", { style: { fontSize: mob ? 13 : 14, fontWeight: 700, color: isBoss ? "#ef4444" : "#fbbf24" } }, blindN[blind]), !mob && /* @__PURE__ */ React.createElement(ProgressMap, { ante, blind, mx: MX })), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#888", letterSpacing: 2 } }, "SCORE"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: mob ? 16 : 18, fontWeight: 900 } }, /* @__PURE__ */ React.createElement("span", { style: { color: rScore >= tgt ? "#4ade80" : "#e8e6e3" } }, rScore.toLocaleString()), /* @__PURE__ */ React.createElement("span", { style: { color: "#666", fontSize: 12 } }, " / ", tgt.toLocaleString())), !mob && rScore < tgt && hLeft > 0 && (() => {
      ...
      e.stopPropagation();
      toggleMute();
    }, style: { background: "none", border: "none", fontSize: 14, cursor: "pointer", opacity: 0.4, padding: 4 }, title: muted ? "Unmute" : "Mute" }, muted ? "\u{1F507}" : "\u{1F50A}"))), false && rScore < tgt && hLeft > 0 && (() => {
      const need = tgt - rScore;
      const nph = Math.ceil(need / hLeft);
      ...
      position: "fixed",
      inset: 0,
      background: mob ? "#000000f0" : "#000000cc",
      zIndex: 100,
      display: "flex",
      flexDirection: "column",
      alignItems: "center",
      ...
      return /* @__PURE__ */ React.createElement("div", { style: {
        ...
        return /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10, alignItems: "center", animation: isFirstStep ? "fadeIn .4s ease-out" : "none" } }, /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", transition: "transform .2s", transform: (isChipStep || isFirstStep) && !done ? "scale(1.2)" : "scale(1)" } }, /* @__PURE__ */ React.createElement("span", { style: { color: "#3b82f6", fontWeight: 900, fontSize: done ? (mob ? 28 : 26) : (mob ? 24 : 22), transition: "font-size .2s", textShadow: (isChipStep || isFirstStep) && !done ? "0 0 12px #3b82f644" : "none", animation: isFirstStep ? "countUp .4s ease-out" : "none" } }, displayChips), !mob && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#3b82f688", letterSpacing: 2 } }, "POWER")), /* @__PURE__ */ React.createElement("span", { style: { color: "#ffffff55", fontSize: 16 } }, "\xD7"), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", transition: "transform .2s", transform: (isMultStep || isFirstStep) && !done ? "scale(1.2)" : "scale(1)" } }, /* @__PURE__ */ React.createElement("span", { style: { color: "#ef4444", fontWeight: 900, fontSize: done ? (mob ? 28 : 26) : (mob ? 24 : 22), transition: "font-size .2s", textShadow: (isMultStep || isFirstStep) && !done ? "0 0 12px #ef444444" : "none", animation: isFirstStep ? "countUp .4s ease-out" : "none" } }, displayMult), !mob && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ef444488", letterSpacing: 2 } }, "BONUS")), /* @__PURE__ */ React.createElement("span", { style: { color: "#ffffff55", fontSize: 16 } }, "="), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center" } }, /* @__PURE__ */ React.createElement(AnimatedScore, { value: displayTotal, style: {
          fontWeight: 900,
          fontSize: done ? (mob ? 44 : 38) : (mob ? 34 : 28),
          color: counterColor,
        ...
        } }), !mob && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: counterColor + "44", letterSpacing: 2 } }, "SCORE")));
      ...
      const branches2 = ["flame", "shelter", "memory"];
      const chosen = dedicateModal;
      const C = SKILL_BRANCHES[chosen];
      const t45 = UPGRADES.filter((u) => u.branch === chosen && u.tier >= 4);
      return /* @__PURE__ */ React.createElement("div", { onClick: () => setDedicateModal(null), style: { position: "fixed", inset: 0, zIndex: 210, display: "flex", alignItems: "center", justifyContent: "center", background: "#000000dd", padding: 16, animation: "fadeIn .2s ease-out" } },
        /* @__PURE__ */ React.createElement("div", { onClick: (e) => e.stopPropagation(), style: { background: `linear-gradient(145deg, ${C.color}08, #0d0d1a)`, border: `2px solid ${C.color}55`, borderRadius: 16, padding: "20px 16px", maxWidth: 380, width: "100%", boxShadow: `0 16px 60px #00000099, 0 0 60px ${C.color}22` } },
          /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", marginBottom: 12 } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 40, marginBottom: 4 } }, C.icon),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 900, color: C.color, letterSpacing: 4 } }, "DEDICATE TO"),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 18, fontWeight: 900, color: C.color, letterSpacing: 6, marginTop: 2 } }, C.name)),
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.color + "aa", fontStyle: "italic", textAlign: "center", lineHeight: 1.5, marginBottom: 6 } }, C.desc),
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff33", fontStyle: "italic", textAlign: "center", lineHeight: 1.6, marginBottom: 12, maxWidth: 320 } }, C.lore),
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: "#ffffff44", letterSpacing: 2, textAlign: "center", marginBottom: 8 } }, "UNLOCKS"),
          t45.map((u) => /* @__PURE__ */ React.createElement("div", { key: u.id, style: { display: "flex", gap: 8, alignItems: "center", padding: "8px 10px", marginBottom: 4, borderRadius: 8, background: C.color + "0a", border: `1px solid ${C.color}33` } },
            /* @__PURE__ */ React.createElement("span", { style: { fontSize: 18 } }, u.icon),
            /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } },
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.color } }, u.name, u.tier === 5 ? " \u2605" : ""),
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#999" } }, u.desc)),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.color + "88" } }, "\u2726", u.cost))),
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: "#ef444488", textAlign: "center", marginTop: 8, lineHeight: 1.5 } }, "This locks ", branches2.filter((b) => b !== chosen).map((b) => SKILL_BRANCHES[b].name).join(" and "), " at tier 3. Respec to change."),
          /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, marginTop: 12, justifyContent: "center" } },
            /* @__PURE__ */ React.createElement("button", { onClick: () => setDedicateModal(null), style: { padding: "10px 20px", borderRadius: 8, background: "transparent", border: "1px solid #ffffff22", color: "#888", cursor: "pointer", fontSize: 12 } }, "Not yet"),
            /* @__PURE__ */ React.createElement("button", { onClick: async () => {
              const nm = { ...meta, primaryBranch: chosen };
              setMeta(nm); await saveS(nm);
              Haptic.heavy();
      ...
      if (blind === 2 && bossTraits.some((bt) => bt.fx.noStrength)) {
        return /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 700, color: "#fbbf24", letterSpacing: 2 } }, preview.type.name, false ? /* @__PURE__ */ React.createElement("span", { style: { fontSize: 9, color: "#fbbf24aa", fontWeight: 400, marginLeft: 3 } }, "(", preview.type.ex, ")") : null), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#ef4444bb", fontStyle: "italic" } }, "\u{1F441}\uFE0F Watchful. it sees your hand"));
      }
      ...
      return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", justifyContent: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 15, fontWeight: 700, color: "#fbbf24", letterSpacing: 2 } }, preview.type.name, false ? /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#fbbf24aa", fontWeight: 400, marginLeft: 4 } }, "(", preview.type.ex, ")") : null), preview.combo && (() => {
        const known = (meta?.stats?.dh || []).includes(preview.combo.name);
        return /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, fontWeight: 700, color: "#c084fc", letterSpacing: 1 } }, "+ ", known ? preview.combo.name : "????", false ? /* @__PURE__ */ React.createElement("span", { style: { fontSize: 9, color: "#c084fc88", fontWeight: 400, marginLeft: 4 } }, "(", preview.combo.ex, ")") : null);
      })(), /* @__PURE__ */ false, false && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#c084fcaa" } }, "Focus \xD7", selC.length === 1 ? "1.5" : selC.length === 2 ? "1.3" : selC.length === 3 ? "1.2" : "1.1"), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14, fontWeight: 900, color: tier.c, letterSpacing: 1, textShadow: `0 0 8px ${tier.c}44` } }, tier.w)), null, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { width: 100, height: 4, background: "#1a1a2e", borderRadius: 3, overflow: "hidden", border: "1px solid #ffffff08" } }, /* @__PURE__ */ React.createElement("div", { style: { height: "100%", width: `${tier.p}%`, borderRadius: 3, background: `linear-gradient(90deg,${tier.c}66,${tier.c})`, transition: "width .3s" } })), need > 0 && hLeft > 0 && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14, fontWeight: 700, color: ratio >= 1 ? "#4ade80" : "#ef4444", letterSpacing: 0.5 } }, "\u{1F3AF} Need ", need.toLocaleString())), false, hLeft <= 1 && need > 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ef4444", fontWeight: 700, animation: "fpp 1.5s ease infinite" } }, "LAST HAND"), (() => {
        const warnings = [];
      ...
      })), false && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 2, alignItems: "center", padding: "2px 6px", borderRadius: 4, background: "#ffffff06", border: "1px solid #ffffff0a" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#888" } }, "\u{1FA91}"), benchTraited.slice(0, 3).map((c) => /* @__PURE__ */ React.createElement("span", { key: c.id, style: { fontSize: 10 }, title: `${c.name.split(" ")[0]}: unplayed bonus` }, (c.trait || {}).icon || "\xB7"))), false && (() => {
      ...
      if (mob) return null;
      return /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, justifyContent: "center", flexWrap: "wrap", marginBottom: 2, zIndex: 1 } }, Object.entries(sc).sort(([, a], [, b]) => b - a).map(
      ...
      return /* @__PURE__ */ React.createElement("div", { onClick: () => {
        ...
        toast("\u{1F0CF}", `${evald.name}: ${htDescs[evald.name] || ""}${runCount >= 1 ? `. Base ${baseC}P + ${baseM}B.` : ""}`, "#fbbf24");
      }, style: { textAlign: "center", padding: "2px 0", zIndex: 1, animation: "fadeIn .15s ease-out", cursor: "help" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 700, color: "#fbbf24", letterSpacing: 2 } }, evald.name, htLv > 1 ? ` Lv${htLv}` : ""), false, bonds.length > 0 && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#4ade80", marginLeft: 6 } }, "\u{1F495}\xD71.5"), false, evald.combo && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#c084fc", marginLeft: 6 } }, "+\u26A1", evald.combo.name));
      ...
      const PLAY_HINTS = isVeryNew ? [
        ...
        mob ? "Match 3+ same season for a Clowder. That's your score." : "Tap cats to select, then Play. Match 3+ of one season (\u{1F342}\u{1F342}\u{1F342}) for a Clowder. That's your bread and butter.",
        ...
        "Wards boost your score every play. Buy one at the Market.",
      ...
      return /* @__PURE__ */ React.createElement("div", { key: p.name, style: { fontSize: 10, color: discovered ? "#555" : "#333" } }, discovered ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("span", { style: { color: "#c084fc", fontWeight: 600 } }, p.name, " \u2728"), " +", /* @__PURE__ */ React.createElement("span", { style: { color: "#3b82f6" } }, p.bonus.c), "P +", /* @__PURE__ */ React.createElement("span", { style: { color: "#ef4444" } }, p.bonus.m), "B ", p.ex, best ? /* @__PURE__ */ React.createElement("span", { style: { color: "#fbbf24bb" } }, " best:", best.toLocaleString()) : "") : /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("span", { style: { color: "#666", fontWeight: 600 } }, "????"), " ", /* @__PURE__ */ React.createElement("span", { style: { fontStyle: "italic", color: "#666666bb" } }, p.ex.includes("consecutive") ? "Consecutive combo" : "Matching combo")));
