...
const Audio={
  ...
  draftFlip(i){this.p(()=>{const t=Tone.now();const notes=["D4","F4","A4"];this.syn.triggerAttackRelease(notes[i%3],"32n",t,0.15);this.perc.triggerAttackRelease("32n",t,0.2);});},
  nightTransition(){this.p(()=>{const t=Tone.now();this.bass.triggerAttackRelease("A1","2n",t,0.3);this.syn.triggerAttackRelease("E4","4n",t+0.1,0.08);});},
  shopAmbient(){this.p(()=>{const t=Tone.now();this.syn.triggerAttackRelease(["C3","G3","E4"],"2n",t,0.06);});},
  campfire(){this.p(()=>{const t=Tone.now();this.syn.triggerAttackRelease("A3","2n",t,0.05);this.syn.triggerAttackRelease("E4","2n",t+0.3,0.04);});},
  discard(){this.p(()=>{const t=Tone.now();this.perc.triggerAttackRelease("16n",t,0.15);this.syn.triggerAttackRelease("D4","32n",t+0.02,0.06);});},
  stepTick(progress=0){this.p(()=>{const n=Math.min(6,Math.floor(Math.min(1.5,progress)*5));this.syn.triggerAttackRelease(this.noteAt(5,n),"64n",Tone.now(),0.06+progress*0.04);});},
  eventReveal(){this.p(()=>{const t=Tone.now();this.syn.triggerAttackRelease("A4","8n",t,0.1);this.bass.triggerAttackRelease("D2","4n",t,0.15);});},
};
...
function addGrudge(c,targetId){
  // "The Forgiven" epithet grants grudge immunity
  if(c.epithetKey==="grudgeResolved")return c;
  if(!(c.grudgedWith||[]).includes(targetId)){c.grudgedWith=[...(c.grudgedWith||[]),targetId];}return c;
}
...
function getDraftVoice(cat,meta){
  // v0.7: The Return ghost voice
  if(cat._returned)return cat._draftVoice||"You remember me. I almost didn't come back.";
  // v0.7: History-aware voices (Kojima — the world knows you've been here)
  const totalDeaths=meta?.stats?.totalFallen||0;const runs=meta?.stats?.r||0;
...
const TRAITS=[
  ...
  {name:"Seer",icon:"👁️",desc:"+4 mult. Same hand type as last: +8 mult",tier:"rare"},
...
function addTrait(cat,trait){
  ...
  if(cat.trait.name==="Plain"){cat._wasPlain=true;cat.trait=trait;assignEpithet(cat);return true;}
...
const NERVE=[
  {name:"Still",     xM:1.0, color:"#666",     glow:"transparent",  desc:""},                     // 0
  {name:"Stirring",  xM:1.0, color:"#666",     glow:"transparent",  desc:""},                     // 1
...
const CAT_NAMES=[
  ...
  "Mabel","Penny","Clover","Milo","Olive","Willow","Maple","Poppy","Wren",
  "Ruby","Percy","Fern","Pearl","Opal","Hazel","Honey","Ada","Flora","Plum","Sage",
  "Lumen","Maren","Solene","Lark","Rue","Sable","Linnea","Iris","Yarrow",
  // Earned-feeling (imply history. survivors, not pets)
  "Cinder","Soot","Brindle","Thistle","Gale","Hob","Rook","Tallow","Spindle","Thresh",
  "Knot","Tinder","Sedge","Wisp","Ghost","Twice","Waif","Vagrant",
  "Notch","Scruff","Burr","Smudge","Patch","Tuft","Scrap","Char","Ravel",
  // Fierce (plosive-forward. memorable and expendable)
  "Bracken","Crook","Pitch","Buckle","Brunt","Knox","Gravel","Stark","Harrow",
  // Nature / world (grounded, real, textured)
  "Thimble","Candle","Locket","Lantern","Compass","Cobalt","Shale","Marrow",
  "Truffle","Pepper","Thorn","Flicker","Hemlock","Cairn","Bramble","Pyre",
  "Quarry","Sullen","Ermine","Wicker","Talon","Anvil","Ember","Slate",
];
...
const EPITHETS={
  scarred:{key:"scarred",test:c=>c.scarred&&!c.epithet,titles:["the Marked","the Scarred","who Bled"],
    bonus:{mult:1},desc:"+1 mult (scar veteran)",flavor:"The scar never healed. It became a map."},
  bonded:{key:"bonded",test:c=>c.bondedTo&&!c.epithet,titles:["the Devoted","the Beloved","who Chose"],
    bonus:{bondMult:true},desc:"+2 mult when bonded partner is played",flavor:"They chose each other in the dark."},
  grudgeResolved:{key:"grudgeResolved",test:c=>c._grudgeResolved&&!c.epithet,titles:["the Forgiven","the Mended","who Let Go"],
    bonus:{grudgeImmune:true},desc:"Immune to future grudges",flavor:"Some wounds close from the inside."},
  bossNight:{key:"bossNight",test:(c,ctx)=>ctx?.bossNight&&!c.epithet,gen:(c,ctx)=>[`of the ${["First","Second","Third","Fourth","Fifth"][Math.min((ctx?.ante||1)-1,4)]} Night`],
    bonus:{mult:3},desc:"+3 mult (boss slayer)",flavor:"The test came. They answered."},
  decisive:{key:"decisive",test:(c,ctx)=>ctx?.decisive&&!c.epithet,titles:["the Decisive","who Tipped the Scale","the Clutch"],
    bonus:{clutchMult:3},desc:"+3 mult on clutch hands",flavor:"One hand. Everything on it."},
  lastStanding:{key:"lastStanding",test:(c,ctx)=>ctx?.lastStanding&&!c.epithet,titles:["the Alone","Last Standing","the Survivor"],
    bonus:{soloMult:4},desc:"+4 mult when only cat of their season",flavor:"Everyone else fell. They didn't."},
  grownUp:{key:"grownUp",test:c=>c._grewUp&&!c.epithet,titles:["the Grown","the Bloomed","who Found Their Name"],
    bonus:{power:2},desc:"+2 Power (matured)",flavor:"Born small. Grew into something."},
  // NEW epithets
  unbreakable:{key:"unbreakable",test:c=>(c.stats?.injuries||0)>=3&&!c.epithet,titles:["the Unbreakable","the Enduring","who Would Not Fall"],
    bonus:{mult:2},desc:"+2 mult",flavor:"What kills others makes them quieter."},
  thunder:{key:"thunder",test:(c,ctx)=>ctx?.thunder&&!c.epithet,titles:["the Thunder","the Storm","who Shook the Dark"],
    bonus:{thunderMult:2},desc:"+2 mult on hands scoring 5k+",flavor:"The number echoed. The dark flinched."},
  parent:{key:"parent",test:c=>(c.stats?.kidsBreed||0)>=2&&!c.epithet,titles:["the Mother","the Father","who Built a Future"],
    bonus:{parentMult:true},desc:"+1 mult per own kitten alive",flavor:"They built a future in the dark."},
  wanderer:{key:"wanderer",test:c=>c._wasPlain&&!c.epithet,titles:["the Wanderer","the Becoming","who Arrived with Nothing"],
    bonus:{mult:2},desc:"+2 mult",flavor:"They arrived with nothing and became something."},
  mourning:{key:"mourning",test:c=>c._mateDied&&!c.epithet,titles:["the Mourning","the Bereft","who Carries Two Names"],
    bonus:{mult:3},desc:"+3 mult (grief)",flavor:"They carry two names now."},
  // v0.7: The Spared — when player refuses The Offering sacrifice
  spared:{key:"spared",test:c=>c._spared&&!c.epithet,titles:["the Spared","the Kept","who Almost Wasn't"],
    bonus:{mult:2},desc:"+2 mult",flavor:"They don't know how close it was."},
  // v0.7: The Returned — ghost from a previous run
  returned:{key:"returned",test:c=>c._returned&&!c.epithet,titles:["the Returned","the Echo","who Came Back"],
    bonus:{mult:2},desc:"+2 mult",flavor:"You remember me. I almost didn't come back."},
};
  ...
  for(const[,ep]of Object.entries(EPITHETS)){
    if(ep.test(cat,ctx)){
      ...
      if(!cat.epithet){
        ...
        // Already has one: track silently for victory choice
...
const BLIND_WHISPER={
  dusk:[ // The dark stirs. first probe
  ...
  midnight:[ // Escalation. the system adjusts
  ...
  boss:[ // The pattern sends its champion
...
const ACHIEVEMENTS=[
  ...
  {id:"first_win",name:"Survivor",desc:"Win your first run",icon:"🏆",dust:5,check:s=>s.w>=1,reward:"5-night mode unlocked"},
  {id:"deathless",name:"Every Single One",desc:"Win with 0 deaths",icon:"💚",dust:15,check:(_,f)=>f===true,reward:"New epigraphs unlocked"},
  {id:"ten_runs",name:"The Stubborn",desc:"Attempt 10 runs",icon:"🔄",dust:5,check:s=>s.r>=10,reward:"Run counter on title screen"},
  {id:"max_fervor",name:"Nerve of Steel",desc:"Reach maximum Nerve",icon:"🔥",dust:5,check:s=>s.mf>=9,reward:"Gold nerve flame visual"},
  // Tier 2. Mastery (medium: 15✦)
  {id:"all_breeds",name:"All Four Seasons",desc:"Save each season to Hearth",icon:"🐾",dust:15,check:s=>{const br=new Set((s.disc||[]).map(d=>d.split("-")[0]));return["Autumn","Winter","Spring","Summer"].every(b=>br.has(b));},reward:"Season icons glow on title"},
  {id:"five_wins",name:"Colony Leader",desc:"Win 5 runs",icon:"👑",dust:15,check:s=>s.w>=5,reward:"Gold colony name on title"},
  {id:"night5",name:"Into the Dark",desc:"Reach Night 5",icon:"🌙",dust:15,check:s=>s.ba>=5,reward:"Boss traits shown on Night Card"},
  {id:"breeder",name:"The Breeder",desc:"5+ kittens in one run",icon:"🐣",dust:15,check:s=>s.kittensTotal>=5,reward:"Kitten celebration"},
  // Tier 3. Legend (hard: 30✦)
  {id:"legend_score",name:"NINTH LIFE",desc:"Score 350,000+ in one hand",icon:"✨",dust:30,check:s=>s.hs>=350000,reward:"Gold card borders"},
  {id:"heat_five",name:"Five by Five",desc:"Win 5 runs at Heat 1+",icon:"🔥",dust:30,check:s=>(s.heatWins||0)>=5,reward:"Career heat wins shown"},
  {id:"diplomat",name:"The Diplomat",desc:"Resolve 10 grudges across all runs",icon:"🕊️",dust:30,check:s=>(s.grudgesResolved||0)>=10,reward:"Upgraded reconciliation"},
  {id:"archivist",name:"The Archivist",desc:"Play 50 hands of one type",icon:"📜",dust:30,check:s=>{const hp=s.handTypePlays||{};return Object.values(hp).some(v=>v>=50);},reward:"Custom hand type sound"},
  // Tier 4. Myth (hard: 30✦)
  {id:"ninth_dawn",name:"The Remembering",desc:"Clear the Ninth Dawn",icon:"🌅",dust:30,check:s=>s.ninthDawnCleared===true,reward:"Secret epigraph pool"},
  ...
  {id:"unbroken",name:"Unbroken Line",desc:"3rd generation cat in colony",icon:"👪",dust:30,check:s=>s.thirdGen===true,reward:"Lineage tree in den"},
  {id:"completionist",name:"Completionist",desc:"All other achievements",icon:"🌟",dust:30,check:(s,_,achv)=>ACHIEVEMENTS.filter(a=>a.id!=="completionist").every(a=>achv.includes(a.id)),reward:"Alternate title gradient"},
  // NEW. Easy (5✦)
  {id:"first_bond",name:"Heartbound",desc:"First bond formed in a run",icon:"💕",dust:5,check:s=>(s.maxBonds||0)>=1,reward:"Bond sparkle effect"},
  {id:"first_epithet",name:"Named",desc:"A cat earns an epithet",icon:"🏷️",dust:5,check:s=>(s.epithetsEarned||0)>=1,reward:"Epithet glow on cards"},
  {id:"camp_five",name:"Firekeeper",desc:"Camp 5 times across all runs",icon:"🏕",dust:5,check:s=>(s.campCount||0)>=5,reward:"Camp whisper pool expanded"},
  // NEW. Medium (15✦)
  {id:"clutch_win",name:"By a Thread",desc:"Win a boss fight on the last hand",icon:"🎯",dust:15,check:s=>(s.clutchBossWins||0)>=1,reward:"Clutch flash enhanced"},
  {id:"hearth_release",name:"Letting Go",desc:"Release a Hearth pair",icon:"🕊️",dust:15,check:s=>(s.hearthReleases||0)>=1,reward:"Release ceremony"},
  {id:"full_bonds",name:"Love Colony",desc:"4+ bonded pairs in one run",icon:"💞",dust:15,check:s=>(s.maxBonds||0)>=4,reward:"Bond constellation visual"},
  {id:"ten_epithets",name:"The Named Ones",desc:"10 epithets earned across runs",icon:"📜",dust:15,check:s=>(s.epithetsEarned||0)>=10,reward:"Epithet history in Hearth"},
  {id:"no_market",name:"Off the Grid",desc:"Win without visiting the market",icon:"🚫",dust:15,check:s=>(s.marketlessWins||0)>=1,reward:"Scavenger bonus increased"},
  // NEW. Hard (30✦)
  {id:"all_bosses",name:"Fear Nothing",desc:"Defeat all 8 unique bosses",icon:"💀",dust:30,check:s=>{const br=s.bossRecord||{};return Object.keys(br).filter(k=>br[k].w>0).length>=8;},reward:"Boss lore expanded"},
  ...
  {id:"heat_deathless",name:"Untouchable",desc:"Win at Heat 3+ with 0 deaths",icon:"🛡️",dust:30,check:s=>(s.mh||0)>=3&&s.deathlessHeatWin===true,reward:"Invincible title card"},
  {id:"dynasty",name:"Dynasty",desc:"A Hearth child earns an epithet",icon:"👪",dust:30,check:s=>(s.dynastyEarned||0)>=1,reward:"Dynasty marker on Hearth"},
];
...
// v0.7: Elder detection — cats with 8+ hands played become elders
function isElder(cat){return(cat.stats?.tp||0)>=8;}
...
{id:"the_question",title:"The Question",icon:"❓",minNight:4,tag:"memory",once:true,
  condFn:(_,ctx)=>(ctx.meta?.stats?.r||0)>=10||(ctx.meta?.stats?.totalFallen||0)>=20,
  textFn:(_,ctx)=>{const runs=(ctx.meta?.stats?.r||0)+1;const dead=ctx.meta?.stats?.totalFallen||0;
...
const EXPANDED_BOSSES=[
  {id:"fraying",name:"The Fraying",icon:"🕸️",
    ...
    tauntFn:(ctx)=>{const pf=ctx.prevFallen||[];
      ...
      return ctx.bonded>3&&ctx.grudges>0?"Love and hate in the same den. I don't even have to try."
  ...
  {id:"eclipse",name:"The Eclipse",icon:"🌑",
    ...
    tauntFn:(ctx)=>{const pf=ctx.prevFallen||[];
      ...
      return ctx.fallen>2?"You've lost so many. Why keep going? For what?"
  ...
  {id:"ember",name:"The Ember That Remains",icon:"🔥",
    ...
    tauntFn:(ctx)=>{const pf=ctx.prevFallen||[];
      ...
      return ctx.clutch?"Cutting it close. I know what 'close' costs."
...
const BOSS_TRAITS=[
  ...
  {id:"bleeding",name:"Bleeding",icon:"🩸",desc:"Target shrinks 2% per hand played",flavor:"It weakens, but will you last?",fx:{bleeding:true}},
  {id:"frozen",name:"Frozen",icon:"🧊",desc:"First 2 cats score half power",flavor:"The cold takes the first ones.",fx:{frozen:true}},
  {id:"enraged",name:"Enraged",icon:"🔥",desc:"Target −15%, all cats +3 mult",flavor:"It fights harder. So do you.",fx:{tgtMult:0.85,enragedMult:3}},
  {id:"fading",name:"Fading",icon:"🕯️",desc:"Target grows 5% per hand remaining",flavor:"It grows stronger as time runs out.",fx:{fading:true}},
...
const WHISPER_SHOP=[
  ...
  "The market doesn't care about your story. It cares about your 🐟. Same as the dark.",
...
// v0.7: Mood-aware colony whispers (replace NIGHT_SUB based on mood)
function getMoodWhisper(mood,colony){
  if(mood<20)return pk(["The colony is tense. You can feel it in the silences.","Something is wrong. The cats won't look at each other.","Too many losses. The fire is smaller tonight."]);
  if(mood<40)return pk(["Quiet tonight. Not peaceful. Just quiet.","The colony is holding together. Barely.","Some of them are sleeping. That's something."]);
  ...
  return null; // mid-range: use default sub
}
const NIGHT_SUB=[
  "They're yours now.",
...
// ★ THE NARRATOR. the voice that watches colonies die and remembers
// Generates a run-aware summary that connects events into a story
function getNarratorRecap(ante,ctx){
  ...
  // NIGHT 5+: Silence. Just names. The loudest thing is what you don't say.
  if(ante>=5){
  ...
  // EVENT ECHOES. personal consequences, always available
  const echoes=[];
  ...
  // NIGHT 2: Only event echoes. The world still feels random.
  if(ante<=2){
    if(echoes.length>0)return echoes[0];
    // Colony state. personal, named
    const ac=allCats||[];
    const fallenName=ac.length===0?"someone":"";
    const bondedPair=ac.filter(c=>c.bondedTo);
    if(fallen>0){const fn=ctx.fallenNames||[];return fn.length?`${fn[0]} is gone. The space where they slept is still warm.`:`${fallen} gone already. The spaces are still warm.`;}
    if(bondedPair.length>=2){const a=bondedPair[0].name.split(" ")[0];const b=ac.find(c=>c.id===bondedPair[0].bondedTo);return b?`${a} and ${b.name.split(" ")[0]} bonded in the dark. The colony is starting to mean something.`:"Someone bonded in the dark.";}
    return null;
  }

  // NIGHT 3: First hint of the pattern. The narrator notices something.
  if(ante===3){
    const ac=allCats||[];
    const scarredCat=ac.find(c=>c.scarred);const grudgedCat=ac.find(c=>(c.grudgedWith||[]).length>0);
    const bondedPair=ac.filter(c=>c.bondedTo);const baby=ac.find(c=>c.stats?.par);
    const stateLines=[];
    if(fallen>0&&bondedPair.length>=2){const bName=bondedPair[0].name.split(" ")[0];stateLines.push(`Loss and love in the same den. ${bName} bonded while the colony grieved. The third colony knew this feeling.`);}
    else if(grudges>=2&&grudgedCat)stateLines.push(`${grudgedCat.name.split(" ")[0]} is fighting their own. It always starts around Night 3. The fifth colony could tell you why.`);
  ...
  // NIGHT 4: The truth. The system reveals itself.
  const ac=allCats||[];
  ...
  // Specific callback + system revelation
  const echo=echoes.length>0?echoes[0]+" ":"";
  const bondedCat=ac.find(c=>c.bondedTo);const grudgedCat=ac.find(c=>(c.grudgedWith||[]).length>0);
  const colState=fallen>0?`${fallen} names the dark already collected. `
    :bondedCat&&bonds>=2?`${bondedCat.name.split(" ")[0]} is bonded. The system doesn't have a test for that. `
    :grudgedCat&&grudges>=2?`${grudgedCat.name.split(" ")[0]} is fighting their own. The dark doesn't need to break what's already cracking. `
    :"";
  return echo+colState+pk(systemLines);
}

// ★ SCORE REACTIONS - escalating hype + ★ v34: narrator voice
const SCORE_TIERS=[{min:0,label:"",color:"",sub:"",nar:""},{min:1200,label:"Alive",color:"#b8956a",sub:"",nar:"Breathing. That counts."},{min:4500,label:"Defiant",color:"#b85c2c",sub:"they felt that one",nar:"Something cracked. They heard it."},{min:12000,label:"ROARING",color:"#f59e0b",sub:"the ground shakes",nar:"The dark stopped moving. It's listening."},{min:35000,label:"UNSTOPPABLE",color:"#fbbf24",sub:"nothing can touch them",nar:"Colonies don't score like this. Legends do."},{min:100000,label:"LEGENDARY",color:"#fef08a",sub:"they will tell stories about this",nar:"Scratch it into the stone. Someone needs to find this number."},{min:350000,label:"NINTH LIFE",color:"#ffffffdd",sub:"the dark blinks first",nar:"Nine lives. Nine colonies. One hand. This is the one they'll remember."}];
...
let _belovedNames=[]; // ★ Names the player gave to cats across runs. they live on
function setBelovedNames(meta){
  if(!meta)return;
  const names=new Set();
  // Hearth cat names. these survived
  (meta.cats||[]).forEach(c=>{if(c.name)names.add(c.name.split(" ")[0]);});
  // Explicitly player-named cats. these were chosen
...
function gN(br,trait){
  ...
  // ★ 50% chance to resurrect a beloved name. names survive across runs
  if(_belovedNames.length>0&&Math.random()<0.5){
    const avail=_belovedNames.filter(n=>!_un.has(n));
    if(avail.length>0){
  ...
  let n=shuffled.find(x=>!_un.has(x)&&x.length<=8); // v0.7: max 8 chars
  if(!n)n=shuffled[0];
  _un.add(n);
...
function resolveDen(denCats,hasMatchmaker,denSafe,heatDenFight,ctx={}){
  ...
  for(let i=0;i<cats.length;i++){
    ...
    for(let j=i+1;j<cats.length;j++){
      ...
      // v0.7: Colony mood affects den — low mood = more fights, high mood = more bonds
      const moodMod=ctx.mood||50;
      if(moodMod<30)a.fightCh=Math.min(80,a.fightCh+10); // tense colony fights more
      if(moodMod>70)a.fightCh=Math.max(0,a.fightCh-8); // happy colony fights less
      let bCh=a.breedCh;if(hasMatchmaker)bCh=Math.min(100,bCh+10);
      if(moodMod>60)bCh=Math.min(100,bCh+5); // good mood encourages bonds
...
// v0.7: Narrative scoring reasons — replace mechanical labels with story (Kasavin: "game pays attention")
function narrativeReason(c,cats,mechanicalReasons){
  const n=c.name.split(" ")[0];const tp=c.stats?.tp||0;
  // Relationship-driven
  if(c.bondedTo){const mate=cats.find(x=>x.id===c.bondedTo);if(mate)return`${n} and ${mate.name.split(" ")[0]}. Together since they chose each other.`;}
  ...
  return mechanicalReasons.join(" \u00B7 "); // fallback to mechanical
}
...
function calcScore(cats,fams,fLvl,cfx={},ctx={}){
  ...
  toS.forEach((c,si)=>{
    ...
    // ★ Epithet bonuses. earned titles grant small mechanical rewards
    if(c.epithetKey&&!c._re){
      ...
      if(epDef?.bonus){
        ...
        if(eb.thunderMult){cm+=eb.thunderMult;icons.push("🏷️");} // always active once earned. the cat IS thunder
        if(eb.parentMult){const kidCount=cats.filter(x=>x.parentIds?.includes(c.id)).length;if(kidCount>0){cm+=kidCount;icons.push("🏷️");}}
      }
    }
    ...
    if(c.epithetKey&&!c._re){const epDef=Object.values(EPITHETS).find(e=>e.key===c.epithetKey);if(epDef?.bonus){
      // v0.7: Mourning split — isolating in grief scores higher than pushing through
      if(c.epithetKey==="mourning"){
        const soloMourning=cats.length===1;
        const mournMult=soloMourning?5:1;
        cm+=mournMult;reasons.push(soloMourning?`"${c.epithet}" alone in grief +5M`:`"${c.epithet}" pushing through +1M`);
    ...
    bd.push({
      ...
      reason:reasons.length>0?(Math.random()<0.15?narrativeReason(c,cats,reasons):reasons.join(" \u00B7 ")):"",
...

  {
    ...
    bondedPairs.forEach(([a,b],pi)=>{
      ...
      // v0.7 Triangle: bonded cat's mate has grudge with someone in this hand
      const allHand=cats;
...
// v0.7: Narrative scoring reasons — 15% chance to replace mechanical label with story
function narrativeLabel(step,allCats){
  if(Math.random()>0.15)return null; // 85% keep mechanical label
  const cn=step.catName||"";
  if(step.type==="bond"&&cn)return`💕 ${step.label.split(" ")[0]}. Together since they chose each other.`;
  if(step.type==="grudge_tension"&&cn)return`⚡ The grudge burns. Scoring through clenched teeth.`;
  if(step.isBigCat&&cn){
    const cat=allCats?.find(c=>c.name.split(" ")[0]===cn);
    if(cat?.scarred)return`🥊 ${cn} scores harder since the scar.`;
    if(cat?.bondedTo){const mate=allCats.find(c=>c.id===cat.bondedTo);if(mate)return`${cn} plays like ${mate.name.split(" ")[0]} is watching.`;}
    if((cat?.stats?.tp||0)>10)return`${cn}. ${cat.stats.tp} hands deep. This is who they are.`;
  }
...
function genDescendant(hearthCats,powerBonus=0,specificPair=null){
  ...
  if(pairs.length===0&&!specificPair)return gC({trait:pickDraftTrait()}); // fallback
  ...
  // Hearth bonus: if baby ended up Plain, give a Common trait
  if(catIsPlain(baby)){
  ...
  baby._parentTrait=pair[0].trait?.name||pair[1].trait?.name||null; // v0.7: for inheritance acknowledgment
  return baby;
}
...
function calcHearthDust(cats){
  return cats.map(c=>{
    ...
    if(c.lineage)d+=3;
    return{cat:c,dust:Math.max(1,Math.ceil(d*0.5))}; // 50% nerf. pairs now valuable for children, not passive dust
  });
}
...
function _CC({cat:_cat,sel,onClick,sm,cw:_cw,dis,hl,fog,chemHint,denMode,onTraitClick}){
  ...
  if(fog)return(<div onClick={dis?undefined:onClick} style={{width:w,height:h,borderRadius:sm?8:12,background:"#0d1117",border:"2px solid #ffffff12",boxShadow:"0 2px 8px #00000066",cursor:dis?"default":"pointer",transition:"all .15s",transform:sel?"translateY(-12px) scale(1.05)":"",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{fontSize:sm?22:30,opacity:.12,color:neon}}>?</span></div>);
  ...
  return(
    <div onClick={dis?undefined:onClick} title={cat.name+" - "+b.name+", P"+cat.power} style={{
      ...
      flexShrink:0,opacity:dis?.4:1,willChange:"transform,box-shadow",
      animation:cat._newEpithet?"epithetFlash 1.2s ease-out":""
    }}
    onMouseEnter={e=>{if(!dis&&!sel){e.currentTarget.style.transform="translateY(-4px) scale(1.02)";e.currentTarget.style.boxShadow=`0 6px 20px ${neon}33,0 0 12px ${neon}22`;}}}
    onMouseLeave={e=>{if(!dis&&!sel){e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="";}}}>

      {/* Selection ripple */}
      {sel&&<div style={{position:"absolute",top:"50%",left:"50%",width:w*0.6,height:w*0.6,borderRadius:"50%",background:neon+"44",transform:"translate(-50%,-50%)",animation:"cardRipple .4s ease-out forwards",pointerEvents:"none",zIndex:5}}/>}
      ...
      <div style={{position:"absolute",top:sm?1:2,right:sm?1:2,zIndex:5,
        ...
        <span style={{fontSize:xs?12:(sm?15:22),fontWeight:900,color:neon,lineHeight:1,
          textShadow:"0 0 8px "+ng}}>{cat.power}</span>
      </div>

      {/* Status badges (top-left). all neon color, vertical stack */}
      {(cat.injured||cat.scarred||cat.bondedTo||(cat.grudgedWith||[]).length>0||cat.parentIds?.length>0||cat._hearthChild)&&
      <div style={{position:"absolute",top:0,left:0,right:0,zIndex:5,display:"flex",gap:0,justifyContent:"center"}}>
        {cat._hearthChild&&<div title="Hearth Legend: returned to guide this colony. Cannot be saved back." style={{
          background:"linear-gradient(180deg,#fbbf2433,#fbbf2411)",padding:sm?"2px 4px":"3px 5px",
          fontSize:sm?9:11,color:"#fbbf24",lineHeight:1,fontWeight:700,borderBottom:"1px solid #fbbf2444"}}>🔥</div>}
        {cat.injured&&<div title="Injured: half power, −2 mult. Heals in 1-2 rounds." style={{
          background:"linear-gradient(180deg,#ef444433,#ef444411)",padding:sm?"2px 4px":"3px 5px",
          fontSize:sm?9:11,color:"#ef4444",lineHeight:1,fontWeight:700,borderBottom:"1px solid #ef444444"}}>✚</div>}
        {!cat.injured&&cat.scarred&&<div title="Scarred: permanent ×1.25 mult bonus." style={{
          background:"linear-gradient(180deg,#fb923c33,#fb923c11)",padding:sm?"2px 4px":"3px 5px",
          fontSize:sm?9:11,color:"#fb923c",lineHeight:1,fontWeight:700,borderBottom:"1px solid #fb923c44"}}>⚔</div>}
        {cat.bondedTo&&<div title="Bonded: ×1.5 mult when played with partner." style={{
          background:"linear-gradient(180deg,#f472b633,#f472b611)",padding:sm?"2px 4px":"3px 5px",
          fontSize:sm?9:11,color:"#f472b6",lineHeight:1,fontWeight:700,borderBottom:"1px solid #f472b644"}}>♡</div>}
        {(cat.grudgedWith||[]).length>0&&<div title="Grudge: −2 mult when played with rival." style={{
          background:"linear-gradient(180deg,#fb923c33,#fb923c11)",padding:sm?"2px 4px":"3px 5px",
          fontSize:sm?9:11,color:"#fb923c",lineHeight:1,fontWeight:700,borderBottom:"1px solid #fb923c44"}}>⚡</div>}
        {cat.parentIds&&cat.parentIds.length>0&&<div title={`Born in the den${cat.stats?.par?", parents: "+cat.stats.par:""}`} style={{
          background:"linear-gradient(180deg,#c084fc33,#c084fc11)",padding:sm?"2px 4px":"3px 5px",
          fontSize:sm?9:11,color:"#c084fc",lineHeight:1,fontWeight:700,borderBottom:"1px solid #c084fc44"}}>👪</div>}
      </div>}
      ...
      {rankLabel&&<div style={{position:"absolute",top:sm?15:20,right:sm?2:3,zIndex:5,
        ...
        padding:"1px 3px",fontSize:sm?6:7,color:neon+"bb",fontWeight:600}}>
        {rankLabel}
      </div>}
      ...
      <div style={{position:"absolute",bottom:0,left:0,right:0,zIndex:4}}>
        ...
        <div style={{background:"#0d1117",padding:sm?"1px 4px 4px":"2px 8px 5px",textAlign:"center"}}>
          <div style={{fontSize:xs?7:(sm?(fn.length>7?8:fn.length>5?9:10):(fn.length>8?10:fn.length>6?11:13)),
            fontWeight:700,color:neon,letterSpacing:xs?0:(sm?0:(fn.length>7?1:3)),
            textShadow:"0 0 6px "+ng,
            textTransform:"uppercase",
            lineHeight:1.2,whiteSpace:"nowrap"}}>{fn}<span style={{fontSize:xs?7:(sm?9:11),fontWeight:700,marginLeft:2,color:cat.sex==="M"?"#60a5fa":"#f472b6"}}>{cat.sex==="M"?"♂":"♀"}</span></div>
        </div>
      </div>
  ...
  return(<div style={{display:"flex",gap:2,alignItems:"center",padding:"2px 0"}}>
    {dots.map((d,i)=>{
      ...
      return(<div key={i} style={{display:"flex",alignItems:"center",gap:1}}>
        ...
        {showAnte&&<span style={{fontSize:10,color:d.done||d.cur?"#fbbf24":"#444",marginRight:1}}>{d.a}</span>}
        <div style={{
  ...
  return(<div style={{width:"100%",maxWidth:700,padding:"0 16px"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
      <div style={{display:"flex",alignItems:"center",gap:6}}>
        <span title={"NERVE multiplies ALL scores.\nGain: +1 per Dusk/Midnight clear.\nBoss: +hands remaining (faster clear = more nerve).\nNo decay. No loss.\nAt NINTH LIFE (max): ×2.2 to ALL scores."} style={{fontSize:12,fontWeight:700,color:fv.color,letterSpacing:2,textShadow:mx?`0 0 14px ${fv.glow}`:level>5?`0 0 6px ${fv.color}44`:"none",animation:mx?"fp 1s ease-in-out infinite":up?"fpp .4s ease-out":dn?"shake .3s ease":"none",cursor:"help"}}>{mx?"✦ ":""}{fv.name}{mx?" ✦":""}</span>
        <span style={{fontSize:fv.xM>1?13:11,color:fv.color,fontWeight:900,opacity:fv.xM>1?1:.3,letterSpacing:fv.xM>1?1:0,textShadow:fv.xM>1.3?`0 0 8px ${fv.color}44`:"none"}}>{fv.xM>1?`×${fv.xM}`:"×1"}</span>
        {ch&&<span style={{fontSize:10,fontWeight:700,animation:"countUp .4s ease-out",color:up?"#4ade80":"#ef4444"}}>{up?"▲":"▼"}</span>}
      </div>
      <span style={{fontSize:10,color:"#666"}}>{level}/{NERVE_MAX}</span>
    </div>
    <div style={{height:8,background:"#1a1a2e",borderRadius:4,overflow:"hidden",border:"1px solid #ffffff08"}}><div style={{height:"100%",width:`${pct}%`,borderRadius:4,background:mx?"linear-gradient(90deg,#b85c2c,#f59e0b,#fef08a,#ffffffcc)":`linear-gradient(90deg,#b8956a,${fv.color})`,transition:"width .5s cubic-bezier(.34,1.56,.64,1)",boxShadow:level>5?`0 0 8px ${fv.color}44`:"none",animation:level>8?`nervePulse ${Math.max(0.6,2-level*0.1)}s ease-in-out infinite`:"none"}}/></div>
    {level===0&&<div style={{fontSize:10,color:"#666",marginTop:2,textAlign:"center"}}>Clear blinds to build Nerve. Faster boss clears give more.</div>}
  </div>);
}
...
// Item 3: Animated score countup
function AnimatedScore({value,style}){
  const ref=useRef(null);const prev=useRef(value);const raf=useRef(null);
  useEffect(()=>{
    const start=prev.current;const end=value;const duration=Math.min(400,Math.max(150,Math.abs(end-start)/50));
    let t0=null;
    function tick(ts){
      if(!t0)t0=ts;const p=Math.min(1,(ts-t0)/duration);
    ...
    if(start!==end){raf.current=requestAnimationFrame(tick);}
...
function ChemPreview({cats}){
  ...
  return(<div style={{display:"flex",gap:6,alignItems:"center",fontSize:10}}>
...
export default function NinthLife(){
  ...
  const setPh=(p)=>{
    ...
    if(document.startViewTransition&&p!=="scoring"){
      document.startViewTransition(()=>{_setPh(p);});
    }else{
      // Fallback: brief opacity dip
      setPhFade(true);setTimeout(()=>{_setPh(p);setPhFade(false);},80);
    }
  };
  ...
  const[mood,setMood]=useState(50); // v0.7: Colony mood 0-100. Affects den + whispers
  ...
  const[sellMode,setSellMode]=useState(false);const[sellConfirm,setSellConfirm]=useState(null);
  ...
  const[hearthConfirm,setHearthConfirm]=useState(null); // v0.7: inline confirm for release
  ...
  function toast(icon,text,color="#fbbf24",dur=2500){const id=++toastRef.current;const neg=color==="#ef4444"||color==="#fb923c";setToasts(t=>[...t,{id,icon,text,color,big:dur>2500,neg}]);setTimeout(()=>setToasts(t=>t.filter(x=>x.id!==id)),dur);}
  ...
  {id:"hunger",name:"The Hunger",icon:"🌪️",taunt:"I was here before you named it.",
    tauntFn:(ctx)=>{const pf=ctx.prevFallen||[];
      ...
      return ctx.markedName?`${ctx.markedName} carries a scar like it means something. It doesn't. It means you bled and the dark noticed.`
      :ctx.fallen>0?`${ctx.fallenName} is gone and the rest are pretending the food will last. It won't.`
      :ctx.scarred>2?`${ctx.scarredName} carries scars like armor. They're not armor. They're receipts.`
      :ctx.colony>18?"All those mouths. I don't have to break in. I just have to wait."
      :ctx.bonded>2?`${ctx.bondedName} and ${ctx.bondedMateName}. Love is expensive. Every bond is a mouth you'll feed twice.`
      :ctx.gold<3?"You feel it already, don't you? That tightness. That counting."
      :"Fresh. Unbroken. I've seen that look eight times. It changes.";},
    ...
    defeatFn:(ctx)=>ctx.clutch?"That close. You smelled it. You'll smell it again."
      :ctx.deathless?`You fed them all. Even ${ctx.strongestName||"the strongest"}. Nobody's done that before.`
      :"You passed the first test. There are others.",
  ...
  {id:"territory",name:"The Territory",icon:"⛰️",taunt:"You built on someone else's grave.",
    tauntFn:(ctx)=>{const pf=ctx.prevFallen||[];
      ...
      return ctx.colony>18?`${ctx.colony} of you? The second colony had more. This ground ate them anyway.`
      :ctx.scarred>3?`${ctx.scarredName} has scars from my ground. Bold. Stupid, but bold.`
      :ctx.bonded>0?`${ctx.bondedName} thinks this is home now. The ground disagrees.`
      :ctx.grudges>2?"Fighting among yourselves on contested ground. The fifth colony did that."
      :"Every colony builds here. Every colony thinks it's different.";},
  ...
  {id:"mother",name:"The Mother",icon:"💔",taunt:"I had names for them, too. Every one.",
    tauntFn:(ctx)=>{const pf=ctx.prevFallen||[];
      ...
      return ctx.mourningName?`${ctx.mourningName} lost someone they loved. I know that look. I wore it until the end.`
      :ctx.fallen>2?`I never lost that many that fast. I lost them slowly. Watching each one go.`
      :ctx.fallen>0?`${ctx.fallenName}. I would have kept ${ctx.fallenName} safe. I would have kept all of them safe. That's what destroyed me.`
      :ctx.colony>18?"So many of them looking at you. I know that weight."
      :ctx.bonded>3?`${ctx.bondedName} loves someone in that colony. It's beautiful. Love is what makes the losing unbearable.`
      :ctx.colony<10?"Getting smaller. I know this part. I watched it happen to me."
      :`${ctx.strongestName} leads them. They look at ${ctx.strongestName} the way mine looked at me.`;},
    ...
    defeatFn:(ctx)=>ctx.clutch?"One number. That's all that stood between them and me."
      :ctx.deathless?"You kept them all. I never could."
      :ctx.fallen>0?"You've already lost some. And you're still going. That's the choice I couldn't make."
  ...
  {id:"swarm",name:"The Swarm",icon:"🐀",taunt:"We don't need to be stronger. Just patient.",
    tauntFn:(ctx)=>{const pf=ctx.prevFallen||[];
      ...
      return ctx.scarred>3?`${ctx.scarredName} is wounded. We can smell it. We're very good at smelling weakness.`
      :ctx.colony<12?`${ctx.colony} of you. Getting smaller. The math gets easier for us every round.`
      :ctx.colony>18?"Impressive numbers! Still not enough. We've done this math eight times."
      :ctx.grudges>2?`${ctx.grudgedName} is fighting their own. We love that. Saves us the trouble.`
      :`We've been watching ${ctx.strongestName}. Strong. But there's only one of them. There are so many of us.`;},
    defeat:"We are still counting.",
    defeatFn:(ctx)=>ctx.clutch?"We almost had you. We always almost have you."
      :ctx.fallen>0?`We took ${ctx.fallenName}. We'll take more next time.`
      :ctx.deathless?`Not a single one. Not even ${ctx.strongestName||"the weakest"}. The math says that shouldn't happen.`
      :"Fewer of us now. But never none. Never, ever none.",
    lore:"The math of extinction."},
  {id:"forgetting",name:"The Forgetting",icon:"🌫️",taunt:"What were their names? Say them. Quickly, now.",
    tauntFn:(ctx)=>{
      // v0.7: Cross-run memory — reference fallen from previous runs
      const pf=ctx.prevFallen||[];const prevName=pf.length>0?pf[0].name:null;
      ...
      return ctx.epithets>3?`They have titles now. "the Marked." "the Devoted." Titles are the last thing I erase. They think a name will save them.`
      :ctx.fallen>0?`What color were ${ctx.fallenName}'s eyes? What sound did they make when they slept? You're already forgetting. I don't have to do anything.`
      :ctx.colony>18?`${ctx.colony} names. Say them. All of them. Without looking. You can't, can you?`
      :ctx.scarred>2?`${ctx.scarredName} has scars. Which night did they get them? You don't remember. I'm already inside.`
      :ctx.bonded>3?`${ctx.bondedName} bonded with someone. What were the exact words? What was the weather? Gone. The details go first.`
      :"All of them? Still? Let me fix that.";},
    defeat:"You remembered. This time.",
    defeatFn:(ctx)=>ctx.clutch?"One number between you and oblivion."
      :ctx.deathless?"They'll remember this colony. I couldn't stop you. I've never failed before."
      :ctx.fallen>2?"You'll tell people about the survivors. Not the fallen. That's how I work. Not with violence. With editing."
      :"You passed. I'll adapt. I always adapt.",
  ...
  function genChronicle(won){
    ...
    // ACT II: The Middle. rich, specific, varied
    const mid=[];
    // Best hand. always include if exists
    if(bestHand){
      const bScore=bestHand.data.score.toLocaleString();
      const bType=bestHand.data.type;
      const bCats=bestHand.data.cats;
      if(bestHand.data.score>=50000)mid.push(`The hand that broke the pattern: ${bCats}, a ${bType} for ${bScore}. The dark doesn't forget numbers like that.`);
    ...
    // Hearth children performance
    if(hearthChildren.length>0){
      const hcBest=hearthChildren.sort((a,b)=>(b.stats?.bs||0)-(a.stats?.bs||0))[0];
      if(hcBest&&hcBest.stats?.bs>5000)mid.push(`${hcBest.name.split(" ")[0]}, child of the Hearth, scored ${hcBest.stats.bs.toLocaleString()} in a single hand. The bloodline proved itself.`);
    }
    // Big total
    if(totalScore>150000)mid.push(`Combined output: ${totalScore.toLocaleString()}. The kind of number that makes you wonder if the dark was ever the threat, or just the audience.`);

    // v0.7: mood-aware and involvement-aware mid entries
    ...
    // Take best 3 mid entries (prioritize variety)
    lines.push(mid.slice(0,3).join(" "));

    // ACT III: The Ending. epithet roll call
  ...
  function startGame(){
    ...
    setBelovedNames(meta); // ★ Load beloved names from hearth + player renames
    ...
    // ★ B1+FALLBACK DRAFT: Hearth children from saved pairs
    // 2+ pairs: both draft waves are Hearth children
    // 1 pair: Wave 1 = Hearth children, Wave 2 = random strangers
    // 0 pairs: both waves random (first run)
    function genHearthWave(size){
      const wave=[];
      const sortedPairs=[...hearthPrs].reverse(); // recent saves first
      for(let i=0;i<size;i++){
        const pair=sortedPairs[i%sortedPairs.length];
        const child=genDescendant(meta.cats,dp+ninthStarBonus,pair);
        // Single pair: force breed variety. first child gets parent 1's breed, second gets parent 2's
    ...
    // Generate waves based on Hearth pair count
    let wave1,wave2;
    if(hearthPrs.length>=2){
      ...
      wave1=genHearthWave(waveSize);
      wave2=genRandomWave(waveSize,true); // breed-weighted strangers
    }else{
      wave1=genRandomWave(waveSize,false);
      wave2=genRandomWave(waveSize,false);
    }
    // Also generate Wave 3 (used if draftPicked includes companion, shifting waves)
    ...
    // v0.7: The Return — rare ghost from a previous run (3% chance, run 3+)
    ...
    if(prevFallen.length>0&&(meta?.stats?.r||0)>=3&&Math.random()<0.03){
      ...
      if(ghostCat&&ghost){
        ...
        ghostCat.name=ghost.name; // Just the first name
        ghostCat.breed=ghost.breed||ghostCat.breed;
        ghostCat.power=Math.max(1,ghostCat.power-1); // Weaker
        ghostCat._returned=true;
        ghostCat._draftVoice="You remember me. I almost didn't come back.";
        ghostCat.name=gN(ghostCat.breed,ghostCat.trait); // Generate full name
        ghostCat.name=ghost.name; // Override with ghost name only
      }
    }

    // Cap traits: no Mythic in draft, no Legendary unless hearthDescendant or upgrade
    allDraft.forEach(c=>{
    ...
    // Guarantee at least 1 Rare across draft
    ...
    // ★ Ensure visual diversity per wave
    function ensureVisualDiversity(wave){
      ...
      for(let i=1;i<wave.length;i++){
        if(types.slice(0,i).includes(types[i])){
          ...
          if(vt&&!wave[i].hearthDescendant){wave[i].trait=pk(vt.pool);wave[i].name=gN(wave[i].breed,wave[i].trait);types[i]=vt.type;}
        }
      }
      return wave;
    }
    ...
    setBoss(BOSSES[0]);setSRes(null);setSStep(-1);setScoringDone(false);setHearthPair(null);
    ...
    setFams(startFams);
    setDen([]);setDenRes(null);setRunLog([]);setFallen([]);setAnteUp(null);setBossRewardChoices([]);prevRewardIdsRef.current=[];setRunBonus({hands:0});setDenNews([]);setFirstHandPlayed(false);setScoringCats([]);setAftermath([]);setColEvent(null);setColTargets([]);setTempMods({hands:0,discs:0,freeRecruits:0,nerveLock:0});setEventDenSafe(false);setEventDenBonus(0);setBabyNamingQueue([]);setFirstDenUsed(false);fcSeenRef.current={};setAutoPlay(null);setHtLevels({});setSScrolls([]);setDevotion({});
    ...
    if(meta&&meta.cats.length>0){
      ...
      if(hd.total>0){
        ...
        const u={...meta,dust:newDust};setMeta(u);saveS(u);
        // Check if player can now afford an upgrade
        ...
        if(canNow)setTimeout(()=>toast("✦","You can afford an upgrade. Check the Hearth.","#c084fc",3000),800);
      }
  ...
  function resumeRun(sr){
    ...
    setBelovedNames(meta); // ★ Load beloved names
    const sanitizeCat=c=>{
      if(!c)return c;
    ...
    setSRes(null);setSStep(-1);setScoringDone(false);setHearthPair(null);
    setBossTraits([]);setDareBet(false);setCurses([]);setCfx({});setOData(null);
    setSellMode(false);setSellsLeft(2);setDen([]);setDenRes(null);
    setScoringCats([]);setAftermath([]);setColEvent(null);setColTargets([]);
    setEventDenSafe(false);setEventDenBonus(0);setBabyNamingQueue([]);
  ...
  function pickDraft(i){
    ...
    if(picked.length>=maxPicks){
      // All picks done — batch naming for returning players, finalize for first run
      setDraftPicked(picked);
      if(!isFirstEverRun){
        // B: Batch naming — go to naming screen with first picked cat
        setBabyNamingQueue(picked.slice(1)); // queue remaining after first
        setNamingCat(picked[0]);
        picked[picked.length-1]._finalPick=true;
        setPh("naming");
      }else{
        finalizeDraft(picked);
      }
    }else{
      // More picks to make — advance to next wave
      setDraftPicked(picked);
  ...
  function advanceFromScoring(){
    ...
    try{
    if(autoPlay){setAutoPlay(null);setGuide({step:2,msg:""});}
    ...
    // ★ First-hand resonance teaching (F+Q): contextual toast AFTER first real play
    if(isFirstRun&&ante===1&&blind===0&&!autoPlay&&!firstHandPlayed){
      ...
      setTimeout(()=>{
        if(matched)toast("✦","Matched seasons resonate. They score together, not separately.","#fbbf24",4000);
        else toast("✦","Try same-season cats. They resonate. Stronger together than apart.","#fbbf24",4000);
      },800);
    }
    // ★ Thunder epithet. hand scored 500%+ of target
    if(tgt>0&&result.total>=tgt*5){
    ...
    cats.forEach(c=>{
      ...
      c.stats.tp++;c.stats.ts+=result.total;
      const wasPB=c.stats.bs;
      if(result.total>c.stats.bs){c.stats.bs=result.total;c.stats.bh=result.ht;
        // v0.7: Personal best notification (only after cat has played 3+ hands)
        if(c.stats.tp>=3&&result.total>wasPB*1.3){
          const fn=c.name.split(" ")[0];
          setDenNews(n=>[...n,{icon:"🏆",text:`${fn}: new personal best! ${result.total.toLocaleString()}`,color:BREEDS[c.breed]?.color||"#fbbf24"}]);
        }
      }
      if(wasKitten){
        const fn=c.name.split(" ")[0];
        ...
        // Propagate growth epithet power bonus (+2) to deck state AFTER growth
        if(c.epithetKey==="grownUp"){
          [setHand,setDraw,setDisc].forEach(s=>{s(arr=>arr.map(x=>x.id===c.id?{...x,power:c.power,epithet:c.epithet,epithetKey:c.epithetKey,_grewUp:true}:x));});
          setDenNews(n=>[...n,{icon:"🏷️",text:`${fn} earned: "${c.epithet}" (+2 Power)`,color:"#fbbf24"}]);
        }
      }
    });
    ...
    if(ns>=tgt){
      setMood(m=>Math.min(100,m+3)); // v0.7: winning feels good
      if(blind===2){
        Audio.bossClear();setMood(m=>Math.min(100,m+10));
        // v0.7: bossNight epithet — only ONE cat per boss, only Night 3+
        // The cat who contributed most earns it. Night 1-2 bosses are too easy to name.
        if(ante>=3){
        ...
        // Collect epithet flags for deferred toasts (fire after reward pick)
        [...cats].forEach(c=>{if(c.epithet){
          if(c._newEpithet){delete c._newEpithet;c._bossEpithet=true;}
          [setHand,setDraw,setDisc].forEach(s=>{s(arr=>arr.map(x=>x.id===c.id?{...x,epithet:c.epithet,epithetKey:c.epithetKey,_bossEpithet:c._bossEpithet}:x));});
        }});
      }
      ...
      setMood(m=>Math.max(0,m-10)); // v0.7: total failure
      endRun(false,ns);
    }
    // Bloodied trait: scars instead of injures.
      ...
      if(shouldInjure){
        ...
        if(eligible.length>0){
          ...
          setMood(m=>Math.max(0,m-5)); // v0.7: injury hurts morale
          ...
          if(isBloodied){
            ...
            // v0.7: Delay epithet toast 1.5s so it doesn't overlap scar toast
            if(victim._newEpithet){delete victim._newEpithet;setTimeout(()=>{toast("🏷️",epithetToastMsg(victim),BREEDS[victim.breed]?.color||"#fbbf24",3000);Audio.epithetEarned();},1500);}
            [setHand,setDraw,setDisc].forEach(s=>{s(arr=>arr.map(x=>x.id===victim.id?{...x,epithet:victim.epithet,epithetKey:victim.epithetKey}:x));});
          }else{
            [setHand,setDraw,setDisc].forEach(s=>{s(arr=>arr.map(x=>x.id===victim.id?{...x,injured:true,injuryTimer:2,power:Math.max(1,x.power),stats:{...x.stats,injuries:(x.stats?.injuries||0)+1}}:x));});
            victim.stats={...victim.stats,injuries:(victim.stats?.injuries||0)+1};
            assignEpithet(victim); // checks unbreakable (3+ injuries)
            toast("💀",`${victim.name.split(" ")[0]} was injured by ${boss?.name||"the boss"}!`,"#ef4444");
            // v0.7: Fix silent unbreakable epithet — delay toast so it doesn't overlap injury
            if(victim._newEpithet){delete victim._newEpithet;setTimeout(()=>{toast("🏷️",epithetToastMsg(victim),BREEDS[victim.breed]?.color||"#fbbf24",3000);Audio.epithetEarned();},1500);}
            [setHand,setDraw,setDisc].forEach(s=>{s(arr=>arr.map(x=>x.id===victim.id?{...x,epithet:victim.epithet,epithetKey:victim.epithetKey}:x));});
          }
        }
  ...
  function playH(){
    ...
    cats.forEach(c=>{
      if(c._mateDied&&!c._griefAcknowledged){
        const mn=fallen.find(f=>true)?.name?.split(" ")[0]; // approximate
        const mateCat=allC.find(x=>x.id===c.bondedTo);
        const mateName=mateCat?.name?.split(" ")[0]||mn||"their mate";
        toast("💔",`${c.name.split(" ")[0]} plays for the first time since ${mateName}. Slower. Heavier. But they play.`,"#f472b6",3000);
        c._griefAcknowledged=true;
        [setHand,setDraw,setDisc].forEach(s=>{s(arr=>arr.map(x=>x.id===c.id?{...x,_griefAcknowledged:true}:x));});
      }
    });
    ...
    function animStep(){
      ...
      if(step<tot){
        ...
        // Item 1: Tier-crossing hold. pause when score crosses a visual tier
        ...
        if(curTier&&(!prevTier||curTier.label!==prevTier.label)){
          setScoreShake(1);setTimeout(()=>setScoreShake(0),200);
        }
        // Item 8: Micro-shake on every scoring step
        if(!s?.xMult&&step!==thresholdStep){
          const magnitude=stepTotals[step].total>10000?2:1;
          setScoreShake(magnitude);setTimeout(()=>setScoreShake(0),100);
        }
        if(s){
          if(s.xMult){
          ...
          // Subtle tick on every step for rhythm
  ...
  function discardH(){
    ...
    // v0.7: Discard narrative awareness — 15% chance, subtle
    if(Math.random()<0.15&&d.length>0){
      const dc=d[0];const dn=dc.name.split(" ")[0];
      ...
      const whisper=mateInHand?`${mateInHand.name.split(" ")[0]} watches ${dn} go.`
        :isKitten?`${dn} never got a chance.`
        :isMvp&&(dc.stats?.tp||0)>5?`The colony's most played cat. Discarded.`
        :dc.epithet?`${dn} ${dc.epithet}. Sent to the bottom of the pile.`
        :null;
      if(whisper)toast("",whisper,"#ffffff33",2000);
    }
  ...
  function recruitCat(){
    ...
    if(actionLock.current)return;actionLock.current=true;requestAnimationFrame(()=>{actionLock.current=false;});
    setGold(g=>g-cost);Audio.recruit();setMood(m=>Math.min(100,m+1)); // v0.7: growing the team
    ...
    setSel(new Set());
    // v0.7: Name the recruit
    const rn=recruited?.name?.split(" ")[0]||"Someone";const rb=BREEDS[recruited?.breed]?.icon||"";
    toast("📣",cost===0?`${rb} ${rn} joins. Free.`:`${rb} ${rn} joins the hand. (${cost}🐟)`,cost>0?"#fbbf24":"#4ade80");
    Audio.cardSelect();
  }
  ...
  function endRun(won,finalScore){
    try{
    try{clearRunSave();setSavedRun(null);}catch(e){}
    ...
    if(meta){(async()=>{try{
      ...
      // Heat stays where player set it. On win, max available increases by 1.
      // On loss, heat stays (player chose this difficulty). No reset.
      ...
      // ★ Collect player-renamed names. these live on across runs
      const existingBeloved=new Set(meta.stats.belovedNames||[]);
      [...allC,...fallen].forEach(c=>{
        const firstName=c.name.split(" ")[0];
        // If the name isn't in any of the default pools, the player renamed it
      ...
      // Count run stats for achievements
      ...
      const u={...meta,heat:newHeat,ninthDawnCleared,relics,
        ...
        stats:{...meta.stats,r:meta.stats.r+1,w:meta.stats.w+(won?1:0),ba:Math.max(meta.stats.ba,ante),hs:Math.max(meta.stats.hs,fScore),mf:Math.max(meta.stats.mf,rMaxF),mh:newMaxHeat,bossRecord,heatWins,ninthDawnCleared,hearthTotal,belovedNames,
          ...
          campCount:meta.stats.campCount||0, // incremented in camp resolution
          hearthReleases:meta.stats.hearthReleases||0, // incremented in release
      ...
      // Award stardust for newly earned achievements
      const prevAchv=meta.achv||[];
      const freshAchvs=newAchv.filter(id=>!prevAchv.includes(id));
      let achvDust=0;
      freshAchvs.forEach(id=>{const a=ACHIEVEMENTS.find(x=>x.id===id);if(a?.dust)achvDust+=a.dust;});
  ...
  function nextBlind(){
    ...
    else{setNightCard({ante:na,blind:nb});setPh("nightCard");Audio.nightTransition();}
  }
  ...
  function sellCat(cat){
    ...
    setMood(m=>Math.max(0,m-4)); // v0.7: losing a member hurts morale
  ...
  function sellFam(f){setGold(g=>g+3);setFams(fs=>fs.filter(x=>x.id!==f.id));toast("🛡️",`${f.name} returned. +3🐟`,"#fbbf24");}
  ...
  function endNight(){
    ...
    if(campMode)setMood(m=>Math.min(100,m+5)); // v0.7: camp rest boosts morale
    setDenNews([]);
    const hasMM=false;
    const heatFight=getHeatFx(meta?.heat).denFight||0;
    const baseCtx={draftRejects,deckSize:dAll.length,nerveLvl:ferv,breedBoost:(getAllDevotionFx(devotion).breedBoost||0)+(getMB().breedBoost||0),mood};
    ...
    results.forEach(r=>{
      if(r.type==="breed"){
        r.baby._bornNight=ante; // v0.7: Track birth night for biography + last words
        setDraw(d=>[...d,r.baby]);
        if(r.twins){const twin=breedC(r.c1,r.c2);twin._bornNight=ante;setDraw(d=>[...d,twin]);r.twin2=twin;}
        // Track kids bred for Parent epithet
        [setHand,setDraw,setDisc].forEach(setter=>{
          setter(arr=>arr.map(c=>{
            if(c.id===r.c1.id){c.stats={...c.stats,kidsBreed:(c.stats?.kidsBreed||0)+1+(r.twins?1:0)};assignEpithet(c);return{...c,bondedTo:r.c2.id,stats:c.stats,epithet:c.epithet,epithetKey:c.epithetKey,earnedEpithets:c.earnedEpithets};}
            if(c.id===r.c2.id){c.stats={...c.stats,kidsBreed:(c.stats?.kidsBreed||0)+1+(r.twins?1:0)};assignEpithet(c);return{...c,bondedTo:r.c1.id,stats:c.stats,epithet:c.epithet,epithetKey:c.epithetKey,earnedEpithets:c.earnedEpithets};}
    ...
    results.forEach(r=>{
      ...
      if(r.type==="death"){logEvent("death",{victim:r.victim.name});
        Audio.denDeath();
        // v0.7: Last words based on relationships
        const vn=r.victim.name.split(" ")[0];const vb=r.victim.bondedTo;const vs=r.victim.scarred;const vk=r.victim._bornNight;
        const mate=vb?[...hand,...draw,...disc].find(x=>x.id===vb):null;const mn=mate?.name.split(" ")[0];
        const lastWords=vb&&mn?pk([`Tell ${mn} I didn't leave on purpose.`,`${mn} will understand. ${mn} always understood.`])
          :vs?pk(["I knew this was coming. The scar told me.","Not surprised. Just tired."])
          :vk?pk(["I never got to play a hand.","Born in the dark. Stayed in the dark."])
          :(r.victim.stats?.tp||0)>8?pk([`${vn} played ${r.victim.stats.tp} hands. That's a life.`,`${vn} scored ${(r.victim.stats?.bs||0).toLocaleString()} once. The colony won't forget.`])
          :pk([`${vn} is gone.`,`The colony is one name lighter.`]);
        toast("💀",lastWords,"#ef4444",3500);
        // Mourning: mark bonded partner
        if(r.victim.bondedTo){
          [setHand,setDraw,setDisc].forEach(s=>{s(arr=>arr.map(x=>{
            if(x.id===r.victim.bondedTo){
              x._mateDied=true;
              assignEpithet(x);
              if(x._newEpithet){delete x._newEpithet;toast("🏷️",epithetToastMsg(x),BREEDS[x.breed]?.color||"#fbbf24",2500);Audio.epithetEarned();}
    ...
    results.forEach(r=>{
      const applyEp=(cat,ctx={})=>{assignEpithet(cat,ctx);if(cat._newEpithet){delete cat._newEpithet;toast("🏷️",epithetToastMsg(cat),BREEDS[cat.breed]?.color||"#fbbf24",2500);Audio.epithetEarned();}[setHand,setDraw,setDisc].forEach(s=>{s(arr=>arr.map(c=>c.id===cat.id?{...c,epithet:cat.epithet,epithetKey:cat.epithetKey,story:cat.story}:c));});};
    ...
    const denSound=(r)=>{
      if(r.type==="breed"){Audio.denBirth();setMood(m=>Math.min(100,m+5));}
      else if(r.type==="death"){Audio.denDeath();setMood(m=>Math.max(0,m-15));}
      else if(r.type==="fight"||r.type==="training"){Audio.denFight();setMood(m=>Math.max(0,m-3));}
      else if(r.type==="bond"||r.type==="reconcile_bond"){Audio.denBond();setMood(m=>Math.min(100,m+10));}
      else if(r.type==="grudge"){Audio.denGrudge();setMood(m=>Math.max(0,m-8));}
      else if(r.type==="reconcile"){Audio.denBond();setMood(m=>Math.min(100,m+8));}
      else if(r.type==="teach"){Audio.denBond();setMood(m=>Math.min(100,m+5));}
  ...
  function fireEvent(){
    ...
    let pool=[...eventSource].filter(e=>{
      ...
      // v0.7: condFn gates events by runtime context (e.g. The Question requires 10+ runs)
      if(e.condFn&&!e.condFn(targets,{meta,colony:all.length,fallen,all}))return false;
      // "once" events only fire once per meta lifetime
    ...
    setColEvent(evt);setColTargets(targets);setPh("event");Audio.eventReveal();
  }
  ...
  function chooseEvent(idx){
    ...
    // v0.7: "once" events persist across runs in meta
    if(colEvent.once&&meta){const so=[...(meta.seenOnce||[]),colEvent.id];setMeta(m=>({...m,seenOnce:so}));}
    ...
    if(fx.sacrifice&&all.length>MIN_DECK){
      ...
      if(victim){
        ...
        setMood(m=>Math.max(0,m-15)); // v0.7: sacrifice drops mood
      }
    }
    // v0.7: Spare target — mark the would-be sacrifice
    if(fx.spareTarget){
      const weakest=targets[0]||[...all].sort((a,b)=>a.power-b.power)[0];
      if(weakest){
        [setHand,setDraw,setDisc].forEach(s=>{s(arr=>arr.map(c=>{
          if(c.id===weakest.id){c._spared=true;assignEpithet(c);
            if(c._newEpithet){delete c._newEpithet;toast("🏷️",epithetToastMsg(c),BREEDS[c.breed]?.color||"#fbbf24",2500);Audio.epithetEarned();}
            return{...c};
          }return c;}));});
        toast("🕊️",`${weakest.name.split(" ")[0]} stays. They don't know how close it was.`,"#4ade80",3000);
        setMood(m=>Math.min(100,m+10)); // v0.7: sparing boosts mood
      }
    }
    ...
    // v0.7: The Offering — spare the weakest, they gain an epithet
    if(fx.spareTarget){
      const weakest=targets[0]||[...all].sort((a,b)=>a.power-b.power)[0];
      if(weakest){
        [setHand,setDraw,setDisc].forEach(s=>{s(arr=>arr.map(x=>{
          if(x.id===weakest.id){x._spared=true;assignEpithet(x);return{...x};}return x;
        }));});
        const wn=weakest.name.split(" ")[0];
        toast("🕊️",`${wn} stays. They don't know how close it was.`,"#4ade80",3000);
      }
    }
    if(fx.targetTrait){
      const traitTarget=targets[0]||(()=>{const plain=all.filter(c=>catIsPlain(c));return plain.length>0?pk(plain):pk(all);})();
      if(traitTarget){const rt=pk(COMMON_TRAITS);
        [setHand,setDraw,setDisc].forEach(s=>{s(arr=>arr.map(x=>{if(x.id===traitTarget.id){addTrait(x,rt);return{...x};}return x;}));});
  ...
  function buildOutcomeText(evt,choice,targets,fx){
    ...
    if(fx.targetTrait)lines.push({text:targets[0]?`${targets[0].name.split(" ")[0]} learned something new.`:"A cat learned something from the old marks.",color:"#c084fc",icon:"📜"});
  ...
  const CSS=`
    ...
    *{box-sizing:border-box;-webkit-tap-highlight-color:transparent}
    :root{
      --gold:oklch(0.82 0.16 85);--red:oklch(0.63 0.24 25);--green:oklch(0.72 0.19 155);
      --purple:oklch(0.7 0.17 300);--pink:oklch(0.7 0.16 340);--blue:oklch(0.62 0.19 260);
      --cyan:oklch(0.82 0.12 200);--orange:oklch(0.76 0.17 55);
      --bg:#06060f;--text:#e8e6e3;
    }
    body{margin:0;background:var(--bg);font-family:'Manrope',sans-serif;color:var(--text)}
    button{
      min-height:44px;min-width:44px;
      &:active:not(:disabled){transform:scale(0.96)!important;transition:transform 0.08s!important}
    }
    ...
    ::view-transition-old(root){animation:fadeOut .12s ease-out}
    ::view-transition-new(root){animation:fadeIn .2s ease-out}
  ...
  const Dust=()=>(<div style={{position:"fixed",inset:0,pointerEvents:"none",zIndex:0,overflow:"hidden",contain:"strict"}}>
    {/* Film grain */}
    <div style={{position:"absolute",inset:0,opacity:.03,mixBlendMode:"screen",backgroundImage:"url(\"data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.85' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='256' height='256' filter='url(%23n)' opacity='0.5'/%3E%3C/svg%3E\")",backgroundSize:"128px 128px",animation:"shimmer 4s ease-in-out infinite"}}/>
    ...
    {/* Parallax: large slow shapes for depth */}
    {Array.from({length:4}).map((_,i)=>{
      const left2=15+i*22;const dur2=25+i*8;const delay2=i*5;const size2=3+i*2;
      const col2=isBoss?"#ef444408":ferv>=12?"#fbbf2406":"#ffffff04";
      return(<div key={`p${i}`} style={{position:"absolute",left:`${left2}%`,bottom:"-10px",
        width:size2,height:size2,borderRadius:"50%",background:col2,
  ...
  if(ph==="coldOpen"){
    ...
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      <div onClick={advance} style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:0,padding:20,cursor:"pointer"}}>
        ...
        {introStep>=coldLines.length-1&&<div style={{fontSize:11,color:"#ffffff66",letterSpacing:3,marginTop:24,animation:"fadeIn 2s ease-out 1s both"}}>tap to continue</div>}
      </div>
    </div>);
  }
  ...
  if(ph==="firstIntro"){
    const pages=[
      ...
      ()=>(<>
        <div style={{fontSize:56,animation:"float 3s ease-in-out infinite",filter:"drop-shadow(0 0 30px #fbbf2444)"}}>🔥</div>
        <div style={{fontSize:20,fontWeight:900,color:"#fbbf24",letterSpacing:6,animation:"fadeIn .8s ease-out",textShadow:"0 0 30px #fbbf2433"}}>THE LAST HEARTH</div>
        <div style={{fontSize:14,color:"#ffffffbb",textAlign:"center",maxWidth:320,lineHeight:2.2,animation:"fadeIn 1.2s ease-out"}}>
          A fire still burns. Cats are gathering.
        </div>
        <div style={{fontSize:13,color:"#ffffff88",textAlign:"center",maxWidth:300,lineHeight:1.9,animation:"fadeIn 1.8s ease-out"}}>
      ...
      ()=>(<>
        <div style={{display:"flex",gap:10,justifyContent:"center",animation:"fadeIn .6s ease-out",marginBottom:8}}>
          {["Autumn","Winter","Spring","Summer"].map(s=>{const b=BREEDS[s];return(
            <div key={s} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,padding:"6px 10px",borderRadius:8,background:`${b.color}11`,border:`1px solid ${b.color}22`}}>
              <span style={{fontSize:18}}>{b.icon}</span>
              <span style={{fontSize:10,color:b.color,fontWeight:700}}>{b.name}</span>
            </div>
          );})}
        </div>
        <div style={{fontSize:16,fontWeight:700,color:"#4ade80",letterSpacing:3,animation:"fadeIn .8s ease-out",textShadow:"0 0 20px #4ade8033"}}>ONE RULE</div>
        <div style={{fontSize:15,color:"#ffffffcc",textAlign:"center",maxWidth:320,lineHeight:2,animation:"fadeIn 1.2s ease-out"}}>
        ...
        <div style={{fontSize:13,color:"#fbbf2466",fontStyle:"italic",textAlign:"center",maxWidth:300,lineHeight:1.8,animation:"fadeIn 2.5s ease-out 0.5s both",marginTop:12,padding:"10px 16px",borderRadius:10,background:"#ffffff04",border:"1px solid #ffffff08"}}>
          Three cats need a name. Choose well.
        </div>
      </>),
    ];
    ...
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:20,padding:20,maxWidth:420}} onClick={()=>{
        ...
        <div style={{display:"flex",gap:8,marginTop:8}}>
          {pages.map((_,i)=>(<div key={i} style={{width:8,height:8,borderRadius:"50%",background:i===introStep?"#fbbf24":i<introStep?"#fbbf2444":"#333",transition:"all .15s"}}/>))}
        </div>
        <div style={{fontSize:12,color:"#ffffff44",animation:"fadeIn 2s ease-out",letterSpacing:2}}>{isLast?"tap to begin":"tap to continue"}</div>
      </div>
    </div>);
  }
  ...
  if(ph==="naming"&&namingCat){
    ...
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:12,padding:20,maxWidth:420}}>
        <div style={{fontSize:10,color:"#ffffff55",letterSpacing:6,animation:"fadeIn .6s ease-out"}}>
          {babyNamingQueue.length>0&&draftPicked.length>0?"NAME YOUR CATS":"NAME YOUR CAT"}
        </div>
        {/* B: Batch naming progress */}
        {draftPicked.length>0&&babyNamingQueue.length>0&&<div style={{display:"flex",gap:8,alignItems:"center",animation:"fadeIn .4s ease-out"}}>
          {draftPicked.map((p,pi)=>{const done=pi<(draftPicked.length-babyNamingQueue.length-1);const active=p.id===namingCat.id;
            return <div key={pi} style={{width:10,height:10,borderRadius:"50%",background:done?"#4ade80":active?"#fbbf24":"#333",boxShadow:active?"0 0 8px #fbbf2444":"none",transition:"all .15s"}}/>;
          })}
          <span style={{fontSize:10,color:"#666"}}>{draftPicked.length-babyNamingQueue.length} of {draftPicked.length}</span>
        </div>}
        ...
        {tr.name!=="Plain"?(<div style={{padding:"12px 16px",borderRadius:10,background:`${tierCol}08`,border:`1px solid ${tierCol}22`,maxWidth:340,width:"100%",animation:"fadeIn 1s ease-out"}}>
          ...
          <div style={{fontSize:12,color:"#ffffff77",lineHeight:1.7}}>{detail}</div>
        </div>):(
          <div style={{padding:"10px 16px",borderRadius:10,background:"#ffffff06",border:"1px solid #ffffff11",maxWidth:340,width:"100%",animation:"fadeIn 1s ease-out"}}>
            <div style={{fontSize:12,color:"#ffffff77",lineHeight:1.7}}>No special trait yet. This cat can earn one through events, breeding, or the shop.</div>
          </div>
        )}
        ...
        />
        <div style={{fontSize:10,color:"#ffffff55",letterSpacing:2}}>TAP TO RENAME</div>
        <button id="nameConfirm" onClick={()=>{
          ...
          if(babyNamingQueue.length>0){
            ...
            if(remaining.length>0){
              ...
              setNamingCat(null);
              // B: If we're naming draft picks, finalize draft. Otherwise advance night.
  ...
  if(ph==="hearthFlash"&&hearthFlash){
    ...
    const companions=hearthCats.map(c=>{
      const breed=c.breed||"Autumn";const tr=c.trait||PLAIN;
    ...
    const colLine=colNum===2?"New names around the same fire."
      :colNum<=4?"The fire still burns. So do you."
      :(meta?.stats?.w||0)===0?"The fire doesn't ask how many times you've tried."
      :(meta?.stats?.w||0)>=5?"The dark knows your name by now."
      :"New colony. Same question. Different answer.";
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:16,padding:20,maxWidth:500}}>
        {/* D: Returning player one-liner — colony number + tone */}
        <div style={{fontSize:11,color:"#ffffff33",letterSpacing:4,animation:"fadeIn 1s ease-out"}}>COLONY {colNum}</div>
        <div style={{fontSize:12,color:"#ffffff55",fontStyle:"italic",textAlign:"center",maxWidth:300,lineHeight:1.6,animation:"fadeIn 1.5s ease-out"}}>{colLine}</div>

        {/* E: Hearth legend pick — narrative, not rules */}
        <div style={{fontSize:48,animation:"float 3s ease-in-out infinite",filter:"drop-shadow(0 0 20px #fbbf2444)",marginTop:8}}>🔥</div>
        <div style={{fontSize:14,color:"#fbbf24cc",letterSpacing:4,fontWeight:700,animation:"fadeIn 1.5s ease-out"}}>WHO WALKS BACK?</div>
        <div style={{fontSize:12,color:"#fbbf24aa",textAlign:"center",maxWidth:340,lineHeight:1.7,animation:"fadeIn 2s ease-out"}}>
          The Hearth holds {hearthCats.length} name{hearthCats.length>1?"s":""}. One can return to the dark. Full strength. One last time.
        </div>
        <div style={{display:"flex",gap:4,flexWrap:"wrap",justifyContent:"center",maxWidth:360,marginBottom:4}}>
          {hearthCats.map((c,i)=>(<span key={i} style={{fontSize:9,color:BREEDS[c.breed]?.color||"#fbbf24",opacity:.4,animation:`fadeIn ${.5+i*.1}s ease-out both`}}>{c.name?.split(" ")[0]||"?"}</span>))}
        </div>
        <div style={{display:"flex",gap:14,flexWrap:"wrap",justifyContent:"center",animation:"fadeIn 2.5s ease-out"}}>
          {companions.map((c,i)=>(<div key={i} onClick={()=>{
            setDraftPicked([c]);
            setHearthFlash(null);setPh("draft");
            toast(BREEDS[c.breed].icon,`${c.name.split(" ")[0]} returns. One last walk into the dark.`,BREEDS[c.breed].color,4000);
            if(meta.stats.r===1){
              setTimeout(()=>toast("🆕","Recruit unlocked! Pay 🐟 to draw extra cats.","#4ade80",4000),1200);
              setTimeout(()=>toast("🆕","Interest unlocked! Save 5+ 🐟 for bonus each round.","#4ade80",4000),2400);
              setTimeout(()=>toast("🆕","Scrolls unlocked! Level up hand types in the shop.","#4ade80",4000),3600);
            }
          }} style={{cursor:"pointer",textAlign:"center",maxWidth:90,transition:"all .15s"}}
          onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-6px)";}}
          onMouseLeave={e=>{e.currentTarget.style.transform="translateY(0)";}}>
            <div style={{animation:`float ${2.5+i*.3}s ease-in-out ${i*.2}s infinite`}}>
              <CC cat={c} sm/>
            </div>
            <div style={{fontSize:12,color:BREEDS[c.breed].color,marginTop:3,fontWeight:700}}>{c.name.split(" ")[0]}</div>
            {c.epithet&&<div style={{fontSize:9,color:"#fbbf2466"}}>{c.epithet}</div>}
            <div style={{fontSize:10,color:"#ffffff77"}}>{(c.trait||PLAIN).icon} P{c.power}{c.scarred?" ⚔":""}</div>
          </div>))}
        </div>
        <button onClick={()=>{setHearthFlash(null);setPh("draft");
          if(meta.stats.r===1){
        ...
        }} style={{...BTN("#1a1a2e","#666"),padding:"8px 20px",fontSize:11,border:"1px solid #ffffff12",marginTop:4}}>No one. Face the dark alone.</button>
      </div>
    </div>);
  }
  ...
  if(ph==="draft"){
    ...
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:14,padding:20,maxWidth:500}}>
        <div style={{fontSize:10,color:"#888",letterSpacing:4,animation:"fadeIn .5s ease-out"}}>{hasHearthLineage?"DESCENDANTS OF THE HEARTH":"THE GATHERING"}</div>
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          ...
          <span style={{fontSize:10,color:"#666",marginLeft:4}}>{draftPicked.length+1} of 3</span>
        </div>
        <div style={{fontSize:11,color:"#d9770699",fontStyle:"italic",textAlign:"center",lineHeight:1.7,maxWidth:320,animation:`fadeIn ${isFirstRun?"1s":"0.4s"} ease-out`}}>{storyText}</div>

        {/* ★ Rubber-banding hint. visible after first run. confusing for brand-new players */}
        {!isFirstRun&&draftPicked.length===0&&<div style={{fontSize:12,color:"#fb923c",textAlign:"center",maxWidth:300,lineHeight:1.6,animation:"fadeIn 0.5s ease-out 0.2s both",padding:"6px 12px",borderRadius:6,background:"#fb923c0d",border:"1px solid #fb923c22",fontWeight:700}}>⚖️ Strong cats attract weak strays. Choose wisely.</div>}

        {isFirstRun&&draftPicked.length===0&&<div style={{fontSize:11,color:"#fbbf2466",textAlign:"center",maxWidth:280,lineHeight:1.5,animation:"fadeIn 1s ease-out .3s both"}}>Pick who catches your eye.</div>}
        ...
        {!isFirstRun&&<div style={{fontSize:10,color:"#888",letterSpacing:1,marginTop:4}}>{
          draftPool[0]?._draftWave==="hearth"?"Choose one. The bloodline continues."
          :"Choose one. The others scatter into the dark."
        }</div>}
        {draftPool[0]?._draftWave&&!isFirstRun&&<div style={{fontSize:10,color:draftPool[0]._draftWave==="hearth"?"#c084fc88":"#ffffff33",letterSpacing:3,marginBottom:2}}>
          {draftPool[0]._draftWave==="hearth"?"HEARTH CHILDREN":"STRANGERS"}
        </div>}
        <div style={{display:"flex",gap:14,justifyContent:"center",animation:`fadeIn .4s ease-out ${isFirstRun?"0.8s":"0.3s"} both`}}>
          {draftPool.map((c,i)=>{
            ...
            return(<div key={c.id} onClick={()=>pickDraft(i)} style={{cursor:"pointer",textAlign:"center",maxWidth:110,
              animation:`cardFlip .5s ease-out ${(isFirstRun?0.9:0.4)+i*.2}s both`,
            ...
            onMouseLeave={e=>{e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.filter="brightness(1)";}}>
              ...
              {c.trait.name!=="Plain"&&(()=>{
                ...
                return(<div style={{fontSize:10,color:tier.color,marginTop:2,lineHeight:1.3,maxWidth:120}}>
                  ...
                  {isFirstRun?(hook[c.trait.name]||c.trait.desc):c.trait.desc}
                </div>);
              })()}
              {c._hearthParents&&<div style={{color:"#c084fc",fontSize:10,fontStyle:"italic",lineHeight:1.3,marginTop:1}}>
                child of {c._hearthParents}.{" "}
                {c._parentTrait&&c.trait.name===c._parentTrait?<span style={{color:"#fbbf24"}}>Carries the same fire.</span>
                :c._parentTrait?<span style={{color:"#c084fc88"}}>Something different in the eyes.</span>
                :null}
              </div>}
              {!c._hearthParents&&c.stats?.par&&<div style={{color:"#c084fc",fontSize:10,fontStyle:"italic",lineHeight:1.3,marginTop:1}}>
                Child of {c.stats.par}
              </div>}
            </div>);
          })}
        </div>
        {!isFirstRun&&draftRejects.length>0&&<div style={{fontSize:10,color:"#666",fontStyle:"italic"}}>The ones you turned away haven't gone far...</div>}
      </div>
    </div>);
  }
  ...
  if(ph==="nightCard"&&nightCard){
    ...
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:0,padding:20}} onClick={()=>{setNightCard(null);setPh("playing");if(isFirstEver)setTimeout(startAutoPlay,400);}}>
        ...
        <div style={{fontSize:isBoss?52:44,fontWeight:700,letterSpacing:"clamp(12px,3vw,20px)",color:glowColor,
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16,marginTop:6}}>
          ...
          <div style={{fontSize:11,color:"#ffffff55",letterSpacing:8}}>NIGHT {nightCard.ante}</div>
        ...
        {isBoss&&boss&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6,padding:"18px 28px",borderRadius:14,
          ...
          <div style={{fontSize:20,color:"#ef4444",fontWeight:700,letterSpacing:3}}>{boss.name}</div>
          <div style={{width:80,height:1,background:"linear-gradient(90deg,transparent,#ef444433,transparent)",margin:"4px 0"}}/>
          <div style={{fontSize:11,color:"#ef444488",fontStyle:"italic",textAlign:"center",lineHeight:1.5}}>{boss.lore}</div>
          {boss.tauntFn&&(()=>{
            const scarredCats=allC.filter(c=>c.scarred);const bondedCats=allC.filter(c=>c.bondedTo);const grudgedCats=allC.filter(c=>(c.grudgedWith||[]).length>0);
            const strongest=[...allC].sort((a,b)=>b.power-a.power)[0];const weakest=[...allC].sort((a,b)=>a.power-b.power)[0];
            const epithetCats=allC.filter(c=>c.epithet);const markedCat=allC.find(c=>c.epithetKey==="scarred");const mourningCat=allC.find(c=>c.epithetKey==="mourning");
            const bCtx={fallen:fallen.length,fallenName:fallen.length>0?fallen[fallen.length-1].name.split(" ")[0]:"",scarred:scarredCats.length,bonded:bondedCats.length,colony:allC.length,gold,grudges:grudgedCats.length/2,
              scarredName:scarredCats[0]?.name.split(" ")[0]||"",bondedName:bondedCats[0]?.name.split(" ")[0]||"",bondedMateName:bondedCats[0]?.bondedTo?allC.find(c=>c.id===bondedCats[0].bondedTo)?.name.split(" ")[0]||"":"",
              grudgedName:grudgedCats[0]?.name.split(" ")[0]||"",strongestName:strongest?.name.split(" ")[0]||"",weakestName:weakest?.name.split(" ")[0]||"",
              epithets:epithetCats.length,markedName:markedCat?.name.split(" ")[0]||"",mourningName:mourningCat?.name.split(" ")[0]||"",clutch:false,
              // v0.7: Cross-run memory
              prevFallen:meta?.allFallen||[],totalRuns:meta?.stats?.r||0,totalDeaths:meta?.stats?.totalFallen||0};
            const taunt=boss.tauntFn(bCtx);
            return taunt?<div style={{fontSize:12,color:"#ef4444bb",fontStyle:"italic",textAlign:"center",lineHeight:1.6,marginTop:4}}>"{taunt}"</div>:null;
          })()}
        </div>}

        {/* Non-boss: whisper + epigraph */}
        {!isBoss&&<>
          <div style={{fontSize:14,color:"#ffffff77",fontStyle:"italic",letterSpacing:2,animation:"fadeIn 1s ease-out",textAlign:"center",textShadow:"0 0 20px #ffffff11",maxWidth:320,marginBottom:8}}>{whisper}</div>
          <div style={{fontSize:13,color:"#ffffff55",textAlign:"center",maxWidth:320,lineHeight:1.8,animation:"fadeIn 1.2s ease-out",letterSpacing:4,fontWeight:500}}>{epi}</div>
          {(()=>{const mw=getMoodWhisper(mood,allC.length);const sub=mw||NIGHT_SUB[Math.min(nightCard.ante-1,4)];return sub?<div style={{fontSize:12,color:mw?"#ffffff66":"#ffffff55",fontStyle:"italic",textAlign:"center",maxWidth:280,lineHeight:1.5,animation:"fadeIn 1.5s ease-out",marginTop:4}}>{sub}</div>:null;})()}
        </>}
        ...
        {nightCard.blind===0&&nightCard.ante>1&&(()=>{
          ...
          // Narrator recap. connects the story, escalates revelation
          const nCtx={fallen:fallen.length,bonds:allC.filter(c=>c.bondedTo).length/2,grudges:allC.reduce((s,c)=>s+(c.grudgedWith||[]).length,0)/2,breeds:runLog.filter(e=>e.type==="breed").length,colony:allC.length,eventHistory:eventHistory,scarred:allC.filter(c=>c.scarred).length,allCats:allC};
          const narLine=getNarratorRecap(nightCard.ante,nCtx);

          return <>
            {showWhisper&&<div style={{fontSize:11,color:"#fbbf2466",fontStyle:"italic",textAlign:"center",maxWidth:300,lineHeight:1.5,animation:"fadeIn 2s ease-out .5s both",marginTop:8}}>{wLine}</div>}
            {narLine&&<div style={{fontSize:11,color:"#c084fc55",fontStyle:"italic",textAlign:"center",maxWidth:320,lineHeight:1.6,animation:"fadeIn 2.5s ease-out 1s both",marginTop:showWhisper?4:8,padding:"6px 12px",borderRadius:8,background:"#c084fc06",border:"1px solid #c084fc11"}}>{narLine}</div>}
            {/* v0.7: Pattern recognition whispers (Kasavin: "the game pays attention") */}
            {(()=>{
              if(Math.random()>0.25)return null; // 25% chance to fire
        ...
        {isNinthDawn&&<div style={{fontSize:11,color:"#fbbf2466",letterSpacing:5,animation:"fadeIn 1.8s ease-out",marginTop:8}}>THE NINTH DAWN</div>}
        ...
        {nightCard.ante>=MX&&nightCard.blind===0&&(fallen.length>0||allC.length>0)&&<div style={{
          ...
          {fallen.length>0&&<div style={{marginBottom:6}}>
            <div style={{fontSize:10,color:"#ef444488",letterSpacing:3,marginBottom:4}}>DID NOT MAKE IT</div>
            {fallen.map((f,i)=><div key={i} style={{fontSize:11,color:BREEDS[f.breed]?.color||"#ef4444",opacity:.6,lineHeight:1.5}}>
          ...
          <div style={{display:"flex",gap:4,flexWrap:"wrap",justifyContent:"center"}}>
            {allC.slice(0,16).map((c,i)=><span key={i} style={{fontSize:10,color:BREEDS[c.breed]?.color||"#888",opacity:.5}}>
        ...
        <div style={{display:"flex",gap:0,alignItems:"stretch",marginTop:20,animation:"fadeIn 1.5s ease-out .3s both",borderRadius:14,overflow:"hidden",border:`1.5px solid ${glowColor}15`,
          ...
          <div style={{textAlign:"center",padding:"16px 24px"}}>
            <div style={{fontSize:10,color:`${glowColor}66`,letterSpacing:3,marginBottom:4}}>TARGET</div>
          ...
          <div style={{textAlign:"center",padding:"16px 20px"}}>
            <div style={{fontSize:10,color:"#3b82f688",letterSpacing:3,marginBottom:4}}>HANDS</div>
            <div style={{fontSize:isFirstEver?34:28,fontWeight:900,color:"#3b82f6"}}>{hLeft}</div>
          </div>
          <div style={{width:1,background:"#ffffff08"}}/>
          <div style={{textAlign:"center",padding:"16px 20px"}}>
            <div style={{fontSize:10,color:"#ef444488",letterSpacing:3,marginBottom:4}}>DISCARDS</div>
            <div style={{fontSize:isFirstEver?34:28,fontWeight:900,color:"#ef4444"}}>{ncDiscs}</div>
          </div>
        </div>);})()}

        {isFirstEver&&<div style={{fontSize:12,color:"#4ade80bb",animation:"fadeIn 2s ease-out .8s both",marginTop:12}}>Tap cats {"\u2192"} Play Hand {"\u2192"} Beat {ncTgt.toLocaleString()}</div>}
        <div style={{fontSize:10,color:"#ffffff55",marginTop:20,animation:`fadeIn 2s ease-out ${isFirstEver?1.2:.6}s both`}}>tap to begin</div>
      </div>
    </div>);
  }
  ...
  if(ph==="colonyFormed"&&colonyData){
    ...
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:14,padding:20,maxWidth:600}}>
        {isNinthDawn&&<div style={{fontSize:10,color:"#fbbf24",letterSpacing:6,animation:"float 3s ease-in-out infinite",marginBottom:-4}}>「 THE NINTH DAWN 」</div>}
        {!isFirstRun&&<div onClick={e=>e.stopPropagation()} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4,animation:"fadeIn 1s ease-out"}}>
          <div style={{fontSize:11,color:"#fbbf24",letterSpacing:4,fontWeight:700}}>NAME YOUR COLONY</div>
        ...
        {isFirstRun?<>
          <div style={{fontSize:10,color:"#999999aa",letterSpacing:6,animation:"fadeIn 1.5s ease-out"}}>YOUR COLONY</div>
          <div style={{display:"flex",gap:16,justifyContent:"center",marginTop:8}}>
            {chosen.map((c,i)=>{const b=BREEDS[c.breed];return(
              <div key={c.id} style={{textAlign:"center",animation:`fadeIn .6s ease-out ${.3+i*.4}s both`}}>
                <CC cat={c} hl/>
                <div style={{fontSize:12,color:b.color,marginTop:4,fontWeight:700}}>{c.name.split(" ")[0]}</div>
                <div style={{fontSize:10,color:"#ffffff77",fontStyle:"italic",lineHeight:1.4,marginTop:2,maxWidth:100}}>"{cpk(flavorCache,`colVoice_${c.id}`,[c],()=>getDraftVoice(c,meta))}"</div>
              </div>);
            })}
          </div>
        ...
        :<>
          <div style={{fontSize:10,color:"#999999aa",letterSpacing:6,animation:"epicReveal 1.2s ease-out forwards"}}>SURVIVE {MX} NIGHTS</div>
          {meta&&meta.stats.r>0&&<div style={{fontSize:10,color:"#777777bb",letterSpacing:3}}>{(()=>{const n=(meta.stats.r||0)+1;const w=meta.stats.w||0;if(n===2)return`Colony #${n}. New names around the same fire.`;if(n<=4)return`Colony #${n}. The fire still burns. So do you.`;if(n<=7&&w===0)return`Colony #${n}. The fire doesn't ask how many times you've tried. It just burns.`;if(n<=7&&w>=1)return`Colony #${n}. The Hearth remembers what you built. Build it again.`;if(w>=5)return`Colony #${n}. The dark knows your name by now. Good.`;return`Colony #${n}. New colony. Same question. Different answer.`;})()}</div>}
          <div style={{fontSize:11,color:"#888",letterSpacing:4,animation:"fadeIn .6s ease-out .2s both"}}>THE COLONY</div>
          <div style={{fontSize:15,color:"#d97706bb",fontStyle:"italic",textAlign:"center",lineHeight:1.6,maxWidth:360,animation:"fadeIn .8s ease-out .15s both"}}>
            {colonyThesis}
          </div>
          ...
          {bloodMemMsg&&<div style={{padding:"8px 16px",borderRadius:8,background:"linear-gradient(135deg,#7a665208,#ef444408)",border:"1px solid #c084fc22",animation:"fadeIn 1s ease-out .3s both",textAlign:"center",maxWidth:380}}>
            <div style={{fontSize:10,color:"#c084fc",fontStyle:"italic",lineHeight:1.6}}>
          ...
          <div style={{display:"flex",gap:20,justifyContent:"center",animation:"fadeIn 1s ease-out .25s both"}}>
            <div style={{textAlign:"center"}}><div style={{fontSize:32,fontWeight:900,color:"#fbbf24",letterSpacing:2,animation:"comboBurst .6s ease-out .35s both"}}>{14}</div><div style={{fontSize:10,color:"#666",letterSpacing:2}}>SOULS</div></div>
            <div style={{width:1,height:40,background:"#ffffff0a"}}/>
            <div style={{textAlign:"center"}}><div style={{fontSize:32,fontWeight:900,color:"#ef4444",letterSpacing:2,animation:"comboBurst .6s ease-out .45s both"}}>{MX}</div><div style={{fontSize:10,color:"#666",letterSpacing:2}}>NIGHTS</div></div>
          </div>
          {strayOffset&&strayOffset!==0&&<div style={{fontSize:10,color:strayOffset>0?"#4ade80aa":"#fb923caa",fontStyle:"italic",textAlign:"center",animation:"fadeIn 1s ease-out .6s both",maxWidth:340,lineHeight:1.5}}>
          ...
          <div style={{width:"100%",marginTop:4}}>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"center",animation:"fadeIn .6s ease-out .6s both"}}>
              {chosen.map(c=>(<div key={c.id} style={{textAlign:"center",cursor:"pointer"}} onClick={()=>setTraitTip(c)}>
                <CC cat={c} hl/>
                <div style={{fontSize:10,color:BREEDS[c.breed]?.color,marginTop:2,fontWeight:700}}>{c.name.split(" ")[0]}</div>
                <div style={{fontSize:10,color:"#888"}}>{c.trait.name!=="Plain"?c.trait.icon+" "+c.trait.name:"plain"}</div>
              </div>))}
            </div>
          </div>
          ...
          <div style={{width:"100%",marginTop:6}}>
            ...
            <div style={{display:"flex",gap:8,justifyContent:"center",animation:"fadeIn 1s ease-out .9s both"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:3,padding:"4px 10px",borderRadius:6,background:"#ffffff04",border:"1px solid #ffffff0a"}}>
                <span style={{fontSize:10,color:"#666"}}>15 strays · all Plain</span>
              </div>
            </div>
          </div>
        </>}
        ...
        {hearthDust>0&&<div style={{display:"flex",alignItems:"center",gap:6,padding:"8px 16px",borderRadius:8,background:"#c084fc0a",border:"1px solid #c084fc22",animation:"fadeIn 1.5s ease-out 1.4s both"}}>
          ...
          <span style={{fontSize:11,color:"#c084fc"}}>The Hearth radiates <span style={{fontWeight:700}}>+{hearthDust}✦</span></span>
        </div>}
  ...
  if(ph==="title"){
    ...
    return(<div style={W}><div style={BG}/><Dust/><style>{CSS}</style>
      ...
      {showSlots&&slotSummaries&&<div style={{position:"fixed",top:40,right:10,zIndex:300,background:"#0d1117ee",border:"1px solid #ffffff15",borderRadius:12,padding:14,width:220,animation:"fadeIn .2s ease-out"}}>
        ...
        {slotSummaries.map((s,i)=>{const n=i+1;const isActive=n===activeSlot;return(
          <div key={n} onClick={async()=>{if(!isActive){await setActiveSlot(n);const d=await loadS(n);setMeta(d);const r=await loadRun();setSavedRun(r);setShowSlots(false);}}} style={{padding:"8px 10px",borderRadius:8,marginBottom:4,cursor:isActive?"default":"pointer",
            background:isActive?"#fbbf2412":"#ffffff06",border:`1px solid ${isActive?"#fbbf2433":"#ffffff0a"}`,transition:"all .15s"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{fontSize:12,fontWeight:isActive?700:400,color:isActive?"#fbbf24":"#888"}}>Slot {n}{isActive?" ✦":""}</span>
              {!s.empty&&<span style={{fontSize:10,color:"#666"}}>{s.wins}W / {s.runs}R</span>}
            </div>
            {s.empty?<div style={{fontSize:10,color:"#666",fontStyle:"italic"}}>Empty</div>
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:12,padding:20,textAlign:"center",maxWidth:600}}>
        <div style={{fontSize:15,color:"#ffffffbb",fontStyle:"italic",textAlign:"center",maxWidth:360,lineHeight:1.7,animation:"fadeIn 2s ease-out",textShadow:"0 0 20px #ffffff08"}}>{getEpigraph(meta)}</div>
        ...
        {/* Founder names. visible after first Hearth save */}
        {meta&&meta.cats.length>=1&&meta.cats.length<=4&&meta.stats.w<=2&&<div style={{fontSize:10,color:"#fbbf2477",fontStyle:"italic",letterSpacing:2,animation:"fadeIn 3s ease-out"}}>
          {meta.cats.length===1?`${meta.cats[0].name.split(" ")[0]} watches from the warm side.`
          :meta.cats.length===2?`${meta.cats[0].name.split(" ")[0]} and ${meta.cats[1].name.split(" ")[0]}. The Founders.`
          :`The Hearth burns. ${meta.cats.slice(0,3).map(c=>c.name.split(" ")[0]).join(", ")} watch.`}
        </div>}
        <h1 style={{fontSize:"clamp(36px,8vw,58px)",fontWeight:700,letterSpacing:"clamp(8px,2vw,18px)",lineHeight:1,background:(meta?.achv||[]).includes("completionist")?"linear-gradient(135deg,#c084fc,#fef08a,#4ade80,#67e8f9)":"linear-gradient(180deg,#fef08a,#fbbf24,#b85c2c)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",margin:0,textTransform:"uppercase",filter:"drop-shadow(0 0 30px #fbbf2418)"}}>NINTH LIFE{meta?.ninthDawnCleared?" 🌅":""}</h1>
        <div style={{fontSize:10,color:"#999999bb",letterSpacing:4,fontWeight:300,lineHeight:1.6,animation:"fadeIn 2.5s ease-out",textTransform:"uppercase"}}>Cats are cards · Seasons are suits · Survive the dark</div>
        {(()=>{const ch=getChapterTitle(meta);return ch?<div style={{fontSize:10,color:"#c084fc88",letterSpacing:4,animation:"fadeIn 3s ease-out"}}>Chapter {ch.num} · {ch.name}</div>:null;})()}
        ...
        {meta&&meta.stats.r>=1&&(()=>{
          ...
          <div style={{fontSize:10,color:"#666",letterSpacing:2}}>ACHIEVEMENTS {earned}/{total}</div>
          <div style={{display:"flex",gap:3,flexWrap:"wrap",justifyContent:"center",maxWidth:200}}>
            {ACHIEVEMENTS.map(a=>{const e=(meta.achv||[]).includes(a.id);return(
              <div key={a.id} onClick={()=>toast(a.icon,`${a.name}: ${a.desc}${e?" ✓ "+a.reward:a.dust?" · "+a.dust+"✦":""}`,e?"#fbbf24":"#555")} style={{width:8,height:8,borderRadius:"50%",cursor:"pointer",
                background:e?"#fbbf24":"#ffffff0a",
                boxShadow:e?`0 0 6px #fbbf2466`:"none",
                border:e?"1px solid #fbbf2466":"1px solid #ffffff12",
                transition:"all .15s"}} title={`${a.name}: ${a.desc}`}/>);
            })}
          </div></div>);
        })()}
        ...
        {hasRun&&meta&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
          <div style={{display:"flex",gap:12,alignItems:"center"}}>
            ...
            {sd>0&&!showUpgrades&&<span style={{fontSize:10,color:"#c084fc88"}}>earned from the Hearth</span>}
            {sd>0&&showUpgrades&&<span style={{fontSize:10,color:"#c084fc88",cursor:"pointer"}} onClick={()=>setTab("✦ upgrades")}>spend on upgrades ▸</span>}
            {meta.cats.length>0&&<div style={{fontSize:10,color:"#c084fcbb"}}>🏠 +{calcTotalHearthDust(meta.cats,getMB().dustBonus||0,getHeatFx(meta?.heat).dustMult||1).total}/run</div>}
          </div>
          {showHeat&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
            <div style={{display:"flex",gap:2,alignItems:"center"}}><span style={{fontSize:10,color:"#ef4444"}}>Heat {meta.heat}</span>{meta.heat>0&&<span style={{fontSize:10,color:"#4ade80"}}>+{(meta.heat)*25}% hearth</span>}</div>
            <div style={{display:"flex",gap:3,alignItems:"center"}}>
              {Array.from({length:(meta.stats.mh||meta.heat||1)+1}).map((_,h)=>{const isActive=h===meta.heat;return(
                <button key={h} onClick={async()=>{if(h!==meta.heat){const u={...meta,heat:h};setMeta(u);await saveS(u);toast("🔥",h===0?"Heat disabled. Standard difficulty.": `Heat set to ${h}. +${h*25}% hearth dust.`,"#ef4444");}}}
                  style={{width:24,height:24,borderRadius:6,cursor:"pointer",fontSize:10,fontWeight:isActive?900:400,border:isActive?"2px solid #ef4444":"1px solid #ffffff15",
                    background:isActive?`linear-gradient(135deg,#ef444422,#ef444408)`:"#ffffff06",
                    color:isActive?"#ef4444":h===0?"#666":"#ef444488"}}>{h}</button>);})}
            </div>
            <div style={{fontSize:10,color:"#ef444488"}}>{meta.heat===0?"Standard difficulty.":"Tap a number to change."}</div>
          </div>}
        </div>}
        ...
        {availTabs.length>1&&<div style={{display:"flex",gap:3,background:"#ffffff08",borderRadius:8,padding:2}}>
          {(()=>{
            ...
            return availTabs.map(t=>{
              ...
              return(<button key={t} onClick={()=>setTab(t)} style={{padding:"5px 14px",fontSize:10,borderRadius:6,cursor:"pointer",
                background:safeTab===t?"#ffffff12":"transparent",
                color:safeTab===t?"#e8e6e3":hasGlow?"#c084fc":"#666",
                letterSpacing:1,textTransform:"uppercase",fontWeight:safeTab===t?700:400,
        ...
        {safeTab==="play"&&<>
          ...
          <button onClick={()=>{try{Audio.init();if(savedRun){setNewColonyConfirm(true);return;}if(!meta?.stats?.r){setGuide({step:0,msg:""});startGame();setPh("coldOpen");}else{startGame();}}catch(e){console.error("Start error:",e);}}} style={{...BTN(savedRun?"#1a1a2e":"linear-gradient(135deg,#fbbf24,#f59e0b)",savedRun?"#fbbf24":"#0a0a1a"),padding:savedRun?"10px 32px":"14px 48px",fontSize:savedRun?13:18,letterSpacing:savedRun?2:3,textTransform:"uppercase",boxShadow:savedRun?"none":"0 0 30px #fbbf2444",border:savedRun?"1px solid #fbbf2444":"none"}}>{savedRun?"New Colony":"Enter the Night"}</button>
          {/* v0.7: New Colony confirmation — warn about losing saved run */}
          {newColonyConfirm&&savedRun&&<div style={{width:"100%",maxWidth:360,padding:"14px 18px",borderRadius:12,background:"linear-gradient(145deg,#2e1b11,#0d0d1a)",border:"1px solid #fbbf2444",animation:"fadeIn .3s ease-out",marginTop:8,textAlign:"center"}}>
            <div style={{fontSize:13,color:"#fbbf24",fontWeight:700,marginBottom:8}}>Abandon saved colony?</div>
            <div style={{fontSize:11,color:"#ffffffaa",lineHeight:1.6,marginBottom:10}}>
              Your colony on Night {savedRun.ante||"?"}, Blind {(savedRun.blind||0)+1} will be lost.
              {savedRun.score>0&&<div style={{color:"#fbbf24",marginTop:3}}>Score: {(savedRun.score||0).toLocaleString()} so far.</div>}
              <div style={{color:"#ef4444aa",marginTop:3}}>This cannot be undone.</div>
            </div>
            <div style={{display:"flex",gap:10,justifyContent:"center"}}>
              <button onClick={()=>setNewColonyConfirm(false)} style={{fontSize:12,padding:"8px 20px",borderRadius:6,border:"1px solid #ffffff22",background:"transparent",color:"#888",cursor:"pointer",minHeight:36}}>Go back</button>
              <button onClick={()=>{setNewColonyConfirm(false);startGame();}} style={{fontSize:12,padding:"8px 20px",borderRadius:6,border:"1px solid #fbbf2466",background:"#fbbf2418",color:"#fbbf24",cursor:"pointer",fontWeight:700,minHeight:36}}>Start fresh</button>
            </div>
          </div>}
          ...
          {meta?.ninthDawnCleared&&<div style={{fontSize:11,color:"#fbbf2466",fontStyle:"italic"}}>🌅 The dawn holds.</div>}
          {showLongDark&&!isFirstRun&&<button onClick={()=>setLongDark(v=>!v)} style={{padding:"6px 16px",fontSize:10,border:`1px solid ${longDark?"#818cf8":"#ffffff22"}`,borderRadius:6,cursor:"pointer",background:longDark?"#818cf822":"transparent",color:longDark?"#818cf8":"#666",letterSpacing:1,marginTop:2}}>
            {longDark?"🌑 THE LONG DARK: 9 nights":"🌑 The Long Dark"}
          </button>}
          {longDark&&<div style={{fontSize:10,color:"#818cf8aa",fontStyle:"italic"}}>Nine nights. Nine colonies. The full weight of the dark.</div>}
          {showHeat&&(meta.heat||0)>0&&HEAT_FLAVOR[meta.heat]&&<div style={{fontSize:11,color:"#ef4444bb",fontStyle:"italic"}}>{HEAT_FLAVOR[meta.heat]}</div>}
          {(meta?.relics||[]).length>0&&<div style={{display:"flex",gap:4,flexWrap:"wrap",justifyContent:"center"}}>
            {(meta.relics||[]).sort().map(h=>{const r=HEAT_RELICS[h];return r?(<div key={h} style={{padding:"3px 8px",borderRadius:5,background:"#fbbf2408",border:"1px solid #fbbf2422",fontSize:10,color:"#fbbf24"}} title={`${r.name}: ${r.desc}
"${r.flavor}"`}>{r.icon} {r.name}</div>):null;})}
          </div>}
          {(!meta||meta.stats.r===0)&&<div style={{fontSize:11,color:"#fbbf2466",lineHeight:1.5,textAlign:"center",maxWidth:300}}>3 nights. 18 cats. The game teaches as you play.</div>}
            ...
            return tiers.map(t=>{
              ...
              return(<details key={t} open={defaultOpen}>
                <summary style={{fontSize:11,color:unlocked?tierColor[t]:tierColor[t]+"66",letterSpacing:2,fontWeight:700,marginTop:t>1?8:0,marginBottom:4,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",listStyle:"none"}}>
                  <span>{unlocked?"":"🔒 "}{tierName[t].toUpperCase()} <span style={{fontSize:10,fontWeight:400,color:"#666"}}>({tierUps.filter(u=>(meta.ups[u.id]||0)>=u.max).length}/{tierUps.length})</span></span>
                  {!unlocked?<span style={{fontSize:10,color:"#666",fontWeight:400}}>Unlocks at {tierReq[t]} upgrades ({totalLevels}/{tierReq[t]})</span>
                ...
                {tierUps.map(u=>{const o=meta.ups[u.id]||0,mx=o>=u.max,can=unlocked&&meta.dust>=u.cost&&!mx;
                  return(<div key={u.id} onClick={()=>can&&buyUpg(u)} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",borderRadius:8,background:mx?"#4ade8008":!unlocked?"#ffffff02":"#ffffff04",border:`1px solid ${mx?"#4ade8033":can?tierColor[t]+"44":"#ffffff0a"}`,cursor:can?"pointer":"default",opacity:!unlocked?0.35:can||mx?1:.5}}>
                    ...
                    <div style={{flex:1,textAlign:"left"}}><div style={{fontSize:11,fontWeight:700,color:mx?"#4ade80":tierColor[t]}}>{u.name}{o>0?` (${o}/${u.max})`:""}</div><div style={{fontSize:10,color:"#888"}}>{u.desc}</div></div>
        ...
        {safeTab==="hearth"&&meta&&<div style={{width:"100%",display:"flex",flexDirection:"column",gap:8}}>
            ...
            return(<>
              <div style={{textAlign:"center",padding:"8px 0"}}>
                {(()=>{const n=meta.cats.length;const tier=n>=15?{name:"THE CONSTELLATION",icon:"⭐",color:"#fef08a",bonus:"+30% stardust"}:n>=6?{name:"THE CIRCLE",icon:"🔥",color:"#fb923c",bonus:"+15% stardust"}:{name:"THE EMBER",icon:"🕯️",color:"#b8956a",bonus:""};
                  return <div style={{fontSize:10,color:tier.color+"88",letterSpacing:3,marginBottom:4}}>{tier.icon} {tier.name}{tier.bonus?` · ${tier.bonus}`:""}</div>;
                })()}
                <div style={{fontSize:10,color:"#c084fcbb",letterSpacing:3}}>STARDUST PER RUN</div>
                <div style={{fontSize:24,fontWeight:900,color:"#c084fc",textShadow:"0 0 20px #c084fc44"}}>+{hd.total}✦</div>
                <div style={{fontSize:10,color:"#c084fc88",lineHeight:1.6}}>
                ...
                <div style={{fontSize:10,color:"#666"}}>Each saved cat radiates stardust when you enter the night</div>
              </div>
              {/* Active Hearth cats */}
              {activeCats.length>0&&<>
                <div style={{fontSize:10,color:"#888",letterSpacing:2}}>HEARTH ({activeCats.length} cats{hd.maintenance>0?`. ${hd.maintenance}✦ upkeep`:""})</div>
                {(()=>{const whisperCat=activeCats[Math.floor(Date.now()/8000)%activeCats.length];if(!whisperCat)return null;
                  ...
                  return <div style={{fontSize:11,color:"#fbbf2466",fontStyle:"italic",textAlign:"center",lineHeight:1.5,animation:"fadeIn 2s ease-out",marginBottom:4}}>
                ...
                <div style={{display:"flex",gap:6,flexWrap:"wrap",justifyContent:"center"}}>
                  {activeCats.map((c,i)=>{const tr=TRAITS.find(t=>t.name===c.trait.name)||c.trait;const perCat=calcHearthDust([c])[0].dust;
                    return(<div key={`h${i}`} style={{textAlign:"center",position:"relative"}} onClick={()=>{if(c.story)toast(BREEDS[c.breed]?.icon||"🐱",c.story,BREEDS[c.breed]?.color||"#fbbf24");}}>
                      ...
                      <div style={{fontSize:10,fontWeight:700,color:"#c084fc",marginTop:1,textShadow:"0 0 6px #c084fc44"}}>+{perCat}✦</div>
                      ...
                      }} style={{fontSize:10,color:"#ef444488",background:"transparent",border:"1px solid #ef444433",borderRadius:4,padding:"4px 8px",cursor:"pointer",marginTop:2,minHeight:28,minWidth:44}}>Let Go</button>
                      {meta.cats.length>=20&&<button onClick={(e)=>{e.stopPropagation();
                        const cn=c.name.split(" ")[0];
                        if(!c._enshrineReady){
                          // First tap: explain what enshrining does
                          toast("🌟",`Enshrine ${cn}? Permanent. No dust cost, but they stop producing children. Tap again to confirm.`,"#fbbf24",3500);
                          c._enshrineReady=true;setTimeout(()=>{c._enshrineReady=false;},4000);
                          return;
                        }
                        c._enshrineReady=false;
                        const u={...meta,cats:meta.cats.map(x=>x.name===c.name&&x.savedAt===c.savedAt?{...x,enshrined:true}:x)};
                        setMeta(u);saveS(u);toast("🌟",`${cn} enshrined. Their light is permanent now.`,"#fbbf24");
                      }} style={{fontSize:10,color:"#fbbf24bb",background:"transparent",border:"1px solid #fbbf2422",borderRadius:4,padding:"4px 8px",cursor:"pointer",marginTop:2,minHeight:28,minWidth:44}}>Enshrine</button>}
                    </div>);
                  })}
                </div>
                {/* v0.7: Inline confirmation panel for Let Go */}
                {hearthConfirm&&<div style={{width:"100%",maxWidth:360,padding:"14px 18px",borderRadius:12,background:"linear-gradient(145deg,#2e1111,#0d0d1a)",border:"1px solid #ef444444",animation:"fadeIn .3s ease-out",marginTop:8,textAlign:"center"}}>
                  <div style={{fontSize:13,color:"#ef4444",fontWeight:700,marginBottom:8}}>Let go of {hearthConfirm.pairNames}?</div>
                  <div style={{fontSize:11,color:"#ef4444aa",lineHeight:1.6,marginBottom:10}}>
                    Both cats leave the Hearth forever. Their children will no longer appear in drafts.
                    <div style={{color:"#c084fc",marginTop:4,fontWeight:700}}>You receive {hearthConfirm.pairDust}✦ stardust.</div>
                  </div>
                  <div style={{display:"flex",gap:10,justifyContent:"center"}}>
                    <button onClick={()=>setHearthConfirm(null)} style={{fontSize:12,padding:"8px 20px",borderRadius:6,border:"1px solid #ffffff22",background:"transparent",color:"#888",cursor:"pointer",minHeight:36}}>Keep them</button>
                    <button onClick={async()=>{
                      ...
                      setMeta(u);await saveS(u);
                      toast("✦",`${pairNames} walk into the light. +${pairDust}✦. The Hearth remembers.`,"#c084fc",3000);
                    }} style={{fontSize:12,padding:"8px 20px",borderRadius:6,border:"1px solid #ef444466",background:"#ef444418",color:"#ef4444",cursor:"pointer",fontWeight:700,minHeight:36}}>Farewell. +{hearthConfirm.pairDust}✦</button>
                  </div>
                </div>}
              </>}
              {enshrinedCats.length>0&&<>
                <div style={{fontSize:10,color:"#fbbf24bb",letterSpacing:2,marginTop:4}}>🌟 ENSHRINED ({enshrinedCats.length}). eternal, no upkeep</div>
                <div style={{display:"flex",gap:4,flexWrap:"wrap",justifyContent:"center"}}>
                  {enshrinedCats.map((c,i)=>{const tr=TRAITS.find(t=>t.name===c.trait.name)||c.trait;
                    return(<div key={`en${i}`} style={{textAlign:"center",position:"relative"}}>
                      ...
                      <div style={{fontSize:10,color:"#fbbf2466"}}>🌟</div>
                    </div>);
                  })}
                </div>
              </>}
            </>);
          })():<div style={{color:"#666",fontSize:11,textAlign:"center",padding:"20px 0"}}>
          ...
          <div style={{display:"flex",gap:6,justifyContent:"center"}}>
            ...
            {BK.every(b=>(meta.stats.disc||[]).some(d=>d.startsWith(b)))&&<span style={{fontSize:10,color:"#4ade80",fontWeight:700}}>✓ All seasons!</span>}
          </div>
          {(meta.achv||[]).length>0&&<div>
            ...
            <div style={{display:"flex",gap:4,flexWrap:"wrap",justifyContent:"center"}}>
              {ACHIEVEMENTS.filter(a=>(meta.achv||[]).includes(a.id)).map(a=>(<span key={a.id} onClick={()=>toast(a.icon,`${a.name}: ${a.reward}`,"#fbbf24")} style={{fontSize:10,padding:"3px 8px",borderRadius:5,background:"#fbbf2411",border:"1px solid #fbbf2422",color:"#fbbf24",cursor:"pointer"}} title={`${a.desc} → ${a.reward}`}>{a.icon} {a.name}</span>))}
            </div>
          </div>}
          <div style={{display:"flex",gap:4,flexWrap:"wrap",justifyContent:"center",opacity:.4}}>
            {ACHIEVEMENTS.filter(a=>!(meta.achv||[]).includes(a.id)).map(a=>(<span key={a.id} onClick={()=>toast(a.icon,`${a.name}: ${a.desc}`,"#555")} style={{fontSize:10,padding:"2px 6px",borderRadius:4,background:"#ffffff04",border:"1px solid #ffffff08",color:"#666",cursor:"pointer"}} title={a.desc}>{a.icon} {a.name}</span>))}
          </div>
          {meta.ninthDawnCleared&&<div style={{marginTop:8,padding:"12px 16px",borderRadius:10,background:"linear-gradient(135deg,#fbbf2408,#7a665208)",border:"1px solid #fbbf2422"}}>
            ...
            <div style={{fontSize:10,color:"#fbbf2466",textAlign:"center",marginTop:3,marginBottom:8}}>Every cat who ever carried the colony's name.</div>
            {/* Constellation view */}
...
Saved from Night ${c.fromAnte||"?"}`} style={{
              ...
              <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",textAlign:"center"}}>
                ...
                <div style={{fontSize:10,color:"#fbbf2466"}}>{meta.cats.length} saved</div>
              </div>
            </div>
            {/* Stats summary */}
            {(()=>{
              ...
              return(<div style={{display:"flex",flexDirection:"column",gap:3,fontSize:10,color:"#888",lineHeight:1.5}}>
            ...
            <div style={{display:"flex",gap:3,flexWrap:"wrap",justifyContent:"center",marginTop:8}}>
              {meta.cats.map((c,i)=>{const hasPair=c.pairId&&meta.cats.some((x,j)=>j!==i&&x.pairId===c.pairId);const fullN=c.epithet?`${c.name.split(" ")[0]} ${c.epithet}`:c.name.split(" ")[0];return(<span key={i} title={`${fullN}: ${c.breed}, P${c.power}, ${c.trait?.name||"Plain"}${c.story?.length?"\n"+c.story.join(", "):""}`} style={{fontSize:10,color:BREEDS[c.breed]?.color||"#888",padding:"2px 6px",borderRadius:3,background:c.enshrined?"#fbbf2411":"#ffffff06",border:c.enshrined?"1px solid #fbbf2433":hasPair?"1px solid #34d39933":"none",cursor:"help"}}>{c.enshrined?"🌟 ":""}{hasPair?"👪 ":""}{fullN} {c.sex==="M"?"♂":"♀"}</span>);})}
            </div>
            <div style={{fontSize:11,color:"#999999aa",textAlign:"center",marginTop:8,fontStyle:"italic"}}>Nine colonies. One survived. Not because it was the strongest. Because someone remembered all the rest.</div>
          </div>}
        </div>}
  ...
  if(anteUp){
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:16,padding:20}}>
        <div style={{fontSize:13,color:"#ffffffbb",fontStyle:"italic",textAlign:"center",maxWidth:300,lineHeight:1.6,animation:"fadeIn 1.5s ease-out",letterSpacing:1}}>{NIGHT_EPI[Math.min(anteUp.to-1,4)]}</div>
        ...
        <div style={{fontSize:10,color:"#666",marginTop:4}}>Target: <span style={{color:"#e8e6e3",fontWeight:700}}>{anteUp.target.toLocaleString()}</span></div>
        {ANTE_ESCALATION[Math.min(anteUp.to-1,4)]&&<div style={{fontSize:12,color:"#fbbf2466",fontStyle:"italic",animation:"fadeIn 1.4s ease-out",textAlign:"center",maxWidth:320,lineHeight:1.6,textShadow:"0 0 15px #fbbf2422"}}>{ANTE_ESCALATION[Math.min(anteUp.to-1,4)]}</div>}
        {anteUp.to>1&&<div style={{fontSize:11,color:"#d97706bb",fontStyle:"italic",animation:"fadeIn 1.2s ease-out",textAlign:"center"}}>⚡ The night deepens. The colony holds.</div>}
        
        {runLog.length>0&&<div style={{maxWidth:350,width:"100%"}}>
          <div style={{fontSize:10,color:"#666",letterSpacing:2,marginBottom:4}}>LAST NIGHT</div>
          <div style={{display:"flex",flexDirection:"column",gap:2,maxHeight:120,overflowY:"auto"}}>
            {runLog.filter(e=>e.ante===anteUp.from).slice(-6).map((e,i)=>(<div key={i} style={{fontSize:10,color:e.type==="death"?"#ef4444":e.type==="breed"?"#4ade80":e.type==="hand"?"#fbbf24":"#666",padding:"1px 4px"}}>
  ...
  if(ph==="bossIntro"){const b=boss||BOSSES[0];
    const bossCtx={fallen:fallen.length,fallenName:fallen.length>0?fallen[fallen.length-1].name.split(" ")[0]:"",scarred:allC.filter(c=>c.scarred).length,bonded:allC.filter(c=>c.bondedTo).length,colony:allC.length,clutch:false,grudges:allC.reduce((s,c)=>(c.grudgedWith||[]).length+s,0)/2,deathless:fallen.length===0,gold,
      prevFallen:meta?.allFallen||[],totalRuns:meta?.stats?.r||0,totalDeaths:meta?.stats?.totalFallen||0,
      strongestName:[...allC].sort((a,b)=>b.power-a.power)[0]?.name.split(" ")[0]||"",
    ...
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
    {/* Boss dark vignette */}
    <div style={{position:"fixed",inset:0,background:"radial-gradient(circle at 50% 40%,transparent 20%,#000000cc 80%)",pointerEvents:"none",zIndex:1,animation:"fadeIn 1s ease-out"}}/>
    {/* Boss red pulse overlay */}
    <div style={{position:"fixed",inset:0,background:"radial-gradient(circle at 50% 40%,#ef444422,transparent 60%)",pointerEvents:"none",zIndex:1,animation:"breathe 3s ease-in-out infinite",opacity:0.5}}/>
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:2,gap:vw<500?10:20,padding:vw<500?16:20,maxWidth:500}}>
      {BOSS_PORTRAITS[b.id]?<div style={{width:vw<500?120:200,height:vw<500?168:280,borderRadius:14,overflow:"hidden",animation:"bossEntrance 1.5s ease-out",boxShadow:"0 0 60px #ef444433, 0 0 120px #ef444411",border:"1px solid #ef444422"}}>
      ...
      </div>:<div style={{fontSize:vw<500?48:72,filter:"drop-shadow(0 0 30px #ef444488)",animation:"bossEntrance 1.5s ease-out"}}>{b.icon}</div>}
      <h2 style={{fontSize:30,color:"#ef4444",letterSpacing:8,margin:0,textShadow:"0 0 40px #ef444488",animation:"epicReveal 1.2s ease-out .3s both"}}>{b.name}</h2>
      {hasMastery&&<div style={{fontSize:11,color:"#fbbf24",letterSpacing:4,fontWeight:700,animation:"fadeIn 1.2s ease-out"}}>YOU ARE THE {mastery.title}</div>}
      {(bWins>0||bLoss>0)&&<div style={{fontSize:10,color:"#ef444488",letterSpacing:2,animation:"fadeIn 1.5s ease-out"}}>{bWins}W / {bLoss}L</div>}
      <div style={{fontSize:17,color:"#ef4444",fontStyle:"italic",textAlign:"center",opacity:.8,animation:"fadeIn 1.5s ease-out",lineHeight:1.6,maxWidth:360,textShadow:"0 0 20px #ef444422"}}>"{b.taunt}"</div>
      {careerTaunt&&<div style={{fontSize:13,color:"#ef4444aa",fontStyle:"italic",textAlign:"center",animation:"fadeIn 2s ease-out",lineHeight:1.5,maxWidth:340}}>"{careerTaunt}"</div>}
      {dynamicTaunt&&!careerTaunt&&<div style={{fontSize:14,color:"#ef4444bb",fontStyle:"italic",textAlign:"center",animation:"fadeIn 2s ease-out"}}>"{dynamicTaunt}"</div>}
      {bossTraits.length>0&&<div style={{display:"flex",gap:6,flexWrap:"wrap",justifyContent:"center",animation:"fadeIn 1.8s ease-out",maxWidth:400}}>
        {bossTraits.map((t,i)=>(<div key={i} style={{padding:"8px 14px",borderRadius:8,background:"#ef444415",border:"1px solid #ef444433",textAlign:"center",minWidth:120,maxWidth:180}}>
          ...
          <div style={{fontSize:13,color:"#ef4444cc",marginTop:3,lineHeight:1.3}}>{t.desc}</div>
          <div style={{fontSize:10,color:"#ef4444bb",fontStyle:"italic",marginTop:2}}>{t.flavor}</div>
        </div>))}
      </div>}
      <div style={{fontSize:11,color:"#ef4444bb",fontStyle:"italic",letterSpacing:2,animation:"fadeIn 2.5s ease-out"}}>{b.lore}</div>
      <div style={{display:"flex",gap:6,flexWrap:"wrap",justifyContent:"center",marginTop:4}}>{curses.map((c,i)=>(<div key={i} style={{padding:"6px 10px",borderRadius:8,background:"#ef444408",border:"1px solid #ef444433",textAlign:"center",minWidth:90}}><span style={{fontSize:16}}>{c.icon}</span><div style={{fontSize:10,color:"#ef4444",fontWeight:600,marginTop:1}}>{c.name}</div><div style={{fontSize:12,color:"#ef4444bb",marginTop:1}}>{c.desc}</div></div>))}</div>
      {cfx.exileBreed&&<div style={{fontSize:11,color:"#ef4444bb"}}>{BREEDS[cfx.exileBreed].icon} {cfx.exileBreed} exiled</div>}
      <div style={{fontSize:12,color:"#666",marginTop:8}}>Target: <span style={{color:"#ef4444",fontWeight:700,fontSize:16}}>{eTgt().toLocaleString()}</span>
        {(meta?.heat||0)>0&&<span style={{color:"#ef4444bb",fontSize:10}}> (Heat +{(meta.heat||0)*10}%)</span>}
      </div>
      {hasRelic(3)&&<div style={{fontSize:10,color:"#4ade8066",fontStyle:"italic",textAlign:"center",maxWidth:300,lineHeight:1.5,animation:"fadeIn 2.5s ease-out",padding:"4px 12px",borderRadius:6,background:"#4ade8008",border:"1px solid #4ade8018"}}>
  ...
  if(ph==="event"&&colEvent){
    ...
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:0,padding:20,maxWidth:480}}>
        <div style={{fontSize:10,color:"#ffffff55",letterSpacing:8,marginBottom:16,animation:"fadeIn 1.2s ease-out",textTransform:"uppercase"}}>Night {ante} {"\u00B7"} {["Dusk","Midnight","After the Boss"][blind]}</div>
        <div style={{fontSize:64,marginBottom:8,animation:"fadeIn .6s ease-out",filter:`drop-shadow(0 0 40px ${evtGlow}22)`}}>{evt.icon}</div>
        <h2 style={{fontSize:20,color:"#e8e6e3",letterSpacing:6,margin:"0 0 6px 0",fontWeight:600,animation:"fadeIn .8s ease-out"}}>{evt.title}</h2>
        {evt.tag&&<div style={{fontSize:10,color:evtGlow+"66",letterSpacing:4,padding:"3px 12px",borderRadius:12,border:`1px solid ${evtGlow}15`,marginBottom:16,animation:"fadeIn 1s ease-out",textTransform:"uppercase"}}>{evt.tag}</div>}
        <div style={{fontSize:15,color:"#c8c2bbdd",textAlign:"center",lineHeight:2.1,maxWidth:360,fontStyle:"italic",animation:"fadeIn 1.2s ease-out",marginBottom:16,padding:"0 8px"}}>{evtText}</div>
        {meta&&meta.cats.length>0&&tgts.length>0&&(()=>{
          ...
          return <div style={{fontSize:11,color:"#c084fc88",fontStyle:"italic",textAlign:"center",maxWidth:300,lineHeight:1.5,animation:"fadeIn 2s ease-out",marginBottom:8}}>
        ...
        {tgts.length>0&&<div style={{display:"flex",gap:14,justifyContent:"center",marginBottom:20,animation:"fadeIn 1s ease-out 0.3s both"}}>
          {tgts.map((t,i)=>(<div key={t.id} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
            <CC cat={t}/>
            <div style={{fontSize:11,color:BREEDS[t.breed]?.color||"#888",letterSpacing:1,fontWeight:700}}>{t.name.split(" ")[0]}</div>
            {t.trait&&t.trait.name!=="Plain"&&<div style={{fontSize:10,color:"#ffffff55"}}>{t.trait.icon} {t.trait.name}</div>}
          </div>))}
        </div>}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10,width:"100%",maxWidth:vw<500?vw-32:380}}>
          {evt.choices.map((ch,i)=>{
            ...
            return(<button key={i} onClick={()=>canAfford&&chooseEvent(i)} style={{
              ...
              transition:"all .15s",opacity:canAfford?1:.25,
              textAlign:"left",
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
                ...
                {hint&&canAfford&&<span style={{fontSize:10,color:hint.c,letterSpacing:1,flexShrink:0,padding:"3px 8px",borderRadius:6,background:hint.c+"11",border:`1px solid ${hint.c}22`,display:"flex",alignItems:"center",gap:3}}>{hint.ic} {hint.t}</span>}
              </div>
              {!canAfford&&<span style={{fontSize:10,color:"#ef4444bb",display:"block",marginTop:4}}>Need {Math.abs(ch.fx.gold||0)}🐟</span>}
            </button>);
          })}
          {hasRelic(1)&&<button onClick={()=>{setFerv(f=>Math.min(NERVE_MAX,f+1));setEventOutcome({title:evt.title,icon:evt.icon,choice:"Stoke the First Flame",desc:[{text:"The first light anyone carried out of the dark. +1 Nerve.",color:"#fbbf24",icon:"🕯️"}],targets:[]});setPh("eventResult");}} style={{
            padding:"16px 22px",borderRadius:14,background:"linear-gradient(145deg,#fbbf2408,#fbbf2402)",border:"1.5px solid #fbbf2420",cursor:"pointer",
        ...
        <div style={{display:"flex",gap:12,marginTop:16,animation:"fadeIn 1.5s ease-out 0.8s both",alignItems:"center"}}>
          {(()=>{const allCount=[...hand,...draw,...disc].length;const scarCount=allC2.filter(c=>c.scarred).length;
            return(<>
              <span style={{fontSize:10,color:"#ffffff55"}}>{allCount} cats</span>
              {scarCount>0&&<span style={{fontSize:10,color:"#fb923c77"}}>{scarCount} scarred</span>}
              <span style={{fontSize:10,color:"#fbbf24",fontWeight:700}}>{gold}🐟</span>
            </>);
          })()}
        </div>
      </div>
  ...
  if(ph==="eventResult"&&eventOutcome){
    ...
    const advanceEvent=()=>{
      ...
      else if(wasCamp&&blind<=1){fireEvent();} // v0.7: Camp now fires event too — missing events was missing the game
      else{nextBlind();}
    };
    ...
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:0,padding:20,maxWidth:480}}>
        ...
        {/* Title */}
        <h2 style={{fontSize:20,color:"#e8e6e3cc",letterSpacing:5,margin:"0 0 6px",fontWeight:500,animation:"fadeIn .6s ease-out"}}>{eo.title}</h2>
        {/* Choice made. styled quote */}
        <div style={{fontSize:13,color:"#ffffff66",fontStyle:"italic",letterSpacing:2,animation:"fadeIn .8s ease-out",padding:"8px 20px",borderRadius:12,background:"linear-gradient(145deg,#ffffff05,#ffffff02)",border:"1px solid #ffffff0a",marginBottom:16}}>"{eo.choice}"</div>
        ...
        {eo.targets&&eo.targets.length>0&&<div style={{display:"flex",gap:14,justifyContent:"center",marginBottom:16,animation:"fadeIn .6s ease-out 0.2s both"}}>
          {eo.targets.map((t,i)=>{
            ...
            return(<div key={t.id} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
              <CC cat={updated}/>
              <div style={{fontSize:11,color:BREEDS[updated.breed]?.color||"#888",fontWeight:700}}>{updated.name.split(" ")[0]}</div>
            </div>);
          })}
        </div>}
        {/* Divider */}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10,width:"100%",maxWidth:vw<500?vw-32:380}}>
          {eo.desc.map((d,i)=>(
            <div key={i} style={{display:"flex",gap:12,alignItems:"center",padding:"14px 18px",borderRadius:14,
              ...
              <span style={{fontSize:14,color:d.color,fontWeight:600,lineHeight:1.6}}>{d.text}</span>
            </div>
          ))}
        </div>
        {/* Continue */}
  ...
  if(ph==="overflow"&&oData){const o=oData;
    ...
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:10,padding:20}}>
      {clutch&&<div style={{fontSize:22,fontWeight:900,color:"#fbbf24",letterSpacing:8,textShadow:"0 0 30px #fbbf24cc",animation:"clutchBurst .8s ease-out",marginBottom:4}}>CLUTCH</div>}
      <div style={{fontSize:14,color:blind>=2?"#4ade80":"#fbbf24",letterSpacing:3}}>{blind>=2?(clutch?"Survived. Barely.":"Survived."):"Cleared."}</div>
      {clearLine&&<div style={{fontSize:13,color:"#ffffffbb",fontStyle:"italic",textAlign:"center",maxWidth:320,lineHeight:1.6,animation:"fadeIn 1.2s ease-out",textShadow:"0 0 15px #ffffff11"}}>{clearLine}</div>}
      {blind>=2&&boss&&(()=>{
        const scarredCats2=allC.filter(c=>c.scarred);const bondedCats2=allC.filter(c=>c.bondedTo);const strongest2=[...allC].sort((a,b)=>b.power-a.power)[0];
        const dCtx={fallen:fallen.length,fallenName:fallen.length>0?fallen[fallen.length-1].name.split(" ")[0]:"",scarred:scarredCats2.length,bonded:bondedCats2.length,colony:allC.length,clutch:clutch,grudges:allC.reduce((s,c)=>(c.grudgedWith||[]).length+s,0)/2,deathless:fallen.length===0,
          strongestName:strongest2?.name.split(" ")[0]||"",bondedName:bondedCats2[0]?.name.split(" ")[0]||""};
        const dLine=boss.defeatFn?boss.defeatFn(dCtx):null;
        return(<div style={{textAlign:"center",padding:"10px 16px",borderRadius:10,background:"#4ade8008",border:"1px solid #4ade8022",maxWidth:360,animation:"fadeIn 1.5s ease-out",position:"relative",overflow:"visible"}}>
          {/* Item 6: Particle burst */}
          {Array.from({length:24}).map((_,i)=>{const angle=i*(360/24);const dist=60+Math.random()*80;const px=Math.cos(angle*Math.PI/180)*dist;const py=Math.sin(angle*Math.PI/180)*dist;const colors=["#4ade80","#fbbf24","#67e8f9","#c084fc"];return(<div key={i} style={{position:"absolute",top:"50%",left:"50%",width:4+Math.random()*3,height:4+Math.random()*3,borderRadius:"50%",background:colors[i%4],opacity:0,pointerEvents:"none","--px":px+"px","--py":py+"px",animation:`burstParticle ${0.6+Math.random()*0.4}s ease-out ${i*0.02}s forwards`}}/>);})}
          <div style={{fontSize:14,color:"#4ade80",fontWeight:700,letterSpacing:2,marginBottom:4}}>{boss.name} falls.</div>
          <div style={{fontSize:14,color:"#4ade80cc",fontStyle:"italic",lineHeight:1.6}}>"{boss.defeat}"</div>
          {dLine&&dLine!==boss.defeat&&<div style={{fontSize:12,color:"#4ade80aa",fontStyle:"italic",marginTop:4,lineHeight:1.5}}>"{dLine}"</div>}
        </div>);
      })()}
      {blind===1&&<div style={{fontSize:13,color:"#ef4444",fontWeight:700,letterSpacing:3,animation:"fpp 1s ease infinite"}}>Something approaches...</div>}
      {ante>=4&&blind<2&&(clutch||o.fs<o.tgt*1.3)&&boss&&(()=>{
        const sc3=allC.filter(c=>c.scarred);const bc3=allC.filter(c=>c.bondedTo);const st3=[...allC].sort((a,b)=>b.power-a.power)[0];
        const bCtx={fallen:fallen.length,fallenName:fallen.length>0?fallen[fallen.length-1].name.split(" ")[0]:"",scarred:sc3.length,bonded:bc3.length,colony:allC.length,clutch,grudges:allC.reduce((s,c)=>(c.grudgedWith||[]).length+s,0)/2,deathless:fallen.length===0,gold,
          scarredName:sc3[0]?.name.split(" ")[0]||"",bondedName:bc3[0]?.name.split(" ")[0]||"",strongestName:st3?.name.split(" ")[0]||"",weakestName:[...allC].sort((a,b)=>a.power-b.power)[0]?.name.split(" ")[0]||""};
        ...
        return earlyTaunt?(<div style={{textAlign:"center",animation:"fadeIn 1.5s ease-out",padding:"6px 14px",borderRadius:8,background:"#ef444408",border:"1px solid #ef444418",maxWidth:340}}>
          ...
          <div style={{fontSize:11,color:"#ef4444bb",fontStyle:"italic",lineHeight:1.5}}>"{earlyTaunt}"</div>
          <div style={{fontSize:10,color:"#ef444499",marginTop:2}}>{boss.name} watches.</div>
        </div>):null;
      })()}
      {(()=>{
        ...
        return(<>
          ...
          <div style={{fontSize:tgtSize,color:pctClear>=200?"#888":"#666"}}>Target: {o.tgt.toLocaleString()}</div>
        </>);
      })()}
      <div style={{display:"flex",flexDirection:"column",gap:5,background:"#ffffff06",borderRadius:12,padding:"14px 22px",border:"1px solid #ffffff0a",minWidth:260}}>
        <div onClick={()=>toast("📊","The higher you score above the target, the more rations you earn. Survived: 2🐟, Comfortable: 3🐟, Crushing: 4🐟, Dominating: 5-6🐟, Legendary: 6🐟. Boss rounds give +2🐟 extra.","#fbbf24",7000)} style={{display:"flex",justifyContent:"space-between",fontSize:11,cursor:"help"}}><span style={{color:"#888"}}>Performance</span><span style={{color:pctClear>=300?"#c084fc":pctClear>=200?"#fbbf24":pctClear>=150?"#4ade80":"#888",fontWeight:700}}>{pctClear>=300?"Legendary":pctClear>=200?"Dominating":pctClear>=150?"Crushing":pctClear>=120?"Comfortable":"Survived"} ({pctClear}%)</span></div>
        {o.uh>0&&<div style={{display:"flex",justifyContent:"space-between",fontSize:11}}><span style={{color:"#888"}}>{o.uh} Unused Hand{o.uh>1?"s":""}</span><span style={{color:"#3b82f6"}}>saved</span></div>}
        <div style={{borderTop:"1px solid #ffffff0a",paddingTop:4,display:"flex",justifyContent:"space-between",fontSize:11}}><span style={{color:"#888"}}>Rations Earned</span><span style={{color:"#fbbf24",fontWeight:700}}>+{o.gR} 🐟</span></div>
        {o.interest>0&&<div style={{display:"flex",justifyContent:"space-between",fontSize:11}}><span style={{color:"#4ade80"}}>Stores (interest)</span><span style={{color:"#4ade80",fontWeight:700}}>+{o.interest} 🐟</span></div>}
        <div style={{borderTop:"1px solid #ffffff12",paddingTop:4,display:"flex",justifyContent:"space-between",fontSize:12}}><span style={{color:"#e8e6e3",fontWeight:700}}>Balance</span><span style={{color:"#fbbf24",fontWeight:900,fontSize:14}}>{gold} 🐟</span></div>
      </div>
      {(()=>{const overflowLines=[
      ...
      return <div style={{fontSize:11,color:moodLine?"#ffffff55":seed<3&&whisperPool?"#fbbf2444":"#ffffff44",fontStyle:"italic",textAlign:"center",maxWidth:280,lineHeight:1.5,marginTop:8,animation:"fadeIn 1.5s ease-out"}}>{line}</div>;})()}
      {blind>=2?(()=>{
        ...
        function claimReward(rw){
          ...
          // Fire boss defeat + epithet toasts on shop screen (after reward picked)
          if(boss){
            ...
            setTimeout(()=>toast(boss.icon,`${boss.name}: "${dLine3||boss.defeat}"`,BREEDS[allC[0]?.breed]?.color||"#4ade80",4000),300);
            // Epithet toasts
            const epCats=[...hand,...draw,...disc].filter(c=>c.epithet&&c._bossEpithet);
            epCats.forEach((c,i)=>{
              setTimeout(()=>{toast("🏷️",epithetToastMsg(c),BREEDS[c.breed]?.color||"#fbbf24",3000);Audio.epithetEarned();},900+i*600);
            });
          }
        }
        return(<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8,marginTop:4}}>
          <div style={{fontSize:10,color:"#4ade80bb",letterSpacing:2}}>CHOOSE YOUR REWARD</div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap",justifyContent:"center"}}>
            {choices.map((rw,i)=>(
              ...
              onMouseLeave={e=>{e.currentTarget.style.border="1px solid #4ade8044";e.currentTarget.style.transform="scale(1)";}}>
                ...
                <div style={{fontSize:10,color:"#888",marginTop:3,lineHeight:1.3}}>{rw.desc}</div>
              </button>
            ))}
          </div>
        </div>);
        ...
        <div style={{display:"flex",gap:6,marginTop:6,flexWrap:"wrap",justifyContent:"center"}}>
          {/* MARKET */}
          <button onClick={()=>{setOData(null);setSkipShop(false);genShop();setPh("shop");Audio.shopAmbient();}} style={{
            ...
            <span style={{fontSize:10,fontWeight:400,opacity:.6}}>buy wards, cats, scrolls</span>
          </button>
          ...
          {(seen.shop||!isFirstRun)&&<button onClick={()=>{
            ...
            if(Math.random()<0.25){
              ...
              results.push({text:pk(flavors),color:"#ffffff55",icon:"🌙"});
            }
            ...
            <span style={{fontSize:16}}>🌿</span>
            <span>Scavenge</span>
            <span style={{fontSize:10,fontWeight:400,opacity:.6}}>+{3+ante}🐟, maybe more</span>
          </button>}
          {/* CAMP. hidden until first market visit on first run */}
          {(seen.shop||!isFirstRun)&&<button onClick={()=>{if(gold<3){toast("🐟","Not enough Rations to camp (need 3🐟).","#ef4444");return;}setCampMode(true);setDen([]);setOData(null);setPh("denSelect");}} style={{
            ...BTN("#1a1a2e",gold>=2?"#c084fc":"#555"),padding:"10px 20px",fontSize:12,
            border:`1px solid ${gold>=2?"#c084fc44":"#33333366"}`,display:"flex",flexDirection:"column",alignItems:"center",gap:2,minWidth:100,
            opacity:gold>=2?1:0.5,cursor:gold>=2?"pointer":"not-allowed",
          }}>
            <span style={{fontSize:16}}>🏕</span>
            <span>Camp</span>
            <span style={{fontSize:10,fontWeight:400,opacity:.6}}>{gold>=3?"3🐟, rest + watch":"Need 3🐟"}</span>
          </button>}
        </div>
        </>}
    </div></div>);}
  ...
  if(ph==="defeat"&&defeatData){
    // v0.7: Personalized defeat — name the cats, show how close, make it hurt
    ...
    const defSentence=deficit>0&&deficit<defeatData.target*0.1
      ?`${defMvpN}'s best hand: ${(defMvp?.stats?.bs||0).toLocaleString()}. The colony needed ${deficit.toLocaleString()} more. That close. That's the whole story.`
      :fallen.length>0&&defBonds.length>0
      ?`${fallen.map(f=>f.name.split(" ")[0]).join(", ")} fell. ${defBonds[0].name.split(" ")[0]} bonded with someone who won't see dawn. The dark doesn't negotiate.`
      :defEpithets.length>0
      ?`${defEpithets[0].name.split(" ")[0]} ${defEpithets[0].epithet} survived. The title outlived the colony. ${deficit>0?deficit.toLocaleString()+" short.":""}`
      :defMvp&&(defMvp.stats?.tp||0)>5
      ?`${defMvpN} played ${defMvp.stats.tp} hands. Scored ${(defMvp.stats?.bs||0).toLocaleString()} at their best. It wasn't enough. ${deficit>0?deficit.toLocaleString()+" short.":""}`
      :fallen.length>0
      ?`${fallen.length} name${fallen.length>1?"s":""} lost before the end. ${fallen[0].name.split(" ")[0]} went first. The rest followed the silence.`
      :`Night ${ante}. ${allC.length} cats. ${defeatData.score.toLocaleString()} scored. ${deficit>0?deficit.toLocaleString()+" short of mattering.":"Not enough."}`;
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:16,padding:20,maxWidth:500}}>
        <div style={{fontSize:64,opacity:.6,animation:"fadeIn 1s ease-out",filter:"drop-shadow(0 0 30px #ef444488)"}}>💀</div>
        <h2 style={{fontSize:28,color:"#ef4444",letterSpacing:6,margin:0,textShadow:"0 0 40px #ef444444",animation:"tierReveal 1s ease-out"}}>COLONY FELL</h2>
        <div style={{fontSize:16,color:"#ef4444aa",fontStyle:"italic",textAlign:"center",maxWidth:360,lineHeight:1.7,animation:"fadeIn 1.5s ease-out"}}>{defeatData.line}</div>
        {/* v0.7: Personalized defeat sentence */}
        <div style={{fontSize:12,color:"#ef4444aa",textAlign:"center",maxWidth:360,lineHeight:1.7,animation:"fadeIn 2s ease-out",padding:"10px 16px",borderRadius:10,background:"#ef444408",border:"1px solid #ef444418"}}>{defSentence}</div>
        <div style={{display:"flex",gap:12,fontSize:13,color:"#666",animation:"fadeIn 2s ease-out"}}>
        ...
        <div style={{display:"flex",gap:2,alignItems:"baseline",animation:"fadeIn 2s ease-out"}}>
          ...
          <span style={{fontSize:12,color:"#666"}}> / {defeatData.target.toLocaleString()} target</span>
        </div>
        {fallen.length>0&&<div style={{display:"flex",gap:4,flexWrap:"wrap",justifyContent:"center",animation:"fadeIn 2.5s ease-out"}}>
          {fallen.map((f,i)=>(<span key={i} style={{fontSize:10,color:BREEDS[f.breed]?.color||"#888",opacity:.5}}>{f.name.split(" ")[0]}</span>))}
        </div>}
  ...
  if(ph==="gameOver"||ph==="victory"){const won=ph==="victory";
    ...
    const cands=[...allC].filter(c=>!c._hearthChild&&!hearthNames.has(c.name)).sort((a,b)=>(b.stats?.bs||0)-(a.stats?.bs||0));const mvp=cands[0]||allC.sort((a,b)=>(b.stats?.bs||0)-(a.stats?.bs||0))[0];
    ...
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:12,padding:20,maxWidth:550}}>
        ...
        {/* ═══ STEP 0: THEY MADE IT / THE DARK WON ═══ */}
        {vStep===0&&<>
        <div style={{fontSize:48,animation:"comboBurst .8s ease-out"}}>{won?(isNinthDawn?"🌅":"👑"):"💀"}</div>
        <h2 style={{fontSize:won&&isNinthDawn?48:32,letterSpacing:won&&isNinthDawn?16:8,margin:0,color:won?undefined:"#ef4444",background:won?"linear-gradient(180deg,#fef08a,#fbbf24)":undefined,WebkitBackgroundClip:won?"text":undefined,WebkitTextFillColor:won?"transparent":undefined,animation:"comboBurst .6s ease-out",fontWeight:700,textTransform:"uppercase",filter:won?"drop-shadow(0 0 20px #fbbf2433)":"none"}}>{won?(isNinthDawn?"DAWN":"THEY MADE IT"):"THE DARK WON"}</h2>

        {/* Win. milestone-aware subtitle */}
        {won&&!isNinthDawn&&(()=>{
          const isFirst3=isFirstRun&&meta&&meta.stats.w===0;
          const isFirst5=!isFirstRun&&meta&&meta.stats.w===0;
          const breath=isFirst3
            ?`Three nights. The dark gave you three and you took all of them. ${allC.length} cats. Every name accounted for.`
            :isFirst5
              ?`Five nights. All five. The system ran every test it had. ${fallen.length===0?"Not a single name lost.":allC.length+" still breathing."} The dark has never seen this before.`
            :fallen.length===0?`${MX} night${MX>1?"s":""}. Every single one of them. Not a name lost.`
            :`${MX} nights. ${allC.length} still breathing. ${fallen.length} carried in their memory.`;
          return <div style={{fontSize:11,color:"#fbbf2466",fontStyle:"italic",letterSpacing:2,animation:"fadeIn 1.5s ease-out",marginTop:4,textAlign:"center",maxWidth:340}}>{breath}</div>;
        })()}

        {/* Win. first-win emotional beat */}
        {won&&!isNinthDawn&&meta&&meta.stats.w<=1&&(()=>{
          const isFirst3=isFirstRun;
          const hookText=isFirst3
            ?"The fire burns. For the first time since the eighth colony fell, someone kept it burning. Three nights. That's how long the dark takes to decide if a colony is real. You're real. Now two names go into the Hearth. Not as survivors. As founders."
            :"Five nights. Every test the dark has run on eight colonies, and you walked through all of it. The eighth colony fell one hand short of this moment. You didn't. The Hearth burns with names the system tried to erase.";
          return <div style={{fontSize:13,color:"#fbbf24cc",fontStyle:"italic",textAlign:"center",maxWidth:360,lineHeight:1.7,animation:"fadeIn 2s ease-out .5s both",padding:"10px 16px",borderRadius:10,background:"#fbbf2406",border:"1px solid #fbbf2418"}}>{hookText}</div>;
        })()}

        {/* Win. veteran flavor */}
        {won&&!isNinthDawn&&meta&&meta.stats.w>1&&<div style={{fontSize:12,color:"#fbbf2466",fontStyle:"italic",letterSpacing:2,animation:"fadeIn 2s ease-out .5s both"}}>The fire burns brighter. Colony #{(meta.stats.r||0)+1}.</div>}

        {/* v0.7: THE SENTENCE — data-driven, unique to this run (Kasavin's "feel seen" moment) */}
        {won&&(()=>{
          ...
          const sentence=fallen.length>0&&mvpPlays>5
            ?`${mvpName} played ${mvpPlays} hands. ${fallen.length===1?fallen[0].name.split(" ")[0]+" didn't make it":fallen.length+" didn't make it"}. The rest walked into the dawn carrying ${fallen.length===1?"one extra name":fallen.length+" extra names"}.`
            :fallen.length===0&&bondCount>0
            ?`${allC.length} cats. ${bondCount} bond${bondCount>1?"s":""}. Zero losses. ${mvpName} led with ${mvpPlays} hands. The dark had nothing to say.`
            :epithetCats2.length>2
            ?`${epithetCats2.map(c=>c.name.split(" ")[0]+" "+c.epithet).join(". ")}. Names that earned their titles.`
            :scarCount>2
            ?`${scarCount} scars across the colony. Every one of them a map that led here.`
            :`${mvpName} carried this colony. ${mvpPlays} hands. The rest followed.`;
          return <div style={{fontSize:13,color:"#e8e6e3cc",fontStyle:"italic",textAlign:"center",maxWidth:380,lineHeight:1.8,animation:"fadeIn 2.5s ease-out 1s both",padding:"12px 18px",borderRadius:10,background:"#ffffff04",border:"1px solid #ffffff0a"}}>{sentence}</div>;
        })()}

        {/* Item 13: Victory constellation — survivors as stars */}
        {won&&allC.length>0&&(()=>{
          const cw=Math.min(340,vw-40); // responsive width
          const ch=Math.round(cw*0.7);
          const cx=cw/2;const cy=ch/2;
          const dotSize=vw<500?8:6;const nameSize=vw<500?10:8;
          const pts=allC.map((c,i)=>{
            const angle=(i/allC.length)*Math.PI*2-Math.PI/2;
            const r=Math.min(cx,cy)*0.7;
            return{x:cx+Math.cos(angle)*r,y:cy+Math.sin(angle)*r,c,i};
          });
          return(<div style={{position:"relative",width:cw,height:ch,animation:"fadeIn 2s ease-out 1s both",opacity:0}}>
            <svg width={cw} height={ch} style={{position:"absolute",inset:0}}>
              {pts.map((p,i)=>{const next=pts[(i+1)%pts.length];return <line key={`l${i}`} x1={p.x} y1={p.y} x2={next.x} y2={next.y} stroke="#fbbf2418" strokeWidth="1"/>;})}
              {pts.map(p=>p.c.bondedTo&&(()=>{const mate=pts.find(q=>q.c.id===p.c.bondedTo);return mate?<line key={`b${p.i}`} x1={p.x} y1={p.y} x2={mate.x} y2={mate.y} stroke="#f472b633" strokeWidth="1.5" strokeDasharray="4 4"/>:null;})())}
            </svg>
            {pts.map(p=><div key={p.c.id} style={{position:"absolute",left:p.x-dotSize/2,top:p.y-dotSize/2,width:dotSize,height:dotSize,borderRadius:"50%",background:BREEDS[p.c.breed]?.color||"#fbbf24",boxShadow:`0 0 ${dotSize+2}px ${BREEDS[p.c.breed]?.color||"#fbbf24"}66`,animation:`fadeIn .5s ease-out ${1.5+p.i*0.15}s both`}} title={p.c.name.split(" ")[0]}/>)}
            {pts.map(p=><div key={`n${p.c.id}`} style={{position:"absolute",left:p.x-24,top:p.y+dotSize,width:48,fontSize:nameSize,textAlign:"center",color:BREEDS[p.c.breed]?.color||"#fbbf2466",fontWeight:600,animation:`fadeIn .5s ease-out ${2+p.i*0.15}s both`,lineHeight:1.2}}>{p.c.name.split(" ")[0]}{p.c.epithet?<div style={{fontSize:nameSize-2,color:"#fbbf2444",fontWeight:400}}>{p.c.epithet}</div>:null}</div>)}
          </div>);
        })()}

        {/* Ninth Dawn special */}
        {won&&isNinthDawn&&<div style={{fontSize:14,color:"#fbbf24aa",fontStyle:"italic",textAlign:"center",maxWidth:360,lineHeight:1.7,animation:"fadeIn 2s ease-out"}}>Nine colonies. One survived. Not because it was the strongest. Because someone remembered all the rest.</div>}

        {/* Defeat */}
        {!won&&(()=>{
          const breath=ante>=MX?`Night ${ante}. So close to dawn. The eighth colony knows what "close" feels like.`
            :ante>=3?`Night ${ante}. ${allC.length} still standing when the dark moved in. They deserved more time.`
            :`Night ${ante}. It ends here. But the fire doesn't go out. It never goes out.`;
          return <div style={{fontSize:11,color:"#ef444488",fontStyle:"italic",letterSpacing:2,animation:"fadeIn 1.5s ease-out",marginTop:4}}>{breath}</div>;
        })()}

        {/* Stats */}
        {won&&statItems.length>0&&<div style={{display:"flex",gap:6,flexWrap:"wrap",justifyContent:"center",maxWidth:400,marginTop:8}}>
          {statItems.map((st,i)=>(<div key={i} style={{textAlign:"center",padding:"6px 12px",borderRadius:8,background:`${st.color}08`,border:`1px solid ${st.color}22`,animation:`scorePop .4s ease-out ${0.8+i*0.3}s both`,minWidth:80}}>
            <div style={{fontSize:10,color:`${st.color}88`,letterSpacing:2}}>{st.label}</div>
            <div style={{fontSize:16,fontWeight:900,color:st.color}}>{st.val}</div>
          </div>))}
        </div>}

        {/* Ninth Dawn star field */}
        ...
        {fallen.length>0&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4,marginTop:4}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:3,alignItems:"center"}}>
            {fallen.map((f,i)=>(<div key={i} style={{textAlign:"center"}}>
              <span style={{fontSize:11,color:BREEDS[f.breed]?.color||"#888",fontWeight:700}}>{f.name.split(" ")[0]}{f.epithet?" "+f.epithet:""}</span>
              {f.memorial&&<div style={{fontSize:10,color:"#ffffff66",fontStyle:"italic",lineHeight:1.3,maxWidth:260}}>{f.memorial}</div>}
            </div>))}
          </div>
        </div>}

        {/* Defeat. survivors list */}
        {!won&&allC.length>0&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4,marginTop:4}}>
          <div style={{fontSize:10,color:"#ffffff55",letterSpacing:3}}>{allC.length} STILL STANDING</div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap",justifyContent:"center",maxWidth:340}}>
            {allC.map((c,i)=>(<span key={i} style={{fontSize:10,color:BREEDS[c.breed]?.color||"#888",opacity:.4}}>{c.name.split(" ")[0]}{c.scarred?"*":""}</span>))}
          </div>
        </div>}

        {/* Closing quote. context-aware */}
        {won&&meta&&meta.stats.w>1&&<div style={{fontSize:14,color:"#fbbf24bb",fontStyle:"italic",textAlign:"center",maxWidth:320,lineHeight:1.6,marginTop:4}}>{
          fallen.length===0?`Every single one. The dark couldn't take ${mvp?mvp.name.split(" ")[0]+" or ":""}any of them.`
          :fallen.length>=3?`${fallen.map(f=>f.name.split(" ")[0]).join(", ")}. The survivors carry those names now.`
          :clutch?"One number. The difference between a colony and a memory."
          :"Not all of them. But the ones who made it carry the ones who didn't."
        }</div>}
        {!won&&<div style={{fontSize:14,color:"#ef4444bb",fontStyle:"italic",textAlign:"center",maxWidth:320,lineHeight:1.6,marginTop:4}}>{
          fallen.length>=3?`${fallen.map(f=>f.name.split(" ")[0]).join(", ")}. Say them. The dark won't.`
          :rMaxF>=6?`${mvp?mvp.name.split(" ")[0]+" fought like something that":"They fought like they"} refused to go quietly.`
          :"They were here. They mattered. The dark can take the colony but it can't take that."
        }</div>}

        <div style={{display:"flex",gap:12,fontSize:10,color:"#666"}}>{meta&&meta.cats.length>0&&<span style={{color:"#c084fc"}}>🏠 +{calcHearthDust(meta.cats).reduce((s,h)=>s+h.dust,0)}✦/run</span>}<span>Peak: <span style={{color:NERVE[rMaxF].color}}>{NERVE[rMaxF].name}</span></span>
        ...
        {vStep===1&&<>
        {hasChronicle&&<>
          <div style={{fontSize:10,color:"#999999bb",letterSpacing:4,textAlign:"center"}}>THE CHRONICLE</div>
          <div style={{padding:"14px 20px",borderRadius:10,background:"#ffffff04",border:"1px solid #ffffff08",maxWidth:400}}>
            {genChronicle(won).map((p,i)=>(<div key={i} style={{fontSize:12,color:"#aaa",fontStyle:"italic",lineHeight:1.7,marginBottom:i<2?8:0}}>{p}</div>))}
          </div>
        </>}
        {!hasChronicle&&<div style={{fontSize:12,color:"#666",fontStyle:"italic"}}>The record is short. But it exists.</div>}
        ...
        {newUnlocks.length>0&&<div style={{display:"flex",flexDirection:"column",gap:4,padding:"12px 20px",borderRadius:10,background:"#fbbf2408",border:"1px solid #fbbf2433"}}>
          ...
          {newUnlocks.map((msg,i)=>(<div key={i} style={{fontSize:11,color:"#fbbf24"}}>{msg}</div>))}
        </div>}
        {meta&&meta.stats.w===1&&<>
          <div style={{position:"fixed",top:0,left:0,right:0,height:"100vh",pointerEvents:"none",zIndex:0,overflow:"hidden"}}>
            {Array.from({length:24}).map((_,i)=><div key={i} style={{position:"absolute",top:-10,left:`${4+i*4}%`,width:5,height:5,borderRadius:"50%",
          ...
          <div style={{fontSize:48,animation:"float 3s ease-in-out infinite",filter:"drop-shadow(0 0 30px #fbbf2444)",marginBottom:4}}>🔥</div>
          <div style={{fontSize:22,fontWeight:700,letterSpacing:12,background:"linear-gradient(180deg,#fef08a,#fbbf24)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",animation:"comboBurst .8s ease-out",textTransform:"uppercase"}}>THE FOUNDING</div>
          <div style={{padding:"16px 24px",borderRadius:14,background:"linear-gradient(145deg,#fbbf2408,#c084fc06)",border:"1.5px solid #fbbf2433",textAlign:"center",maxWidth:380,animation:"fadeIn 1.5s ease-out .5s both",boxShadow:"0 0 40px #fbbf2418"}}>
            <div style={{fontSize:13,color:"#fbbf24cc",lineHeight:1.8,marginBottom:10}}>A colony survived three nights. For the first time since the eighth colony fell, someone kept a fire burning through the dark. Two names go into the Hearth. Not as memories. As <b style={{color:"#fef08a"}}>founders</b>. Everything that comes after carries their blood.</div>
            <div style={{fontSize:11,color:"#c084fcbb",fontStyle:"italic",lineHeight:1.6}}>The dark gave you three nights to see if you were worth five. You were.</div>
          </div>
          <div style={{display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap",fontSize:10,animation:"fadeIn 2s ease-out 1s both"}}>
            <span style={{color:"#fbbf24",padding:"4px 10px",borderRadius:6,background:"#fbbf2411",border:"1px solid #fbbf2422"}}>5 Nights Unlocked</span>
            <span style={{color:"#c084fc",padding:"4px 10px",borderRadius:6,background:"#c084fc11",border:"1px solid #c084fc22"}}>🏠 The Hearth</span>
            <span style={{color:"#4ade80",padding:"4px 10px",borderRadius:6,background:"#4ade8011",border:"1px solid #4ade8022"}}>✦ Upgrades</span>
          </div>
        </>}
        {meta&&meta.stats.w===2&&!isFirstRun&&MX>=5&&<>
          <div style={{position:"fixed",top:0,left:0,right:0,height:"100vh",pointerEvents:"none",zIndex:0,overflow:"hidden"}}>
            {Array.from({length:30}).map((_,i)=><div key={i} style={{position:"absolute",top:-10,left:`${3+i*3.2}%`,width:5,height:5,borderRadius:"50%",
          ...
          <div style={{padding:"16px 24px",borderRadius:14,background:"linear-gradient(145deg,#fbbf2410,#f59e0b08)",border:"2px solid #fbbf2444",animation:"comboBurst .8s ease-out",textAlign:"center",maxWidth:380,boxShadow:"0 0 40px #fbbf2422"}}>
            <div style={{fontSize:22,fontWeight:700,letterSpacing:10,marginBottom:8,background:"linear-gradient(180deg,#fef08a,#fbbf24)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",textTransform:"uppercase"}}>THE DARK LOST</div>
            <div style={{fontSize:13,color:"#fbbf24cc",lineHeight:1.7,marginBottom:10}}>Five nights. Every test. Every boss. Every pattern the dark has run on eight colonies. And you walked through all of it. The Hearth burns with {meta.cats.length} name{meta.cats.length!==1?"s":""}. The system doesn't know what to do with that.</div>
            <div style={{fontSize:11,color:"#fbbf2488",fontStyle:"italic"}}>Heat is unlocked. The dark adapts. So can you.</div>
          </div>
        </>}
        ...
        {/* ═══ STEP 3: EPITHET CHOICE + HEARTH PICK + FINISH ═══ */}
        {vStep===3&&<>
        {/* Epithet chooser. cats with 2+ earned epithets get a compact picker */}
        {(()=>{
          ...
          return(<div style={{width:"100%",marginBottom:16}}>
            <div style={{fontSize:10,color:"#fbbf2488",letterSpacing:3,textAlign:"center",marginBottom:8}}>TITLES EARNED</div>
            <div style={{display:"flex",flexDirection:"column",gap:8,maxWidth:380,margin:"0 auto"}}>
              {multiEp.map(cat=>{
                ...
                const fn=cat.name.split(" ")[0];
                return(<div key={cat.id} style={{padding:"8px 12px",borderRadius:8,background:"#ffffff04",border:"1px solid #fbbf2422"}}>
                  <div style={{fontSize:12,fontWeight:700,color:BREEDS[cat.breed]?.color||"#fbbf24",marginBottom:4}}>{fn}</div>
                  <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                    {allEps.map((ep,j)=>{
                      const isActive=ep.key===cat.epithetKey;
                      return(<button key={j} onClick={()=>{
                        if(!isActive){
                          cat.epithet=ep.title;cat.epithetKey=ep.key;
                          [setHand,setDraw,setDisc].forEach(s=>{s(arr=>arr.map(x=>x.id===cat.id?{...x,epithet:ep.title,epithetKey:ep.key}:x));});
                          toast("🏷️",`${fn} is now "${ep.title}"`,BREEDS[cat.breed]?.color||"#fbbf24");
                        }
                      }} style={{padding:"4px 8px",borderRadius:6,fontSize:10,cursor:isActive?"default":"pointer",
                        background:isActive?"#fbbf2422":"#ffffff06",
                        border:isActive?"1px solid #fbbf2466":"1px solid #ffffff12",
                        color:isActive?"#fbbf24":"#888",fontWeight:isActive?700:400,
                      }}>"{ep.title}"{isActive?" ★":""}</button>);
                    })}
                  </div>
                  <div style={{fontSize:10,color:"#fbbf2466",fontStyle:"italic",marginTop:2}}>
                    {EPITHETS[cat.epithetKey]?.flavor||""}
                  </div>
                </div>);
              })}
            </div>
        ...
        {hasHearthPick&&(()=>{
          ...
          {picked.length===1&&(()=>{
            ...
            const line=p.epithet?`${pn} ${p.epithet} is saved. The title will echo in every colony that follows.`
              :p.bondedTo&&cands.find(c=>c.id===p.bondedTo)?`${pn} is saved. ${cands.find(c=>c.id===p.bondedTo).name.split(" ")[0]} waits to be chosen beside them.`
              :(p.stats?.tp||0)>10?`${pn}. ${p.stats.tp} hands played. More than anyone. Saved.`
              :p.scarred?`${pn} carries a scar into the Hearth. It becomes part of the legacy.`
              :null;
            return line?<div style={{fontSize:10,color:"#c084fc66",fontStyle:"italic",textAlign:"center",maxWidth:340,lineHeight:1.5,marginBottom:6,animation:"fadeIn 1s ease-out"}}>{line}</div>:null;
          })()}
          <div style={{fontSize:12,color:"#fbbf24",letterSpacing:3,marginBottom:4}}>
            {picked.length===0?(isFirstHearthSave?"THE FOUNDERS":"CHOOSE THE FIRST SOUL"):"CHOOSE THEIR MATE"}
          </div>
          <div style={{fontSize:10,color:"#c084fcbb",marginBottom:6}}>
            {picked.length===0?(isFirstHearthSave?"Choose carefully. This is the beginning of everything."
            :"A male and female carry the story to the Hearth. Their descendants begin the next colony.")
            :<><span style={{color:"#fbbf24",fontWeight:700}}>{getFullName(picked[0])}</span>{` (${picked[0].sex==="M"?"♂":"♀"}) ${isFirstHearthSave?"is the first Founder":"will be remembered"}. Now choose ${isFirstHearthSave?"the second Founder":needSex==="M"?"a male companion":"a female companion"}.`}</>}
          </div>
          {noMate&&<div style={{fontSize:11,color:"#fb923c",marginBottom:8}}>No {needSex==="M"?"males":"females"} survived. {picked[0].name.split(" ")[0]} goes alone.
            <button onClick={async()=>{
            ...
            {availCands.filter(c=>!pickedIds.includes(c.id)&&!pickedIds.includes(c.name)).slice(0,12).map(c=>{const b=BREEDS[c.breed];
              ...
              <div key={c.id} onClick={()=>saveCatM(c)} style={{cursor:"pointer",textAlign:"center",padding:5,borderRadius:8,border:"1px solid #ffffff0a",background:"#ffffff04",width:96}}>
                ...
                <div style={{marginTop:3,fontSize:10,lineHeight:1.4}}>
                  ...
                  {c.stats.tp>0?<div style={{color:"#888"}}>{c.stats.tp}x Best:{c.stats.bs.toLocaleString()}</div>:<div style={{color:"#666",fontStyle:"italic"}}>Never played</div>}
        ...
        {hearthPair===null&&runLog.length>0&&<div style={{width:"100%",maxWidth:400}}>
          <details style={{cursor:"pointer"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:2,maxHeight:200,overflowY:"auto",marginTop:4,paddingBottom:4}}>
              {runLog.map((e,i)=>(<div key={i} style={{fontSize:10,display:"flex",gap:4,padding:"2px 0",
                ...
                <span style={{color:"#666",minWidth:35}}>N{e.ante}.{e.blind+1}</span>
  ...
  if(ph==="denSelect"){
    ...
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-start",minHeight:"100vh",zIndex:1,gap:10,padding:mob?"16px 12px":"20px",maxWidth:600,paddingTop:mob?40:60}}>
        <h2 style={{fontSize:mob?18:20,color:campMode?"#fbbf24":"#c084fc",letterSpacing:4,margin:0}}>{campMode?"🏕 CAMP":"THE DEN"}</h2>
        {campMode&&<div style={{fontSize:12,color:"#fbbf24",fontWeight:700}}>{gold} 🐟</div>}
        <div style={{fontSize:mob?11:12,color:"#ffffff55",fontStyle:"italic",textAlign:"center",maxWidth:320,lineHeight:1.6}}>{campMode?"Pick 2 cats for the watch. The rest sleep by the fire.":nightText}</div>

        {campMode?<div style={{padding:"10px 14px",borderRadius:10,background:"#fbbf2408",border:"1px solid #fbbf2422",fontSize:12,color:"#fbbf24cc",lineHeight:1.7,textAlign:"center",maxWidth:380,animation:"fadeIn .6s ease-out"}}>
          <b>Pick 2 cats for the watch.</b> Unbonded ♂+♀ may bond. Grudged pairs may reconcile. Others gain +1 Power.
        </div>
        :!seen.den&&<div style={{padding:"10px 14px",borderRadius:10,background:"#c084fc08",border:"1px solid #c084fc22",fontSize:12,color:"#c084fccc",lineHeight:1.7,textAlign:"center",maxWidth:380,animation:"fadeIn .6s ease-out"}}>
          <b>Shelter a ♂ + ♀ pair to breed safely.</b> Everyone else enters the wilds. they train, bond, or fight.
          <div style={{marginTop:6}}><button onClick={()=>setSeen(s=>({...s,den:true}))} style={{fontSize:10,background:"none",border:"1px solid #c084fc33",borderRadius:4,color:"#c084fc",cursor:"pointer",padding:"3px 10px"}}>Got it</button></div>
        </div>}
        ...
        <div style={{width:"100%",maxWidth:480}}>
          {den.length>0&&<div style={{display:"flex",gap:8,justifyContent:"center",alignItems:"center",marginBottom:8,padding:"8px 12px",borderRadius:10,background:"#fbbf2408",border:"1px solid #fbbf2422"}}>
            ...
            <div style={{display:"flex",gap:4}}>
              {den.map(c=><div key={c.id} onClick={()=>toggleDen(c)} style={{cursor:"pointer",textAlign:"center"}}><CC cat={c} sm cw={denCardW} sel/><div style={{fontSize:9,color:BREEDS[c.breed]?.color||"#fbbf24",marginTop:1,fontWeight:700,lineHeight:1}}>{c.name.split(" ")[0]}</div></div>)}
            </div>
          </div>}
          {den.length<2&&<div style={{fontSize:10,color:"#fbbf2466",textAlign:"center",marginBottom:6}}>{den.length===0?"Tap 2 cats below":"Pick 1 more"}</div>}
          <div style={{display:"flex",gap:4,flexWrap:"wrap",justifyContent:"center"}}>
            {(()=>{
              ...
              return sorted.map(c=>{
                ...
                return <div key={c.id} onClick={den.length<2?()=>toggleDen(c):undefined} style={{cursor:den.length<2?"pointer":"default",opacity:den.length>=2?.4:1,position:"relative",textAlign:"center"}}>
                  <CC cat={c} sm cw={denCardW} hl={isGrudgedWith||canBondWith||isMateOf}/>
                  <div style={{fontSize:8,color:BREEDS[c.breed]?.color||"#888",marginTop:1,maxWidth:denCardW+8,lineHeight:1,whiteSpace:"nowrap"}}>{c.name.split(" ")[0]}</div>
          ...
          {injured.length>0&&<div style={{marginTop:8}}>
            <div style={{fontSize:10,color:"#fb923c88",letterSpacing:1,marginBottom:3}}>🩹 RESTING ({injured.length})</div>
          ...
          <div style={{flex:1,padding:"8px 10px",borderRadius:10,background:"#4ade8006",border:"1px solid #4ade8022",minHeight:mob?80:120}}>
            ...
            {eventDenSafe&&<div style={{fontSize:10,color:"#4ade80aa",marginBottom:4}}>🕊️ Shrine protection active</div>}
            {isolated.length===0?<div style={{fontSize:10,color:"#4ade8066",fontStyle:"italic"}}>Tap cats below to shelter them</div>
            :<>
              <div style={{display:"flex",gap:3,flexWrap:"wrap",justifyContent:"center"}}>
              ...
              {!campMode&&(()=>{
                ...
                return breedPairs>0?<div style={{fontSize:10,color:"#4ade80",textAlign:"center",marginTop:4}}>🤝 {breedPairs} breeding pair{breedPairs>1?"s":""}</div>
                :isolated.length>=2?<div style={{fontSize:10,color:"#4ade8066",textAlign:"center",marginTop:4}}>No M/F pairs to breed</div>:null;
              })()}
            </>}
          </div>
          ...
          <div style={{flex:2,padding:"8px 10px",borderRadius:10,background:"#fb923c06",border:"1px solid #fb923c22",minHeight:mob?100:120}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                ...
                return <span style={{fontSize:10,color:riskColor,fontWeight:600}}>{risk}</span>;
              })()}
              </div>
              <button onClick={()=>setSeen(s=>({...s,denSort:s.denSort==="power"?"season":"power"}))} style={{background:"#ffffff08",border:"1px solid #ffffff15",borderRadius:5,padding:"3px 10px",fontSize:10,color:"#aaa",cursor:"pointer",fontWeight:600}}>{(seen.denSort||"season")==="season"?"🍂 Season":"⚡ Power"}</button>
            </div>
            <div style={{display:"flex",gap:3,flexWrap:"wrap",justifyContent:"center"}}>
              {(()=>{
                const sOrder={Autumn:0,Winter:1,Spring:2,Summer:3};
                ...
                return sorted.map(c=><div key={c.id} onClick={den.length<MAX_ISOLATE?()=>toggleDen(c):undefined} style={{cursor:den.length<MAX_ISOLATE?"pointer":"default",opacity:den.length>=MAX_ISOLATE?.6:1,position:"relative",textAlign:"center"}}>
            ...
            {injured.length>0&&<div style={{marginTop:6}}>
              ...
              <div style={{display:"flex",gap:3,flexWrap:"wrap"}}>
                {injured.map(c=><div key={c.id} style={{position:"relative",textAlign:"center"}}><CC cat={c} sm cw={denCardW} dis onTraitClick={ct=>setTraitTip(ct)}/><div style={{fontSize:8,color:"#fb923c66",marginTop:1,maxWidth:denCardW+8,lineHeight:1,whiteSpace:"nowrap"}}>{c.name.split(" ")[0]}</div></div>)}
              </div>
            </div>}
          </div>
        </div>}
        ...
        {(()=>{
          ...
          if(campMode&&den.length>=2){
            ...
            else{campHint="85% +1P one, 15% +1P both";campHintColor="#fbbf24";}
          }
          return <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4,marginTop:4}}>
          {campMode&&campHint&&<div style={{fontSize:10,color:campHintColor,letterSpacing:1,animation:"fadeIn .3s ease-out"}}>{campHint}</div>}
          <div style={{display:"flex",gap:8}}>
          <button onClick={campMode?()=>{
            // Camp costs 3 rations
            const campCost=3;
            if(gold<campCost){toast("🐟","Not enough Rations to camp (need 3🐟).","#ef4444");return;}
            setGold(g=>g-campCost);
            const results=[];
            results.push({text:`-${campCost}🐟. The colony makes camp.`,color:"#888",icon:"🏕"});
            ...
            if(den.length>=2){
              ...
              if(isGrudged&&Math.random()<0.30){
                ...
                if(b._newEpithet){delete b._newEpithet;setTimeout(()=>{toast("🏷️",epithetToastMsg(b),BREEDS[b.breed]?.color||"#fbbf24",2500);Audio.epithetEarned();},800);}
                ...
                if(aCamped&&bCamped){
                  ...
                  if(roll<0.85){
                    // +1P to one uncamped cat (or both if both uncamped and one already camped)
            ...
            if(meta){const u={...meta,stats:{...meta.stats,campCount:(meta.stats.campCount||0)+1}};setMeta(u);saveS(u);}
            setPh("eventResult");
  ...
  if(ph==="denResults"&&denRes){
    ...
    denRes.forEach(r=>{
      if(r.type==="breed")beats.push({pri:0,el:<div key={beats.length} style={{width:"100%",padding:mob?"8px 10px":"10px 14px",borderRadius:10,background:"linear-gradient(145deg,#1b2e1b,#0d0d1a)",border:"1px solid #4ade8044"}}>
        <div style={{fontStyle:"italic",color:"#4ade80bb",fontSize:12,lineHeight:1.5,marginBottom:6}}>{cpk(flavorCache,`breed_${r.baby.id}`,DEN_BREED,fn=>fn(n(r.c1),n(r.c2),n(r.baby)))}</div>
      ...
      if(r.type==="death")beats.push({pri:1,el:<div key={beats.length} style={{width:"100%",padding:mob?"8px 10px":"10px 14px",borderRadius:10,background:"linear-gradient(145deg,#2e1111,#0d0d1a)",border:"1px solid #ef4444bb"}}>
        <div style={{fontSize:10,color:"#ef4444",letterSpacing:3,fontWeight:700,marginBottom:4}}>💀 DEATH</div>
        <div style={{fontStyle:"italic",color:"#ef4444bb",fontSize:13,lineHeight:1.5,marginBottom:6}}>{cpk(flavorCache,`death_${r.victim.id}`,DEN_DEATH,fn=>fn(n(r.c1),n(r.c2),n(r.victim)))}</div>
        <div style={{textAlign:"center",padding:"8px 12px",borderRadius:8,background:"#ef444411",border:"1px solid #ef444422"}}>
          ...
          <div style={{fontSize:10,color:"#888",marginTop:2}}>P{r.victim.power} {BREEDS[r.victim.breed]?.icon} {r.victim.breed} {(r.victim.trait||PLAIN).icon} {(r.victim.trait||PLAIN).name}</div>
          <div style={{fontSize:11,color:"#ef4444bb",marginTop:2}}>{r.victim.stats?.tp>0?`Played ${r.victim.stats.tp} hands. Best: ${r.victim.stats.bs?.toLocaleString()}.`:"Never got to play."}</div>
          <div style={{fontSize:11,color:"#ef4444",marginTop:4,fontWeight:700,letterSpacing:2}}>Gone from the colony.</div>
        </div>
      </div>});
      if(r.type==="phoenix")beats.push({pri:0,el:<div key={beats.length} style={{width:"100%",padding:mob?"8px 10px":"10px 14px",borderRadius:10,background:"linear-gradient(145deg,#2e2211,#0d0d1a)",border:"1px solid #fbbf2466"}}>
        ...
        <div style={{fontStyle:"italic",color:"#fbbf24bb",fontSize:11}}>{cpk(flavorCache,`phoenix_${r.risen.id}`,DEN_PHOENIX,fn=>fn(r.c1.name,r.c2.name,r.risen.name))}</div>
        <div style={{fontSize:10,color:"#fbbf24",marginTop:4}}>Now Eternal at P1. The fire changes everything.</div>
      </div>});
      if(r.type==="bond")beats.push({pri:2,el:<div key={beats.length} style={{fontSize:11,color:"#f472b6",lineHeight:1.4,padding:"4px 0"}}>💕 {n(r.c1)} & {n(r.c2)} bonded <span style={{fontSize:10,color:"#ffffff55"}}>: {bondStr} mult together</span></div>});
      if(r.type==="reconcile_bond")beats.push({pri:2,el:<div key={beats.length} style={{fontSize:11,color:"#f472b6",lineHeight:1.4,padding:"4px 0"}}>💕 {n(r.c1)} & {n(r.c2)} reconciled + bonded <span style={{fontSize:10,color:"#ffffff55"}}>: {bondStr} mult together</span></div>});
      if(r.type==="reconcile")beats.push({pri:2,el:<div key={beats.length} style={{fontSize:11,color:"#67e8f9",lineHeight:1.4,padding:"4px 0"}}>🕊️ {n(r.c1)} & {n(r.c2)} made peace <span style={{fontSize:10,color:"#ffffff55"}}>: grudge cleared</span></div>});
      if(r.type==="fight")beats.push({pri:3,el:<div key={beats.length} style={{fontSize:11,color:"#ef4444",lineHeight:1.4,padding:"4px 0"}}>⚔ {n(r.loser)} {r.wasInjured?"injured":"scarred"} <span style={{fontSize:10,color:"#ffffff55"}}>({r.wasInjured?"heals in 2 rounds":"×1.25 mult"})</span></div>});
      if(r.type==="grudge")beats.push({pri:3,el:<div key={beats.length} style={{fontSize:11,color:"#fb923c",lineHeight:1.4,padding:"4px 0"}}>⚡ {n(r.c1)} & {n(r.c2)} grudge <span style={{fontSize:10,color:"#ffffff55"}}>: −2M when together</span></div>});
      if(r.type==="wanderer")beats.push({pri:4,el:<div key={beats.length} style={{fontSize:11,color:"#67e8f9",lineHeight:1.4,padding:"4px 0"}}>🐱 {n(r.cat)} wandered in <span style={{fontSize:10,color:"#ffffff55"}}>: P{r.cat.power} {r.cat.breed}</span></div>});
      if(r.type==="growth")beats.push({pri:5,el:<div key={beats.length} style={{fontSize:11,color:"#4ade80",lineHeight:1.4,padding:"4px 0"}}>⭐ {n(r.cat)} +1P</div>});
      if(r.type==="mentor")beats.push({pri:5,el:<div key={beats.length} style={{fontSize:11,color:"#c084fc",lineHeight:1.4,padding:"4px 0"}}>📖 {n(r.elder)} mentored {n(r.young)} <span style={{fontSize:10,color:"#ffffff55"}}>: +1P</span></div>});
      if(r.type==="training")beats.push({pri:5,el:<div key={beats.length} style={{fontSize:11,color:"#60a5fa",lineHeight:1.4,padding:"4px 0"}}>⚔️ {n(r.c1)} & {n(r.c2)} sparred <span style={{fontSize:10,color:"#ffffff55"}}>: +1P each</span></div>});
      if(r.type==="teach")beats.push({pri:4,el:<div key={beats.length} style={{fontSize:11,color:"#34d399",fontWeight:700,lineHeight:1.4,padding:"4px 0"}}>👪 {n(r.parent)} taught {n(r.child)} {r.trait.icon} <span style={{fontSize:10,color:"#ffffff55"}}>: {r.trait.name}</span></div>});
      if(r.type==="found")beats.push({pri:5,el:<div key={beats.length} style={{fontSize:11,color:"#fbbf24",lineHeight:1.4,padding:"4px 0"}}>🐟 {n(r.cat)} found {r.gold}🐟</div>});
      if(r.traitGained)beats.push({pri:4,el:<div key={beats.length} style={{fontSize:11,color:"#fbbf24",fontWeight:700,lineHeight:1.4,padding:"4px 0"}}>✨ {n(r.traitGained.cat)} gained {r.traitGained.trait.icon} {r.traitGained.trait.name}</div>});
    });
    ...
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-start",minHeight:"100vh",zIndex:1,gap:10,padding:mob?"16px 12px":"20px",maxWidth:550,overflowY:"auto",paddingTop:mob?30:40}}>
        <h2 style={{fontSize:mob?16:18,color:"#c084fc",letterSpacing:4,margin:0,animation:"fadeIn .6s ease-out"}}>WHAT HAPPENED IN THE DARK</h2>
        {denRes.length===0&&<div style={{color:"#666",fontSize:13,fontStyle:"italic",textAlign:"center",lineHeight:1.6,maxWidth:340}}>{cpk(flavorCache,"denQuiet",DEN_QUIET,fn=>fn())}</div>}

        {/* Major events. births, deaths, phoenix. full narrative cards */}
        {majors.map((b,i)=><div key={i} style={{width:"100%",animation:`slideInLeft .4s ease-out ${i*0.4}s both`}}>{b.el}</div>)}

        {/* Minor events. compact report with staggered cascade */}
        {minors.length>0&&<div style={{display:"flex",flexDirection:"column",gap:1,width:"100%",maxWidth:420,padding:"8px 12px",borderRadius:10,background:"#ffffff04",border:"1px solid #ffffff08",animation:`slideInLeft .4s ease-out ${majors.length*0.4+0.2}s both`}}>
          <div style={{fontSize:10,color:"#c084fc88",letterSpacing:4,textAlign:"center",fontWeight:700,marginBottom:4}}>DEN REPORT</div>
          {minors.map((b,i)=><div key={i} style={{animation:`slideInLeft .3s ease-out ${majors.length*0.6+0.5+i*0.15}s both`}}>{b.el}</div>)}
        </div>}
        ...
        <button onClick={()=>{
          ...
          setBabyNamingQueue([]);
          if(denStRef.current)clearTimeout(denStRef.current);setDenRes(null);
    ...
    return(<div style={W}><div style={BG}/><Dust/><style>{CSS}</style>
      ...
      <div style={{width:"100%",maxWidth:700,padding:"10px 16px",zIndex:1,display:"flex",flexDirection:"column",gap:8,alignItems:"center",paddingBottom:100}}>
        <div style={{display:"flex",justifyContent:"space-between",width:"100%",alignItems:"center"}}>
          <h2 style={{fontSize:20,color:"#fbbf24",letterSpacing:4,margin:0}}>THE MARKET</h2>
          <div style={{display:"flex",gap:12,alignItems:"center"}}>
            <span style={{fontSize:16,fontWeight:900,color:"#fbbf24"}}>{gold} 🐟</span>
            <ProgressMap ante={ante} blind={blind} mx={MX}/>
          </div>
        </div>
        <FM level={ferv} prev={pFerv}/>
        {(()=>{const wSeed=(ante*11+blind*7+gold)%10;if(wSeed>3||isFirstRun)return null;
          // v0.7: Pattern-aware shop whispers (Kasavin: "the game pays attention")
          const wardCount=fams.length;const scrollsBought=Object.values(htLevels).filter(v=>typeof v==="number"&&v>1).length;
          const patternLine=wardCount>=3&&scrollsBought===0?"You always buy wards. Never scrolls. The keeper has noticed."
            :scrollsBought>=2&&wardCount===0?"All scrolls, no wards. The keeper raises an eyebrow."
            :gold>15?"Carrying that many rations into a shop. The keeper has seen this before. They never spend it."
            :allC.filter(c=>c.scarred).length>=3?"The scarred ones come through here a lot. The keeper leaves bandages by the door now."
            :null;
          const line=patternLine||WHISPER_SHOP[(ante*3+blind)%WHISPER_SHOP.length];
          return <div style={{fontSize:11,color:"#fbbf2466",fontStyle:"italic",textAlign:"center",maxWidth:320,lineHeight:1.4,animation:"fadeIn 1.5s ease-out"}}>{line}</div>;
        })()}
        ...
        <div style={{display:"flex",gap:0,width:"100%",borderBottom:"1px solid #ffffff0a"}}>
          {[["cats","🐱 Cats"],["upgrades","⚡ Upgrades"],["colony","👥 Colony"]].map(([id,label])=>{
            ...
            return(<button key={id} onClick={()=>{setShopTab(id);if(id!=="colony")setSellMode(false);}} style={{flex:1,padding:vw<500?"12px 4px":"8px 4px",fontSize:12,fontWeight:shopTab===id?700:400,color:shopTab===id?"#fbbf24":isNudge?"#fbbf24":"#666",background:shopTab===id?"#fbbf2408":isNudge?"#fbbf2406":"transparent",border:"none",borderBottom:shopTab===id?"2px solid #fbbf24":isNudge?"2px solid #fbbf2466":"2px solid transparent",cursor:"pointer",transition:"all .15s",animation:isNudge?"breathe 2s ease-in-out infinite":"none"}}>{label}{isNudge?" ✦":""}</button>);
          })}
        </div>
          ...
          <div style={{display:"flex",gap:3,flexWrap:"wrap",justifyContent:"center",padding:"4px 0 8px",borderBottom:"1px solid #ffffff08",marginBottom:8}}>
            {(()=>{const uAll2=[...hand,...draw,...disc];const bCounts={};uAll2.forEach(c=>{bCounts[c.breed]=(bCounts[c.breed]||0)+1;});
              return Object.entries(bCounts).sort(([,a],[,b])=>b-a).map(([br,ct])=>
                <span key={br} style={{fontSize:10,color:BREEDS[br]?.color||"#888",padding:"1px 5px",borderRadius:3,background:BREEDS[br]?.color+"11",border:"1px solid "+BREEDS[br]?.color+"22"}}>{BREEDS[br]?.icon}{ct}</span>
              );})()}
            <span style={{fontSize:10,color:"#666",padding:"1px 5px"}}>{[...hand,...draw,...disc].length} cats · avg P{Math.round([...hand,...draw,...disc].reduce((s,c)=>s+c.power,0)/Math.max(1,[...hand,...draw,...disc].length))}</span>
          </div>
          {(()=>{
              ...
              {featured&&ftl&&<div style={{marginBottom:8,animation:"scorePop .5s ease-out .1s both"}}>
                ...
                <div onClick={()=>buyCat(sCats.indexOf(featured))} style={{cursor:gold>=(featured._price||4)?"pointer":"not-allowed",display:"flex",flexDirection:vw<500?"column":"row",gap:vw<500?6:12,alignItems:"center",padding:vw<500?"12px 10px":"8px 12px",borderRadius:10,background:isHighTier(featured.trait)?`linear-gradient(135deg,${ftl.color}08,${ftl.color}02)`:"#ffffff04",border:`1px solid ${isHighTier(featured.trait)?ftl.color+"33":"#ffffff0a"}`,transition:"all .15s"}}>
                  ...
                  <div style={{flex:1,textAlign:vw<500?"center":"left"}}>
                    <div style={{fontSize:11,fontWeight:700,color:BREEDS[featured.breed]?.color}}>{featured.name}</div>
                    <div style={{fontSize:12,color:"#888",marginTop:2}}>Power {featured.power} {BREEDS[featured.breed]?.icon} {featured.breed} {featured.sex==="M"?"♂":"♀"}</div>
                    <div style={{fontSize:12,color:featured.trait.name==="Plain"?"#555":tierColor(featured.trait),marginTop:3}}>{featured.trait.name==="Plain"?"No trait yet":featured.trait.icon+" "+featured.trait.name+": "+featured.trait.desc}</div>
                    {(featured.extraTraits||[]).map((t,ti)=><div key={ti} style={{fontSize:12,color:tierColor(t),marginTop:1}}>+{t.icon} {t.name}: {t.desc}</div>)}
          ...
          {sFams.length>0?<div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
            {sFams.map((f,i)=>{const wp=famPrice(f);const can=gold>=wp&&fams.length<MF;
              ...
              return(<div key={f.id} onClick={()=>can&&buyFam(i)} style={{flex:"1 1 140px",maxWidth:180,padding:"10px 12px",borderRadius:10,
                ...
                cursor:can?"pointer":"default",opacity:can?1:.4,transition:"all .15s",
                ...
                <div style={{fontSize:10,color:"#888",lineHeight:1.4,marginTop:2}}>{f.desc}</div>
                {isHtWard&&<div style={{fontSize:10,color:"#c084fc88",marginTop:3,letterSpacing:1}}>HAND TYPE BONUS</div>}
              </div>);
            })}
          </div>:(!ul.fams&&!isFirstRun?<div style={{color:"#666",fontSize:10,fontStyle:"italic",textAlign:"center",padding:12,marginBottom:12}}>🔒 Complete a run to unlock Wards</div>
          :<div style={{color:"#666",fontSize:10,textAlign:"center",padding:12,marginBottom:12}}>No wards available</div>)}
          ...
          {fams.length>0&&<div style={{marginBottom:12}}>
            ...
            <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
              {fams.map(f=><div key={f.id} onClick={()=>toast(f.icon,`${f.name}: ${f.desc}`,"#fbbf24")} style={{display:"flex",alignItems:"center",gap:4,padding:"4px 8px",borderRadius:6,background:"#ffffff06",border:"1px solid #ffffff0a",cursor:"pointer"}}>
                ...
                <div style={{flex:1}}><span style={{fontSize:10,color:"#aaa"}}>{f.name}</span><div style={{fontSize:10,color:"#666"}}>{f.desc}</div></div>
                <button onClick={e=>{e.stopPropagation();sellFam(f);}} style={{fontSize:10,color:"#ef4444aa",background:"none",border:"none",cursor:"pointer",padding:0}} title="Sell for 3🐟">✕ 3🐟</button>
              </div>)}
            </div>
          </div>}
          ...
          <div style={{borderTop:"1px solid #ffffff0a",paddingTop:8,marginBottom:8}}>
            ...
            <div style={{display:"flex",gap:3,flexWrap:"wrap"}}>
              {HT.filter(h=>!h.hidden).map(h=>{const lv=getHtLevel(h.name,htLevels);const sc=getHtScaled(h,lv);
                return(<div key={h.name} style={{padding:"2px 6px",borderRadius:4,background:lv>1?"#fbbf2408":"transparent",border:lv>1?"1px solid #fbbf2422":"1px solid #ffffff08",fontSize:10,color:lv>1?"#fbbf24":"#555"}}>
              ...
              {Object.keys(DEVOTION_MILESTONES).map(breed=>{
                ...
                return(<div key={breed} style={{padding:"6px 10px",borderRadius:8,background:dev.count>0?color+"08":"transparent",border:`1px solid ${dev.count>0?color+"22":"#ffffff08"}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                    <span style={{fontSize:11,color,fontWeight:700}}>{icon} {breed}</span>
                    <span style={{fontSize:10,color:color+"88"}}>{dev.count} played</span>
                  </div>
                  {/* Progress segments */}
                  ...
                  {dev.unlocked.map((m,i)=><div key={i} style={{fontSize:10,color:color+"99"}}>{"✓ "+m.name+": "+m.desc}</div>)}
                  {dev.next&&<div style={{fontSize:10,color:"#666"}}>{"○ "+dev.next.at+" plays: "+dev.next.name+": "+dev.next.desc}</div>}
                </div>);
              })}
            </div>
          </div>
        ...
        {shopTab==="colony"&&<div style={{width:"100%",animation:"fadeIn .3s ease-out"}}>
          {/* Deck stats */}
          {(()=>{const ds=getDeckStats();return(<div style={{padding:"8px 12px",borderRadius:8,background:"#ffffff06",border:"1px solid #ffffff0a",fontSize:10,display:"flex",gap:6,flexWrap:"wrap",color:"#888",marginBottom:8,lineHeight:1.6}}>
            ...
            {ds.scarred>0&&<><span style={{color:"#666"}}>·</span><span style={{color:"#ef4444"}}>🩹 {ds.scarred} scarred</span></>}
            {ds.bonded>0&&<><span style={{color:"#666"}}>·</span><span style={{color:"#4ade80"}}>💕 {ds.bonded} bonded</span></>}
          </div>);})()}
          {/* Release mode */}
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
            ...
            <button onClick={()=>setSellMode(!sellMode)} style={{fontSize:10,padding:"4px 12px",borderRadius:6,border:`1px solid ${sellMode?"#c084fc44":"#fbbf2444"}`,background:sellMode?"#c084fc11":"transparent",color:sellMode?"#c084fc":"#fbbf24",cursor:"pointer"}}>{sellMode?"Done":"Let Go..."}</button>
          </div>
          {sellMode&&<div style={{fontSize:10,color:"#c084fcbb",marginBottom:6,fontStyle:"italic"}}>Each departure costs 1 Nerve. Scarred cats leave without penalty. Tap a cat to release them.</div>}
          <div style={{display:"flex",gap:4,flexWrap:"wrap",maxHeight:300,overflowY:"auto",paddingBottom:4}}>
            {uAll.map(c=>{
              ...
              return(<div key={c.id} style={{position:"relative",textAlign:"center"}} className="scroll-reveal">
                <CC cat={c} sm onClick={sellMode&&canSell?()=>setSellConfirm(c):undefined} dis={sellMode&&!canSell} onTraitClick={ct=>setTraitTip(ct)}/>
                <div style={{fontSize:8,color:BREEDS[c.breed]?.color||"#888",marginTop:1,maxWidth:88,lineHeight:1,whiteSpace:"nowrap"}}>{c.name.split(" ")[0]}</div>
                {sellMode&&canSell&&<div style={{position:"absolute",top:2,right:2,fontSize:10,color:"#c084fc",fontWeight:700,background:"#000000dd",borderRadius:3,padding:"1px 3px",cursor:"pointer",maxWidth:72,lineHeight:1.3}}>
                  ...
                  {!cat.scarred&&<span style={{color:"#ef4444"}}>-1🔥 </span>}
                  {pg.gifts.map((g,gi)=><span key={gi} style={{color:"#c084fc"}}>{g} </span>)}
                  {pg.goldVal===0&&pg.gifts.length===0&&cat.scarred&&<span style={{color:"#888"}}>free</span>}
                </div>}
              </div>);
            })}
          </div>
          {/* v0.7: Sell confirmation panel */}
          {sellConfirm&&sellMode&&(()=>{
            const sc=sellConfirm;const pg=getPartingGifts(sc);
            const n=sc.name.split(" ")[0];
            const mate=sc.bondedTo?uAll.find(x=>x.id===sc.bondedTo):null;
            const mn=mate?.name.split(" ")[0];
            return <div style={{width:"100%",maxWidth:360,padding:"12px 16px",borderRadius:10,background:"linear-gradient(145deg,#1b1133,#0d0d1a)",border:"1px solid #c084fc44",animation:"fadeIn .3s ease-out",marginTop:6,textAlign:"center"}}>
              <div style={{fontSize:13,color:"#c084fc",fontWeight:700,marginBottom:6}}>Let {n} go?</div>
              <div style={{fontSize:11,color:"#ffffffaa",lineHeight:1.6,marginBottom:8}}>
                {!sc.scarred&&<div style={{color:"#ef4444"}}>Costs 1 Nerve. {n} wasn't scarred.</div>}
                {sc.scarred&&<div style={{color:"#4ade80"}}>No Nerve cost. Scarred cats leave freely.</div>}
                {mate&&<div style={{color:"#f472b6",marginTop:3}}>Bonded to {mn}. {mn} loses the bond and gains +2 Power from the grief.</div>}
                {pg.goldVal>0&&<div style={{color:"#fbbf24",marginTop:3}}>The colony receives {pg.goldVal}🐟.</div>}
                {sc.epithet&&<div style={{color:"#fbbf24aa",marginTop:3}}>Carries the title "{sc.epithet}."</div>}
                {(sc.stats?.tp||0)>0&&<div style={{color:"#888",marginTop:3}}>Played {sc.stats.tp} hands. Best: {(sc.stats?.bs||0).toLocaleString()}.</div>}
              </div>
              <div style={{display:"flex",gap:10,justifyContent:"center"}}>
                <button onClick={()=>setSellConfirm(null)} style={{fontSize:12,padding:"8px 20px",borderRadius:6,border:"1px solid #ffffff22",background:"transparent",color:"#888",cursor:"pointer",minHeight:36}}>Keep {n}</button>
                <button onClick={()=>{sellCat(sc);setSellConfirm(null);}} style={{fontSize:12,padding:"8px 20px",borderRadius:6,border:"1px solid #c084fc66",background:"#c084fc18",color:"#c084fc",cursor:"pointer",fontWeight:700,minHeight:36}}>Let go</button>
              </div>
            </div>;
          })()}
        </div>}

        {/* ═══ REROLL — shared across all tabs ═══ */}
        {shopTab!=="colony"&&<div style={{display:"flex",justifyContent:"center",marginTop:8}}>
          <button onClick={reroll} disabled={gold<rc} style={{...BTN("#1a1a2e","#fbbf24",gold>=rc),border:`1px solid ${gold>=rc?"#fbbf2444":"#222"}`,fontSize:11}} title="Rerolls cats, wards, and scrolls. Cost increases each time.">Reroll All ({rc}🐟{rerollCount>0?" ↑":""})</button>
        </div>}
        ...
        <div style={{maxWidth:700,width:"100%",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
          <div style={{display:"flex",flexDirection:"column",gap:2}}>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              <span onClick={()=>toast("🐟",`Rations: ${gold}🐟. Spend in the Market on cats, wards, scrolls. Earn more by scoring higher. Save 5+ for interest.`,"#fbbf24",5000)} style={{color:"#fbbf24",fontSize:14,fontWeight:700,cursor:"help"}}>🐟{gold}</span>
              {runCount>=1&&interestPreview>0&&<span onClick={()=>toast("📈",`Interest: +${interestPreview}🐟 next round (floor of Rations÷5, max 5). Save Rations to earn more.`,"#4ade80")} style={{color:"#4ade80",fontSize:10,cursor:"help"}}>+{interestPreview} stores{interestPreview>=5?" MAX":""}</span>}
              <span onClick={()=>toast("✦",`Stardust: ${meta?.dust||0}✦. Meta-currency from Hearth cats. Spend on permanent upgrades between runs.`,"#c084fc",4000)} style={{color:"#c084fc",fontSize:10,cursor:"help"}}>✦{meta?.dust||0}</span>
            </div>
    ...
    <div style={{...W,animation:fFlash==="down"?"shake .3s ease":"none"}}>
      ...
      {autoPlay&&ph==="playing"&&(()=>{
        ...
        if(isUiTour&&tour)return(<>
          ...
          <div style={{position:"fixed",top:tour.pos==="center"?"35%":tour.pos==="top"?80:"auto",bottom:tour.pos==="bottom"?240:"auto",left:"50%",transform:"translateX(-50%)",zIndex:142,
            padding:"20px 28px",borderRadius:14,background:"#0d1117",border:"1.5px solid #fbbf2444",maxWidth:360,animation:"fadeIn .5s ease-out",textAlign:"center",boxShadow:"0 12px 48px #00000099"}}>
            ...
            <button onClick={()=>setAutoPlay(a=>a?{...a,step:a.step+1}:null)} style={{marginTop:12,fontSize:12,background:"linear-gradient(135deg,#fbbf24,#f59e0b)",border:"none",borderRadius:6,color:"#0a0a1a",cursor:"pointer",padding:"8px 20px",fontWeight:700,letterSpacing:1}}>
              {step===-1?"Show me →":"Next →"}
            </button>
        ...
        return(<>
          ...
          <div style={{position:"fixed",bottom:mob?70:60,left:"50%",transform:"translateX(-50%)",zIndex:150,padding:"16px 28px",borderRadius:14,background:"#0a0a1aee",border:"1.5px solid #4ade8044",maxWidth:360,animation:"fadeIn .6s ease-out",textAlign:"center",boxShadow:"0 8px 32px #00000088"}}>
      ...
      {guide&&!autoPlay&&ante===1&&blind===0&&ph==="playing"&&(()=>{
        ...
        if(guide.step===0){
          ...
          sub="Matching seasons multiply everything. Some trait combos unlock hidden hand types.";
        ...
        return(<div style={{position:"fixed",bottom:guide.step>=2?280:220,left:"50%",transform:"translateX(-50%)",zIndex:150,padding:"12px 20px",borderRadius:10,background:"#0a0a1aee",border:"1px solid #fbbf2444",maxWidth:320,animation:"fadeIn .6s ease-out",textAlign:"center",boxShadow:"0 8px 32px #00000088"}}>
          <div style={{fontSize:14,color:"#fbbf24",fontWeight:700}}>{msg}</div>
          {sub&&<div style={{fontSize:11,color:"#fbbf24aa",marginTop:4,lineHeight:1.5}}>{sub}</div>}
          {guide.step<=2?<button onClick={()=>{
            if(guide.step===0)setGuide(g=>({...g,step:1}));
            else if(guide.step===1)setGuide(g=>({...g,step:2}));
            else if(guide.step===2)setGuide(g=>({...g,step:3}));
          }} style={{marginTop:8,fontSize:11,background:"#fbbf2422",border:"1px solid #fbbf2444",borderRadius:6,color:"#fbbf24",cursor:"pointer",padding:"5px 16px",fontWeight:700,letterSpacing:1}}>Next →</button>
          :<button onClick={()=>{setGuide(null);setSeen(s=>({...s,guided:true}));}} style={{marginTop:8,fontSize:11,background:"#fbbf2422",border:"1px solid #fbbf2444",borderRadius:6,color:"#fbbf24",cursor:"pointer",padding:"5px 16px",fontWeight:700,letterSpacing:1}}>Got it</button>}
          <div style={{display:"flex",gap:6,justifyContent:"center",marginTop:6}}>
            {[0,1,2,3].map(i=>(<div key={i} style={{width:6,height:6,borderRadius:"50%",background:i===guide.step?"#fbbf24":i<guide.step?"#fbbf2444":"#333",transition:"all .15s"}}/>))}
          </div>
        </div>);
      })()}
      ...
      <div style={{width:"100%",maxWidth:700,padding:mob?"6px 12px":"8px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",zIndex:1,borderBottom:`1px solid ${isBoss?"#ef444422":"#ffffff0a"}`}}>
        <div><div style={{fontSize:11,color:"#888",letterSpacing:2}}>NIGHT {ante}/{MX}{!mob&&<span style={{color:"#666"}}> Round {blind+1}/3</span>}</div><div style={{fontSize:mob?13:14,fontWeight:700,color:isBoss?"#ef4444":"#fbbf24"}}>{blindN[blind]}</div>{!mob&&<ProgressMap ante={ante} blind={blind} mx={MX}/>}</div>
        <div style={{textAlign:"center"}}><div style={{fontSize:11,color:"#888",letterSpacing:2}}>SCORE</div><div style={{fontSize:mob?16:18,fontWeight:900}}><span style={{color:rScore>=tgt?"#4ade80":"#e8e6e3"}}>{rScore.toLocaleString()}</span><span style={{color:"#666",fontSize:12}}> / {tgt.toLocaleString()}</span></div>
            {!mob&&rScore<tgt&&hLeft>0&&(()=>{const need=tgt-rScore;return (<div style={{fontSize:12,color:need/hLeft>45000?"#ef4444":need/hLeft>24000?"#fb923c":"#aaa",fontWeight:700,animation:need/hLeft>45000?"fpp 2s ease infinite":"none"}}>🎯 Need {need.toLocaleString()}</div>);})()}
        </div>
        <div style={{textAlign:"right"}}><div style={{color:"#fbbf24",fontWeight:700,fontSize:13}}>🐟{gold}</div>{runCount>=1&&<div style={{fontSize:10,color:"#4ade80"}}>{Math.min(5,Math.floor(gold/5))>0?`+${Math.min(5,Math.floor(gold/5))} stores`:""}</div>}</div>
      ...
      {mob&&rScore<tgt&&hLeft>0&&(()=>{const nph=Math.ceil((tgt-rScore)/hLeft);return(
        <div style={{textAlign:"center",padding:"3px 0",zIndex:1,maxWidth:700,width:"100%"}}>
          <span style={{fontSize:13,color:nph>45000?"#ef4444":nph>24000?"#fb923c":"#aaa",fontWeight:800,letterSpacing:1}}>🎯 Need {(tgt-rScore).toLocaleString()}</span>
        </div>);})()}
      ...
      {mob?<div style={{display:"flex",gap:4,padding:"2px 12px",zIndex:1,maxWidth:700,width:"100%",justifyContent:"center",alignItems:"center",flexWrap:"nowrap"}}>
        {!seen.mobBar&&(fams.length>0||Object.values(devotion).some(v=>v>0))&&runCount>=1&&<span onClick={()=>{setSeen(s=>({...s,mobBar:true}));toast("ℹ️","Tap any icon for details","#fbbf24");}} style={{fontSize:10,color:"#fbbf2466",cursor:"pointer",animation:"breathe 2s ease-in-out infinite",marginRight:2}}>ℹ️</span>}
      ...
      {runCount>=1&&<div style={{display:"flex",gap:3,padding:"2px 16px",zIndex:1,maxWidth:700,width:"100%",justifyContent:"center",flexWrap:"wrap"}}>
        {Object.keys(DEVOTION_MILESTONES).map(breed=>{
          ...
          return(<div key={breed} title={dev.next?`${breed}: ${dev.count}/${dev.next.at} → ${dev.next.name}: ${dev.next.desc}`:`${breed}: ALL UNLOCKED`} style={{display:"flex",alignItems:"center",gap:2,fontSize:10,color:color+"88"}}>
      ...
      {isBoss&&curses.length>0&&<div style={{display:"flex",gap:4,padding:"3px 16px",zIndex:1,maxWidth:700,width:"100%",flexWrap:"wrap"}}>
        {curses.map((c,i)=>(<div key={i} title={c.desc} style={{display:"flex",alignItems:"center",gap:2,padding:"2px 6px",borderRadius:5,background:"#ef444411",border:"1px solid #ef444433",fontSize:10,color:"#ef4444"}}>{c.icon} <span style={{fontWeight:600}}>{c.name}</span></div>))}
        {cfx.exileBreed&&<div style={{display:"flex",alignItems:"center",gap:2,padding:"2px 6px",borderRadius:5,background:"#ef444411",border:"1px solid #ef444433",fontSize:10,color:"#ef4444"}}>{BREEDS[cfx.exileBreed].icon} Exiled</div>}
      </div>}
      </>}
      {hLeft<=2&&rScore<tgt&&ph==="playing"&&<div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6,padding:hLeft===1?"6px 16px":"4px 16px",zIndex:1,maxWidth:700,width:"100%",animation:hLeft===1?"fpp 1.2s ease infinite":"none",background:hLeft===1?"#ef444418":"#fb923c08",borderRadius:6,border:`1px solid ${hLeft===1?"#ef444444":"#fb923c22"}`}}>
        <span style={{fontSize:hLeft===1?12:10,fontWeight:900,color:hLeft===1?"#ef4444":"#fb923c",letterSpacing:hLeft===1?4:2}}>{hLeft===1?"⚠ LAST HAND ⚠":"⚡ 2 HANDS REMAINING"}</span>
      </div>}
      ...
      {!mob&&fams.length>0&&<div style={{display:"flex",gap:3,padding:"0 16px",zIndex:1,maxWidth:700,width:"100%",justifyContent:"center",alignItems:"center",flexWrap:"wrap"}}>
        ...
        {cfx.silence&&<span style={{fontSize:12,color:"#ef4444bb"}}>🤐</span>}
      </div>}
      ...
      {denNews.length>0&&ph==="playing"&&<div style={{display:"flex",gap:4,padding:"4px 16px",zIndex:1,maxWidth:700,width:"100%",justifyContent:"center",flexWrap:"wrap"}}>
        {denNews.slice(-3).map((n,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:4,padding:"4px 10px",borderRadius:6,
          background:n.color+"11",border:`1px solid ${n.color}33`,animation:"fadeIn .4s ease-out"}}>
          <span style={{fontSize:14}}>{n.icon}</span>
          <span style={{fontSize:11,color:n.color,fontWeight:700}}>{n.text}</span>
        </div>))}
        <button onClick={()=>setDenNews([])} style={{background:"none",border:"none",color:"#666",fontSize:10,cursor:"pointer",padding:"2px 4px"}}>✕</button>
      </div>}

      <div style={{width:"100%",maxWidth:700,padding:"0 16px",zIndex:1}}><div style={{height:3,background:"#1a1a2e",borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:`${Math.min(100,(rScore/tgt)*100)}%`,background:isNL?"linear-gradient(90deg,#b85c2c,#f59e0b,#fef08a,#ffffffcc)":"linear-gradient(90deg,#fbbf24,#4ade80)",borderRadius:2,transition:"width .5s cubic-bezier(.34,1.56,.64,1)"}}/></div></div>
      ...
      {ph==="scoring"&&sRes&&(
        <div onClick={scoringDone?advanceFromScoring:skipScoring} style={{cursor:"pointer",position:"fixed",inset:0,background:"#000000cc",zIndex:100,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:0,
          willChange:"transform",animation:scoreShake>0?`bigShake ${0.2+scoreShake*0.08}s ease`:"none"}}>
          ...
          {multPop&&multPop.mode==="xmult"&&(()=>{
            ...
            return(<div style={{position:"absolute",top:"12%",left:0,right:0,zIndex:102,pointerEvents:"none",textAlign:"center",animation:"multFlash .7s ease-out forwards"}}>
              ...
              <div style={{fontSize:18,color:srcColor,letterSpacing:3,fontWeight:700,marginBottom:8,textShadow:`0 0 20px ${srcColor}44`}}>{multPop.label}</div>
              {/* The number. enormous */}
              <div style={{fontSize:multPop.val>=2?110:multPop.val>=1.5?90:72,fontWeight:900,color:multPop.val>=2?"#fef08a":"#fbbf24",
                ...
                letterSpacing:12,lineHeight:1}}>{"×"}{multPop.val}</div>
            </div>);
          })()}
            ...
            return(<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:0,
              ...
              {(()=>{
                ...
                return(<>
                  <div style={{display:"flex",alignItems:"center",gap:8,justifyContent:"center",marginTop:isReveal?20:0,marginBottom:isReveal?12:0,transition:"margin .4s"}}>
                    ...
                    {htLvMatch&&!done&&<span style={{fontSize:isReveal?13:11,fontWeight:700,color:"#fbbf24",background:"#fbbf2418",padding:isReveal?"3px 9px":"2px 7px",borderRadius:6,letterSpacing:1,border:"1px solid #fbbf2433",animation:isReveal?"fadeIn .8s ease-out":"none"}}>LV{htLvMatch[1]}</span>}
                  </div>
                  {!isReveal&&<div style={{marginBottom:done?2:4}}/>}
                  {anyDisc&&done&&<div style={{fontSize:10,color:"#c084fc",fontWeight:700,letterSpacing:2,animation:"fadeIn .5s ease-out",marginBottom:4}}>✨ SECRET COMBO DISCOVERED ✨</div>}
                  {done&&!anyDisc&&(()=>{const eo=htObj?.echo;return eo?<div style={{fontSize:10,color:"#ffffff66",fontStyle:"italic",letterSpacing:2,animation:"fadeIn 1s ease-out",marginBottom:2}}>{eo}</div>:null;})()}
                </>);
              })()}
              ...
              {(()=>{
                ...
                <div style={{textAlign:"center",transition:"transform .2s",transform:(isChipStep||isFirstStep)&&!done?"scale(1.2)":"scale(1)"}}>
                  ...
                  <div style={{fontSize:10,color:"#3b82f688",letterSpacing:2}}>CHIPS</div>
                </div>
                <span style={{color:"#ffffff55",fontSize:16}}>{"×"}</span>
                <div style={{textAlign:"center",transition:"transform .2s",transform:(isMultStep||isFirstStep)&&!done?"scale(1.2)":"scale(1)"}}>
                  ...
                  <div style={{fontSize:10,color:"#ef444488",letterSpacing:2}}>MULT</div>
                </div>
                <span style={{color:"#ffffff55",fontSize:16}}>=</span>
                <div style={{textAlign:"center"}}>
                  <AnimatedScore value={displayTotal} style={{fontWeight:900,
                    fontSize:done?38:28,
                    color:counterColor,
                  ...
                  <div style={{fontSize:10,color:counterColor+"44",letterSpacing:2}}>SCORE</div>
                </div>
              </div>);
              })()}

              {/* Flavor + attribution. subtitle OF the counter, not a separate zone */}
              {!done&&s?.reason&&<div style={{fontSize:10,color:stepColor+"cc",letterSpacing:.5,marginTop:1,animation:"fadeIn .15s ease-out",maxWidth:280,textAlign:"center",lineHeight:1.3}}>{s.reason}</div>}
              {!done&&flavor&&!s?.reason&&<div style={{fontSize:11,color:counterColor+"bb",fontStyle:"italic",letterSpacing:1.5,marginTop:2,animation:"fadeIn .15s ease-out",maxWidth:260,textAlign:"center",lineHeight:1.3}}>{flavor}</div>}
              {!done&&annotation&&<div style={{fontSize:10,color:"#4ade80",marginTop:4,padding:"3px 12px",borderRadius:8,background:"#4ade800a",border:"1px solid #4ade8022",animation:"fadeIn .3s ease-out",maxWidth:280,textAlign:"center",lineHeight:1.4,letterSpacing:.5}}>💡 {annotation}</div>}
              ...
              {s&&!done&&<div style={{marginTop:2,textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",gap:1}}>
                {(s.type==="hand"||s.type==="combo")?<>
                  {(()=>{
                    ...
                    return(<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4,animation:shakeAnim}}>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        <div style={{
                          fontSize:nameSize,letterSpacing:rarity>=4?6:4,fontWeight:900,
                          color:htColor,
                        ...
                        {(()=>{const lvMatch=s.label.match(/Lv(\d+)$/);return lvMatch?<span style={{fontSize:rarity>=4?14:12,fontWeight:700,color:htColor,background:`${htColor}18`,padding:"2px 8px",borderRadius:6,letterSpacing:1,border:`1px solid ${htColor}33`}}>LV{lvMatch[1]}</span>:null;})()}
                      </div>
                  ...
                  }}>{(()=>{const nl=narrativeLabel(s,allC);return nl||s.label;})()}</div>
                  {hasX&&(()=>{
                  ...
                  {s.type!=="hand"&&s.type!=="combo"&&!hasX&&(s.chips||s.mult)?<div style={{fontSize:11,fontWeight:700,display:"flex",gap:6}}>
              ...
              {done&&pct>=100&&<div style={{fontSize:14,color:"#4ade80",letterSpacing:4,fontWeight:900,animation:"targetCross 1.2s ease-out, comboBurst .6s ease-out",marginTop:4,textShadow:"0 0 16px #4ade8066"}}>TARGET PASSED ✦</div>}
              {done&&pct<100&&<div style={{fontSize:12,color:"#ef4444",letterSpacing:3,fontWeight:900,animation:"fpp 1.5s ease infinite",marginTop:4}}>BELOW TARGET</div>}

              {/* Near miss indicator during cascade. subtle, not a celebration */}
              {!done&&nearMiss&&pct<100&&<div style={{fontSize:10,color:"#ef4444aa",letterSpacing:2,animation:"fpp 1.5s ease infinite",marginTop:2}}>CLOSE...</div>}
              ...
              {done&&tier?.label&&<div style={{textAlign:"center",animation:"tierReveal .5s ease-out",marginTop:4}}>
                ...
                {tier.sub&&<div style={{fontSize:11,color:tier.color,opacity:.7,letterSpacing:3,marginTop:2}}>{tier.sub}</div>}
                {tier.nar&&<div style={{fontSize:12,color:"#aaa",fontStyle:"italic",marginTop:3,opacity:.7}}>{tier.nar}</div>}
              </div>}

              {/* New best. part of the counter celebration */}
              {done&&newBest&&<div style={{fontSize:10,fontWeight:700,color:"#fbbf24",letterSpacing:3,animation:"newBestPop .5s ease-out",background:"#fbbf2418",padding:"3px 12px",borderRadius:20,border:"1px solid #fbbf2433",marginTop:4}}>NEW BEST {newBest.toUpperCase()}</div>}

              {done&&_fc&&<div style={{fontSize:11,color:"#4ade80",marginTop:6,padding:"8px 16px",borderRadius:8,background:"#4ade800a",border:"1px solid #4ade8022",animation:"fadeIn .6s ease-out .3s both",maxWidth:320,textAlign:"center",lineHeight:1.6}}>
          ...
          {!scoringDone&&sStep>=2&&<div style={{fontSize:10,color:"#ffffff55",letterSpacing:2,animation:"fadeIn 1.5s ease-out"}}>TAP TO SKIP ⏭</div>}

          {/* Continue. replaces the breath */}
          {scoringDone&&(
            <button onClick={e=>{e.stopPropagation();advanceFromScoring();}} style={{background:"linear-gradient(135deg,#fbbf24,#f59e0b)",color:"#0a0a1a",border:"none",borderRadius:10,padding:"10px 32px",fontSize:13,fontWeight:700,cursor:"pointer",letterSpacing:1,boxShadow:"0 0 24px #fbbf2444",animation:"fadeIn .3s ease-out"}}>Continue</button>
          )}
                ...
                <div key={cat.id} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,
                  ...
                  <div style={{minHeight:20,display:"flex",flexDirection:"column",alignItems:"center",gap:0}}>
                    {hasFired&&<div style={{display:"flex",gap:4,alignItems:"center",animation:isActive?"countUp .3s ease-out":"none"}}>
                      {totalC!==0&&<span style={{fontSize:11,color:"#3b82f6",fontWeight:700}}>{totalC>0?"+":""}{totalC}</span>}
                      {totalM!==0&&<span style={{fontSize:12,color:"#ef4444",fontWeight:800}}>{totalM>0?"+":""}{totalM}</span>}
                    </div>}
                    {xVals.map((x,xi)=><span key={xi} style={{fontSize:14,color:"#fbbf24",fontWeight:900,animation:"scorePop .3s ease",textShadow:"0 0 10px #fbbf2466"}}>{"×"}{x}</span>)}
                  </div>
                  {isActive&&activeStep?.reason&&<div style={{fontSize:10,color:"#ffffff66",textAlign:"center",maxWidth:100,lineHeight:1.3,animation:"fadeIn .2s ease-out"}}>{activeStep.reason}</div>}
                </div>
              );
            })}
          </div>
          ...
          <div style={{display:"flex",gap:4,flexWrap:"wrap",justifyContent:"center",maxWidth:400,marginTop:4}}>
            {sRes.bd.slice(0,sStep+1).filter(s=>s.catIdx===undefined&&s.type!=="hand"&&s.type!=="combo"&&s.type!=="boss_trait").map((s,i)=>{
              ...
              <div key={i} style={{display:"flex",gap:4,alignItems:"center",
                ...
                transform:isCurrentStep?"scale(1.05)":"scale(1)",transition:"all .15s"}}>
          ...
          {sStep>=sRes.bd.length-1&&aftermath.length>0&&(
            <div style={{display:"flex",flexDirection:"column",gap:3,alignItems:"center",marginTop:2,padding:"6px 12px",borderRadius:8,background:"#ffffff06",border:"1px solid #ffffff0a"}}>
              {aftermath.map((a,i)=>(
                <div key={i} style={{fontSize:10,color:a.color,fontWeight:600,
        ...
        <div onClick={()=>setTraitTip(null)} style={{position:"fixed",inset:0,zIndex:150,display:"flex",alignItems:"center",justifyContent:"center",background:"#000000aa",padding:16}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"linear-gradient(145deg,#1a1a2e,#0d0d1a)",border:`2px solid ${tierColor(traitTip.trait||{tier:"common"})}`,borderRadius:14,padding:"20px 24px",maxWidth:340,width:"100%",animation:"fadeIn .15s ease-out",boxShadow:`0 12px 48px #00000099,0 0 30px ${BREEDS[traitTip.breed]?.glow||"#fff"}11`}}>
            ...
            <div style={{textAlign:"center",marginBottom:10}}>
              ...
              {traitTip.name?.includes(" ")&&<div style={{fontSize:10,color:"#666",fontStyle:"italic"}}>{traitTip.name}</div>}
              <div style={{fontSize:11,color:BREEDS[traitTip.breed]?.color||"#888",marginTop:4}}>{BREEDS[traitTip.breed]?.icon} {traitTip.breed} · Power {traitTip.power} · {traitTip.sex==="M"?"♂ Male":"♀ Female"}</div>
              {traitTip.quirk&&<div style={{fontSize:11,color:"#999",fontStyle:"italic",marginTop:4,lineHeight:1.4}}>"{traitTip.quirk}"</div>}
            </div>
            {/* Traits section */}
            {(()=>{const allT=catAllTraits(traitTip);return allT.length===0?(
              <div style={{fontSize:12,color:"#666",textAlign:"center",padding:"8px 0",lineHeight:1.5}}>No trait yet. Cats can earn traits through den events, breeding, or the shop.</div>
              ...
              <div key={ti} style={{marginTop:ti>0?12:0,paddingTop:ti>0?12:0,borderTop:ti>0?"1px solid #ffffff0a":"none"}}>
                ...
                <div style={{fontSize:16,fontWeight:700,color:tl.color,textAlign:"center",letterSpacing:2}}>{t.name}</div>
                ...
                <div style={{fontSize:13,color:"#c8c8c8",textAlign:"center",lineHeight:1.5,marginTop:6}}>{TRAIT_DETAIL[t.name]||t.desc}</div>
              </div>
            );}));})()}
            ...
            {(()=>{const dev=getDevotionLevel(traitTip.breed,devotion);return dev.count>0||dev.next?<div style={{marginTop:10,padding:"8px 12px",borderRadius:8,background:"#ffffff06",border:"1px solid #ffffff0a"}}>
              ...
              {dev.unlocked.map((m,i)=><div key={i} style={{fontSize:11,color:BREEDS[traitTip.breed]?.color||"#888"}}>✓ {m.name}: {m.desc}</div>)}
              {dev.next&&<div style={{fontSize:11,color:"#666"}}>○ {dev.next.at} plays: {dev.next.name}. {dev.next.desc}</div>}
            </div>:null;})()}
            {/* Relationships & status */}
            <div style={{display:"flex",flexDirection:"column",gap:4,marginTop:8}}>
              {traitTip.epithet&&(()=>{
                const epDef=traitTip.epithetKey?Object.values(EPITHETS).find(e=>e.key===traitTip.epithetKey):null;
                return <div style={{fontSize:11,color:"#fbbf24",textAlign:"center",lineHeight:1.6}}>
                  🏷️ <b>"{traitTip.epithet}"</b>{epDef?.desc?<span style={{color:"#fbbf24aa"}}>: {epDef.desc}</span>:""}
                  {epDef?.flavor&&<div style={{fontSize:10,color:"#fbbf2466",fontStyle:"italic",marginTop:2}}>"{epDef.flavor}"</div>}
                  {(traitTip.earnedEpithets||[]).length>0&&<div style={{marginTop:4,fontSize:10,color:"#ffffff55"}}>
                    {traitTip.earnedEpithets.map((e,i)=><span key={i} style={{marginRight:6}}>○ {e.title}</span>)}
                  </div>}
                </div>;
              })()}
              {traitTip.bondedTo&&<div style={{fontSize:11,color:"#f472b6",textAlign:"center"}}>💕 Bonded. ×1.5 mult{traitTip.epithetKey==="bonded"?" (+2 from epithet)":""}, ×1.25 for second pair</div>}
              {(traitTip.grudgedWith||[]).length>0&&<div style={{fontSize:11,color:"#fb923c",textAlign:"center"}}>⚡ {(traitTip.grudgedWith||[]).length} Grudge{(traitTip.grudgedWith||[]).length>1?"s":""}. 75% tension / 25% prove per pair</div>}
              {traitTip.epithetKey==="grudgeResolved"&&<div style={{fontSize:11,color:"#4ade80",textAlign:"center"}}>🕊️ Immune to future grudges</div>}
              {traitTip.scarred&&!traitTip.injured&&<div style={{fontSize:11,color:"#fbbf24",textAlign:"center"}}>⚔ Battle-Hardened. ×1.25 mult</div>}
              {traitTip.injured&&<div style={{fontSize:11,color:"#ef4444",textAlign:"center"}}>🩹 Injured. Half power, less mult (heals in {traitTip.injuryTimer||1})</div>}
              {traitTip._hearthChild&&<div style={{fontSize:11,color:"#fbbf24aa",textAlign:"center"}}>🔥 Hearth Legend. Cannot be saved back.</div>}
            </div>
            {/* Story timeline */}
            {(traitTip.story||[]).length>0&&<div style={{marginTop:10,padding:"8px 12px",borderRadius:8,background:"#ffffff04",border:"1px solid #ffffff08"}}>
              ...
              {(traitTip.story||[]).slice(-3).map((s,i)=>(<div key={i} style={{fontSize:11,color:"#999",lineHeight:1.5}}>{s}</div>))}
            </div>}
            {/* v0.7: Cat biography — full life story */}
            {traitTip.stats&&<div style={{marginTop:8,padding:"8px 12px",borderRadius:8,background:"#ffffff04",border:"1px solid #ffffff08"}}>
              <div style={{fontSize:10,color:"#666",letterSpacing:2,marginBottom:4}}>BIOGRAPHY</div>
              <div style={{display:"flex",flexDirection:"column",gap:2,fontSize:11,color:"#999",lineHeight:1.5}}>
                {traitTip.stats.tp>0&&<div>Played {traitTip.stats.tp} hand{traitTip.stats.tp>1?"s":""}. {traitTip.stats.bs>0?`Best: ${traitTip.stats.bs.toLocaleString()}.`:""}</div>}
                {traitTip.stats.tp===0&&<div style={{color:"#666",fontStyle:"italic"}}>Hasn't played a hand yet. Waiting for a chance.</div>}
                {traitTip.bondedTo&&(()=>{const mate=allC.find(c=>c.id===traitTip.bondedTo);return mate?<div style={{color:"#f472b6"}}>Bonded to {mate.name.split(" ")[0]}.</div>:null;})()}
                {traitTip._mateDied&&<div style={{color:"#ef4444"}}>Lost their partner. Carries the weight.</div>}
                {traitTip.scarred&&<div style={{color:"#fbbf24"}}>Carries a scar. The colony remembers how.</div>}
                {traitTip._bornNight&&<div style={{color:"#c084fc"}}>Born Night {traitTip._bornNight}. {traitTip._hearthParents?"Child of "+traitTip._hearthParents+".":"Grew up in the dark."}</div>}
                {traitTip.hearthDescendant&&<div style={{color:"#fbbf24aa"}}>Descended from the Hearth. The bloodline continues.</div>}
                {traitTip.epithet&&<div style={{color:"#fbbf24"}}>Earned the title "{traitTip.epithet}."</div>}
                {(traitTip.earnedEpithets||[]).length>1&&<div style={{color:"#666"}}>Also qualified for: {traitTip.earnedEpithets.filter(e=>e!==traitTip.epithetKey).map(e=>{const ep=Object.values(EPITHETS).find(x=>x.key===e);return ep?pk(ep.titles):e;}).join(", ")}</div>}
                {(()=>{const xp=getCatXP(traitTip.stats?.tp||0,!!(getMB().xp));return xp?<div style={{color:xp.color,fontWeight:700}}>{xp.icon} {xp.label}</div>:null;})()}
              </div>
            </div>}
          </div>
        </div>
      ...
      {clutch&&<div style={{position:"fixed",inset:0,zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",background:"radial-gradient(circle,#fbbf2422,transparent 60%)",pointerEvents:"none"}}>
        <div style={{fontSize:32,fontWeight:900,color:"#fbbf24",letterSpacing:8,textShadow:"0 0 40px #fbbf24cc",animation:"clutchBurst .6s ease-out"}}>THEY SHOULD HAVE STAYED DOWN</div>
      </div>}
        ...
        {preview?(()=>{
          if(blind===2&&bossTraits.some(bt=>bt.fx.noStrength)){
            return(<div style={{display:"flex",alignItems:"center",gap:6}}>
              ...
              <span style={{fontSize:10,color:"#ef4444bb",fontStyle:"italic"}}>👁️ Watchful. it sees your hand</span>
            </div>);
          }
          ...
          return(<>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <span style={{fontSize:11,color:"#888"}}>{baseC}×{baseM}</span>
              <span style={{fontSize:14,fontWeight:900,color:tier.c,letterSpacing:1,textShadow:`0 0 8px ${tier.c}44`}}>{tier.w}</span>
            </div>
            {/* v0.7: Hand preview narrative — 20% chance, relationship-aware */}
            {(()=>{
              if(Math.random()>0.20||selC.length<2)return null;
              ...
              const line=bondPair&&mate?`${bondPair.name.split(" ")[0]} and ${mate.name.split(" ")[0]} together again.`
                :kitten?`First hand for ${kitten.name.split(" ")[0]}. Everything starts here.`
                :allSame?`All ${BREEDS[selC[0].breed]?.name}. The season runs deep.`
                :scarCount>=2?`${scarCount} scars in this hand. They've earned whatever comes next.`
                :selC.length===1?`${selC[0].name.split(" ")[0]} alone. Sometimes that's enough.`
                :null;
              return line?<div style={{fontSize:10,color:"#ffffff22",fontStyle:"italic",animation:"fadeIn .5s ease-out"}}>{line}</div>:null;
            })()}
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              ...
              {need>0&&hLeft>0&&<span style={{fontSize:14,fontWeight:700,color:ratio>=1?"#4ade80":"#ef4444",letterSpacing:0.5}}>🎯 Need {need.toLocaleString()}</span>}
            </div>
            {sel.size>1&&<button onClick={()=>setSel(new Set())} style={{background:"none",border:"1px solid #ffffff12",borderRadius:4,color:"#666",fontSize:10,cursor:"pointer",padding:"2px 8px"}}>Clear</button>}
            {hLeft<=1&&need>0&&<div style={{fontSize:10,color:"#ef4444",fontWeight:700,animation:"fpp 1.5s ease infinite"}}>LAST HAND</div>}
            {(()=>{
              const warnings=[];
              ...
              return warnings.length>0?<div style={{display:"flex",flexDirection:"column",gap:1}}>
                {warnings.map((w,i)=><div key={i} style={{fontSize:10,color:w.color}}>{w.icon} {w.text}</div>)}
              </div>:null;
            })()}
          </>);
        })():(<div style={{textAlign:"center"}}><span style={{color:"#666",fontSize:10}}>Select up to 5 cats to play</span></div>)}
      </div>
      ...
      {sel.size>=1&&(()=>{
        ...
        return(
          <div style={{display:"flex",gap:6,padding:"4px 16px",zIndex:1,maxWidth:700,width:"100%",justifyContent:"center",alignItems:"center"}}>
            <div style={{display:"flex",gap:3,alignItems:"flex-end"}}>
              {ordered.map((cat,pos)=>{
                ...
                return(<div key={cat.id} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:0}}>
                  <span style={{fontSize:10,color:"#fbbf24",fontWeight:700}}>{pos+1}</span>
            ...
            {/* v0.7: Bench whisper — involvement-aware one-liner */}
            {benchCats.length>0&&(()=>{
              ...
              const line=mourner?`${mourner.name.split(" ")[0]} watches. Still carrying two names.`
                :never.length>=2?`${never[0].name.split(" ")[0]} and ${never[1].name.split(" ")[0]} haven't played yet. They're waiting.`
                :never.length===1?`${never[0].name.split(" ")[0]} hasn't played a hand. Still waiting for a chance.`
                :veteran?`${veteran.name.split(" ")[0]} rests. ${veteran.stats.tp} hands played. They've earned it.`
                :null;
              return line?<div style={{fontSize:9,color:"#ffffff1a",fontStyle:"italic",maxWidth:200,textAlign:"center",lineHeight:1.3}}>{line}</div>:null;
            })()}
          </div>);
      })()}
      ...
      <div style={{display:"flex",gap:mob?2:3,padding:"0 4px",zIndex:1,justifyContent:"center",flexWrap:"nowrap",maxWidth:mob?(vw-8):840,width:"100%",overflowX:"visible"}}>
        {(()=>{
          ...
          return(<div key={c.id} style={{position:"relative",flexShrink:0,animation:isGuideHL?"guidePulse 1.5s ease-in-out infinite":"none"}}>
            ...
            {isRelated&&<div style={{position:"absolute",top:-10,left:"50%",transform:"translateX(-50%)",fontSize:11,fontWeight:700,background:relType==="mate"?"#f472b622":"#4ade8022",padding:"1px 6px",borderRadius:4,border:`1px solid ${relType==="mate"?"#f472b644":"#4ade8044"}`,whiteSpace:"nowrap",animation:"countUp .3s ease-out",zIndex:10,color:relType==="mate"?"#f472b6":"#4ade80"}}>{relType==="mate"?"💕 mate":"👪 kin"}</div>}
            {isGuideHL&&<div style={{position:"absolute",top:-10,left:"50%",transform:"translateX(-50%)",fontSize:11,fontWeight:700,background:"#fbbf2422",padding:"1px 6px",borderRadius:4,border:"1px solid #fbbf2444",whiteSpace:"nowrap",animation:"guidePulse 1.5s ease-in-out infinite",zIndex:10,color:"#fbbf24"}}>👆 select</div>}
          </div>);
        });})()}
      </div>
      ...
      {sel.size>=1&&ph==="playing"&&!autoPlay&&(()=>{
        ...
        return(<div onClick={()=>{
          ...
          <span style={{fontSize:13,fontWeight:700,color:"#fbbf24",letterSpacing:2}}>{evald.name}{htLv>1?` Lv${htLv}`:""}</span>
          {runCount>=1&&<><span style={{fontSize:11,color:"#3b82f6",fontWeight:700,marginLeft:8}}>{baseC}C</span>
          <span style={{fontSize:11,color:"#ffffff55",margin:"0 2px"}}>×</span>
          <span style={{fontSize:11,color:"#ef4444",fontWeight:700}}>{baseM}M</span></>}
      ...
      <div style={{display:"flex",gap:8,padding:"8px",zIndex:10,alignItems:"center",position:mob?"sticky":"static",bottom:mob?0:"auto",background:mob?"#0a0a1aee":"transparent",borderTop:mob?"1px solid #ffffff0a":"none",justifyContent:"center"}}>
        ...
        {ph==="playing"&&hLeft<=0&&rScore<tgt?(<div style={{textAlign:"center"}}>
          ...
          <div style={{fontSize:10,color:"#ef4444bb",marginTop:4}}>No hands remaining</div>
        </div>):(<>
        <div style={{textAlign:"center"}}>
          {runCount>=1?(()=>{const cost=recruitCost();const canRecruit=ph==="playing"&&!autoPlay&&gold>=cost&&(draw.length>0||disc.length>0);
            return(<>
              ...
              <div onClick={()=>toast("🐱","Recruit: draw an extra cat into your hand. Cost doubles each time (1→2→4→8🐟). More cats = better hands. Unplayed cats with traits give passive bonuses.","#4ade80",5500)} style={{fontSize:10,color:cost===0?"#4ade80":gold>=cost?"#fbbf24":"#ef4444",marginTop:2,cursor:"help"}}>{cost===0?"Free!":cost+"🐟"}</div>
            </>);
          })():null}
        </div>
        <div style={{textAlign:"center"}}>
          ...
          <div onClick={()=>toast("♻️",`Discard: swap up to ${MAX_DISCARD} selected cats for new draws. Free!`,"#ef4444")} style={{fontSize:10,color:cfx.noDisc?"#ef4444bb":sel.size>MAX_DISCARD?"#ef4444":dLeft<=0?"#ef4444":"#888",marginTop:2,cursor:"help"}}>{cfx.noDisc?"Disabled":sel.size>MAX_DISCARD?`Max ${MAX_DISCARD}`:`${dLeft} left`}</div>
          {sel.size>0&&dLeft>0&&!cfx.noDisc&&ph==="playing"&&(()=>{
            ...
            selCats2.forEach(c=>{
              ...
              else if(catHas(c,"Loyal"))hints.push({icon:"🫂",text:"+1 mult all",color:"#f472b6"});
              else if(catHas(c,"Scavenger"))hints.push({icon:"🌾",text:"+2🐟",color:"#4ade80"});
            });
            return hints.length>0?<div style={{fontSize:10,color:"#4ade80",marginTop:1}}>
        ...
        <div style={{textAlign:"center"}}>
          ...
          <div style={{fontSize:10,color:"#666",marginTop:2,cursor:"pointer"}} onClick={()=>setHandSort(s=>s==="season"?"power":"season")}>{handSort==="season"?"☀ szn":"⚡ pwr"}</div>
        </div>
        </>)}
      </div>
      ...
      {meta&&meta.stats.w<3&&ph==="playing"&&!autoPlay&&(()=>{
        ...
        return <div style={{textAlign:"center",padding:"4px 16px",maxWidth:400,margin:"0 auto",animation:"fadeIn 1s ease-out"}}>
          <div style={{fontSize:isVeryNew?12:10,color:isVeryNew?"#fbbf24aa":"#ffffff44",lineHeight:1.5,padding:isVeryNew?"4px 12px":"0",borderRadius:8,background:isVeryNew?"#fbbf2408":"transparent",border:isVeryNew?"1px solid #fbbf2415":"none"}}>{isVeryNew?"🎯 ":""}{PLAY_HINTS[hIdx]}</div>
        </div>;
      })()}

      <div style={{maxWidth:700,width:"100%",padding:"2px 16px",zIndex:1,marginTop:"auto"}}>
        <details style={{cursor:"pointer"}}><summary style={{fontSize:10,color:"#666",letterSpacing:2}}>REFERENCE</summary>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:2,marginTop:4}}>
            {HT.slice(1).map(h=>{const best=handBests[h.name];const isHidden=h.hidden;const discovered=(meta?.stats?.dh||[]).includes(h.name);const show=!isHidden||discovered;return(<div key={h.name} style={{fontSize:10,color:show?"#555":"#333"}}>{show?<><span style={{color:discovered?"#c084fc":"#888",fontWeight:600}}>{h.name}{discovered?" ✨":""}</span> <span style={{color:"#3b82f6"}}>{h.base.c}</span>x<span style={{color:"#ef4444"}}>{h.base.m}</span> {h.ex}{best?<span style={{color:"#fbbf24bb"}}> best:{best.toLocaleString()}</span>:""}</>:<><span style={{color:"#666",fontWeight:600}}>????</span> <span style={{fontStyle:"italic",color:"#666666bb"}}>Trait combo</span></>}</div>);})}
            <div style={{fontSize:10,color:"#666",letterSpacing:1,marginTop:4,borderTop:"1px solid #ffffff08",paddingTop:3}}>POWER COMBOS <span style={{color:"#666",fontStyle:"italic"}}>(stack on season hands)</span></div>
            {POWER_COMBOS.map(p=>{const best=handBests[p.name];const discovered=(meta?.stats?.dh||[]).includes(p.name);return(<div key={p.name} style={{fontSize:10,color:discovered?"#555":"#333"}}>{discovered?<><span style={{color:"#c084fc",fontWeight:600}}>{p.name} ✨</span> +<span style={{color:"#3b82f6"}}>{p.bonus.c}</span>C +<span style={{color:"#ef4444"}}>{p.bonus.m}</span>M {p.ex}{best?<span style={{color:"#fbbf24bb"}}> best:{best.toLocaleString()}</span>:""}</>:<><span style={{color:"#666",fontWeight:600}}>????</span> <span style={{fontStyle:"italic",color:"#666666bb"}}>{p.ex.includes("consecutive")?"Consecutive combo":"Matching combo"}</span></>}</div>);})}
          </div>
          <div style={{borderTop:"1px solid #ffffff0a",marginTop:4,paddingTop:4}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:1}}>
              {TRAITS.map(t=>{const tl=traitTierLabel(t);return(<div key={t.name} style={{fontSize:10,color:tl.color}}>{t.icon} <span style={{fontWeight:600}}>{t.name}</span> <span style={{opacity:.6,fontSize:10}}>({tl.label})</span> {t.desc}</div>);})}
            </div>
          </div>
          <div style={{display:"flex",gap:8,paddingTop:4,fontSize:10,color:"#666",flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",gap:8,paddingBottom:4,fontSize:10,color:"#666",flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",gap:8,paddingBottom:6,fontSize:10,color:"#666",flexWrap:"wrap"}}>
      ...
      {showAbandon&&<div style={{position:"fixed",top:8,left:8,zIndex:200,display:"flex",gap:4,alignItems:"center"}}>
        <span style={{fontSize:10,color:"#ffffff55",marginRight:2}}>v0.7</span>
        {abandonConfirm?<>
          <button onClick={()=>{setAbandonConfirm(false);setPh("title");setTab("play");}} style={{background:"#ef444433",border:"1px solid #ef4444",borderRadius:6,fontSize:10,cursor:"pointer",padding:"4px 10px",color:"#ef4444",animation:"fadeIn .2s ease-out"}}>Abandon Run</button>
          <button onClick={()=>setAbandonConfirm(false)} style={{background:"none",border:"1px solid #ffffff22",borderRadius:6,fontSize:10,cursor:"pointer",padding:"4px 8px",color:"#888"}}>✕</button>
        </>:<div style={{display:"flex",gap:3}}>
          <button onClick={()=>setAbandonConfirm(true)} style={{background:"#ffffff08",border:"1px solid #ffffff22",borderRadius:8,fontSize:11,cursor:"pointer",padding:"6px 10px",color:"#aaa",display:"flex",alignItems:"center",gap:4}} title="Abandon run"><span style={{fontSize:14}}>☰</span><span style={{fontSize:10,letterSpacing:1}}>MENU</span></button>
      ...
      {showLog&&<div style={{position:"fixed",top:0,left:0,bottom:0,width:Math.min(320,vw-40),zIndex:250,background:"#0d1117f8",borderRight:"1px solid #ffffff15",overflowY:"auto",padding:"40px 12px 20px",animation:"slideIn .2s ease-out"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <div style={{fontSize:11,color:"#fbbf24",letterSpacing:3,fontWeight:700}}>📋 RUN LOG</div>
          <button onClick={()=>setShowLog(false)} style={{background:"none",border:"none",color:"#666",fontSize:14,cursor:"pointer"}}>✕</button>
        </div>
        <div style={{fontSize:10,color:"#666",marginBottom:8}}>Night {ante} · Round {blind+1}/3 · {runLog.length} events</div>
        {runLog.length===0?<div style={{fontSize:11,color:"#666",fontStyle:"italic"}}>Nothing has happened yet.</div>
        :<div style={{display:"flex",flexDirection:"column",gap:3}}>
          {[...runLog].reverse().map((e,i)=>{
            ...
            return(<div key={i} style={{fontSize:10,color,lineHeight:1.4,padding:"2px 0",borderBottom:"1px solid #ffffff06"}}>
              ...
              <span style={{fontSize:10,color:"#666",marginLeft:4}}>N{e.ante}.{e.blind+1}</span>
            </div>);
          })}
        </div>}
      </div>}
