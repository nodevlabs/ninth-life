...
const BREEDS = {
  Autumn: { color: "#b8956a", glow: "#9a7a52", bg: "#1f1a14", icon: "\u{1F342}", name: "Autumn", lore: "Born when the leaves fell. They know what it means to let go." },
...
const TRAITS = [
  ...
  { name: "Scrapper", icon: "\u{1F94A}", desc: "+3 mult, +5 if scarred. Discard: +1 Nerve", tier: "rare" },
  ...
  { name: "Seer", icon: "\u{1F441}\uFE0F", desc: "+4 mult. Correctly predicted hand type: +8", tier: "rare" },
  { name: "Guardian", icon: "\u{1F6E1}\uFE0F", desc: "+2 mult per injured or scarred cat played. Discard: heal 1", tier: "rare" },
  ...
  { name: "Phoenix", icon: "\u{1F525}", desc: "\xD72.5 mult. Scarred: \xD74. Revives once on death", tier: "mythic" }
];
...
function pickBreedTrait(p1, p2) {
  if (p1 && p2) return pickBreedInheritTrait(p1, p2);
  const r = Math.random();
...
function catHas(cat, name) {
  return (cat.trait || PLAIN).name === name || (cat.extraTraits || []).some((t) => t.name === name);
}
function catAllTraits(cat) {
  const all = [cat.trait || PLAIN, ...cat.extraTraits || []];
  return all.filter((t) => t.name !== "Plain");
}
function catIsPlain(cat) {
  return (cat.trait || PLAIN).name === "Plain" && !(cat.extraTraits || []).length;
}
function catIsKitten(cat) {
  return !!(cat.parentIds && cat.parentIds.length > 0 && (cat.stats?.tp || 0) === 0);
}
function addTrait(cat, trait) {
  if (!cat.trait) cat.trait = PLAIN;
  if (cat.trait.name === "Plain") {
    cat.trait = trait;
    return true;
  }
  if (!(cat.extraTraits || []).length && cat.trait.name !== trait.name) {
    cat.extraTraits = [trait];
    return true;
  }
  return false;
}
...
const FAMS = [
  { id: "f1", name: "Falling Leaf", icon: "\u{1F342}", desc: "+2 mult per Autumn cat, +5 if 3+", eff: (c) => {
    const n = c.filter((x) => getCatBreeds(x).includes("Autumn")).length;
    return { mult: n * 2 + (n >= 3 ? 5 : 0) };
  } },
  { id: "f2", name: "Warm Hearth", icon: "\u2600\uFE0F", desc: "+2 mult per Summer cat, +5 if 3+", eff: (c) => {
    const n = c.filter((x) => getCatBreeds(x).includes("Summer")).length;
    return { mult: n * 2 + (n >= 3 ? 5 : 0) };
  } },
  { id: "f3", name: "Snowglobe", icon: "\u{1F52E}", desc: "+2 mult per Winter cat, +5 if 3+", eff: (c) => {
    const n = c.filter((x) => getCatBreeds(x).includes("Winter")).length;
    return { mult: n * 2 + (n >= 3 ? 5 : 0) };
  } },
  { id: "f4", name: "First Bud", icon: "\u{1F338}", desc: "+2 mult per Spring cat, +5 if 3+", eff: (c) => {
    const n = c.filter((x) => getCatBreeds(x).includes("Spring")).length;
    return { mult: n * 2 + (n >= 3 ? 5 : 0) };
  } },
  { id: "f5", name: "Golden Yarn", icon: "\u{1F9F6}", desc: "+15 chips per cat", eff: (c) => ({ chips: c.length * 15 }) },
  ...
  { id: "f9", name: "Stubborn's Stone", icon: "\u{1FAA8}", desc: "+6 mult with Stubborn", eff: (c) => c.some((x) => catHas(x, "Stubborn")) ? { mult: 6 } : {} },
  { id: "f10", name: "Wild Card", icon: "\u{1F0CF}", desc: "\xD72 with Wild cat", eff: (c) => c.some((x) => catHas(x, "Wild")) ? { xMult: 2 } : {} },
  { id: "f11", name: "Echo Chamber", icon: "\u{1F50A}", desc: "+5 mult per Echo cat", eff: (c) => ({ mult: c.filter((x) => catHas(x, "Echo")).length * 5 }) },
  { id: "f12", name: "Brawler's Belt", icon: "\u{1F94B}", desc: "+3 mult per Scrapper", eff: (c) => ({ mult: c.filter((x) => catHas(x, "Scrapper")).length * 3 }) },
  { id: "f18", name: "Iron Will", icon: "\u{1F6E1}\uFE0F", desc: "\xD71.15 per scarred cat", eff: (c) => {
  ...
  { id: "f22", name: "Harmony", icon: "\u{1F3B5}", desc: "Two Kin \xD71.7, Full Den \xD71.6", eff: () => ({}), htBonus: { "Two Kin": { xMult: 1.7 }, "Full Den": { xMult: 1.6 } } },
  { id: "f23", name: "Lone Wolf", icon: "\u{1F311}", desc: "Stray \xD72.5", eff: () => ({}), htBonus: { Stray: { xMult: 2.5 } } },
  { id: "f24", name: "Bench Coach", icon: "\u{1FA91}", desc: "+2M per unplayed cat", eff: (c, ctx) => ({ mult: (ctx?.benchSize || 0) * 2 }) },
...
const CURSES = [
  ...
  { id: "c_exile", name: "Exile", icon: "\u{1F6AB}", desc: "One season exiled", tier: 2, fx: { exile: true } },
  ...
  { id: "c_double", name: "Double Down", icon: "\u{1F3B2}", desc: "Threshold \xD71.3", tier: 3, fx: { tgtMult: 1.3 } }
];
const NERVE = [
  { name: "Still", xM: 1, color: "#555", glow: "transparent", desc: "" },
  // 0
  { name: "Stirring", xM: 1, color: "#555", glow: "transparent", desc: "" },
  // 1
  { name: "Awake", xM: 1.05, color: "#6b7280", glow: "transparent", desc: "" },
  // 2
  { name: "Alert", xM: 1.05, color: "#6b7280", glow: "#6b728033", desc: "" },
  // 3
  { name: "Tense", xM: 1.1, color: "#b8956a", glow: "#b8956a33", desc: "" },
  // 4
  { name: "Focused", xM: 1.1, color: "#b8956a", glow: "#b8956a44", desc: "" },
  // 5
  { name: "Cornered", xM: 1.15, color: "#b85c2c", glow: "#b85c2c44", desc: "" },
  // 6
  { name: "Cornered", xM: 1.2, color: "#b85c2c", glow: "#b85c2c55", desc: "" },
  // 7
  { name: "Defiant", xM: 1.25, color: "#d97706", glow: "#d9770655", desc: "backs against the wall" },
  // 8
  { name: "Defiant", xM: 1.3, color: "#d97706", glow: "#d9770666", desc: "backs against the wall" },
  // 9
  { name: "Burning", xM: 1.35, color: "#f59e0b", glow: "#f59e0b66", desc: "past the point of fear" },
  // 10
  { name: "Burning", xM: 1.4, color: "#f59e0b", glow: "#f59e0b77", desc: "past the point of fear" },
  // 11
  { name: "Fury", xM: 1.5, color: "#fb923c", glow: "#fb923c77", desc: "nothing left to lose" },
  // 12
  { name: "Blazing", xM: 1.6, color: "#fbbf24", glow: "#fbbf2488", desc: "the air catches fire" },
  // 13
  { name: "Blazing", xM: 1.7, color: "#fbbf24", glow: "#fbbf24aa", desc: "the air catches fire" },
  // 14
  { name: "Undying", xM: 1.85, color: "#fef08a", glow: "#fef08aaa", desc: "they should have stayed down" },
  // 15
  { name: "Undying", xM: 2, color: "#fef08a", glow: "#fef08abb", desc: "they should have stayed down" },
  // 16
...
const UPGRADES = [
  ...
  { id: "u_g", name: "Buried Provisions", icon: "\u{1F41F}", desc: "+2 Rations each night", cost: 25, b: { gold: 2 }, max: 3, tier: 1 },
  { id: "u_d", name: "Quick Instincts", icon: "\u{1F4E3}", desc: "+1 Free Recruit per round", cost: 40, b: { freeRecruits: 1 }, max: 2, tier: 1 },
  { id: "u_den", name: "Deeper Burrow", icon: "\u{1F3E0}", desc: "+1 Shelter slot in the den", cost: 50, b: { shelter: 1 }, max: 2, tier: 1 },
  { id: "u_keen", name: "Keen Eye", icon: "\u{1F3AF}", desc: "Recruit costs 1\u{1F41F} less (min free)", cost: 35, b: { recruitDiscount: 1 }, max: 1, tier: 1 },
  ...
  { id: "u_f", name: "The Old Fire", icon: "\u{1F525}", desc: "Start with +2 Nerve", cost: 70, b: { fervor: 2 }, max: 2, tier: 2 },
  { id: "u_pot", name: "The Warden", icon: "\u{1F6E1}\uFE0F", desc: "Start each run with a random ward", cost: 75, b: { startWard: 1 }, max: 2, tier: 2 },
  { id: "u_b", name: "Blood Memory", icon: "\u{1FA78}", desc: "Starter cat inherits a Hearth cat's trait", cost: 80, b: { bloodMemory: 1 }, max: 1, tier: 2 },
  { id: "u_fertile", name: "Fertile Ground", icon: "\u{1F33F}", desc: "Den breed chance +15%", cost: 60, b: { breedBoost: 0.15 }, max: 1, tier: 2 },
  { id: "u_scroll", name: "Ancient Scrolls", icon: "\u{1F4DC}", desc: "Start with a random hand type leveled up", cost: 75, b: { startScroll: 1 }, max: 1, tier: 2 },
  ...
  { id: "u_bench", name: "Deep Bench", icon: "\u{1FA91}", desc: "Unplayed cats give +50% passive bonus", cost: 90, b: { doubleBench: 1 }, max: 1, tier: 3 },
  { id: "u_combo", name: "Power Resonance", icon: "\u{1F4A5}", desc: "Power combos give +50% bonus", cost: 110, b: { comboBoost: 0.5 }, max: 1, tier: 3 },
  ...
  { id: "u_draft", name: "Wider Horizons", icon: "\u{1F305}", desc: "See 4 cats per draft wave instead of 3", cost: 130, b: { draftSize: 1 }, max: 1, tier: 4 },
...
function epithetToastMsg(cat) {
  const fn = cat.name.split(" ")[0];
  return `${fn} earned: "${cat.epithet}"`;
}
const DEN_BREED = [
...
function getDeathMemorial(cat, ante) {
  ...
  const bs = cat.stats?.bs || 0;
  const bonded = cat.bondedTo;
  const scarred = cat.scarred;
...
const BOSS_FLAVOR = [
  "The cats go still. All of them. At once. They've never felt this before. The dark has sent it eight times.",
  "A sound at the edge of the dark. Patient. Certain. It has done this before. It expects to do it again.",
  "The pattern sends its test. Same test it sent the first colony. Same test it sent the eighth.",
  "Everything you built. Everyone you kept alive. The system doesn't care. It only measures the number.",
  "Something older than your colony steps forward. It remembers every name it's ever erased.",
  "The dark doesn't improvise. It sends what worked before. This is what worked before.",
  "There is nowhere left to retreat. The den is behind you. The test is in front. It has always been in front.",
  "This is the pattern's favorite part. The part where colonies break. Prove it wrong."
];
const BLIND_WHISPER = {
  dusk: [
    ...
    "Prove you exist. The threshold doesn't believe you yet."
  ],
  midnight: [
    ...
    "The threshold rises. Eight colonies met this number. Six couldn't match it."
  ],
  boss: [
...
function getThresholdClear(ante, blind, clutch, pct) {
  ...
  if (clutch) {
    return pk([
      ...
      "The threshold held. You held harder.",
  ...
  if (pct >= 300) {
    return pk([
      "The threshold didn't break. It evaporated.",
  ...
  if (pct >= 200) {
    const crush = [
      ...
      "The threshold shattered like it was never there."
    ];
    if (ante >= 4) crush.push("Four nights in and still hitting like this. The dark is running out of responses.");
    return pk(crush);
  }
  const lines = [
    ["The dark heard something. Wasn't sure what. Decided not to check.", "One hand in. One threshold down. The smallest victory is still a victory.", "They existed loud enough to matter. For one more round."],
    ...
    ["Three nights deep. The dark knows your name, your number, your shape. You're still here.", "Most colonies break on Night 3. You bent. That's different.", "The threshold gives like it's getting tired of you."],
    ...
    ["The last threshold. You walked through it like a door you've opened before.", "Five nights and every threshold broken. The dark has nothing left to say.", "You are not forgettable. You never were."]
  ];
...
const ANTE_ESCALATION = [
  ...
  "It remembers your shape now. The thresholds tighten.",
  "Four nights. The dark sends the patterns that killed the other eight.",
  "The last threshold. Everything the dark has. Everything you have. One of you is wrong."
];
const ACHIEVEMENTS = [
  // Tier 1 — Foundation
  { id: "first_win", name: "Survivor", desc: "Win your first run", icon: "\u{1F3C6}", check: (s) => s.w >= 1, reward: "5-night mode unlocked" },
  { id: "deathless", name: "Every Single One", desc: "Win with 0 deaths", icon: "\u{1F49A}", check: (_, f) => f === true, reward: "New epigraphs unlocked" },
  { id: "ten_runs", name: "The Stubborn", desc: "Attempt 10 runs", icon: "\u{1F504}", check: (s) => s.r >= 10, reward: "Run counter on title screen" },
  { id: "max_fervor", name: "Nerve of Steel", desc: "Reach maximum Nerve", icon: "\u{1F525}", check: (s) => s.mf >= 9, reward: "Gold nerve flame visual" },
  // Tier 2 — Mastery
  { id: "all_breeds", name: "All Four Seasons", desc: "Save each season to Hearth", icon: "\u{1F43E}", check: (s) => {
  ...
  { id: "five_wins", name: "Colony Leader", desc: "Win 5 runs", icon: "\u{1F451}", check: (s) => s.w >= 5, reward: "Gold colony name on title" },
  { id: "night5", name: "Into the Dark", desc: "Reach Night 5", icon: "\u{1F319}", check: (s) => s.ba >= 5, reward: "Boss traits shown on Night Card" },
  { id: "breeder", name: "The Breeder", desc: "5+ kittens in one run", icon: "\u{1F423}", check: (s) => s.kittensTotal >= 5, reward: "Kitten celebration" },
  // Tier 3 — Legend
  { id: "legend_score", name: "NINTH LIFE", desc: "Score 350,000+ in one hand", icon: "\u2728", check: (s) => s.hs >= 35e4, reward: "Gold card borders" },
  { id: "heat_five", name: "Five by Five", desc: "Win 5 runs at Heat 1+", icon: "\u{1F525}", check: (s) => (s.heatWins || 0) >= 5, reward: "Career heat wins shown" },
  { id: "diplomat", name: "The Diplomat", desc: "Resolve 10 grudges across all runs", icon: "\u{1F54A}\uFE0F", check: (s) => (s.grudgesResolved || 0) >= 10, reward: "Upgraded reconciliation" },
  { id: "archivist", name: "The Archivist", desc: "Play 50 hands of one type", icon: "\u{1F4DC}", check: (s) => {
    const hp = s.handTypePlays || {};
    return Object.values(hp).some((v) => v >= 50);
  }, reward: "Custom hand type sound" },
  // Tier 4 — Myth
  { id: "ninth_dawn", name: "The Remembering", desc: "Clear the Ninth Dawn", icon: "\u{1F305}", check: (s) => s.ninthDawnCleared === true, reward: "Secret epigraph pool" },
  ...
  { id: "unbroken", name: "Unbroken Line", desc: "3rd generation cat in colony", icon: "\u{1F46A}", check: (s) => s.thirdGen === true, reward: "Lineage tree in den" },
  { id: "completionist", name: "Completionist", desc: "All other achievements", icon: "\u{1F31F}", check: (s, _, achv) => ACHIEVEMENTS.filter((a) => a.id !== "completionist").every((a) => achv.includes(a.id)), reward: "Alternate title gradient" }
];
...
const BOSS_REWARDS = [
  ...
  { id: "br_blood", name: "Blood Price", desc: "Strongest cat scarred. All others +2 Power.", icon: "\u{1FA78}", type: "blood_price", minNight: 3 }
];
...
const EXPANDED_BOSSES = [
  {
    ...
    tauntFn: (ctx) => ctx.bonded > 3 && ctx.grudges > 0 ? "Love and hate in the same den. I don't even have to try." : ctx.grudges > 0 ? `They resent each other. I can taste it.` : "No grudges? Give it time. I'm patient.",
  ...
  },
  {
    ...
    tauntFn: (ctx) => ctx.fallen > 2 ? "You've lost so many. Why keep going? For what?" : ctx.colony < 12 ? "So few of you. You've done enough. It's okay." : "All that nerve. All that fire. Aren't you tired?",
  ...
  },
  {
    ...
    tauntFn: (ctx) => ctx.clutch ? "Cutting it close. I know what 'close' costs." : ctx.colony < 10 ? "Not enough of you. There weren't enough of us either." : ctx.fallen > 0 ? "We lost three on Night 4. Thought we could still make it. 'Still' is a lie." : "This is where we died. Right here. Make it different.",
...
const BOSS_TRAITS = [
  { id: "armored", name: "Armored", icon: "\u{1F6E1}\uFE0F", desc: "+20% threshold", flavor: "It remembers how to protect itself.", fx: { tgtMult: 1.2 } },
  ...
  { id: "bleeding", name: "Bleeding", icon: "\u{1FA78}", desc: "Threshold \u22122% per hand played", flavor: "It weakens, but will you last?", fx: { bleeding: true } },
  ...
  { id: "enraged", name: "Enraged", icon: "\u{1F525}", desc: "Threshold \u221215%, all cats +3 mult", flavor: "It fights harder. So do you.", fx: { tgtMult: 0.85, enragedMult: 3 } },
  { id: "fading", name: "Fading", icon: "\u{1F56F}\uFE0F", desc: "Threshold +5% per hand remaining", flavor: "It grows stronger as time runs out.", fx: { fading: true } },
  { id: "marked", name: "Marked", icon: "\u{1FA9E}", desc: "One cat scarred before the fight", flavor: "The dark reached in and chose one.", fx: { marked: true } },
...
function getNarratorRecap(ante, ctx) {
  ...
  if (eh.sickness_quarantine) echoes.push("The one you quarantined carries the scar like a question you haven't answered.");
  ...
  if (ante === 3) {
    ...
    else if (scarred >= 3 && scarredCat) stateLines.push(`${scarredCat.name.split(" ")[0]} carries scars now. The colony is starting to rhyme with the ones that came before.`);
  ...
  for (let i = 0; i < cats.length; i++) {
    ...
    for (let j = i + 1; j < cats.length; j++) {
      ...
      if (!noBreed && roll < bCh) {
        ...
        if (loser.injured) forceDeath = true;
        else if (loser.scarred && Math.random() < 0.09) forceDeath = true;
        if (forceDeath) {
          if (hasPhoenix) {
            loser.power = 1;
            loser.scarred = true;
...
function getCatBreeds(cat) {
  if (catHas(cat, "Wild") || catHas(cat, "Chimera")) return [...BK];
  return [cat.breed];
}
function evalH(cats) {
...
function calcScore(cats, fams, fLvl, cfx = {}, ctx = {}) {
  ...
  toS.forEach((c, si) => {
    ...
    bd.push({
      ...
      reason: reasons.length > 0 ? reasons.join(" \xB7 ") : "",
      chips: cc,
      mult: cm,
      xMult: cx !== 1 ? cx : null,
      type: catType,
  ...
  if (bench.length > 0) {
    ...
    if (bc > 0) {
      chips += bc;
      bd.push({ label: `\u{1FA91} Unplayed +${bc}C`, chips: bc, mult: 0, type: "bench" });
    }
    if (bm > 0) {
      mult += bm;
      bd.push({ label: `\u{1FA91} Unplayed +${bm}M`, chips: 0, mult: bm, type: "bench" });
    }
  }
  if (!cfx.silence) {
...
function getTarget(a, b, firstRun, isLong) {
  ...
  const base = (firstRun ? [1200, 2400, 3500] : [2e3, 4e3, 8500])[b] * Math.pow(scale, a - 1);
  const bossDiscount = b === 2 ? a === 1 ? 0.8 : a === 2 && firstRun ? 0.85 : 1 : 1;
...
function genDescendant(hearthCats, powerBonus = 0) {
  ...
  if (pairs.length === 0) return gC({ trait: pickDraftTrait() });
  const pair = pk(pairs);
...
function CatPortrait({ cat, sm, b, fill }) {
  ...
  if (fill) return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(
    "img",
    {
      src: url,
      alt: "",
      loading: "eager",
      style: { width: "100%", height: "100%", objectFit: "cover", display: status === "loaded" ? "block" : "none", filter: cat.injured ? "saturate(0.3) brightness(0.5)" : cat.scarred ? "contrast(1.15) brightness(0.9)" : "none", opacity: 0.85 },
      onLoad: () => {
        setStatus("loaded");
        _portraitsAvailable = true;
      },
  ...
  const sz = sm ? 40 : 56;
  return /* @__PURE__ */ React.createElement("div", { style: { width: sz, height: sz, position: "relative", marginBottom: sm ? 1 : 2, flexShrink: 0, borderRadius: "50%", overflow: "hidden", border: `1.5px solid ${b.color}33` } }, /* @__PURE__ */ React.createElement(
    "img",
    {
      src: url,
      alt: "",
      loading: "eager",
      style: { width: "100%", height: "100%", objectFit: "cover", display: status === "loaded" ? "block" : "none", filter: cat.injured ? "saturate(0.4) brightness(0.7)" : cat.scarred ? "contrast(1.2)" : "none" },
      onLoad: () => {
        setStatus("loaded");
        _portraitsAvailable = true;
      },
...
function CC({ cat: _cat, sel, onClick, sm, cw: _cw, dis, hl, fog, chemHint, denMode, onTraitClick }) {
  ...
  let rankLabel = null;
  if (fog) return /* @__PURE__ */ React.createElement("div", { onClick: dis ? void 0 : onClick, style: { width: w, height: h, borderRadius: sm ? 8 : 12, background: "#0d1117", border: "2px solid #ffffff12", boxShadow: "0 2px 8px #00000066", cursor: dis ? "default" : "pointer", transition: "all .15s", transform: sel ? "translateY(-12px) scale(1.05)" : "", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontFamily: "'Manrope',sans-serif" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: sm ? 22 : 30, opacity: 0.12, color: neon } }, "?"));
  return /* @__PURE__ */ React.createElement("div", { onClick: dis ? void 0 : onClick, title: cat.name + " - " + b.name + ", P" + cat.power, style: {
    width: w,
    height: h,
    ...
    zIndex: 2
  } }, _portraitsAvailable !== false && getPortraitUrl(cat) ? /* @__PURE__ */ React.createElement(
    "img",
    {
      src: getPortraitUrl(cat),
      alt: "",
      loading: "eager",
      style: {
        width: "100%",
    ...
    fontFamily: "'Manrope',sans-serif"
  } }, cat.power)), (cat.injured || cat.scarred || cat.bondedTo || (cat.grudgedWith || []).length > 0 || cat.parentIds?.length > 0 || cat._hearthChild) && /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", top: 0, left: 0, right: 0, zIndex: 5, display: "flex", gap: 0, justifyContent: "center" } }, cat._hearthChild && /* @__PURE__ */ React.createElement("div", { title: "Hearth Legend: returned to guide this colony. Cannot be saved back.", style: {
  ...
  } }, "\u{1F525}"), cat.injured && /* @__PURE__ */ React.createElement("div", { title: "Injured: half power, \u22122 mult. Heals in 1-2 rounds.", style: {
  ...
  } }, "\u271A"), !cat.injured && cat.scarred && /* @__PURE__ */ React.createElement("div", { title: "Scarred: permanent \xD71.25 mult bonus.", style: {
  ...
  } }, "\u2694"), cat.bondedTo && /* @__PURE__ */ React.createElement("div", { title: "Bonded: \xD71.5 mult when played with partner.", style: {
  ...
  } }, "\u2661"), (cat.grudgedWith || []).length > 0 && /* @__PURE__ */ React.createElement("div", { title: "Grudge: \u22122 mult when played with rival.", style: {
    background: "linear-gradient(180deg,#fb923c33,#fb923c11)",
    padding: sm ? "2px 4px" : "3px 5px",
    fontSize: sm ? 9 : 11,
    color: "#fb923c",
    ...
    fontWeight: 700,
    borderBottom: "1px solid #fb923c44"
  } }, "\u26A1"), cat.parentIds && cat.parentIds.length > 0 && /* @__PURE__ */ React.createElement("div", { title: `Born in the den${cat.stats?.par ? " \u2014 parents: " + cat.stats.par : ""}`, style: {
  ...
  } }, "\u{1F46A}")), rankLabel && /* @__PURE__ */ React.createElement("div", { style: {
    position: "absolute",
    top: sm ? 15 : 20,
    right: sm ? 2 : 3,
    zIndex: 5,
    ...
    fontFamily: "'Manrope',sans-serif",
    fontWeight: 600
  } }, rankLabel), /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", bottom: 0, left: 0, right: 0, zIndex: 4 } }, /* @__PURE__ */ React.createElement("div", { style: { position: "relative", height: sm ? 14 : 18 } }, /* @__PURE__ */ React.createElement("div", { style: {
    position: "absolute",
    top: "50%",
    left: sm ? 4 : 5,
    right: sm ? 4 : 5,
    ...
    gap: sm ? 2 : 3,
    zIndex: 1
  } }, [cat.trait, ...cat.extraTraits || []].filter(function(t) {
    return t.name !== "Plain";
  }).map(function(t, ti) {
    return /* @__PURE__ */ React.createElement("div", { key: ti, onClick: function(e) {
      e.stopPropagation();
      if (onTraitClick) onTraitClick(cat);
    }, style: {
      background: "#0d1117",
      padding: "0 2px",
    ...
    lineHeight: 1,
    zIndex: 1
  } }, isWild ? "\u2726" : b.icon)), /* @__PURE__ */ React.createElement("div", { style: { background: "#0d1117", padding: sm ? "1px 6px 4px" : "2px 8px 5px", textAlign: "center" } }, /* @__PURE__ */ React.createElement("div", { style: {
    fontSize: xs ? fn.length > 5 ? 5 : 7 : sm ? fn.length > 7 ? 7 : fn.length > 5 ? 8 : 10 : fn.length > 8 ? 10 : fn.length > 6 ? 11 : 13,
    fontWeight: 700,
    color: neon,
    letterSpacing: xs ? 0 : sm ? fn.length > 6 ? 0 : 1 : fn.length > 7 ? 1 : 3,
    ...
    lineHeight: 1.2
  } }, fn, /* @__PURE__ */ React.createElement("span", { style: { fontSize: xs ? 7 : sm ? 9 : 11, fontWeight: 700, marginLeft: 2, color: cat.sex === "M" ? "#60a5fa" : "#f472b6" } }, cat.sex === "M" ? "\u2642" : "\u2640")))), chemHint && chemHint.grudge && /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", top: sm ? 3 : 4, left: "50%", transform: "translateX(-50%)", zIndex: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { width: 5, height: 5, borderRadius: 3, background: "#fb923c", boxShadow: "0 0 6px #fb923c88" } })));
}
function FS({ f, onClick, sm, off }) {
  const isChem = f.isChem;
  return /* @__PURE__ */ React.createElement("div", { onClick, title: f.desc, style: { width: sm ? 68 : 84, height: sm ? 44 : 54, borderRadius: 8, border: `1.5px solid ${off ? "#ef444444" : isChem ? "#818cf844" : "#fbbf2444"}`, background: off ? "linear-gradient(145deg,#2a1111,#0d0d1a)" : isChem ? "linear-gradient(145deg,#1a1a3a,#0d0d1a)" : "linear-gradient(145deg,#2a2200,#0d0d1a)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: onClick ? "pointer" : "default", padding: 2, flexShrink: 0, fontFamily: "'Manrope',sans-serif", opacity: off ? 0.5 : 1 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: sm ? 13 : 16 } }, off ? "\u{1F910}" : f.icon), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: off ? "#ef4444" : isChem ? "#818cf8" : "#fbbf24", textAlign: "center", lineHeight: 1.1, marginTop: 1 } }, f.name));
}
function ProgressMap({ ante, blind, mx }) {
  ...
  return /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 2, alignItems: "center", padding: "2px 0" } }, dots.map((d, i) => {
    ...
    return /* @__PURE__ */ React.createElement("div", { key: i, style: { display: "flex", alignItems: "center", gap: 1 } }, showAnte && i > 0 && /* @__PURE__ */ React.createElement("div", { style: { width: 4, height: 1, background: "#333", margin: "0 1px" } }), showAnte && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: d.done || d.cur ? "#fbbf24" : "#444", fontFamily: "'Manrope',sans-serif", marginRight: 1 } }, d.a), /* @__PURE__ */ React.createElement("div", { style: {
...
function FM({ level, prev }) {
  ...
  return /* @__PURE__ */ React.createElement("div", { style: { width: "100%", maxWidth: 700, padding: "0 16px" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6 } }, /* @__PURE__ */ React.createElement("span", { title: "NERVE multiplies ALL scores.\nGain: +1 per Dusk/Midnight clear.\nBoss: +hands remaining (faster clear = more nerve).\nNo decay. No loss.\nAt NINTH LIFE (max): \xD72.2 to ALL scores.", style: { fontSize: 12, fontWeight: 700, color: fv.color, letterSpacing: 2, textShadow: mx ? `0 0 14px ${fv.glow}` : level > 5 ? `0 0 6px ${fv.color}44` : "none", animation: mx ? "fp 1s ease-in-out infinite" : up ? "fpp .4s ease-out" : dn ? "shake .3s ease" : "none", fontFamily: "'Manrope',sans-serif", cursor: "help" } }, mx ? "\u2726 " : "", fv.name, mx ? " \u2726" : ""), /* @__PURE__ */ React.createElement("span", { style: { fontSize: fv.xM > 1 ? 13 : 11, color: fv.color, fontFamily: "'Manrope',sans-serif", fontWeight: 900, opacity: fv.xM > 1 ? 1 : 0.3, letterSpacing: fv.xM > 1 ? 1 : 0, textShadow: fv.xM > 1.3 ? `0 0 8px ${fv.color}44` : "none" } }, fv.xM > 1 ? `\xD7${fv.xM}` : "\xD71"), ch && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, fontWeight: 700, animation: "countUp .4s ease-out", color: up ? "#4ade80" : "#ef4444" } }, up ? "\u25B2" : "\u25BC")), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#555", fontFamily: "'Manrope',sans-serif" } }, level, "/", NERVE_MAX)), /* @__PURE__ */ React.createElement("div", { style: { height: 8, background: "#1a1a2e", borderRadius: 4, overflow: "hidden", border: "1px solid #ffffff08" } }, /* @__PURE__ */ React.createElement("div", { style: { height: "100%", width: `${pct}%`, borderRadius: 4, background: mx ? "linear-gradient(90deg,#b85c2c,#f59e0b,#fef08a,#ffffffcc)" : `linear-gradient(90deg,#b8956a,${fv.color})`, transition: "width .5s cubic-bezier(.4,0,.2,1)", boxShadow: level > 5 ? `0 0 8px ${fv.color}44` : "none" } })), level === 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#666", fontFamily: "'Manrope',sans-serif", marginTop: 2, textAlign: "center" } }, "Clear blinds to build Nerve. Faster boss clears give more."));
}
function ChemPreview({ cats }) {
  if (cats.length < 2) return null;
  const grudges = getGrudges(cats);
  if (!grudges.length) return null;
  return /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, alignItems: "center", fontSize: 10, fontFamily: "'Manrope',sans-serif" } }, grudges.map(([a, b], i) => /* @__PURE__ */ React.createElement("span", { key: `g${i}`, style: { color: "#fb923c" } }, "\u26A1", a.name.split(" ")[0], "+", b.name.split(" ")[0])));
}
function NinthLife() {
  ...
  function toast(icon, text, color = "#fbbf24", dur = 2500) {
    ...
    setToasts((t) => [...t, { id, icon, text, color, big: dur > 2500 }]);
  ...
  const BOSSES = [
    ...
    },
    {
      ...
      tauntFn: (ctx) => ctx.colony > 18 ? `${ctx.colony} of you? The second colony had more. This ground ate them anyway.` : ctx.scarred > 3 ? `${ctx.scarredName} has scars from my ground. Bold. Stupid, but bold.` : ctx.bonded > 0 ? `${ctx.bondedName} thinks this is home now. The ground disagrees.` : ctx.grudges > 2 ? "Fighting among yourselves on contested ground. The fifth colony did that." : "Every colony builds here. Every colony thinks it's different.",
    ...
    },
    {
      ...
      tauntFn: (ctx) => ctx.scarred > 3 ? `${ctx.scarredName} is wounded. We can smell it. We're very good at smelling weakness.` : ctx.colony < 12 ? `${ctx.colony} of you. Getting smaller. The math gets easier for us every round.` : ctx.colony > 18 ? "Impressive numbers! Still not enough. We've done this math eight times." : ctx.grudges > 2 ? `${ctx.grudgedName} is fighting their own. We love that. Saves us the trouble.` : `We've been watching ${ctx.strongestName}. Strong. But there's only one of ${ctx.strongestName}. There are so many of us.`,
  ...
  function genChronicle(won) {
    ...
    if (ante <= 2) lines.push(`It lasted ${ante} night${ante > 1 ? "s" : ""}. Not long enough for anyone to learn each other's names properly. ${draftNames} were the first to step out of the dark, and the colony that formed around them never found its footing.`);
    else if (ante <= 3) lines.push(`Three nights. The colony started with ${draftNames} and ${allC.length + fallen.length} others who gathered around a fire that nobody remembers lighting. By the second night they had a system. By the third, they had a story.`);
    ...
    if (bestHand) {
      ...
      mid.push(`The hand they'll remember: ${bCats}, playing a ${bType} for ${bScore}. The kind of number that makes the dark flinch.`);
    }
    if (deaths.length > 0 && bonds.length > 0) {
      const dName = deaths[0].data.victim?.split(" ")[0] || "one";
      const bPair = bonds[0];
      mid.push(`${dName} fell in the den. That same night, ${bPair?.data?.c1?.split(" ")[0] || "two cats"} and ${bPair?.data?.c2?.split(" ")[0] || "another"} bonded. The colony learned that grief and love run through the same dark.`);
    } else if (deaths.length > 0) {
      const dNames = deaths.map((d) => d.data.victim?.split(" ")[0] || "one").join(", ");
      mid.push(`The den took ${deaths.length === 1 ? dNames : `${dNames}. ${deaths.length} names scratched off the wall`}. The colony got quieter after that. Not weaker. Quieter. There's a difference.`);
    } else if (bonds.length > 0) {
      if (bonds.length === 1) mid.push(`${bonds[0].data.c1?.split(" ")[0]} and ${bonds[0].data.c2?.split(" ")[0]} bonded. Nobody announced it. They just started sleeping closer to the fire, and everyone understood.`);
      else mid.push(`${bonds.length} pairs bonded over the course of ${ante} nights. The colony stopped being a collection of survivors and became something that had a pulse.`);
    }
    if (grudges.length > 0 && reconciles.length > 0) mid.push(`There were fights. ${grudges.length} grudges that cut the den in two. But ${reconciles.length} of them healed. Forgiveness, it turns out, is its own kind of scoring.`);
    else if (grudges.length >= 2) mid.push(`${grudges.length} grudges. None forgiven. The colony survived the dark, but some nights it wasn't clear if the real enemy was outside.`);
    if (breeds.length > 0) mid.push(`${breeds.length} kitten${breeds.length > 1 ? "s were" : " was"} born in the dark. New life, impossibly small, in a world that had stopped believing in new things.`);
    if (totalScore > 1e5) mid.push(`Total output: ${totalScore.toLocaleString()}. The kind of combined score that makes you wonder if the dark was ever really a threat, or just an audience.`);
    lines.push(mid.slice(0, 2).join(" "));
    if (won && fallen.length === 0) lines.push(`All of them. Every single one walked out of night ${ante} and into a dawn that had no right to exist. ${mvpName} carried the colony on their back, scoring ${mvpCat?.stats?.bs?.toLocaleString() || "more than anyone"} in a single hand. Their names belong to the Hearth now. The fire burns because of what they did.`);
    else if (won) lines.push(`${fallen.length} didn't make it. ${fallen.map((f) => f.name.split(" ")[0]).join(", ")}. The rest walked into the dawn carrying names that weren't their own. ${mvpName} scored the highest, but the real score was the number of names they refused to forget.`);
    else lines.push(`Night ${ante}. The numbers fell short and the dark moved in. ${fallen.length > 0 ? fallen.map((f) => f.name.split(" ")[0]).join(", ") + " went first. " : ""}The colony didn't scream. They'd already said everything worth saying. Somewhere in the ashes, a number is scratched into the floor. ${totalScore.toLocaleString()}. That's what they were worth. Remember it.`);
    return lines;
  }
  ...
  function updFerv(s, cs, ct, ch, hasGrudgeProve) {
    ...
    if (totalScore >= ct) {
      let gain = 1;
      if (blind === 2) gain = Math.max(0, handsAfter);
      if (gain > 0) {
  ...
  function startGame(st) {
    ...
    do {
      ...
      allDraft = breeds.map((br) => {
        const c = genDraft();
        c.breed = br;
        c.name = gN(br, c.trait);
        c.quirk = pk(QUIRKS[br] || QUIRKS.Autumn);
        return c;
      });
      allDraft.forEach((c) => {
        c.stats.tp = 3;
    ...
    if (dp > 0) allDraft.forEach((c) => {
      c.power = Math.min(15, c.power + dp);
    });
    ...
    setHLeft(4 + mb.hands + (hfx.handMod || 0));
    ...
    if (meta && meta.cats.length > 0) {
      ...
      if (hd.total > 0) {
        const u = { ...meta, dust: meta.dust + hd.total };
        setMeta(u);
        saveS(u);
      }
    } else {
  ...
  function pickDraft(i) {
    const cat = draftPool[i];
    const picked = [...draftPicked, cat];
    const tn = (cat.trait || PLAIN).name;
    const p = cat.power;
    const draftNarr = tn !== "Plain" && (cat.trait || {}).tier === "legendary" ? `${cat.name.split(" ")[0]}. P${p}. You don't find cats like this. They find you.` : tn !== "Plain" && (cat.trait || {}).tier === "rare" ? `${cat.name.split(" ")[0]}. P${p} ${tn}. They came with a story already started.` : p >= 8 ? `${cat.name.split(" ")[0]}. P${p}. The others step aside when this one walks past.` : p <= 3 ? `${cat.name.split(" ")[0]}. P${p}. No power yet. Give them time.` : `${cat.name.split(" ")[0]}. P${p}. Another name for the record.`;
  ...
  function advanceFromScoring() {
    ...
    try {
      ...
      setLastHandLost(result.total < tgt2 * 0.3);
        ...
        if (shouldInjure) {
          ...
          if (eligible.length > 0) {
            ...
            if (isBloodied) {
              ...
              toast("\u{1FA79}", `${victim.name.split(" ")[0]} was scarred by ${boss?.name || "the boss"}!`, "#ef4444");
              victim.scarred = true;
              assignEpithet(victim);
              if (victim._newEpithet) {
                delete victim._newEpithet;
                toast("\u{1F3F7}\uFE0F", epithetToastMsg(victim), BREEDS[victim.breed]?.color || "#fbbf24", 2500);
                Audio.epithetEarned();
              }
              [setHand, setDraw, setDisc].forEach((s) => {
              ...
              [setHand, setDraw, setDisc].forEach((s) => {
                s((arr) => arr.map((x) => x.id === victim.id ? { ...x, injured: true, injuryTimer: 2, power: Math.max(1, x.power) } : x));
              });
  ...
  function recruitCat() {
    ...
    setSel(/* @__PURE__ */ new Set());
    toast("\u{1F4E3}", `Recruited! Hand +1 (cost ${cost}\u{1F41F})`, "#fbbf24");
    Audio.cardSelect();
  }
    ...
    try {
      ...
      if (won) {
        ...
        const defeatLines = [
          ...
          "Below the threshold. The silence didn't come fast. It came slow. That's worse.",
        ...
        (async () => {
          try {
            ...
            const u = {
              ...
              ninthDawnCleared,
              relics,
              stats: { ...meta.stats, r: meta.stats.r + 1, w: meta.stats.w + (won ? 1 : 0), ba: Math.max(meta.stats.ba, ante), hs: Math.max(meta.stats.hs, fScore), mf: Math.max(meta.stats.mf, rMaxF), mh: newMaxHeat, bossRecord, heatWins, ninthDawnCleared, hearthTotal, belovedNames }
            };
  ...
  async function saveCatM(cat) {
    if (!meta || savingRef.current) return;
    savingRef.current = true;
    try {
      const allC2 = [...hand, ...draw, ...disc];
      const hasKids = allC2.some((c) => c.parentIds?.includes(cat.id));
      const hasParents = cat.parentIds?.some((pid) => allC2.some((c) => c.id === pid));
      const ser = {
        id: cat.id,
        breed: cat.breed,
        power: cat.power,
        ...
        name: cat.name,
        parentBreeds: cat.parentBreeds,
        parentIds: cat.parentIds || null,
        bonded: !!cat.bondedTo,
        scarred: !!cat.scarred,
  ...
  function nextBlind() {
    ...
    if (nb === 2) {
      ...
      const c = genCurses(na, hfxC.extraCurse || 0);
      setCurses(c);
      const fx = buildCfx(c);
      setCfx(fx);
  ...
  function getPartingGifts(cat) {
    ...
    if (isNeg) {
      return { goldVal: 0, gifts, narr: `${cat.name.split(" ")[0]} slipped away before dawn. No one stopped them.` };
    }
    ...
    if (cat.scarred) {
      goldVal += 1;
      gifts.push("+1 Nerve");
      narr = `${cat.name.split(" ")[0]} walked different after the scar. Heavier. But they walked.`;
    }
    if (cat.power >= 8) {
      gifts.push("Weakest gains power");
      if (!narr) narr = `${cat.name.split(" ")[0]} was the strongest. Now someone else has to be.`;
    }
    if (cat.bondedTo) {
  ...
  function sellCat(cat) {
    ...
    setHand((h) => h.filter((c) => c.id !== cat.id));
    setDraw((d) => d.filter((c) => c.id !== cat.id));
    setDisc((di) => di.filter((c) => c.id !== cat.id));
    ...
    toast("\u{1F54A}\uFE0F", `${cat.name.split(" ")[0]} released. ${sellLines[ante % sellLines.length]}${giftStr}`, pg.goldVal >= 0 ? "#c084fc" : "#ef4444");
  }
  function sellFam(f) {
    setGold((g) => g + 3);
  ...
  function endNight() {
    ...
    results.forEach((r) => {
      ...
      if (r.type === "death") {
        ...
        Audio.denDeath();
        toast("\u{1F480}", `${r.victim.name.split(" ")[0]} is gone.`, "#ef4444", 3500);
        setFallen((f) => {
    ...
    const news = results.map((r) => {
      ...
      if (r.type === "fight") return { icon: "\u{1FA79}", text: `${r.loser.name.split(" ")[0]} scarred`, color: "#ef4444" };
      ...
      if (r.type === "mentor") return { icon: "\u{1F4D6}", text: `${r.elder.name.split(" ")[0]} taught ${r.young.name.split(" ")[0]}`, color: "#c084fc" };
    ...
    results.forEach((r) => {
      ...
      if (r.type === "fight") {
        addStory(r.loser.id, `Scarred N${ante}`);
    ...
    results.forEach((r) => {
      const applyEp = (cat, ctx = {}) => {
        assignEpithet(cat, ctx);
        if (cat._newEpithet) {
          delete cat._newEpithet;
          toast("\u{1F3F7}\uFE0F", epithetToastMsg(cat), BREEDS[cat.breed]?.color || "#fbbf24", 2500);
          Audio.epithetEarned();
        }
        [setHand, setDraw, setDisc].forEach((s) => {
          s((arr) => arr.map((c) => c.id === cat.id ? { ...c, epithet: cat.epithet, epithetKey: cat.epithetKey, story: cat.story } : c));
        });
      };
    ...
    const denSound = (r) => {
      ...
      else if (r.type === "teach") Audio.denBond();
  ...
  function buildOutcomeText(evt, choice, targets, fx) {
    ...
    if (fx.choiceSave !== void 0 && targets.length >= 2) {
      ...
      lines.push({ text: `${targets[sv].name.split(" ")[0]} saved. ${targets[sc].name.split(" ")[0]} scarred. They won't forget.`, color: "#fbbf24", icon: "\u2696\uFE0F" });
    }
    ...
    if (fx.tideWade && targets.length === 0) lines.push({ text: "The strongest waded in. Came back changed. And scarred.", color: "#fbbf24", icon: "\u{1F30A}" });
  ...
  const BTN = (bg, col, on = true) => ({ padding: "9px 24px", fontSize: 13, fontWeight: 700, border: "none", borderRadius: 8, cursor: on ? "pointer" : "not-allowed", fontFamily: "'Manrope',sans-serif", letterSpacing: 1, background: on ? bg : "#222", color: on ? col : "#555", transition: "all .2s" });
  const CSS = `
    ...
    *{box-sizing:border-box;-webkit-tap-highlight-color:transparent}
    ...
    body{margin:0;background:#06060f;font-family:'Manrope',sans-serif}`;
  const Dust = () => /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, overflow: "hidden" } }, Array.from({ length: 8 }).map((_, i) => {
  ...
  if (ph === "coldOpen") {
    ...
    }), introStep >= coldLines.length - 1 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#ffffff66", fontFamily: "'Manrope',sans-serif", letterSpacing: 3, marginTop: 24, animation: "fadeIn 2s ease-out 1s both" } }, "tap to continue")));
  }
  if (ph === "firstIntro") {
    const pages = [
      // Page 0: Your role — exciting, not clinical
      () => /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 48, animation: "fadeIn .6s ease-out", opacity: 0.8 } }, "\u{1F525}"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 18, fontWeight: 900, color: "#fbbf24", letterSpacing: 5, fontFamily: "'Manrope',sans-serif", animation: "fadeIn .8s ease-out" } }, "THE LAST HEARTH"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: "#ffffffaa", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 340, lineHeight: 2, animation: "fadeIn 1.2s ease-out" } }, "A fire still burns. Cats are gathering.", /* @__PURE__ */ React.createElement("br", null), "Strays, fighters, survivors. They need someone to lead them."), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: "#e8e6e3cc", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 300, lineHeight: 1.8, animation: "fadeIn 1.6s ease-out", marginTop: 8 } }, "That's you. Keep the fire burning.", /* @__PURE__ */ React.createElement("br", null), "Survive the dark. Make them remember you.")),
      // Page 1: Reading a card — real card with callouts
      () => {
        const sampleCat = { id: "intro", name: "Ember", breed: "Autumn", power: 5, sex: "F", trait: { name: "Alpha", icon: "\u{1F43A}", desc: "\xD71.3 if highest power", tier: "rare" }, scarred: false, injured: false, bondedTo: null, grudgedWith: [], parentIds: null, stats: { tp: 0, bs: 0 } };
        return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff55", letterSpacing: 6, fontFamily: "'Manrope',sans-serif", animation: "fadeIn .4s ease-out" } }, "READING A CARD"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 16, alignItems: "center", justifyContent: "center", animation: "fadeIn .6s ease-out", flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement("div", { style: { position: "relative" } }, /* @__PURE__ */ React.createElement(CC, { cat: sampleCat }), /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", top: 10, left: "100%", marginLeft: 6, display: "flex", alignItems: "center", gap: 4 } }, /* @__PURE__ */ React.createElement("div", { style: { width: 20, height: 1, background: "#ffffff44" } }), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: "#ffffffbb", fontFamily: "'Manrope',sans-serif", whiteSpace: "nowrap" } }, "Power \u2192 chips")), /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", bottom: 28, right: "100%", marginRight: 6, display: "flex", alignItems: "center", gap: 4 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: "#38bdf8", fontFamily: "'Manrope',sans-serif", whiteSpace: "nowrap" } }, "Trait"), /* @__PURE__ */ React.createElement("div", { style: { width: 20, height: 1, background: "#ffffff44" } })), /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", bottom: 28, left: "100%", marginLeft: 6, display: "flex", alignItems: "center", gap: 4 } }, /* @__PURE__ */ React.createElement("div", { style: { width: 20, height: 1, background: "#ffffff44" } }), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: BREEDS.Autumn.color, fontFamily: "'Manrope',sans-serif", whiteSpace: "nowrap" } }, "Season")), /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", bottom: 6, right: "100%", marginRight: 6, display: "flex", alignItems: "center", gap: 4 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: "#e8e6e3aa", fontFamily: "'Manrope',sans-serif", whiteSpace: "nowrap" } }, "Name \u2642\u2640"), /* @__PURE__ */ React.createElement("div", { style: { width: 20, height: 1, background: "#ffffff44" } })))), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "#4ade80bb", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 320, lineHeight: 1.7, animation: "fadeIn .8s ease-out", fontWeight: 600 } }, "Same-season cats score better together.", /* @__PURE__ */ React.createElement("br", null), /* @__PURE__ */ React.createElement("span", { style: { color: "#ffffff55", fontSize: 11, fontWeight: 400 } }, "Higher power = more chips. Traits add multipliers.")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, justifyContent: "center", animation: "fadeIn 1s ease-out", marginTop: 4 } }, ["Autumn", "Winter", "Spring", "Summer"].map((s) => {
          const b = BREEDS[s];
          return /* @__PURE__ */ React.createElement("div", { key: s, style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 1, padding: "4px 8px", borderRadius: 6, background: `${b.color}11`, border: `1px solid ${b.color}22`, minWidth: 48 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14 } }, b.icon), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: b.color, fontWeight: 700 } }, b.name));
        })));
      },
      // Page 2: Draft — your 3 picks as the last trained cats
      () => /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 36, animation: "fadeIn .6s ease-out", opacity: 0.7 } }, "\u2694"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 18, fontWeight: 700, color: "#fbbf24", letterSpacing: 4, fontFamily: "'Manrope',sans-serif", animation: "fadeIn .8s ease-out" } }, "THE BLOODLINES"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: "#ffffffaa", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 320, lineHeight: 1.9, animation: "fadeIn 1s ease-out" } }, "Three cats still carry the old blood.", /* @__PURE__ */ React.createElement("br", null), "Trained. Named. ", /* @__PURE__ */ React.createElement("b", { style: { color: "#fbbf24" } }, "Born with gifts the strays don't have.")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#ffffff88", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 300, lineHeight: 1.7, animation: "fadeIn 1.2s ease-out", marginTop: 8 } }, "Choose them. The rest of the colony will follow."))
    ];
    ...
    } }, pages[introStep](), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, marginTop: 8 } }, pages.map((_, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { width: 8, height: 8, borderRadius: "50%", background: i === introStep ? "#fbbf24" : i < introStep ? "#fbbf2444" : "#333", transition: "all .3s" } }))), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#ffffff66", fontFamily: "'Manrope',sans-serif", animation: "fadeIn 2s ease-out", letterSpacing: 2 } }, isLast ? "Tap to start drafting" : "Tap to continue")));
  }
  if (ph === "naming" && namingCat) {
    ...
    return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", zIndex: 1, gap: 12, padding: 20, maxWidth: 420 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff55", letterSpacing: 6, fontFamily: "'Manrope',sans-serif", animation: "fadeIn .6s ease-out" } }, "NAME YOUR CAT"), /* @__PURE__ */ React.createElement("div", { style: { animation: "float 3s ease-in-out infinite" } }, /* @__PURE__ */ React.createElement(CC, { cat: namingCat })), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 12, alignItems: "center", animation: "fadeIn .8s ease-out" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: b.color, fontWeight: 700, letterSpacing: 1 } }, b.icon, " ", b.name), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#ffffff66" } }, "Power ", namingCat.power)), tr.name !== "Plain" ? /* @__PURE__ */ React.createElement("div", { style: { padding: "12px 16px", borderRadius: 10, background: `${tierCol}08`, border: `1px solid ${tierCol}22`, maxWidth: 340, width: "100%", animation: "fadeIn 1s ease-out" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "center", marginBottom: 6 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 18 } }, tr.icon), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14, color: tierCol, fontWeight: 700 } }, tr.name), tierLabel && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: tierCol, opacity: 0.6, letterSpacing: 1, textTransform: "uppercase" } }, tierLabel)), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#ffffff77", fontFamily: "'Manrope',sans-serif", lineHeight: 1.7 } }, detail)) : /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 16px", borderRadius: 10, background: "#ffffff06", border: "1px solid #ffffff11", maxWidth: 340, width: "100%", animation: "fadeIn 1s ease-out" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#ffffff77", fontFamily: "'Manrope',sans-serif", lineHeight: 1.7 } }, "No special trait yet. This cat can earn one through events, breeding, or the shop.")), /* @__PURE__ */ React.createElement(
      "input",
      {
        id: "nameInput",
        type: "text",
        ...
        style: { fontSize: 22, fontWeight: 700, color: b.color, background: "#0a0a1a", border: `2px solid ${b.color}44`, borderRadius: 8, padding: "10px 16px", textAlign: "center", outline: "none", fontFamily: "'Manrope',sans-serif", width: 220, letterSpacing: 1, marginTop: 4 }
      }
    ), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff44", fontFamily: "'Manrope',sans-serif", letterSpacing: 2 } }, "TAP TO RENAME"), /* @__PURE__ */ React.createElement("button", { id: "nameConfirm", onClick: () => {
    ...
    }, style: { ...BTN(`linear-gradient(135deg,${b.color},${b.color}cc)`, "#0a0a1a"), padding: "10px 32px", fontSize: 14, letterSpacing: 2 } }, babyNamingQueue.length > 0 ? "Name this kitten" : "This is their name")));
  }
  if (ph === "hearthFlash" && hearthFlash) {
    ...
    const companions = hearthCats.map((c) => {
      ...
      return { ...gC({ breed, trait: tr, sex: c.sex || "M" }), name: c.name || "Ghost", power: c.power || 3, scarred: !!c.scarred, epithet: c.epithet || null, epithetKey: c.epithetKey || null, _hearthChild: true, stats: { tp: 0, bs: 0 } };
    }).slice(0, 6);
    return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", zIndex: 1, gap: 16, padding: 20 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 48, animation: "float 3s ease-in-out infinite", filter: "drop-shadow(0 0 20px #fbbf2444)" } }, "\u{1F525}"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#fbbf2466", letterSpacing: 6, fontFamily: "'Manrope',sans-serif", animation: "fadeIn 1s ease-out" } }, "THE HEARTH REMEMBERS"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", justifyContent: "center", maxWidth: 360, animation: "fadeIn 1.5s ease-out" } }, hearthCats.map((c, i) => /* @__PURE__ */ React.createElement("span", { key: i, style: { fontSize: 10, color: BREEDS[c.breed]?.color || "#fbbf24", fontFamily: "'Manrope',sans-serif", opacity: 0.5, animation: `fadeIn ${0.5 + i * 0.15}s ease-out both` } }, c.name?.split(" ")[0] || "?"))), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "#fbbf24cc", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 360, lineHeight: 1.7, animation: "fadeIn 2s ease-out" } }, "The Hearth remembers. One legend can walk back into the dark \u2014 as strong as the day they were saved. They cannot be saved again. Their time in the Hearth is earned. This time, they fight so that others may take their place."), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center", animation: "fadeIn 2.5s ease-out" } }, companions.map((c, i) => /* @__PURE__ */ React.createElement("div", { key: i, onClick: () => {
      setDraftPicked([c]);
      setHearthFlash(null);
      setPh("draft");
      toast(BREEDS[c.breed].icon, `${c.name.split(" ")[0]} returns as a guide. P${c.power}. They fight so new legends can rise.`, BREEDS[c.breed].color, 4e3);
      if (meta.stats.r === 1) {
    ...
    }, style: { cursor: "pointer", textAlign: "center", maxWidth: 90, animation: `float ${2.5 + i * 0.3}s ease-in-out ${i * 0.2}s infinite` } }, /* @__PURE__ */ React.createElement(CC, { cat: c, sm: true }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontFamily: "'Manrope',sans-serif", color: BREEDS[c.breed].color, marginTop: 3, fontWeight: 700 } }, c.name.split(" ")[0], c.epithet ? " " + c.epithet : ""), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff88", fontFamily: "'Manrope',sans-serif" } }, (c.trait || PLAIN).icon, " ", (c.trait || PLAIN).name), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff55", fontFamily: "'Manrope',sans-serif" } }, "P", c.power, c.scarred ? " \u2694" : "")))), /* @__PURE__ */ React.createElement("button", { onClick: () => {
      setHearthFlash(null);
      setPh("draft");
      if (meta.stats.r === 1) {
    ...
    }, style: { ...BTN("#1a1a2e", "#888"), padding: "6px 16px", fontSize: 10, border: "1px solid #ffffff12", marginTop: 4 } }, "Skip. go alone")));
  }
  if (ph === "draft") {
    ...
    return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", zIndex: 1, gap: 14, padding: 20, maxWidth: 500 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888", letterSpacing: 4, fontFamily: "'Manrope',sans-serif", animation: "fadeIn .5s ease-out" } }, hasHearthLineage ? "DESCENDANTS OF THE HEARTH" : "THE GATHERING"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, alignItems: "center" } }, [0, 1, 2].map((i) => /* @__PURE__ */ React.createElement("div", { key: i, style: {
      width: 10,
      height: 10,
      borderRadius: "50%",
    ...
    } })), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#666", fontFamily: "'Manrope',sans-serif", marginLeft: 4 } }, draftPicked.length + 1, " of 3")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#d9770699", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", lineHeight: 1.7, maxWidth: 320, animation: `fadeIn ${isFirstRun2 ? "1s" : "0.4s"} ease-out` } }, storyText), !isFirstRun2 && draftPicked.length === 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#fb923c", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 300, lineHeight: 1.6, animation: "fadeIn 0.5s ease-out 0.2s both", padding: "6px 12px", borderRadius: 6, background: "#fb923c0d", border: "1px solid #fb923c22", fontWeight: 700 } }, "\u2696\uFE0F Strong cats attract weak strays. Choose wisely."), isFirstRun2 && draftPicked.length === 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf2466", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 280, lineHeight: 1.5, animation: "fadeIn 1s ease-out .3s both" } }, "Pick who catches your eye."), draftPicked.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, alignItems: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#4ade80bb", letterSpacing: 2 } }, "CHOSEN"), draftPicked.map((c, i) => /* @__PURE__ */ React.createElement(CC, { key: i, cat: c, sm: true, hl: true }))), !isFirstRun2 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888", fontFamily: "'Manrope',sans-serif", letterSpacing: 1, marginTop: 4 } }, "Choose one. The others scatter into the dark."), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 14, justifyContent: "center", animation: `fadeIn .4s ease-out ${isFirstRun2 ? "0.8s" : "0.3s"} both` } }, draftPool.map((c, i) => {
      ...
      return /* @__PURE__ */ React.createElement(
        "div",
        {
          ...
          style: {
            ...
            textAlign: "center",
            maxWidth: 110,
            animation: `fadeIn .3s ease-out ${(isFirstRun2 ? 0.9 : 0.4) + i * 0.15}s both`,
        ...
        /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, fontFamily: "'Manrope',sans-serif", color: b.color, marginTop: 4, lineHeight: 1.3, fontWeight: 700 } }, c.name.split(" ")[0], " ", /* @__PURE__ */ React.createElement("span", { style: { fontWeight: 400, opacity: 0.6, fontSize: 10 } }, c.sex === "M" ? "\u2642" : "\u2640")),
        /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff99", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", lineHeight: 1.4, minHeight: 28, marginTop: 3 } }, '"', voice, '"'),
        c.trait.name !== "Plain" && (() => {
          ...
          return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: tierColor(c.trait), fontFamily: "'Manrope',sans-serif", marginTop: 2, lineHeight: 1.3, maxWidth: 120 } }, isFirstRun2 ? hook[c.trait.name] || c.trait.desc : c.trait.desc);
        })(),
        c.stats?.par && /* @__PURE__ */ React.createElement("div", { style: { color: "#c084fc", fontSize: 10, fontFamily: "'Manrope',sans-serif", fontStyle: "italic", lineHeight: 1.3, marginTop: 1 } }, meta?.cats?.find((h) => c.parentIds?.includes(h.name)) ? `${c.stats.par.split("\xD7")[0]?.trim()}'s blood. The Hearth remembers.` : `Child of ${c.stats.par}`)
      );
    })), !isFirstRun2 && draftRejects.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", fontFamily: "'Manrope',sans-serif", fontStyle: "italic" } }, "The ones you turned away haven't gone far...")));
  }
  if (ph === "nightCard" && nightCard) {
    ...
    return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: {
      position: "fixed",
      top: "10%",
      left: "50%",
      ...
      marginBottom: 2
    } }, blindNames[nightCard.blind].toUpperCase()), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 12, marginBottom: 16, marginTop: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { width: 60, height: 1, background: `linear-gradient(90deg,transparent,${glowColor}33)` } }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#ffffff55", letterSpacing: 8, fontFamily: "'Manrope',sans-serif" } }, "NIGHT ", nightCard.ante), /* @__PURE__ */ React.createElement("div", { style: { width: 60, height: 1, background: `linear-gradient(90deg,${glowColor}33,transparent)` } })), isBoss2 && boss && /* @__PURE__ */ React.createElement("div", { style: {
      display: "flex",
    ...
    } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 48 } }, boss.icon), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 20, color: "#ef4444", fontWeight: 700, letterSpacing: 3, fontFamily: "'Manrope',sans-serif" } }, boss.name), /* @__PURE__ */ React.createElement("div", { style: { width: 80, height: 1, background: "linear-gradient(90deg,transparent,#ef444433,transparent)", margin: "4px 0" } }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#ef444466", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", lineHeight: 1.5 } }, boss.lore), boss.tauntFn && (() => {
      ...
      return taunt ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#ef4444bb", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", lineHeight: 1.6, marginTop: 4 } }, '"', taunt, '"') : null;
    })()), !isBoss2 && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: "#ffffff77", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", letterSpacing: 2, animation: "fadeIn 1s ease-out", textAlign: "center", textShadow: "0 0 20px #ffffff11", maxWidth: 320, marginBottom: 8 } }, whisper), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "#ffffff33", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 320, lineHeight: 1.8, animation: "fadeIn 1.2s ease-out", letterSpacing: 4, fontWeight: 500 } }, epi), NIGHT_SUB[Math.min(nightCard.ante - 1, 4)] && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#ffffff55", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 280, lineHeight: 1.5, animation: "fadeIn 1.5s ease-out", marginTop: 4 } }, NIGHT_SUB[Math.min(nightCard.ante - 1, 4)])), nightCard.blind === 0 && nightCard.ante > 1 && (() => {
      ...
      return /* @__PURE__ */ React.createElement(React.Fragment, null, showWhisper && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf2433", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 300, lineHeight: 1.5, animation: "fadeIn 2s ease-out .5s both", marginTop: 8 } }, wLine), narLine && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#c084fc55", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 320, lineHeight: 1.6, animation: "fadeIn 2.5s ease-out 1s both", marginTop: showWhisper ? 4 : 8, padding: "6px 12px", borderRadius: 8, background: "#c084fc06", border: "1px solid #c084fc11" } }, narLine));
    })(), isNinthDawn && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf2466", letterSpacing: 5, fontFamily: "'Manrope',sans-serif", animation: "fadeIn 1.8s ease-out", marginTop: 8 } }, "THE NINTH DAWN"), nightCard.ante >= MX && nightCard.blind === 0 && (fallen.length > 0 || allC.length > 0) && /* @__PURE__ */ React.createElement("div", { style: {
      ...
      marginTop: 12
    } }, fallen.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ef444466", letterSpacing: 3, marginBottom: 4 } }, "DID NOT MAKE IT"), fallen.map((f, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { fontSize: 11, color: BREEDS[f.breed]?.color || "#ef4444", fontFamily: "'Manrope',sans-serif", opacity: 0.6, lineHeight: 1.5 } }, f.name.split(" ")[0], " ", /* @__PURE__ */ React.createElement("span", { style: { color: "#ffffff66", fontSize: 10 } }, "Night ", f.night)))), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#4ade8066", letterSpacing: 3, marginBottom: 3 } }, allC.length, " STILL HERE"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", justifyContent: "center" } }, allC.slice(0, 16).map((c, i) => /* @__PURE__ */ React.createElement("span", { key: i, style: { fontSize: 10, color: BREEDS[c.breed]?.color || "#888", fontFamily: "'Manrope',sans-serif", opacity: 0.5 } }, c.name.split(" ")[0], c.scarred ? "*" : "")))), (() => {
      const ncMb = getMB();
      ...
      } }, /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", padding: "16px 24px" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: `${glowColor}66`, letterSpacing: 3, fontFamily: "'Manrope',sans-serif", marginBottom: 4 } }, "TARGET"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: isFirstEver ? 34 : 28, fontWeight: 900, color: glowColor, textShadow: `0 0 20px ${glowColor}33` } }, ncTgt.toLocaleString())), /* @__PURE__ */ React.createElement("div", { style: { width: 1, background: "#ffffff08" } }), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", padding: "16px 20px" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#3b82f666", letterSpacing: 3, fontFamily: "'Manrope',sans-serif", marginBottom: 4 } }, "HANDS"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: isFirstEver ? 34 : 28, fontWeight: 900, color: "#3b82f6" } }, hLeft)), /* @__PURE__ */ React.createElement("div", { style: { width: 1, background: "#ffffff08" } }), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", padding: "16px 20px" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ef444466", letterSpacing: 3, fontFamily: "'Manrope',sans-serif", marginBottom: 4 } }, "DISCARDS"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: isFirstEver ? 34 : 28, fontWeight: 900, color: "#ef4444" } }, ncDiscs)));
    })(), isFirstEver && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#4ade80bb", fontFamily: "'Manrope',sans-serif", animation: "fadeIn 2s ease-out .8s both", marginTop: 12 } }, "Tap cats ", "\u2192", " Play Hand ", "\u2192", " Beat ", ncTgt.toLocaleString()), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff55", fontFamily: "'Manrope',sans-serif", marginTop: 20, animation: `fadeIn 2s ease-out ${isFirstEver ? 1.2 : 0.6}s both` } }, "tap to begin")));
  }
  if (ph === "colonyFormed" && colonyData) {
    ...
    return /* @__PURE__ */ React.createElement("div", { style: W, onClick: enterNight }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", zIndex: 1, gap: 14, padding: 20, maxWidth: 600 } }, isNinthDawn && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fbbf24", letterSpacing: 6, fontFamily: "'Manrope',sans-serif", animation: "float 3s ease-in-out infinite", marginBottom: -4 } }, "\u300C THE NINTH DAWN \u300D"), !isFirstRun2 && /* @__PURE__ */ React.createElement("div", { onClick: (e) => e.stopPropagation(), style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 4, animation: "fadeIn 1s ease-out" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf24", letterSpacing: 4, fontFamily: "'Manrope',sans-serif", fontWeight: 700 } }, "NAME YOUR COLONY"), /* @__PURE__ */ React.createElement(
      "input",
      {
        value: colonyName,
        ...
        style: { background: "#ffffff08", border: `2px solid ${(meta?.achv || []).includes("five_wins") ? "#fbbf24" : "#fbbf2444"}`, borderRadius: 8, padding: "8px 14px", color: "#fbbf24", fontSize: 16, fontFamily: "'Manrope',sans-serif", letterSpacing: 3, width: 220, outline: "none", textAlign: "center", boxShadow: (meta?.achv || []).includes("five_wins") ? "0 0 16px #fbbf2433" : "none" }
      }
    )), isFirstRun2 ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#77777799", letterSpacing: 6, fontFamily: "'Manrope',sans-serif", animation: "fadeIn 1.5s ease-out" } }, "YOUR COLONY"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 16, justifyContent: "center", marginTop: 8 } }, chosen.map((c, i) => {
      const b = BREEDS[c.breed];
      return /* @__PURE__ */ React.createElement("div", { key: c.id, style: { textAlign: "center", animation: `fadeIn .6s ease-out ${0.3 + i * 0.4}s both` } }, /* @__PURE__ */ React.createElement(CC, { cat: c, hl: true }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: b.color, fontFamily: "'Manrope',sans-serif", marginTop: 4, fontWeight: 700 } }, c.name.split(" ")[0]), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff77", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", lineHeight: 1.4, marginTop: 2, maxWidth: 100 } }, '"', cpk(flavorCache, `colVoice_${c.id}`, [c], () => getDraftVoice(c)), '"'));
    })), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#888", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", lineHeight: 1.6, maxWidth: 340, marginTop: 8, animation: "fadeIn .8s ease-out .5s both" } }, "Three you chose. Fifteen who were already here, waiting for someone worth following.", /* @__PURE__ */ React.createElement("br", null), /* @__PURE__ */ React.createElement("span", { style: { color: "#77777799" } }, "Eighteen against the dark. It only takes one night to find out if that's enough."))) : /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#77777799", letterSpacing: 6, fontFamily: "'Manrope',sans-serif", animation: "epicReveal 1.2s ease-out forwards" } }, "SURVIVE ", MX, " NIGHTS"), meta && meta.stats.r > 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#777777bb", fontFamily: "'Manrope',sans-serif", letterSpacing: 3 } }, (() => {
    ...
    })()), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#888", letterSpacing: 4, fontFamily: "'Manrope',sans-serif", animation: "fadeIn .6s ease-out .2s both" } }, "THE COLONY"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 15, color: "#d97706bb", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", lineHeight: 1.6, maxWidth: 360, animation: "fadeIn .8s ease-out .15s both" } }, colonyThesis), bloodMemMsg && /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 16px", borderRadius: 8, background: "linear-gradient(135deg,#7a665208,#ef444408)", border: "1px solid #c084fc22", animation: "fadeIn 1s ease-out .3s both", textAlign: "center", maxWidth: 380 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#c084fc", fontFamily: "'Manrope',sans-serif", fontStyle: "italic", lineHeight: 1.6 } }, "\u{1FA78} ", /* @__PURE__ */ React.createElement("b", null, bloodMemMsg.heir.split(" ")[0]), " carries something old. ", bloodMemMsg.trait.icon, " ", /* @__PURE__ */ React.createElement("span", { style: { color: "#e8e6e3" } }, bloodMemMsg.trait.name), ". inherited from ", /* @__PURE__ */ React.createElement("span", { style: { color: "#fbbf24" } }, bloodMemMsg.ancestor), " of the Hearth.", bloodMemMsg.scarred && /* @__PURE__ */ React.createElement("span", { style: { color: "#d97706" } }, " The scar came with it."))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 20, justifyContent: "center", animation: "fadeIn 1s ease-out .25s both" } }, /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 32, fontWeight: 900, color: "#fbbf24", letterSpacing: 2, animation: "comboBurst .6s ease-out .35s both" } }, 14), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", letterSpacing: 2, fontFamily: "'Manrope',sans-serif" } }, "SOULS")), /* @__PURE__ */ React.createElement("div", { style: { width: 1, height: 40, background: "#ffffff0a" } }), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 32, fontWeight: 900, color: "#ef4444", letterSpacing: 2, animation: "comboBurst .6s ease-out .45s both" } }, MX), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", letterSpacing: 2, fontFamily: "'Manrope',sans-serif" } }, "NIGHTS"))), strayOffset && strayOffset !== 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: strayOffset > 0 ? "#4ade80aa" : "#fb923caa", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", animation: "fadeIn 1s ease-out .6s both", maxWidth: 340, lineHeight: 1.5 } }, strayOffset >= 2 ? "Weak cats attract strong strays. They had to be. your chosen carry less." : strayOffset === 1 ? "Weak cats attract strong strays. Something to prove." : strayOffset === -1 ? "Strong cats attract weak strays. They followed the light, not the fight." : "Strong cats attract weak strays. Your chosen burn bright. the rest just watched."), /* @__PURE__ */ React.createElement("div", { style: { width: "100%", marginTop: 4 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#4ade80", letterSpacing: 2, marginBottom: 6, textAlign: "center" } }, "CHOSEN. YOUR EDGE"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10, justifyContent: "center", animation: "fadeIn .6s ease-out .6s both" } }, chosen.map((c) => /* @__PURE__ */ React.createElement("div", { key: c.id, style: { textAlign: "center", cursor: "pointer" }, onClick: () => setTraitTip(c) }, /* @__PURE__ */ React.createElement(CC, { cat: c, hl: true }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: BREEDS[c.breed]?.color, fontFamily: "'Manrope',sans-serif", marginTop: 2, fontWeight: 700 } }, c.name.split(" ")[0]), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888", fontFamily: "'Manrope',sans-serif" } }, c.trait.name !== "Plain" ? c.trait.icon + " " + c.trait.name : "plain"))))), /* @__PURE__ */ React.createElement("div", { style: { width: "100%", marginTop: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888", letterSpacing: 2, marginBottom: 6, textAlign: "center" } }, "THE ONES WHO WERE ALREADY HERE"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, justifyContent: "center", animation: "fadeIn 1s ease-out .9s both" } }, BK.map((b) => {
    ...
    }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 3, padding: "4px 10px", borderRadius: 6, background: "#ffffff04", border: "1px solid #ffffff0a" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#666", fontFamily: "'Manrope',sans-serif" } }, "15 strays \xB7 all Plain"))))), hearthDust > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 8, background: "#c084fc0a", border: "1px solid #c084fc22", animation: "fadeIn 1.5s ease-out 1.4s both" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14 } }, "\u{1F3E0}"), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: "#c084fc", fontFamily: "'Manrope',sans-serif" } }, "The Hearth radiates ", /* @__PURE__ */ React.createElement("span", { style: { fontWeight: 700 } }, "+", hearthDust, "\u2726"))), /* @__PURE__ */ React.createElement("button", { onClick: (e) => {
      e.stopPropagation();
      enterNight();
    }, style: { background: "linear-gradient(135deg,#fbbf24,#f59e0b)", color: "#0a0a1a", border: "none", borderRadius: 10, padding: "14px 44px", fontSize: 17, fontWeight: 900, cursor: "pointer", letterSpacing: 4, fontFamily: "'Manrope',sans-serif", marginTop: 8, boxShadow: "0 0 40px #fbbf2433", textTransform: "uppercase", zIndex: 2 } }, "Enter the Night"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff55", fontFamily: "'Manrope',sans-serif", marginTop: 4 } }, "tap anywhere to continue")));
  }
  if (ph === "title") {
    ...
    return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement(Dust, null), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", top: 10, right: 10, zIndex: 200, display: "flex", gap: 6, alignItems: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#ffffff55", fontFamily: "'Manrope',sans-serif", letterSpacing: 1 } }, "v0.65"), meta && meta.stats.r >= 1 && /* @__PURE__ */ React.createElement("button", { onClick: async () => {
      const sums = [];
      for (let i = 1; i <= SLOT_COUNT; i++) sums.push(await getSlotSummary(i));
      setSlotSummaries(sums);
      setShowSlots(!showSlots);
    }, style: { background: "none", border: "1px solid #ffffff15", borderRadius: 6, fontSize: 10, cursor: "pointer", opacity: 0.5, padding: "3px 8px", color: "#888", fontFamily: "'Manrope',sans-serif" }, title: "Save Slots" }, "\u{1F4BE} ", activeSlot), /* @__PURE__ */ React.createElement("button", { onClick: toggleMute, style: { background: "none", border: "none", fontSize: 16, cursor: "pointer", opacity: 0.4, padding: 4 }, title: muted ? "Unmute" : "Mute" }, muted ? "\u{1F507}" : "\u{1F50A}")), showSlots && slotSummaries && /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", top: 40, right: 10, zIndex: 300, background: "#0d1117ee", border: "1px solid #ffffff15", borderRadius: 12, padding: 14, width: 220, animation: "fadeIn .2s ease-out", fontFamily: "'Manrope',sans-serif" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888", letterSpacing: 2, marginBottom: 8 } }, "SAVE SLOTS"), slotSummaries.map((s, i) => {
      const n = i + 1;
      ...
      return /* @__PURE__ */ React.createElement("div", { key: n, onClick: async () => {
        ...
        transition: "all .2s"
      } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, fontWeight: isActive ? 700 : 400, color: isActive ? "#fbbf24" : "#888" } }, "Slot ", n, isActive ? " \u2726" : ""), !s.empty && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#555" } }, s.wins, "W / ", s.runs, "R")), s.empty ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", fontStyle: "italic" } }, "Empty") : /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#666", marginTop: 2 } }, "\u{1F3E0}", s.cats, " cats \xB7 \u2726", s.dust, " \xB7 ", s.heat > 0 ? `\u{1F525}H${s.heat}` : ""));
    ...
    }, style: { fontSize: 10, background: "#fb923c", border: "none", borderRadius: 4, color: "#0a0a1a", cursor: "pointer", padding: "3px 10px", marginTop: 3, fontWeight: 700 } }, "Import to Slot ", activeSlot))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", zIndex: 1, gap: 12, padding: 20, textAlign: "center", maxWidth: 600 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 15, color: "#ffffffbb", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 360, lineHeight: 1.7, animation: "fadeIn 2s ease-out", textShadow: "0 0 20px #ffffff08" } }, getEpigraph(meta)), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 1, alignItems: "flex-end", justifyContent: "center", height: meta && meta.cats.length > 0 ? 28 : 16, marginTop: 8, marginBottom: -4 } }, meta && meta.cats.length > 0 ? meta.cats.slice(0, 20).map((c, i) => {
      ...
      opacity: 0.3
    } }))), meta && meta.cats.length >= 1 && meta.cats.length <= 4 && meta.stats.w <= 2 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fbbf2455", fontFamily: "'Manrope',sans-serif", fontStyle: "italic", letterSpacing: 2, animation: "fadeIn 3s ease-out" } }, meta.cats.length === 1 ? `${meta.cats[0].name.split(" ")[0]} watches from the warm side.` : meta.cats.length === 2 ? `${meta.cats[0].name.split(" ")[0]} and ${meta.cats[1].name.split(" ")[0]}. The Founders.` : `The Hearth burns. ${meta.cats.slice(0, 3).map((c) => c.name.split(" ")[0]).join(", ")} watch.`), /* @__PURE__ */ React.createElement("h1", { style: { fontSize: "clamp(36px,8vw,58px)", fontWeight: 700, letterSpacing: "clamp(8px,2vw,18px)", lineHeight: 1, background: (meta?.achv || []).includes("completionist") ? "linear-gradient(135deg,#c084fc,#fef08a,#4ade80,#67e8f9)" : "linear-gradient(180deg,#fef08a,#fbbf24,#b85c2c)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", margin: 0, fontFamily: "'Manrope',sans-serif", textTransform: "uppercase", filter: "drop-shadow(0 0 30px #fbbf2418)" } }, "NINTH LIFE", meta?.ninthDawnCleared ? " \u{1F305}" : ""), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888888aa", letterSpacing: 4, fontFamily: "'Manrope',sans-serif", fontWeight: 300, lineHeight: 1.6, animation: "fadeIn 2.5s ease-out", textTransform: "uppercase" } }, "Cats are cards \xB7 Seasons are suits \xB7 Survive the dark"), (() => {
      const ch = getChapterTitle(meta);
      return ch ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#c084fc66", letterSpacing: 4, fontFamily: "'Manrope',sans-serif", animation: "fadeIn 3s ease-out" } }, "Chapter ", ch.num, " \xB7 ", ch.name) : null;
    })(), meta && meta.stats.r >= 1 && (() => {
      const earned = (meta.achv || []).length;
      const total = ACHIEVEMENTS.length;
      return /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 3, animation: "fadeIn 2s ease-out" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", letterSpacing: 2, fontFamily: "'Manrope',sans-serif" } }, "ACHIEVEMENTS ", earned, "/", total), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 3, flexWrap: "wrap", justifyContent: "center", maxWidth: 200 } }, ACHIEVEMENTS.map((a) => {
        const e = (meta.achv || []).includes(a.id);
        return /* @__PURE__ */ React.createElement("div", { key: a.id, onClick: () => toast(a.icon, `${a.name}: ${a.desc}${e ? " \u2713 " + a.reward : ""}`, e ? "#fbbf24" : "#555"), style: {
          width: 8,
          height: 8,
          borderRadius: "50%",
          cursor: "pointer",
    ...
    })(), hasRun && meta && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 2 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 12, alignItems: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { color: "#c084fc", fontSize: 13, fontWeight: 700 } }, "\u2726 ", sd, " Stardust"), sd > 0 && !showUpgrades && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#c084fc66", fontFamily: "'Manrope',sans-serif" } }, "earned from the Hearth"), sd > 0 && showUpgrades && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#c084fc66", fontFamily: "'Manrope',sans-serif", cursor: "pointer" }, onClick: () => setTab("\u2726 upgrades") }, "spend on upgrades \u25B8"), meta.cats.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#c084fcbb", fontFamily: "'Manrope',sans-serif" } }, "\u{1F3E0} +", calcTotalHearthDust(meta.cats, getMB().dustBonus || 0, getHeatFx(meta?.heat).dustMult || 1).total, "/run")), showHeat && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 4 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 2, alignItems: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#ef4444", fontFamily: "'Manrope',sans-serif" } }, "Heat ", meta.heat), meta.heat > 0 && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#4ade80", fontFamily: "'Manrope',sans-serif" } }, "+", meta.heat * 25, "% hearth")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 3, alignItems: "center" } }, Array.from({ length: (meta.stats.mh || meta.heat || 1) + 1 }).map((_, h) => {
    ...
    })), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ef444466", fontFamily: "'Manrope',sans-serif" } }, meta.heat === 0 ? "Standard difficulty." : "Tap a number to change."))), availTabs.length > 1 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 3, background: "#ffffff08", borderRadius: 8, padding: 2 } }, availTabs.map((t) => /* @__PURE__ */ React.createElement("button", { key: t, onClick: () => setTab(t), style: { padding: "5px 14px", fontSize: 10, border: "none", borderRadius: 6, cursor: "pointer", background: safeTab === t ? "#ffffff12" : "transparent", color: safeTab === t ? "#e8e6e3" : "#666", fontFamily: "'Manrope',sans-serif", letterSpacing: 1, textTransform: "uppercase", fontWeight: safeTab === t ? 700 : 400 } }, t))), safeTab === "play" && /* @__PURE__ */ React.createElement(React.Fragment, null, false, savedRun && /* @__PURE__ */ React.createElement("button", { onClick: () => {
      Audio.init();
      resumeRun(savedRun);
    }, style: { ...BTN("linear-gradient(135deg,#4ade80,#22c55e)", "#0a0a1a"), padding: "14px 48px", fontSize: 18, letterSpacing: 3, textTransform: "uppercase", boxShadow: "0 0 30px #4ade8044", animation: "float 3s ease-in-out infinite", marginBottom: 2 } }, "Continue Colony ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, opacity: 0.7 } }, "Night ", savedRun.ante)), /* @__PURE__ */ React.createElement("button", { onClick: () => {
      try {
        Audio.init();
        if (!meta?.stats?.r) {
          setGuide({ step: 0, msg: "" });
    ...
    }, style: { ...BTN(savedRun ? "#1a1a2e" : "linear-gradient(135deg,#fbbf24,#f59e0b)", savedRun ? "#fbbf24" : "#0a0a1a"), padding: savedRun ? "10px 32px" : "14px 48px", fontSize: savedRun ? 13 : 18, letterSpacing: savedRun ? 2 : 3, textTransform: "uppercase", boxShadow: savedRun ? "none" : "0 0 30px #fbbf2444", border: savedRun ? "1px solid #fbbf2444" : "none" } }, savedRun ? "New Colony" : "Enter the Night"), meta && canUnlockNinthDawn(meta) && !meta.ninthDawnCleared && /* @__PURE__ */ React.createElement("button", { onClick: () => {
      Audio.init();
      startNinthDawn();
    }, style: { ...BTN("linear-gradient(135deg,#fbbf24,#fef08a)", "#0a0a1a"), padding: "10px 36px", fontSize: 13, letterSpacing: 4, textTransform: "uppercase", boxShadow: "0 0 20px #fbbf2444", animation: "float 3s ease-in-out infinite" } }, "\u300C THE NINTH DAWN \u300D"), meta?.ninthDawnCleared && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf2466", fontFamily: "'Manrope',sans-serif", fontStyle: "italic" } }, "\u{1F305} The dawn holds."), showLongDark && !isFirstRun && /* @__PURE__ */ React.createElement("button", { onClick: () => setLongDark((v) => !v), style: { padding: "6px 16px", fontSize: 10, border: `1px solid ${longDark ? "#818cf8" : "#ffffff22"}`, borderRadius: 6, cursor: "pointer", background: longDark ? "#818cf822" : "transparent", color: longDark ? "#818cf8" : "#666", fontFamily: "'Manrope',sans-serif", letterSpacing: 1, marginTop: 2 } }, longDark ? "\u{1F311} THE LONG DARK \u2014 9 nights" : "\u{1F311} The Long Dark"), longDark && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#818cf8aa", fontStyle: "italic", fontFamily: "'Manrope',sans-serif" } }, "Nine nights. Nine colonies. The full weight of the dark."), showHeat && (meta.heat || 0) > 0 && HEAT_FLAVOR[meta.heat] && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#ef4444bb", fontStyle: "italic", fontFamily: "'Manrope',sans-serif" } }, HEAT_FLAVOR[meta.heat]), (meta?.relics || []).length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", justifyContent: "center" } }, (meta.relics || []).sort().map((h) => {
      const r = HEAT_RELICS[h];
      return r ? /* @__PURE__ */ React.createElement("div", { key: h, style: { padding: "3px 8px", borderRadius: 5, background: "#fbbf2408", border: "1px solid #fbbf2422", fontSize: 10, color: "#fbbf24", fontFamily: "'Manrope',sans-serif" }, title: `${r.name}: ${r.desc}
"${r.flavor}"` }, r.icon, " ", r.name) : null;
    })), (!meta || meta.stats.r === 0) && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf2466", fontFamily: "'Manrope',sans-serif", lineHeight: 1.5, textAlign: "center", maxWidth: 300 } }, "3 nights. 18 cats. The game teaches as you play."), meta && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 14, fontFamily: "'Manrope',sans-serif", fontSize: 10, color: "#555", alignItems: "center", flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement("button", { onClick: () => setSeen((s) => ({ ...s, howToPlay: !s.howToPlay })), style: { background: "none", border: "1px solid #ffffff12", borderRadius: 12, color: "#555", fontSize: 10, cursor: "pointer", padding: "2px 8px", fontFamily: "'Manrope',sans-serif" } }, "How to Play"), /* @__PURE__ */ React.createElement("span", null, meta.stats.r, " runs"), /* @__PURE__ */ React.createElement("span", null, meta.stats.w, " wins")), seen.howToPlay && /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 16px", borderRadius: 10, background: "#ffffff06", border: "1px solid #ffffff0a", maxWidth: 400, fontSize: 13, fontFamily: "'Manrope',sans-serif", color: "#aaa", lineHeight: 1.6, animation: "fadeIn .4s ease-out", textAlign: "left" } }, /* @__PURE__ */ React.createElement("div", { style: { fontWeight: 700, color: "#fbbf24", marginBottom: 4 } }, "Quick Rules"), "Draw 6 cats. Pick up to 5 \u2014 match 3+ of one season for Clowder or Colony. Discard to swap cards (free). Recruit to draw extra cats (costs \u{1F41F}). Unplayed cats give bench bonuses. Chips \xD7 Mult = score. Beat the target.", /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6 } }, "Scars (\xD71.25) and Bonds (\xD71.5) multiply your score. Nerve builds every blind you clear \u2014 fast boss clears give more. In the Den, shelter a \u2642+\u2640 pair to breed. Everyone else enters the wilds."), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6, color: "#67e8f9" } }, "\u{1F3AF} Match seasons for Clowder or Colony hands. Stack traits for big multipliers. Unplayed cats in your hand give bench bonuses."), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6, color: "#34d399" } }, "\u{1F46A} Shelter a parent with their child to teach traits. Save a M/F pair to the Hearth \u2014 their descendants begin your next colony.")), meta && (mb.gold > 0 || mb.hands > 0 || mb.discards > 0 || mb.fervor > 0) && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "center", fontFamily: "'Manrope',sans-serif", fontSize: 10 } }, mb.gold > 0 && /* @__PURE__ */ React.createElement("span", { style: { color: "#fbbf24" } }, "+", mb.gold, "\u{1F41F}"), mb.hands > 0 && /* @__PURE__ */ React.createElement("span", { style: { color: "#3b82f6" } }, "+", mb.hands, "H"), mb.discards > 0 && /* @__PURE__ */ React.createElement("span", { style: { color: "#ef4444" } }, "+", mb.discards, "D"), mb.fervor > 0 && /* @__PURE__ */ React.createElement("span", { style: { color: "#d97706" } }, "N+", mb.fervor))), safeTab === "\u2726 upgrades" && meta && /* @__PURE__ */ React.createElement("div", { style: { width: "100%", display: "flex", flexDirection: "column", gap: 6 } }, (() => {
      ...
      return tiers.map((t) => {
        ...
        return /* @__PURE__ */ React.createElement("details", { key: t, open: defaultOpen }, /* @__PURE__ */ React.createElement("summary", { style: { fontSize: 11, color: unlocked ? tierColor2[t] : tierColor2[t] + "66", letterSpacing: 2, fontWeight: 700, marginTop: t > 1 ? 8 : 0, marginBottom: 4, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", listStyle: "none" } }, /* @__PURE__ */ React.createElement("span", null, unlocked ? "" : "\u{1F512} ", tierName[t].toUpperCase(), " ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, fontWeight: 400, color: "#555" } }, "(", tierUps.filter((u) => (meta.ups[u.id] || 0) >= u.max).length, "/", tierUps.length, ")")), !unlocked ? /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#555", fontWeight: 400 } }, "Unlocks at ", tierReq[t], " upgrades (", totalLevels, "/", tierReq[t], ")") : allMaxed ? /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#4ade80", fontWeight: 400 } }, "\u2713 COMPLETE") : /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#555" } }, "\u25BE")), tierUps.map((u) => {
          const o = meta.ups[u.id] || 0, mx = o >= u.max, can = unlocked && meta.dust >= u.cost && !mx;
          return /* @__PURE__ */ React.createElement("div", { key: u.id, onClick: () => can && buyUpg(u), style: { display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderRadius: 8, background: mx ? "#4ade8008" : !unlocked ? "#ffffff02" : "#ffffff04", border: `1px solid ${mx ? "#4ade8033" : can ? tierColor2[t] + "44" : "#ffffff0a"}`, cursor: can ? "pointer" : "default", opacity: !unlocked ? 0.35 : can || mx ? 1 : 0.5 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 18 } }, u.icon), /* @__PURE__ */ React.createElement("div", { style: { flex: 1, textAlign: "left" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: mx ? "#4ade80" : tierColor2[t] } }, u.name, o > 0 ? ` (${o}/${u.max})` : ""), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888", fontFamily: "'Manrope',sans-serif" } }, u.desc)), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: mx ? "#4ade80" : "#c084fc", fontWeight: 700 } }, mx ? "MAX" : `\u2726${u.cost}`));
        }));
      });
      ...
      return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", padding: "8px 0" } }, (() => {
        ...
        return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: tier.color + "88", letterSpacing: 3, fontFamily: "'Manrope',sans-serif", marginBottom: 4 } }, tier.icon, " ", tier.name, tier.bonus ? ` \xB7 ${tier.bonus}` : "");
      })(), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#c084fcbb", letterSpacing: 3, fontFamily: "'Manrope',sans-serif" } }, "STARDUST PER RUN"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 24, fontWeight: 900, color: "#c084fc", textShadow: "0 0 20px #c084fc44" } }, "+", hd.total, "\u2726"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#c084fc66", fontFamily: "'Manrope',sans-serif", lineHeight: 1.6 } }, (dustBonus > 0 || heatMult > 1 || hd.maintenance > 0) && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("span", null, "Gross ", hd.gross, "\u2726"), dustBonus > 0 && /* @__PURE__ */ React.createElement("span", null, " (\xD7", 1 + dustBonus, " bonus)"), heatMult > 1 && /* @__PURE__ */ React.createElement("span", null, " (\xD7", heatMult, " heat)"), hd.maintenance > 0 && /* @__PURE__ */ React.createElement("span", { style: { color: "#ef4444bb" } }, " \u2212 ", hd.maintenance, "\u2726 upkeep (", hd.activeCats - 8, " above 8)"))), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#666", fontFamily: "'Manrope',sans-serif" } }, "Each saved cat radiates stardust when you enter the night")), activeCats.length > 0 && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888", letterSpacing: 2, fontFamily: "'Manrope',sans-serif" } }, "HEARTH (", activeCats.length, " cats", hd.maintenance > 0 ? `. ${hd.maintenance}\u2726 upkeep` : "", ")"), (() => {
        const whisperCat = activeCats[Math.floor(Date.now() / 8e3) % activeCats.length];
        ...
        return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf2444", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", lineHeight: 1.5, animation: "fadeIn 2s ease-out", marginBottom: 4 } }, /* @__PURE__ */ React.createElement("span", { style: { color: BREEDS[whisperCat.breed]?.color || "#888", fontWeight: 600 } }, fn, ":"), " ", templates[Math.floor(Date.now() / 8e3) % templates.length]);
        ...
        return /* @__PURE__ */ React.createElement("div", { key: `h${i}`, style: { textAlign: "center", position: "relative" }, onClick: () => {
          if (c.story) toast(BREEDS[c.breed]?.icon || "\u{1F431}", c.story, BREEDS[c.breed]?.color || "#fbbf24");
        } }, /* @__PURE__ */ React.createElement(CC, { cat: { ...c, id: `ht${i}`, trait: tr }, sm: true }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: "#c084fc", fontFamily: "'Manrope',sans-serif", marginTop: 1, textShadow: "0 0 6px #c084fc44" } }, "+", perCat, "\u2726"), meta.cats.length >= 20 && /* @__PURE__ */ React.createElement("button", { onClick: () => {
          const u = { ...meta, cats: meta.cats.map((x) => x.name === c.name && x.savedAt === c.savedAt ? { ...x, enshrined: true } : x) };
          setMeta(u);
          saveS(u);
          toast("\u{1F31F}", `${c.name.split(" ")[0]} enshrined. Their light is permanent now.`, "#fbbf24");
        }, style: { fontSize: 10, color: "#fbbf24bb", background: "transparent", border: "1px solid #fbbf2422", borderRadius: 3, padding: "1px 4px", cursor: "pointer", marginTop: 1 } }, "Enshrine"));
      }))), enshrinedCats.length > 0 && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fbbf24bb", letterSpacing: 2, fontFamily: "'Manrope',sans-serif", marginTop: 4 } }, "\u{1F31F} ENSHRINED (", enshrinedCats.length, "). eternal, no upkeep"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", justifyContent: "center" } }, enshrinedCats.map((c, i) => {
        const tr = TRAITS.find((t) => t.name === c.trait.name) || c.trait;
        return /* @__PURE__ */ React.createElement("div", { key: `en${i}`, style: { textAlign: "center", position: "relative" } }, /* @__PURE__ */ React.createElement("div", { style: { border: "2px solid #fbbf2444", borderRadius: 8, padding: 1 } }, /* @__PURE__ */ React.createElement(CC, { cat: { ...c, id: `en${i}`, trait: tr }, sm: true })), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fbbf2466", fontFamily: "'Manrope',sans-serif" } }, "\u{1F31F}"));
      }))));
    })() : /* @__PURE__ */ React.createElement("div", { style: { color: "#555", fontSize: 11, fontFamily: "'Manrope',sans-serif", textAlign: "center", padding: "20px 0" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 28, opacity: 0.3, marginBottom: 8 } }, "\u{1F3E0}"), "No survivors yet. Win a run and save a cat to the Hearth.", /* @__PURE__ */ React.createElement("br", null), /* @__PURE__ */ React.createElement("span", { style: { color: "#c084fcbb", fontSize: 10 } }, "Each saved cat will generate \u2726 Stardust at the start of every run.")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, justifyContent: "center" } }, BK.map((b) => {
    ...
    }), BK.every((b) => (meta.stats.disc || []).some((d) => d.startsWith(b))) && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#4ade80", fontFamily: "'Manrope',sans-serif", fontWeight: 700 } }, "\u2713 All seasons!")), (meta.achv || []).length > 0 && /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888", letterSpacing: 2, marginBottom: 3 } }, "ACHIEVEMENTS (", (meta.achv || []).length, "/", ACHIEVEMENTS.length, ")"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", justifyContent: "center" } }, ACHIEVEMENTS.filter((a) => (meta.achv || []).includes(a.id)).map((a) => /* @__PURE__ */ React.createElement("span", { key: a.id, onClick: () => toast(a.icon, `${a.name}: ${a.reward}`, "#fbbf24"), style: { fontSize: 10, padding: "3px 8px", borderRadius: 5, background: "#fbbf2411", border: "1px solid #fbbf2422", color: "#fbbf24", fontFamily: "'Manrope',sans-serif", cursor: "pointer" }, title: `${a.desc} \u2192 ${a.reward}` }, a.icon, " ", a.name)))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", justifyContent: "center", opacity: 0.4 } }, ACHIEVEMENTS.filter((a) => !(meta.achv || []).includes(a.id)).map((a) => /* @__PURE__ */ React.createElement("span", { key: a.id, onClick: () => toast(a.icon, `${a.name}: ${a.desc}`, "#555"), style: { fontSize: 10, padding: "2px 6px", borderRadius: 4, background: "#ffffff04", border: "1px solid #ffffff08", color: "#555", fontFamily: "'Manrope',sans-serif", cursor: "pointer" }, title: a.desc }, a.icon, " ", a.name))), meta.ninthDawnCleared && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, padding: "12px 16px", borderRadius: 10, background: "linear-gradient(135deg,#fbbf2408,#7a665208)", border: "1px solid #fbbf2422" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#fbbf24", letterSpacing: 4, textAlign: "center", fontWeight: 700 } }, "\u{1F305} THE RECORD"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fbbf2466", fontFamily: "'Manrope',sans-serif", textAlign: "center", marginTop: 3, marginBottom: 8 } }, "Every cat who ever carried the colony's name."), /* @__PURE__ */ React.createElement("div", { style: { position: "relative", width: "100%", maxWidth: 300, height: 300, margin: "0 auto", marginBottom: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", inset: 0, borderRadius: "50%", border: "1px solid #ffffff06" } }), /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", inset: "15%", borderRadius: "50%", border: "1px solid #ffffff04" } }), meta.cats.map((c, i) => {
...
Saved from Night ${c.fromAnte || "?"}`, style: {
    ...
    }), /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", textAlign: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 24, opacity: 0.5 } }, "\u{1F525}"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fbbf2466", fontFamily: "'Manrope',sans-serif" } }, meta.cats.length, " saved"))), (() => {
      ...
      return /* @__PURE__ */ React.createElement("span", { key: i, title: `${fullN} \u2014 ${c.breed}, P${c.power}, ${c.trait?.name || "Plain"}${c.story?.length ? "\n" + c.story.join(", ") : ""}`, style: { fontSize: 10, color: BREEDS[c.breed]?.color || "#888", fontFamily: "'Manrope',sans-serif", padding: "2px 6px", borderRadius: 3, background: c.enshrined ? "#fbbf2411" : "#ffffff06", border: c.enshrined ? "1px solid #fbbf2433" : hasPair ? "1px solid #34d39933" : "none", cursor: "help" } }, c.enshrined ? "\u{1F31F} " : "", hasPair ? "\u{1F46A} " : "", fullN, " ", c.sex === "M" ? "\u2642" : "\u2640");
    })), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#77777799", fontFamily: "'Manrope',sans-serif", textAlign: "center", marginTop: 8, fontStyle: "italic" } }, "Nine colonies. One survived. Not because it was the strongest. Because someone remembered all the rest.")))));
  }
  if (anteUp) {
    return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", zIndex: 1, gap: 16, padding: 20 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "#ffffffbb", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 300, lineHeight: 1.6, animation: "fadeIn 1.5s ease-out", letterSpacing: 1 } }, NIGHT_EPI[Math.min(anteUp.to - 1, 4)]), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 72, fontWeight: 900, background: "linear-gradient(135deg,#f59e0b,#fef08a)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", animation: "scorePop .6s ease-out", letterSpacing: 8, marginTop: 8 } }, anteUp.to), /* @__PURE__ */ React.createElement(ProgressMap, { ante: anteUp.to, blind: 0, mx: MX }), /* @__PURE__ */ React.createElement("div", { style: { fontFamily: "'Manrope',sans-serif", fontSize: 10, color: "#666", marginTop: 4 } }, "Threshold: ", /* @__PURE__ */ React.createElement("span", { style: { color: "#e8e6e3", fontWeight: 700 } }, anteUp.target.toLocaleString())), ANTE_ESCALATION[Math.min(anteUp.to - 1, 4)] && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#fbbf2466", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", animation: "fadeIn 1.4s ease-out", textAlign: "center", maxWidth: 320, lineHeight: 1.6, textShadow: "0 0 15px #fbbf2422" } }, ANTE_ESCALATION[Math.min(anteUp.to - 1, 4)]), anteUp.to > 1 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#d97706bb", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", animation: "fadeIn 1.2s ease-out", textAlign: "center" } }, "\u26A1 The night deepens. The colony holds."), runLog.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { maxWidth: 350, width: "100%" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", letterSpacing: 2, marginBottom: 4 } }, "LAST NIGHT"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 2, maxHeight: 120, overflowY: "auto" } }, runLog.filter((e) => e.ante === anteUp.from).slice(-6).map((e, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { fontSize: 10, fontFamily: "'Manrope',sans-serif", color: e.type === "death" ? "#ef4444" : e.type === "breed" ? "#4ade80" : e.type === "hand" ? "#fbbf24" : "#666", padding: "1px 4px" } }, e.type === "draft" && /* @__PURE__ */ React.createElement("span", null, "Drafted: ", e.data.picked), e.type === "hand" && /* @__PURE__ */ React.createElement("span", null, e.data.type, ": ", e.data.score.toLocaleString()), e.type === "breed" && /* @__PURE__ */ React.createElement("span", null, e.data.baby, " born (", e.data.breed, ")"), e.type === "fight" && /* @__PURE__ */ React.createElement("span", null, e.data.loser, " scarred (-", e.data.dmg, "P)"), e.type === "death" && /* @__PURE__ */ React.createElement("span", null, e.data.victim, " died"), e.type === "night" && /* @__PURE__ */ React.createElement("span", null, "Night ", e.data.to, " begins"), e.type === "phoenix" && /* @__PURE__ */ React.createElement("span", null, e.data.risen, " rose from the ashes!"), e.type === "mentor" && /* @__PURE__ */ React.createElement("span", null, e.data.elder, " mentored ", e.data.young), e.type === "found" && /* @__PURE__ */ React.createElement("span", null, e.data.cat, " found rations"), e.type === "growth" && /* @__PURE__ */ React.createElement("span", null, e.data.cat, " grew stronger"), e.type === "wanderer" && /* @__PURE__ */ React.createElement("span", null, e.data.cat, " joined"), e.type === "reward" && /* @__PURE__ */ React.createElement("span", null, "Reward: ", e.data.name), e.type === "event" && /* @__PURE__ */ React.createElement("span", null, e.data.title, ": ", e.data.choice))))), /* @__PURE__ */ React.createElement("button", { onClick: () => {
      setAnteUp(null);
  ...
  if (ph === "bossIntro") {
    ...
    return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", zIndex: 1, gap: vw < 500 ? 10 : 20, padding: vw < 500 ? 16 : 20, maxWidth: 500 } }, BOSS_PORTRAITS[b.id] ? /* @__PURE__ */ React.createElement("div", { style: { width: vw < 500 ? 120 : 200, height: vw < 500 ? 168 : 280, borderRadius: 14, overflow: "hidden", animation: "fadeIn 1.2s ease-out", boxShadow: "0 0 60px #ef444433, 0 0 120px #ef444411", border: "1px solid #ef444422" } }, /* @__PURE__ */ React.createElement("img", { src: BOSS_PORTRAIT_BASE + BOSS_PORTRAITS[b.id], alt: b.name, style: { width: "100%", height: "100%", objectFit: "cover" }, onError: (e) => {
      e.target.style.display = "none";
      e.target.nextSibling.style.display = "flex";
    } }), /* @__PURE__ */ React.createElement("div", { style: { display: "none", width: "100%", height: "100%", alignItems: "center", justifyContent: "center", fontSize: 72, background: "#0d1117" } }, b.icon)) : /* @__PURE__ */ React.createElement("div", { style: { fontSize: vw < 500 ? 48 : 72, filter: "drop-shadow(0 0 30px #ef444488)", animation: "fadeIn 1.2s ease-out" } }, b.icon), /* @__PURE__ */ React.createElement("h2", { style: { fontSize: 30, color: "#ef4444", letterSpacing: 8, margin: 0, textShadow: "0 0 40px #ef444488", animation: "tierReveal 1s ease-out", fontFamily: "'Manrope',sans-serif" } }, b.name), hasMastery && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf24", letterSpacing: 4, fontFamily: "'Manrope',sans-serif", fontWeight: 700, animation: "fadeIn 1.2s ease-out" } }, "YOU ARE THE ", mastery.title), (bWins > 0 || bLoss > 0) && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ef444466", fontFamily: "'Manrope',sans-serif", letterSpacing: 2, animation: "fadeIn 1.5s ease-out" } }, bWins, "W / ", bLoss, "L"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 17, color: "#ef4444", fontFamily: "'Manrope',sans-serif", fontStyle: "italic", textAlign: "center", opacity: 0.8, animation: "fadeIn 1.5s ease-out", lineHeight: 1.6, maxWidth: 360, textShadow: "0 0 20px #ef444422" } }, '"', b.taunt, '"'), careerTaunt && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "#ef4444aa", fontFamily: "'Manrope',sans-serif", fontStyle: "italic", textAlign: "center", animation: "fadeIn 2s ease-out", lineHeight: 1.5, maxWidth: 340 } }, '"', careerTaunt, '"'), dynamicTaunt && !careerTaunt && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: "#ef4444bb", fontFamily: "'Manrope',sans-serif", fontStyle: "italic", textAlign: "center", animation: "fadeIn 2s ease-out" } }, '"', dynamicTaunt, '"'), bossTraits.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "center", animation: "fadeIn 1.8s ease-out", maxWidth: 400 } }, bossTraits.map((t, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { padding: "8px 14px", borderRadius: 8, background: "#ef444415", border: "1px solid #ef444433", textAlign: "center", minWidth: 120, maxWidth: 180 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "center", gap: 4 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 16 } }, t.icon), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: "#ef4444", fontWeight: 700, letterSpacing: 1 } }, t.name)), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "#ef4444cc", fontFamily: "'Manrope',sans-serif", marginTop: 3, lineHeight: 1.3 } }, t.desc), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ef4444bb", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", marginTop: 2 } }, t.flavor)))), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#ef4444bb", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", letterSpacing: 2, animation: "fadeIn 2.5s ease-out" } }, b.lore), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "center", marginTop: 4 } }, curses.map((c, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { padding: "6px 10px", borderRadius: 8, background: "#ef444408", border: "1px solid #ef444433", textAlign: "center", minWidth: 90 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 16 } }, c.icon), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ef4444", fontWeight: 600, marginTop: 1 } }, c.name), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#ef4444bb", fontFamily: "'Manrope',sans-serif", marginTop: 1 } }, c.desc)))), cfx.exileBreed && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#ef4444bb", fontFamily: "'Manrope',sans-serif" } }, BREEDS[cfx.exileBreed].icon, " ", cfx.exileBreed, " exiled"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#666", fontFamily: "'Manrope',sans-serif", marginTop: 8 } }, "Threshold: ", /* @__PURE__ */ React.createElement("span", { style: { color: "#ef4444", fontWeight: 700, fontSize: 16 } }, eTgt().toLocaleString()), (meta?.heat || 0) > 0 && /* @__PURE__ */ React.createElement("span", { style: { color: "#ef4444bb", fontSize: 10 } }, " (Heat +", (meta.heat || 0) * 10, "%)")), hasRelic(3) && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#4ade8066", fontFamily: "'Manrope',sans-serif", fontStyle: "italic", textAlign: "center", maxWidth: 300, lineHeight: 1.5, animation: "fadeIn 2.5s ease-out", padding: "4px 12px", borderRadius: 6, background: "#4ade8008", border: "1px solid #4ade8018" } }, "\u{1F441}\uFE0F The Vigil whispers: ", b.id === "hunger" ? "Bonds score double here. Fill every hand." : b.id === "territory" ? "Scars make you stronger. The Territory respects fighters." : b.id === "mother" ? "Don't spread thin. Pick your best five and commit." : b.id === "swarm" ? "Nerve is everything. Build it before you get here." : b.id === "forgetting" ? "Every name matters. Play your bonded pairs." : b.id === "fraying" ? "Resolve your grudges before you get here. Every grudge is \u22122 mult." : b.id === "eclipse" ? "Don't rest. Momentum carries through." : b.id === "ember" ? "Give everything. One more hand is all it takes." : "Trust the colony."), /* @__PURE__ */ React.createElement("button", { onClick: () => {
      if (bossTraits.some((bt) => bt.fx.marked)) {
        ...
        if (eligible.length > 0) {
          ...
          toast("\u{1FA9E}", `${victim.name.split(" ")[0]} was scarred before the fight began.`, "#ef4444", 2e3);
        }
      }
      setPh("playing");
  ...
  if (ph === "event" && colEvent) {
    ...
    return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", top: "12%", left: "50%", transform: "translateX(-50%)", width: 350, height: 350, borderRadius: "50%", background: `radial-gradient(circle,${evtGlow}06,transparent 70%)`, pointerEvents: "none", zIndex: 0 } }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", zIndex: 1, gap: 0, padding: 20, maxWidth: 480 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff12", letterSpacing: 8, fontFamily: "'Manrope',sans-serif", marginBottom: 16, animation: "fadeIn 1.2s ease-out", textTransform: "uppercase" } }, "Night ", ante, " ", "\xB7", " ", ["Dusk", "Midnight", "After the Boss"][blind]), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 64, marginBottom: 8, animation: "fadeIn .6s ease-out", filter: `drop-shadow(0 0 40px ${evtGlow}22)` } }, evt.icon), /* @__PURE__ */ React.createElement("h2", { style: { fontSize: 20, color: "#e8e6e3", letterSpacing: 6, margin: "0 0 6px 0", fontWeight: 600, animation: "fadeIn .8s ease-out", fontFamily: "'Manrope',sans-serif" } }, evt.title), evt.tag && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: evtGlow + "66", fontFamily: "'Manrope',sans-serif", letterSpacing: 4, padding: "3px 12px", borderRadius: 12, border: `1px solid ${evtGlow}15`, marginBottom: 16, animation: "fadeIn 1s ease-out", textTransform: "uppercase" } }, evt.tag), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 15, color: "#c8c2bbdd", fontFamily: "'Manrope',sans-serif", textAlign: "center", lineHeight: 2.1, maxWidth: 360, fontStyle: "italic", animation: "fadeIn 1.2s ease-out", marginBottom: 16, padding: "0 8px" } }, evtText), meta && meta.cats.length > 0 && tgts.length > 0 && (() => {
      ...
      return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#c084fc44", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 300, lineHeight: 1.5, animation: "fadeIn 2s ease-out", marginBottom: 8 } }, cpk(flavorCache, "bossHearth", templates));
    })(), tgts.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 14, justifyContent: "center", marginBottom: 20, animation: "fadeIn 1s ease-out 0.3s both" } }, tgts.map((t, i) => /* @__PURE__ */ React.createElement("div", { key: t.id, style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 4 } }, /* @__PURE__ */ React.createElement(CC, { cat: t }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: BREEDS[t.breed]?.color || "#888", fontFamily: "'Manrope',sans-serif", letterSpacing: 1, fontWeight: 700 } }, t.name.split(" ")[0]), t.trait && t.trait.name !== "Plain" && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff44", fontFamily: "'Manrope',sans-serif" } }, t.trait.icon, " ", t.trait.name)))), /* @__PURE__ */ React.createElement("div", { style: { width: 60, height: 1, background: `linear-gradient(90deg,transparent,${evtGlow}22,transparent)`, marginBottom: 16 } }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 10, width: "100%", maxWidth: vw < 500 ? vw - 32 : 380 } }, evt.choices.map((ch, i) => {
      const canAfford = !ch.fx.gold || ch.fx.gold >= 0 || gold >= Math.abs(ch.fx.gold);
      const label = ch.labelFn ? ch.labelFn(tgts) : ch.label;
      const hint = (() => {
        const fx = ch.fx;
        const hints = [];
        if (fx.gold && fx.gold < 0) hints.push({ t: "Costly", c: "#fbbf24", ic: "\u{1F41F}" });
      ...
      } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 15, color: canAfford ? "#e8e6e3" : "#555", fontWeight: 500, letterSpacing: 1, lineHeight: 1.4 } }, label), hint && canAfford && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: hint.c, fontFamily: "'Manrope',sans-serif", letterSpacing: 1, flexShrink: 0, padding: "3px 8px", borderRadius: 6, background: hint.c + "11", border: `1px solid ${hint.c}22`, display: "flex", alignItems: "center", gap: 3 } }, hint.ic, " ", hint.t)), !canAfford && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#ef4444bb", fontFamily: "'Manrope',sans-serif", display: "block", marginTop: 4 } }, "Need ", Math.abs(ch.fx.gold || 0), "\u{1F41F}"));
      ...
      return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#ffffff55", fontFamily: "'Manrope',sans-serif" } }, allCount, " cats"), scarCount > 0 && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#fb923c33", fontFamily: "'Manrope',sans-serif" } }, scarCount, " scarred"), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#fbbf24", fontFamily: "'Manrope',sans-serif", fontWeight: 700 } }, gold, "\u{1F41F}"));
    })())));
  }
  ...
  if (ph === "overflow" && oData) {
    ...
    return /* @__PURE__ */ React.createElement("div", { style: { ...W, animation: clutch ? "flash .6s ease-out" : "none" } }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), clutch && /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", inset: 0, zIndex: 50, background: "radial-gradient(circle,#fbbf2433,transparent 70%)", pointerEvents: "none", animation: "flash 1.5s ease-out" } }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", zIndex: 1, gap: 10, padding: 20 } }, clutch && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 22, fontWeight: 900, color: "#fbbf24", letterSpacing: 8, textShadow: "0 0 30px #fbbf24cc", animation: "clutchBurst .8s ease-out", fontFamily: "'Manrope',sans-serif", marginBottom: 4 } }, "CLUTCH"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: blind >= 2 ? "#4ade80" : "#fbbf24", letterSpacing: 3 } }, blind >= 2 ? clutch ? "Survived. Barely." : "Survived." : "The threshold breaks."), clearLine && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "#ffffffbb", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 320, lineHeight: 1.6, animation: "fadeIn 1.2s ease-out", textShadow: "0 0 15px #ffffff11" } }, clearLine), blind >= 2 && boss && (() => {
      ...
      return /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", padding: "10px 16px", borderRadius: 10, background: "#4ade8008", border: "1px solid #4ade8022", maxWidth: 360, animation: "fadeIn 1.5s ease-out" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: "#4ade80", fontWeight: 700, fontFamily: "'Manrope',sans-serif", letterSpacing: 2, marginBottom: 4 } }, boss.name, " falls."), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: "#4ade80cc", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", lineHeight: 1.6 } }, '"', boss.defeat, '"'), dLine && dLine !== boss.defeat && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#4ade80aa", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", marginTop: 4, lineHeight: 1.5 } }, '"', dLine, '"'));
    })(), blind === 1 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "#ef4444", fontWeight: 700, letterSpacing: 3, fontFamily: "'Manrope',sans-serif", animation: "fpp 1s ease infinite" } }, "Something approaches..."), ante >= 4 && blind < 2 && (clutch || o.fs < o.tgt * 1.3) && boss && (() => {
      ...
      return earlyTaunt ? /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", animation: "fadeIn 1.5s ease-out", padding: "6px 14px", borderRadius: 8, background: "#ef444408", border: "1px solid #ef444418", maxWidth: 340 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14 } }, boss.icon), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#ef4444bb", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", lineHeight: 1.5 } }, '"', earlyTaunt, '"'), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ef444433", fontFamily: "'Manrope',sans-serif", marginTop: 2 } }, boss.name, " watches.")) : null;
    })(), (() => {
      ...
      return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: scoreSize, fontWeight: 900, color: scoreColor, textShadow: `0 0 ${glowSize}px ${scoreColor}44`, animation: shakeAnim || "fadeIn .6s ease-out", letterSpacing: pctClear >= 200 ? 3 : 1 } }, o.fs.toLocaleString()), /* @__PURE__ */ React.createElement("div", { style: { fontSize: tgtSize, color: pctClear >= 200 ? "#888" : "#666", fontFamily: "'Manrope',sans-serif" } }, "Threshold: ", o.tgt.toLocaleString()));
    })(), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 5, background: "#ffffff06", borderRadius: 12, padding: "14px 22px", border: "1px solid #ffffff0a", minWidth: 260 } }, /* @__PURE__ */ React.createElement("div", { onClick: () => toast("\u{1F4CA}", "The higher you score above the target, the more rations you earn. Survived: 2\u{1F41F}, Comfortable: 3\u{1F41F}, Crushing: 4\u{1F41F}, Dominating: 5-6\u{1F41F}, Legendary: 6\u{1F41F}. Boss rounds give +2\u{1F41F} extra.", "#fbbf24"), style: { display: "flex", justifyContent: "space-between", fontFamily: "'Manrope',sans-serif", fontSize: 11, cursor: "help" } }, /* @__PURE__ */ React.createElement("span", { style: { color: "#888" } }, "Performance"), /* @__PURE__ */ React.createElement("span", { style: { color: pctClear >= 300 ? "#c084fc" : pctClear >= 200 ? "#fbbf24" : pctClear >= 150 ? "#4ade80" : "#888", fontWeight: 700 } }, pctClear >= 300 ? "Legendary" : pctClear >= 200 ? "Dominating" : pctClear >= 150 ? "Crushing" : pctClear >= 120 ? "Comfortable" : "Survived", " (", pctClear, "%)")), o.uh > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", fontFamily: "'Manrope',sans-serif", fontSize: 11 } }, /* @__PURE__ */ React.createElement("span", { style: { color: "#888" } }, o.uh, " Unused Hand", o.uh > 1 ? "s" : ""), /* @__PURE__ */ React.createElement("span", { style: { color: "#3b82f6" } }, "saved")), /* @__PURE__ */ React.createElement("div", { style: { borderTop: "1px solid #ffffff0a", paddingTop: 4, display: "flex", justifyContent: "space-between", fontFamily: "'Manrope',sans-serif", fontSize: 11 } }, /* @__PURE__ */ React.createElement("span", { style: { color: "#888" } }, "Rations Earned"), /* @__PURE__ */ React.createElement("span", { style: { color: "#fbbf24", fontWeight: 700 } }, "+", o.gR, " \u{1F41F}")), o.interest > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", fontFamily: "'Manrope',sans-serif", fontSize: 11 } }, /* @__PURE__ */ React.createElement("span", { style: { color: "#4ade80" } }, "Stores (interest)"), /* @__PURE__ */ React.createElement("span", { style: { color: "#4ade80", fontWeight: 700 } }, "+", o.interest, " \u{1F41F}"))), (() => {
      const overflowLines = [
      ...
      return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: seed < 3 && whisperPool ? "#fbbf2444" : "#ffffff44", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 280, lineHeight: 1.5, marginTop: 8, animation: "fadeIn 1.5s ease-out" } }, line);
    })(), blind >= 2 ? (() => {
      ...
      return /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 8, marginTop: 4 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#4ade80bb", letterSpacing: 2, fontFamily: "'Manrope',sans-serif" } }, "CHOOSE YOUR REWARD"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "center" } }, choices.map((rw, i) => /* @__PURE__ */ React.createElement(
        "button",
        {
          key: rw.id || i,
        ...
        /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888", fontFamily: "'Manrope',sans-serif", marginTop: 3, lineHeight: 1.3 } }, rw.desc)
      ))));
    })() : /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, marginTop: 6, flexWrap: "wrap", justifyContent: "center" } }, /* @__PURE__ */ React.createElement("button", { onClick: () => {
      setOData(null);
      setSkipShop(false);
      genShop();
      setPh("shop");
      ...
      padding: isFirstRun && !seen.shop ? "12px 28px" : "10px 20px",
      fontSize: isFirstRun && !seen.shop ? 15 : 12,
      ...
      gap: 2,
      minWidth: 100
    } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 16 } }, "\u{1F3EA}"), /* @__PURE__ */ React.createElement("span", null, isFirstRun && !seen.shop ? "Enter the Market \u2726" : "Market"), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, fontWeight: 400, opacity: 0.6, fontFamily: "'Manrope',sans-serif" } }, "buy wards, cats, scrolls")), (seen.shop || !isFirstRun) && /* @__PURE__ */ React.createElement("button", { onClick: () => {
      const baseG = 3 + ante;
      const results = [];
      let bonusG = 0;
      ...
      if (Math.random() < 0.25) {
        ...
        if (r < 0.35) {
          ...
          const nc = gC({ trait: PLAIN, power: 3 + Math.floor(Math.random() * 3) });
        ...
        results.push({ text: pk(flavors), color: "#ffffff44", icon: "\u{1F319}" });
      }
      ...
      gap: 2,
      minWidth: 100
    } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 16 } }, "\u{1F33F}"), /* @__PURE__ */ React.createElement("span", null, "Scavenge"), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, fontWeight: 400, opacity: 0.6, fontFamily: "'Manrope',sans-serif" } }, "+", 3 + ante, "\u{1F41F}, maybe more")), (seen.shop || !isFirstRun) && /* @__PURE__ */ React.createElement("button", { onClick: () => {
      setCampMode(true);
      setDen([]);
      setOData(null);
      setPh("denSelect");
      ...
      gap: 2,
      minWidth: 100
    } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 16 } }, "\u{1F3D5}"), /* @__PURE__ */ React.createElement("span", null, "Camp"), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, fontWeight: 400, opacity: 0.6, fontFamily: "'Manrope',sans-serif" } }, "heal, +1 nerve, watch pair"))))));
  }
  if (ph === "defeat" && defeatData) {
    return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", zIndex: 1, gap: 16, padding: 20, maxWidth: 500 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 64, opacity: 0.6, animation: "fadeIn 1s ease-out", filter: "drop-shadow(0 0 30px #ef444488)" } }, "\u{1F480}"), /* @__PURE__ */ React.createElement("h2", { style: { fontSize: 28, color: "#ef4444", letterSpacing: 6, margin: 0, textShadow: "0 0 40px #ef444444", animation: "tierReveal 1s ease-out", fontFamily: "'Manrope',sans-serif" } }, "COLONY FELL"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, color: "#ef4444aa", fontFamily: "'Manrope',sans-serif", fontStyle: "italic", textAlign: "center", maxWidth: 360, lineHeight: 1.7, animation: "fadeIn 1.5s ease-out" } }, defeatData.line), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 12, fontFamily: "'Manrope',sans-serif", fontSize: 13, color: "#666", animation: "fadeIn 2s ease-out" } }, /* @__PURE__ */ React.createElement("span", null, "Night ", ante), /* @__PURE__ */ React.createElement("span", { style: { color: "#ef4444" } }, defeatData.blind)), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 2, alignItems: "baseline", animation: "fadeIn 2s ease-out" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 28, fontWeight: 900, color: "#ef4444bb" } }, defeatData.score.toLocaleString()), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: "#555", fontFamily: "'Manrope',sans-serif" } }, " / ", defeatData.target.toLocaleString(), " threshold")), fallen.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", justifyContent: "center", animation: "fadeIn 2.5s ease-out" } }, fallen.map((f, i) => /* @__PURE__ */ React.createElement("span", { key: i, style: { fontSize: 10, color: BREEDS[f.breed]?.color || "#888", fontFamily: "'Manrope',sans-serif", opacity: 0.5 } }, f.name.split(" ")[0]))), /* @__PURE__ */ React.createElement("button", { onClick: () => {
      setDefeatData(null);
      setPh("gameOver");
    }, style: { ...BTN("linear-gradient(135deg,#ef4444,#dc2626)", "#fff"), padding: "12px 40px", fontSize: 15, animation: "fadeIn 2.5s ease-out", marginTop: 8 } }, "Continue")));
  }
  if (ph === "gameOver" || ph === "victory") {
    ...
    return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", zIndex: 1, gap: 12, padding: 20, maxWidth: 550 } }, vStep === 0 && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 48, animation: "comboBurst .8s ease-out" } }, won ? isNinthDawn ? "\u{1F305}" : "\u{1F451}" : "\u{1F480}"), /* @__PURE__ */ React.createElement("h2", { style: { fontSize: won && isNinthDawn ? 48 : 32, letterSpacing: won && isNinthDawn ? 16 : 8, margin: 0, color: won ? void 0 : "#ef4444", background: won ? "linear-gradient(180deg,#fef08a,#fbbf24)" : void 0, WebkitBackgroundClip: won ? "text" : void 0, WebkitTextFillColor: won ? "transparent" : void 0, animation: "comboBurst .6s ease-out", fontFamily: "'Manrope',sans-serif", fontWeight: 700, textTransform: "uppercase", filter: won ? "drop-shadow(0 0 20px #fbbf2433)" : "none" } }, won ? isNinthDawn ? "DAWN" : "THEY MADE IT" : "THE DARK WON"), won && !isNinthDawn && (() => {
      ...
      return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf2466", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", letterSpacing: 2, animation: "fadeIn 1.5s ease-out", marginTop: 4, textAlign: "center", maxWidth: 340 } }, breath);
      ...
      return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "#fbbf24cc", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 360, lineHeight: 1.7, animation: "fadeIn 2s ease-out .5s both", padding: "10px 16px", borderRadius: 10, background: "#fbbf2406", border: "1px solid #fbbf2418" } }, hookText);
    })(), won && !isNinthDawn && meta && meta.stats.w > 1 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#fbbf2444", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", letterSpacing: 2, animation: "fadeIn 2s ease-out .5s both" } }, "The fire burns brighter. Colony #", (meta.stats.r || 0) + 1, "."), won && isNinthDawn && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: "#fbbf24aa", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 360, lineHeight: 1.7, animation: "fadeIn 2s ease-out" } }, "Nine colonies. One survived. Not because it was the strongest. Because someone remembered all the rest."), !won && (() => {
      ...
      return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#ef444466", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", letterSpacing: 2, animation: "fadeIn 1.5s ease-out", marginTop: 4 } }, breath);
    })(), won && statItems.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "center", maxWidth: 400, marginTop: 8 } }, statItems.map((st, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { textAlign: "center", padding: "6px 12px", borderRadius: 8, background: `${st.color}08`, border: `1px solid ${st.color}22`, animation: `scorePop .4s ease-out ${0.8 + i * 0.3}s both`, minWidth: 80 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: `${st.color}88`, letterSpacing: 2, fontFamily: "'Manrope',sans-serif" } }, st.label), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, fontWeight: 900, color: st.color, fontFamily: "'Manrope',sans-serif" } }, st.val)))), won && isNinthDawn && meta && meta.cats.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 2, flexWrap: "wrap", justifyContent: "center", maxWidth: 400, animation: "fadeIn 3s ease-out 1s both" } }, meta.cats.map((c, i) => /* @__PURE__ */ React.createElement("span", { key: i, style: { width: 4, height: 4, borderRadius: "50%", background: BREEDS[c.breed]?.color || "#fbbf24", boxShadow: `0 0 4px ${BREEDS[c.breed]?.color || "#fbbf24"}`, animation: `fadeIn ${0.5 + i * 0.1}s ease-out ${1 + i * 0.05}s both` }, title: c.name }))), fallen.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 4, marginTop: 4 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ef4444bb", letterSpacing: 3 } }, "THEY WERE HERE"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 3, alignItems: "center" } }, fallen.map((f, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { textAlign: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: BREEDS[f.breed]?.color || "#888", fontFamily: "'Manrope',sans-serif", fontWeight: 700 } }, f.name.split(" ")[0], f.epithet ? " " + f.epithet : ""), f.memorial && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff66", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", lineHeight: 1.3, maxWidth: 260 } }, f.memorial))))), !won && allC.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 4, marginTop: 4 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff44", letterSpacing: 3 } }, allC.length, " STILL STANDING"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "center", maxWidth: 340 } }, allC.map((c, i) => /* @__PURE__ */ React.createElement("span", { key: i, style: { fontSize: 10, color: BREEDS[c.breed]?.color || "#888", fontFamily: "'Manrope',sans-serif", opacity: 0.4 } }, c.name.split(" ")[0], c.scarred ? "*" : "")))), won && meta && meta.stats.w > 1 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: "#fbbf24bb", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 320, lineHeight: 1.6, marginTop: 4 } }, fallen.length === 0 ? `Every single one. The dark couldn't take ${mvp ? mvp.name.split(" ")[0] + " or " : ""}any of them.` : fallen.length >= 3 ? `${fallen.map((f) => f.name.split(" ")[0]).join(", ")}. The survivors carry those names now.` : clutch ? "One number. The difference between a colony and a memory." : "Not all of them. But the ones who made it carry the ones who didn't."), !won && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: "#ef4444bb", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 320, lineHeight: 1.6, marginTop: 4 } }, fallen.length >= 3 ? `${fallen.map((f) => f.name.split(" ")[0]).join(", ")}. Say them. The dark won't.` : rMaxF >= 6 ? `${mvp ? mvp.name.split(" ")[0] + " fought like something that" : "They fought like they"} refused to go quietly.` : "They were here. They mattered. The dark can take the colony but it can't take that."), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 12, fontFamily: "'Manrope',sans-serif", fontSize: 10, color: "#666" } }, meta && meta.cats.length > 0 && /* @__PURE__ */ React.createElement("span", { style: { color: "#c084fc" } }, "\u{1F3E0} +", calcHearthDust(meta.cats).reduce((s, h) => s + h.dust, 0), "\u2726/run"), /* @__PURE__ */ React.createElement("span", null, "Peak: ", /* @__PURE__ */ React.createElement("span", { style: { color: NERVE[rMaxF].color } }, NERVE[rMaxF].name)), (meta?.heat || 0) > 0 && /* @__PURE__ */ React.createElement("span", { style: { color: "#ef4444" } }, "Heat ", meta.heat, "\u{1F525}")), won ? /* @__PURE__ */ React.createElement("button", { onClick: () => setVictoryStep(1), style: { ...BTN("linear-gradient(135deg,#fbbf24,#f59e0b)", "#0a0a1a"), padding: "10px 36px", fontSize: 14, marginTop: 8 } }, "Continue") : /* @__PURE__ */ React.createElement("button", { onClick: () => setPh("title"), style: { ...BTN("linear-gradient(135deg,#ef4444,#dc2626)", "#fff"), padding: "10px 36px", fontSize: 14, marginTop: 8 } }, "The Hearth Still Burns")), vStep === 1 && /* @__PURE__ */ React.createElement(React.Fragment, null, hasChronicle && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888888aa", letterSpacing: 4, textAlign: "center", fontFamily: "'Manrope',sans-serif" } }, "THE CHRONICLE"), /* @__PURE__ */ React.createElement("div", { style: { padding: "14px 20px", borderRadius: 10, background: "#ffffff04", border: "1px solid #ffffff08", maxWidth: 400 } }, genChronicle(won).map((p, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { fontSize: 12, color: "#aaa", fontFamily: "'Manrope',sans-serif", fontStyle: "italic", lineHeight: 1.7, marginBottom: i < 2 ? 8 : 0 } }, p)))), !hasChronicle && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#666", fontStyle: "italic", fontFamily: "'Manrope',sans-serif" } }, "The record is short. But it exists."), /* @__PURE__ */ React.createElement("button", { onClick: () => setVictoryStep(hasUnlocks ? 2 : 3), style: { ...BTN("linear-gradient(135deg,#fbbf24,#f59e0b)", "#0a0a1a"), padding: "10px 36px", fontSize: 14, marginTop: 8 } }, "Continue")), vStep === 2 && /* @__PURE__ */ React.createElement(React.Fragment, null, newUnlocks.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 4, padding: "12px 20px", borderRadius: 10, background: "#fbbf2408", border: "1px solid #fbbf2433" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fbbf24", letterSpacing: 2, fontWeight: 700 } }, "NEW UNLOCKS"), newUnlocks.map((msg, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { fontSize: 11, color: "#fbbf24", fontFamily: "'Manrope',sans-serif" } }, msg))), meta && meta.stats.w === 1 && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", top: 0, left: 0, right: 0, height: "100vh", pointerEvents: "none", zIndex: 0, overflow: "hidden" } }, Array.from({ length: 24 }).map((_, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: {
      position: "absolute",
      top: -10,
      left: `${4 + i * 4}%`,
      width: 5,
    ...
    } }))), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 48, animation: "float 3s ease-in-out infinite", filter: "drop-shadow(0 0 30px #fbbf2444)", marginBottom: 4 } }, "\u{1F525}"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 22, fontWeight: 700, letterSpacing: 12, fontFamily: "'Manrope',sans-serif", background: "linear-gradient(180deg,#fef08a,#fbbf24)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", animation: "comboBurst .8s ease-out", textTransform: "uppercase" } }, "THE FOUNDING"), /* @__PURE__ */ React.createElement("div", { style: { padding: "16px 24px", borderRadius: 14, background: "linear-gradient(145deg,#fbbf2408,#c084fc06)", border: "1.5px solid #fbbf2433", textAlign: "center", maxWidth: 380, animation: "fadeIn 1.5s ease-out .5s both", boxShadow: "0 0 40px #fbbf2418" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "#fbbf24cc", fontFamily: "'Manrope',sans-serif", lineHeight: 1.8, marginBottom: 10 } }, "A colony survived three nights. For the first time since the eighth colony fell, someone kept a fire burning through the dark. Two names go into the Hearth. Not as memories. As ", /* @__PURE__ */ React.createElement("b", { style: { color: "#fef08a" } }, "founders"), ". Everything that comes after carries their blood."), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#c084fcbb", fontFamily: "'Manrope',sans-serif", fontStyle: "italic", lineHeight: 1.6 } }, "The dark gave you three nights to see if you were worth five. You were.")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap", fontSize: 10, fontFamily: "'Manrope',sans-serif", animation: "fadeIn 2s ease-out 1s both" } }, /* @__PURE__ */ React.createElement("span", { style: { color: "#fbbf24", padding: "4px 10px", borderRadius: 6, background: "#fbbf2411", border: "1px solid #fbbf2422" } }, "5 Nights Unlocked"), /* @__PURE__ */ React.createElement("span", { style: { color: "#c084fc", padding: "4px 10px", borderRadius: 6, background: "#c084fc11", border: "1px solid #c084fc22" } }, "\u{1F3E0} The Hearth"), /* @__PURE__ */ React.createElement("span", { style: { color: "#4ade80", padding: "4px 10px", borderRadius: 6, background: "#4ade8011", border: "1px solid #4ade8022" } }, "\u2726 Upgrades"))), meta && meta.stats.w === 2 && !isFirstRun && MX >= 5 && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", top: 0, left: 0, right: 0, height: "100vh", pointerEvents: "none", zIndex: 0, overflow: "hidden" } }, Array.from({ length: 30 }).map((_, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: {
      position: "absolute",
      top: -10,
      left: `${3 + i * 3.2}%`,
      width: 5,
    ...
    } }))), /* @__PURE__ */ React.createElement("div", { style: { padding: "16px 24px", borderRadius: 14, background: "linear-gradient(145deg,#fbbf2410,#f59e0b08)", border: "2px solid #fbbf2444", animation: "comboBurst .8s ease-out", textAlign: "center", maxWidth: 380, boxShadow: "0 0 40px #fbbf2422" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 22, fontWeight: 700, letterSpacing: 10, marginBottom: 8, fontFamily: "'Manrope',sans-serif", background: "linear-gradient(180deg,#fef08a,#fbbf24)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", textTransform: "uppercase" } }, "THE DARK LOST"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "#fbbf24cc", fontFamily: "'Manrope',sans-serif", lineHeight: 1.7, marginBottom: 10 } }, "Five nights. Every test. Every boss. Every pattern the dark has run on eight colonies. And you walked through all of it. The Hearth burns with ", meta.cats.length, " name", meta.cats.length !== 1 ? "s" : "", ". The system doesn't know what to do with that."), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf2488", fontFamily: "'Manrope',sans-serif", fontStyle: "italic" } }, "Heat is unlocked. The dark adapts. So can you."))), /* @__PURE__ */ React.createElement("button", { onClick: () => setVictoryStep(3), style: { ...BTN("linear-gradient(135deg,#fbbf24,#f59e0b)", "#0a0a1a"), padding: "10px 36px", fontSize: 14, marginTop: 8 } }, "Continue")), vStep === 3 && /* @__PURE__ */ React.createElement(React.Fragment, null, hasHearthPick && (() => {
      ...
      return /* @__PURE__ */ React.createElement("div", { style: { width: "100%", textAlign: "center" } }, isFirstHearthSave && picked.length === 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#c084fcbb", fontFamily: "'Manrope',sans-serif", fontStyle: "italic", lineHeight: 1.6, marginBottom: 10, maxWidth: 360, margin: "0 auto 10px", padding: "10px 14px", borderRadius: 10, background: "#c084fc06", border: "1px solid #c084fc15" } }, "The Hearth has been dark since the eighth colony fell. Two names will relight it. Not as memories. As founders. Their blood flows through every colony that comes after."), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#fbbf24", letterSpacing: 3, marginBottom: 4, fontFamily: "'Manrope',sans-serif" } }, picked.length === 0 ? isFirstHearthSave ? "THE FOUNDERS" : "CHOOSE THE FIRST SOUL" : "CHOOSE THEIR MATE"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#c084fcbb", fontFamily: "'Manrope',sans-serif", marginBottom: 6 } }, picked.length === 0 ? isFirstHearthSave ? "Choose carefully. This is the beginning of everything." : "A male and female carry the story to the Hearth. Their descendants begin the next colony." : /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("span", { style: { color: "#fbbf24", fontWeight: 700 } }, getFullName(picked[0])), ` (${picked[0].sex === "M" ? "\u2642" : "\u2640"}) ${isFirstHearthSave ? "is the first Founder" : "will be remembered"}. Now choose ${isFirstHearthSave ? "the second Founder" : needSex === "M" ? "a male companion" : "a female companion"}.`)), noMate && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fb923c", fontFamily: "'Manrope',sans-serif", marginBottom: 8 } }, "No ", needSex === "M" ? "males" : "females", " survived. ", picked[0].name.split(" ")[0], " goes alone.", /* @__PURE__ */ React.createElement("button", { onClick: async () => {
        ...
        return /* @__PURE__ */ React.createElement("div", { key: c.id, onClick: () => saveCatM(c), style: { cursor: "pointer", textAlign: "center", padding: 5, borderRadius: 8, border: "1px solid #ffffff0a", background: "#ffffff04", width: 96 } }, /* @__PURE__ */ React.createElement(CC, { cat: c, sm: true, hl: c.id === mvp?.id }), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 3, fontFamily: "'Manrope',sans-serif", fontSize: 10, lineHeight: 1.4 } }, /* @__PURE__ */ React.createElement("div", { style: { color: c.sex === "M" ? "#60a5fa" : "#f472b6", fontWeight: 700 } }, c.sex === "M" ? "\u2642" : "\u2640"), c.stats.tp > 0 ? /* @__PURE__ */ React.createElement("div", { style: { color: "#888" } }, c.stats.tp, "x Best:", c.stats.bs.toLocaleString()) : /* @__PURE__ */ React.createElement("div", { style: { color: "#555", fontStyle: "italic" } }, "Never played"), /* @__PURE__ */ React.createElement("div", { style: { color: "#c084fc", fontWeight: 700, fontSize: 10 } }, "+", hVal, "\u2726/run"), c.id === mvp?.id && c.stats.tp > 0 && /* @__PURE__ */ React.createElement("div", { style: { color: "#fbbf24", fontWeight: 700, fontSize: 10 } }, "\u2605 MVP")));
  ...
  if (ph === "denSelect") {
    ...
    return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start", minHeight: "100vh", zIndex: 1, gap: 10, padding: mob2 ? "16px 12px" : "20px", maxWidth: 600, paddingTop: mob2 ? 40 : 60 } }, /* @__PURE__ */ React.createElement("h2", { style: { fontSize: mob2 ? 18 : 20, color: campMode ? "#fbbf24" : "#c084fc", letterSpacing: 4, margin: 0, fontFamily: "'Manrope',sans-serif" } }, campMode ? "\u{1F3D5} CAMP" : "THE DEN"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: mob2 ? 11 : 12, color: "#ffffff55", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 320, lineHeight: 1.6 } }, campMode ? "Pick 2 cats for the watch. The rest sleep by the fire." : nightText), campMode ? /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 14px", borderRadius: 10, background: "#fbbf2408", border: "1px solid #fbbf2422", fontSize: 12, fontFamily: "'Manrope',sans-serif", color: "#fbbf24cc", lineHeight: 1.7, textAlign: "center", maxWidth: 380, animation: "fadeIn .6s ease-out" } }, /* @__PURE__ */ React.createElement("b", null, "Pick 2 cats for the watch."), " Unbonded \u2642+\u2640 may bond. Grudged pairs may reconcile. Others gain +1 Power.") : !seen.den && /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 14px", borderRadius: 10, background: "#c084fc08", border: "1px solid #c084fc22", fontSize: 12, fontFamily: "'Manrope',sans-serif", color: "#c084fccc", lineHeight: 1.7, textAlign: "center", maxWidth: 380, animation: "fadeIn .6s ease-out" } }, /* @__PURE__ */ React.createElement("b", null, "Shelter a \u2642 + \u2640 pair to breed safely."), " Everyone else enters the wilds \u2014 they train, bond, or fight.", /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => setSeen((s) => ({ ...s, den: true })), style: { fontSize: 10, background: "none", border: "1px solid #c084fc33", borderRadius: 4, color: "#c084fc", cursor: "pointer", padding: "3px 10px", fontFamily: "'Manrope',sans-serif" } }, "Got it"))), campMode ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { width: "100%", maxWidth: 480 } }, den.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, justifyContent: "center", alignItems: "center", marginBottom: 8, padding: "8px 12px", borderRadius: 10, background: "#fbbf2408", border: "1px solid #fbbf2422" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fbbf24", letterSpacing: 2, fontWeight: 700 } }, "\u{1F525} THE WATCH"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4 } }, den.map((c) => /* @__PURE__ */ React.createElement("div", { key: c.id, onClick: () => toggleDen(c), style: { cursor: "pointer" } }, /* @__PURE__ */ React.createElement(CC, { cat: c, sm: true, cw: denCardW, sel: true }))))), den.length < 2 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fbbf2466", fontFamily: "'Manrope',sans-serif", textAlign: "center", marginBottom: 6 } }, den.length === 0 ? "Tap 2 cats below" : "Pick 1 more"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", justifyContent: "center" } }, (() => {
      ...
      return sorted.map((c) => {
        ...
        return /* @__PURE__ */ React.createElement("div", { key: c.id, onClick: den.length < 2 ? () => toggleDen(c) : void 0, style: { cursor: den.length < 2 ? "pointer" : "default", opacity: den.length >= 2 ? 0.4 : 1, position: "relative" } }, /* @__PURE__ */ React.createElement(CC, { cat: c, sm: true, cw: denCardW, hl: isGrudgedWith || canBondWith || isMateOf }), isGrudgedWith && /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", top: -4, right: -2, fontSize: 10, color: "#fb923c" } }, "\u26A1"), canBondWith && !isGrudgedWith && /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", top: -4, right: -2, fontSize: 10, color: "#f472b6" } }, "\u2661"), isMateOf && /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", top: -4, right: -2, fontSize: 10, color: "#f472b6" } }, "\u{1F495}"));
      });
    })()), injured.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fb923c88", letterSpacing: 1, fontFamily: "'Manrope',sans-serif", marginBottom: 3 } }, "\u{1FA79} RESTING (", injured.length, ")"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 3, flexWrap: "wrap", justifyContent: "center" } }, injured.map((c) => /* @__PURE__ */ React.createElement("div", { key: c.id, style: { position: "relative" } }, /* @__PURE__ */ React.createElement(CC, { cat: c, sm: true, cw: denCardW, dis: true }))))))) : /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: mob2 ? "column" : "row", gap: 12, width: "100%" } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 1, padding: "8px 10px", borderRadius: 10, background: "#4ade8006", border: "1px solid #4ade8022", minHeight: mob2 ? 80 : 120 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: campMode ? "#fbbf24" : "#4ade80", letterSpacing: 2, fontWeight: 700, marginBottom: 6 } }, campMode ? "\u{1F525} THE WATCH" : "\u{1F6E1} SHELTER", " (", isolated.length, "/", MAX_ISOLATE, ")"), eventDenSafe && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#4ade80aa", fontFamily: "'Manrope',sans-serif", marginBottom: 4 } }, "\u{1F54A}\uFE0F Shrine protection active"), isolated.length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#4ade8044", fontFamily: "'Manrope',sans-serif", fontStyle: "italic" } }, "Tap cats below to shelter them") : /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 3, flexWrap: "wrap", justifyContent: "center" } }, isolated.map((c) => /* @__PURE__ */ React.createElement("div", { key: c.id, onClick: () => toggleDen(c), style: { cursor: "pointer", position: "relative" } }, /* @__PURE__ */ React.createElement(CC, { cat: c, sm: true, cw: denCardW, sel: true, onTraitClick: (ct) => setTraitTip(ct) })))), !campMode && (() => {
      let breedPairs = 0;
      ...
      return breedPairs > 0 ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#4ade80", fontFamily: "'Manrope',sans-serif", textAlign: "center", marginTop: 4 } }, "\u{1F91D} ", breedPairs, " breeding pair", breedPairs > 1 ? "s" : "") : isolated.length >= 2 ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#4ade8066", fontFamily: "'Manrope',sans-serif", textAlign: "center", marginTop: 4 } }, "No M/F pairs to breed") : null;
      ...
      return sorted.map((c) => /* @__PURE__ */ React.createElement("div", { key: c.id, onClick: den.length < MAX_ISOLATE ? () => toggleDen(c) : void 0, style: { cursor: den.length < MAX_ISOLATE ? "pointer" : "default", opacity: den.length >= MAX_ISOLATE ? 0.6 : 1, position: "relative" } }, /* @__PURE__ */ React.createElement(CC, { cat: c, sm: true, cw: denCardW, denMode: true, onTraitClick: (ct) => setTraitTip(ct) })));
    })()), injured.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fb923c88", letterSpacing: 1, fontFamily: "'Manrope',sans-serif", marginBottom: 3 } }, "\u{1FA79} RESTING (", injured.length, ")"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 3, flexWrap: "wrap" } }, injured.map((c) => /* @__PURE__ */ React.createElement("div", { key: c.id, style: { position: "relative" } }, /* @__PURE__ */ React.createElement(CC, { cat: c, sm: true, cw: denCardW, dis: true, onTraitClick: (ct) => setTraitTip(ct) }))))))), (() => {
      ...
      if (campMode && den.length >= 2) {
        ...
        if (isGrudged) {
          campHint = "30% chance to reconcile";
          campHintColor = "#c084fc";
        } else if (canBond && bothDevoted) {
          campHint = "Devoted pair \u2014 guaranteed bond \u{1F495}";
          campHintColor = "#f472b6";
        } else if (canBond) {
          campHint = "40% chance to bond";
          campHintColor = "#f472b6";
        } else {
          campHint = "70% +1P, or rations, or quiet night";
          campHintColor = "#fbbf24";
        }
      }
      return /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 4, marginTop: 4 } }, campMode && campHint && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: campHintColor, fontFamily: "'Manrope',sans-serif", letterSpacing: 1, animation: "fadeIn .3s ease-out" } }, campHint), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8 } }, /* @__PURE__ */ React.createElement("button", { onClick: campMode ? () => {
        const results = [];
        [setHand, setDraw, setDisc].forEach((s) => {
          s((arr) => arr.map((x) => x.injured ? { ...x, injuryTimer: Math.max(0, (x.injuryTimer || 2) - 1), injured: (x.injuryTimer || 2) <= 1 ? false : x.injured } : x));
        });
        results.push({ text: "Injuries tended. The colony rests.", color: "#4ade80", icon: "\u{1FA79}" });
        setFerv((f) => Math.min(NERVE_MAX, f + 1));
        Audio.nerveUp();
        setFFlash("up");
        setTimeout(() => setFFlash(null), 400);
        ...
        if (den.length >= 2) {
          ...
          const canBond = a.sex !== b.sex && !a.bondedTo && !b.bondedTo;
          const bothDevoted = catHas(a, "Devoted") && catHas(b, "Devoted");
          const bondChance = bothDevoted ? 1 : 0.4;
          if (isGrudged && Math.random() < 0.3) {
            [setHand, setDraw, setDisc].forEach((s) => {
              s((arr) => arr.map((x) => {
            ...
            if (aCamped && bCamped) {
              ...
              if (roll < 0.7) {
                ...
                setGold((g) => g + found);
                results.push({ text: `${a.name.split(" ")[0]} found ${found}\u{1F41F} stashed near the fire.`, color: "#fbbf24", icon: "\u{1F41F}" });
              } else {
                const quiet = pk([
                ...
                results.push({ text: quiet, color: "#888", icon: "\u{1F319}" });
              }
            }
          }
        } else {
  ...
  if (ph === "denResults" && denRes) {
    ...
    const beats = [];
    denRes.forEach((r) => {
      if (r.type === "breed") beats.push({ pri: 0, el: /* @__PURE__ */ React.createElement("div", { key: beats.length, style: { width: "100%", padding: mob2 ? "8px 10px" : "10px 14px", borderRadius: 10, background: "linear-gradient(145deg,#1b2e1b,#0d0d1a)", border: "1px solid #4ade8044" } }, /* @__PURE__ */ React.createElement("div", { style: { fontStyle: "italic", color: "#4ade80bb", fontSize: 12, lineHeight: 1.5, fontFamily: "'Manrope',sans-serif", marginBottom: 6 } }, cpk(flavorCache, `breed_${r.baby.id}`, DEN_BREED, (fn) => fn(n(r.c1), n(r.c2), n(r.baby)))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, alignItems: "center" } }, /* @__PURE__ */ React.createElement(CC, { cat: r.c1, sm: true }), /* @__PURE__ */ React.createElement("span", { style: { color: "#4ade80bb" } }, "+"), /* @__PURE__ */ React.createElement(CC, { cat: r.c2, sm: true }), /* @__PURE__ */ React.createElement("span", { style: { color: "#4ade80bb" } }, "="), /* @__PURE__ */ React.createElement(CC, { cat: r.baby, sm: true, hl: true }), r.twins && r.twin2 && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("span", { style: { color: "#fbbf24bb" } }, "+"), /* @__PURE__ */ React.createElement(CC, { cat: r.twin2, sm: true, hl: true }))), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888", fontFamily: "'Manrope',sans-serif", marginTop: 3 } }, BREEDS[r.baby.breed].icon, " ", r.baby.name, " P", r.baby.power, " ", (r.baby.trait || PLAIN).icon, " ", (r.baby.trait || PLAIN).name, " ", r.baby.sex === "M" ? "\u2642" : "\u2640", (r.baby.trait || PLAIN).tier !== "common" && (r.baby.trait || PLAIN).name !== "Plain" && /* @__PURE__ */ React.createElement("span", { style: { color: traitTierLabel(r.baby.trait).color, fontWeight: 700 } }, " \u2605 ", traitTierLabel(r.baby.trait).label.toUpperCase()))) });
      if (r.type === "death") beats.push({ pri: 1, el: /* @__PURE__ */ React.createElement("div", { key: beats.length, style: { width: "100%", padding: mob2 ? "8px 10px" : "10px 14px", borderRadius: 10, background: "linear-gradient(145deg,#2e1111,#0d0d1a)", border: "1px solid #ef4444bb" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ef4444", letterSpacing: 3, fontWeight: 700, fontFamily: "'Manrope',sans-serif", marginBottom: 4 } }, "\u{1F480} DEATH"), /* @__PURE__ */ React.createElement("div", { style: { fontStyle: "italic", color: "#ef4444bb", fontSize: 13, lineHeight: 1.5, fontFamily: "'Manrope',sans-serif", marginBottom: 6 } }, cpk(flavorCache, `death_${r.victim.id}`, DEN_DEATH, (fn) => fn(n(r.c1), n(r.c2), n(r.victim)))), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", padding: "8px 12px", borderRadius: 8, background: "#ef444411", border: "1px solid #ef444422" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: "#ef4444", fontWeight: 700 } }, r.victim.name), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888", fontFamily: "'Manrope',sans-serif", marginTop: 2 } }, "P", r.victim.power, " ", BREEDS[r.victim.breed]?.icon, " ", r.victim.breed, " ", (r.victim.trait || PLAIN).icon, " ", (r.victim.trait || PLAIN).name), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#ef4444bb", fontFamily: "'Manrope',sans-serif", marginTop: 2 } }, r.victim.stats?.tp > 0 ? `Played ${r.victim.stats.tp} hands. Best: ${r.victim.stats.bs?.toLocaleString()}.` : "Never got to play."), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#ef4444", fontFamily: "'Manrope',sans-serif", marginTop: 4, fontWeight: 700, letterSpacing: 2 } }, "Gone from the colony."))) });
      if (r.type === "phoenix") beats.push({ pri: 0, el: /* @__PURE__ */ React.createElement("div", { key: beats.length, style: { width: "100%", padding: mob2 ? "8px 10px" : "10px 14px", borderRadius: 10, background: "linear-gradient(145deg,#2e2211,#0d0d1a)", border: "1px solid #fbbf2466" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "#fbbf24", fontWeight: 700, marginBottom: 4 } }, "\u{1F525} ", n(r.risen), " RISES FROM THE ASHES!"), /* @__PURE__ */ React.createElement("div", { style: { fontStyle: "italic", color: "#fbbf24bb", fontSize: 11, fontFamily: "'Manrope',sans-serif" } }, cpk(flavorCache, `phoenix_${r.risen.id}`, DEN_PHOENIX, (fn) => fn(r.c1.name, r.c2.name, r.risen.name))), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fbbf24", fontFamily: "'Manrope',sans-serif", marginTop: 4 } }, "Now Eternal at P1. The fire changes everything.")) });
      if (r.type === "bond") beats.push({ pri: 2, el: /* @__PURE__ */ React.createElement("div", { key: beats.length, style: { fontSize: 11, fontFamily: "'Manrope',sans-serif", color: "#f472b6", lineHeight: 1.4, padding: "4px 0" } }, "\u{1F495} ", n(r.c1), " & ", n(r.c2), " bonded ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#ffffff55" } }, "\u2014 ", bondStr, " mult together")) });
      if (r.type === "reconcile_bond") beats.push({ pri: 2, el: /* @__PURE__ */ React.createElement("div", { key: beats.length, style: { fontSize: 11, fontFamily: "'Manrope',sans-serif", color: "#f472b6", lineHeight: 1.4, padding: "4px 0" } }, "\u{1F495} ", n(r.c1), " & ", n(r.c2), " reconciled + bonded ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#ffffff55" } }, "\u2014 ", bondStr, " mult together")) });
      if (r.type === "reconcile") beats.push({ pri: 2, el: /* @__PURE__ */ React.createElement("div", { key: beats.length, style: { fontSize: 11, fontFamily: "'Manrope',sans-serif", color: "#67e8f9", lineHeight: 1.4, padding: "4px 0" } }, "\u{1F54A}\uFE0F ", n(r.c1), " & ", n(r.c2), " made peace ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#ffffff55" } }, "\u2014 grudge cleared")) });
      if (r.type === "fight") beats.push({ pri: 3, el: /* @__PURE__ */ React.createElement("div", { key: beats.length, style: { fontSize: 11, fontFamily: "'Manrope',sans-serif", color: "#ef4444", lineHeight: 1.4, padding: "4px 0" } }, "\u2694 ", n(r.loser), " ", r.wasInjured ? "injured" : "scarred", " ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#ffffff55" } }, "\u2014 ", r.wasInjured ? "heals in 2 rounds" : "\xD71.25 mult")) });
      if (r.type === "grudge") beats.push({ pri: 3, el: /* @__PURE__ */ React.createElement("div", { key: beats.length, style: { fontSize: 11, fontFamily: "'Manrope',sans-serif", color: "#fb923c", lineHeight: 1.4, padding: "4px 0" } }, "\u26A1 ", n(r.c1), " & ", n(r.c2), " grudge ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#ffffff55" } }, "\u2014 \u22122M when together")) });
      if (r.type === "wanderer") beats.push({ pri: 4, el: /* @__PURE__ */ React.createElement("div", { key: beats.length, style: { fontSize: 11, fontFamily: "'Manrope',sans-serif", color: "#67e8f9", lineHeight: 1.4, padding: "4px 0" } }, "\u{1F431} ", n(r.cat), " wandered in ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#ffffff55" } }, "\u2014 P", r.cat.power, " ", r.cat.breed)) });
      if (r.type === "growth") beats.push({ pri: 5, el: /* @__PURE__ */ React.createElement("div", { key: beats.length, style: { fontSize: 11, fontFamily: "'Manrope',sans-serif", color: "#4ade80", lineHeight: 1.4, padding: "4px 0" } }, "\u2B50 ", n(r.cat), " +1P") });
      if (r.type === "mentor") beats.push({ pri: 5, el: /* @__PURE__ */ React.createElement("div", { key: beats.length, style: { fontSize: 11, fontFamily: "'Manrope',sans-serif", color: "#c084fc", lineHeight: 1.4, padding: "4px 0" } }, "\u{1F4D6} ", n(r.elder), " mentored ", n(r.young), " ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#ffffff55" } }, "\u2014 +1P")) });
      if (r.type === "training") beats.push({ pri: 5, el: /* @__PURE__ */ React.createElement("div", { key: beats.length, style: { fontSize: 11, fontFamily: "'Manrope',sans-serif", color: "#60a5fa", lineHeight: 1.4, padding: "4px 0" } }, "\u2694\uFE0F ", n(r.c1), " & ", n(r.c2), " sparred ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#ffffff55" } }, "\u2014 +1P each")) });
      if (r.type === "teach") beats.push({ pri: 4, el: /* @__PURE__ */ React.createElement("div", { key: beats.length, style: { fontSize: 11, fontFamily: "'Manrope',sans-serif", color: "#34d399", fontWeight: 700, lineHeight: 1.4, padding: "4px 0" } }, "\u{1F46A} ", n(r.parent), " taught ", n(r.child), " ", r.trait.icon, " ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#ffffff55" } }, "\u2014 ", r.trait.name)) });
      if (r.type === "found") beats.push({ pri: 5, el: /* @__PURE__ */ React.createElement("div", { key: beats.length, style: { fontSize: 11, fontFamily: "'Manrope',sans-serif", color: "#fbbf24", lineHeight: 1.4, padding: "4px 0" } }, "\u{1F41F} ", n(r.cat), " found ", r.gold, "\u{1F41F}") });
      if (r.traitGained) beats.push({ pri: 4, el: /* @__PURE__ */ React.createElement("div", { key: beats.length, style: { fontSize: 11, fontFamily: "'Manrope',sans-serif", color: "#fbbf24", fontWeight: 700, lineHeight: 1.4, padding: "4px 0" } }, "\u2728 ", n(r.traitGained.cat), " gained ", r.traitGained.trait.icon, " ", r.traitGained.trait.name) });
    });
    beats.sort((a, b) => a.pri - b.pri);
    const majors = beats.filter((b) => b.pri <= 1);
    const minors = beats.filter((b) => b.pri >= 2);
    return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start", minHeight: "100vh", zIndex: 1, gap: 10, padding: mob2 ? "16px 12px" : "20px", maxWidth: 550, overflowY: "auto", paddingTop: mob2 ? 30 : 40 } }, /* @__PURE__ */ React.createElement("h2", { style: { fontSize: mob2 ? 16 : 18, color: "#c084fc", letterSpacing: 4, margin: 0, fontFamily: "'Manrope',sans-serif", animation: "fadeIn .6s ease-out" } }, "WHAT HAPPENED IN THE DARK"), denRes.length === 0 && /* @__PURE__ */ React.createElement("div", { style: { color: "#666", fontSize: 13, fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", lineHeight: 1.6, maxWidth: 340 } }, cpk(flavorCache, "denQuiet", DEN_QUIET, (fn) => fn())), majors.map((b, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { width: "100%", animation: `scoreSlide .5s ease-out ${i * 0.6}s both` } }, b.el)), minors.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 1, width: "100%", maxWidth: 420, padding: "8px 12px", borderRadius: 10, background: "#ffffff04", border: "1px solid #ffffff08", animation: `fadeIn .5s ease-out ${majors.length * 0.6 + 0.3}s both` } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#c084fc88", letterSpacing: 4, textAlign: "center", fontFamily: "'Manrope',sans-serif", fontWeight: 700, marginBottom: 4 } }, "DEN REPORT"), minors.map((b, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { animation: `scoreSlide .3s ease-out ${majors.length * 0.6 + 0.5 + i * 0.15}s both` } }, b.el))), /* @__PURE__ */ React.createElement("button", { onClick: () => {
      const babies = [];
  ...
  if (ph === "shop") {
    ...
    return /* @__PURE__ */ React.createElement("div", { style: W }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement(Dust, null), /* @__PURE__ */ React.createElement("style", null, CSS), toasts.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", top: 12, right: 12, zIndex: 250, display: "flex", flexDirection: "column", gap: 6, pointerEvents: "none", maxWidth: 280 } }, toasts.map((t) => /* @__PURE__ */ React.createElement("div", { key: t.id, style: { display: "flex", gap: 8, alignItems: "center", padding: t.big ? "12px 18px" : "8px 14px", borderRadius: t.big ? 10 : 8, background: "#1a1a2eee", border: `1.5px solid ${t.color}${t.big ? "66" : "44"}`, boxShadow: `0 4px 16px #00000066,0 0 ${t.big ? 16 : 8}px ${t.color}${t.big ? "44" : "22"}`, animation: "slideIn .3s ease-out", fontFamily: "'Manrope',sans-serif" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: t.big ? 22 : 16, flexShrink: 0 } }, t.icon), /* @__PURE__ */ React.createElement("span", { style: { fontSize: t.big ? 14 : 12, color: t.color, fontWeight: t.big ? 700 : 600, lineHeight: 1.3 } }, t.text)))), /* @__PURE__ */ React.createElement("div", { style: { width: "100%", maxWidth: 700, padding: "10px 16px", zIndex: 1, display: "flex", flexDirection: "column", gap: 8, alignItems: "center", paddingBottom: 100 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", width: "100%", alignItems: "center" } }, /* @__PURE__ */ React.createElement("h2", { style: { fontSize: 20, color: "#fbbf24", letterSpacing: 4, margin: 0, fontFamily: "'Manrope',sans-serif" } }, "THE MARKET"), /* @__PURE__ */ React.createElement(ProgressMap, { ante, blind, mx: MX })), /* @__PURE__ */ React.createElement(FM, { level: ferv, prev: pFerv }), (() => {
      const wSeed = (ante * 11 + blind * 7 + gold) % 10;
      if (wSeed > 2 || isFirstRun) return null;
      return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf2433", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 320, lineHeight: 1.4, animation: "fadeIn 1.5s ease-out" } }, WHISPER_SHOP[(ante * 3 + blind) % WHISPER_SHOP.length]);
    })(), !seen.shop && /* @__PURE__ */ React.createElement("div", { style: { padding: "12px 18px", borderRadius: 10, background: "linear-gradient(145deg,#fbbf2408,#f59e0b04)", border: "1px solid #fbbf2433", fontSize: 12, fontFamily: "'Manrope',sans-serif", color: "#e8e6e3cc", lineHeight: 1.7, textAlign: "left", maxWidth: 400, animation: "fadeIn .6s ease-out" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: "#fbbf24", marginBottom: 6, letterSpacing: 1 } }, "THE MARKET"), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("span", { style: { color: "#fbbf24" } }, "\u{1F41F} Rations"), " are currency. Save them for ", /* @__PURE__ */ React.createElement("span", { style: { color: "#4ade80" } }, "interest"), " (every 5\u{1F41F} = +1 per round) or spend on:"), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6, display: "flex", flexDirection: "column", gap: 3 } }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("span", { style: { color: "#fbbf24" } }, "\u{1F431} Cats"), " \u2014 stronger cats with traits boost your score"), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("span", { style: { color: "#fbbf24" } }, "\u{1F4DC} Scrolls"), " \u2014 level up hand types for bigger base scores"), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("span", { style: { color: "#fbbf24" } }, "\u{1F6E1}\uFE0F Wards"), " \u2014 passive bonuses that trigger every hand")), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, padding: "6px 10px", borderRadius: 6, background: "#ffffff06", border: "1px solid #ffffff0a" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf24bb" } }, "\u{1F4A1} ", /* @__PURE__ */ React.createElement("b", null, "Season Devotion"), ": The more cats of one season you play, the stronger that season becomes. Check your progress in the Upgrades tab.")), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, textAlign: "center" } }, /* @__PURE__ */ React.createElement("button", { onClick: () => setSeen((s) => ({ ...s, shop: true })), style: { fontSize: 11, background: "linear-gradient(135deg,#fbbf24,#f59e0b)", border: "none", borderRadius: 5, color: "#0a0a1a", cursor: "pointer", padding: "5px 16px", fontWeight: 700 } }, "Got it"))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 0, width: "100%", borderBottom: "1px solid #ffffff0a" } }, [["cats", "\u{1F431} Cats"], ["upgrades", "\u26A1 Upgrades"], ["colony", "\u{1F465} Colony"]].map(([id, label]) => {
      ...
      }, style: { flex: 1, padding: vw < 500 ? "12px 4px" : "8px 4px", fontSize: 12, fontFamily: "'Manrope',sans-serif", fontWeight: shopTab === id ? 700 : 400, color: shopTab === id ? "#fbbf24" : isNudge ? "#fbbf24" : "#666", background: shopTab === id ? "#fbbf2408" : isNudge ? "#fbbf2406" : "transparent", border: "none", borderBottom: shopTab === id ? "2px solid #fbbf24" : isNudge ? "2px solid #fbbf2466" : "2px solid transparent", cursor: "pointer", transition: "all .2s", animation: isNudge ? "breathe 2s ease-in-out infinite" : "none" } }, label, isNudge ? " \u2726" : "");
    })), shopTab === "cats" && /* @__PURE__ */ React.createElement("div", { style: { width: "100%", animation: "fadeIn .3s ease-out" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 3, flexWrap: "wrap", justifyContent: "center", padding: "4px 0 8px", borderBottom: "1px solid #ffffff08", marginBottom: 8 } }, (() => {
      ...
      return Object.entries(bCounts).sort(([, a], [, b]) => b - a).map(
        ([br, ct]) => /* @__PURE__ */ React.createElement("span", { key: br, style: { fontSize: 10, color: BREEDS[br]?.color || "#888", fontFamily: "'Manrope',sans-serif", padding: "1px 5px", borderRadius: 3, background: BREEDS[br]?.color + "11", border: "1px solid " + BREEDS[br]?.color + "22" } }, BREEDS[br]?.icon, ct)
      );
    })(), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#666", fontFamily: "'Manrope',sans-serif", padding: "1px 5px" } }, [...hand, ...draw, ...disc].length, " cats \xB7 avg P", Math.round([...hand, ...draw, ...disc].reduce((s, c) => s + c.power, 0) / Math.max(1, [...hand, ...draw, ...disc].length)))), (() => {
      ...
      return /* @__PURE__ */ React.createElement("div", null, featured && ftl && /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 8, animation: "scorePop .5s ease-out .1s both" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: featured.trait.name === "Plain" ? "#4ade80" : ftl.color, letterSpacing: 2, marginBottom: 4, animation: "fadeIn .4s ease-out" } }, featured.trait.tier === "mythic" ? "\u2728 MYTHIC FIND" : featured.trait.tier === "legendary" ? "\u2B50 LEGENDARY FIND" : featured.trait.tier === "rare" ? "\u2605 RARE FIND" : featured.trait.tier === "rare_neg" ? "\u26A1 RISKY BET" : featured.trait.name !== "Plain" ? "\u2605 TRAINED CAT" : "FOR SALE"), /* @__PURE__ */ React.createElement("div", { onClick: () => buyCat(sCats.indexOf(featured)), style: { cursor: gold >= (featured._price || 4) ? "pointer" : "not-allowed", display: "flex", flexDirection: vw < 500 ? "column" : "row", gap: vw < 500 ? 6 : 12, alignItems: "center", padding: vw < 500 ? "12px 10px" : "8px 12px", borderRadius: 10, background: isHighTier(featured.trait) ? `linear-gradient(135deg,${ftl.color}08,${ftl.color}02)` : "#ffffff04", border: `1px solid ${isHighTier(featured.trait) ? ftl.color + "33" : "#ffffff0a"}`, transition: "all .2s" } }, /* @__PURE__ */ React.createElement(CC, { cat: featured, dis: gold < (featured._price || 4), onTraitClick: (ct) => setTraitTip(ct), cw: vw < 500 ? 70 : void 0 }), /* @__PURE__ */ React.createElement("div", { style: { flex: 1, textAlign: vw < 500 ? "center" : "left" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: BREEDS[featured.breed]?.color, fontFamily: "'Manrope',sans-serif" } }, featured.name), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#888", fontFamily: "'Manrope',sans-serif", marginTop: 2 } }, "Power ", featured.power, " ", BREEDS[featured.breed]?.icon, " ", featured.breed, " ", featured.sex === "M" ? "\u2642" : "\u2640"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: featured.trait.name === "Plain" ? "#555" : tierColor(featured.trait), fontFamily: "'Manrope',sans-serif", marginTop: 3 } }, featured.trait.name === "Plain" ? "No trait yet" : featured.trait.icon + " " + featured.trait.name + ": " + featured.trait.desc), (featured.extraTraits || []).map((t, ti) => /* @__PURE__ */ React.createElement("div", { key: ti, style: { fontSize: 12, color: tierColor(t), fontFamily: "'Manrope',sans-serif", marginTop: 1 } }, "+", t.icon, " ", t.name, ": ", t.desc)), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fbbf24", fontWeight: 700, marginTop: 4 } }, featured._price || 4, "\u{1F41F}")))), rest.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 4 } }, /* @__PURE__ */ React.createElement("div", { style: { borderTop: "1px solid #ffffff08", paddingTop: 8, marginTop: 4 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, flexWrap: "wrap" } }, rest.map((c2, ri2) => {
        const ri = sCats.indexOf(c2);
        const p = c2._price || 4;
        const gI = c2.sex === "M" ? "\u2642" : "\u2640";
        const gC2 = c2.sex === "M" ? "#60a5fa" : "#f472b6";
        return /* @__PURE__ */ React.createElement("div", { key: c2.id, onClick: () => buyCat(ri), style: { cursor: gold >= p ? "pointer" : "not-allowed", textAlign: "center", maxWidth: 110, animation: `scorePop .4s ease-out ${0.2 + ri2 * 0.15}s both` } }, /* @__PURE__ */ React.createElement(CC, { cat: c2, dis: gold < p, onTraitClick: (ct) => setTraitTip(ct) }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: c2.trait.tier === "rare_neg" ? "#ef4444" : BREEDS[c2.breed].color, marginTop: 1, fontFamily: "'Manrope',sans-serif" } }, c2.name, " ", /* @__PURE__ */ React.createElement("span", { style: { color: gC2 } }, gI), " ", /* @__PURE__ */ React.createElement("span", { style: { color: "#fbbf24" } }, p, "\u{1F41F}")));
      })))), !sCats.length && /* @__PURE__ */ React.createElement("div", { style: { color: "#555", fontSize: 10, fontFamily: "'Manrope',sans-serif", textAlign: "center", padding: 16 } }, "Sold out"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "center", marginTop: 8 } }, /* @__PURE__ */ React.createElement("button", { onClick: reroll, disabled: gold < rc, style: { ...BTN("#1a1a2e", "#fbbf24", gold >= rc), border: `1px solid ${gold >= rc ? "#fbbf2444" : "#222"}`, fontSize: 11 } }, "Reroll (", rc, "\u{1F41F})")));
    })()), (shopTab === "upgrades" || shopTab === "scrolls" || shopTab === "wards") && /* @__PURE__ */ React.createElement("div", { style: { width: "100%", animation: "fadeIn .3s ease-out" } }, !seen.shop2 && /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 14px", borderRadius: 8, background: "#fbbf2408", border: "1px solid #fbbf2422", marginBottom: 10, animation: "fadeIn .6s ease-out" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf24cc", fontFamily: "'Manrope',sans-serif", lineHeight: 1.6 } }, /* @__PURE__ */ React.createElement("b", null, "\u{1F6E1}\uFE0F Wards"), " give passive bonuses every hand. ", /* @__PURE__ */ React.createElement("b", null, "\u{1F4DC} Scrolls"), " level up hand types. Both shape your strategy."), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6, textAlign: "center" } }, /* @__PURE__ */ React.createElement("button", { onClick: () => setSeen((s) => ({ ...s, shop2: true })), style: { fontSize: 10, background: "#fbbf24", border: "none", borderRadius: 4, color: "#0a0a1a", cursor: "pointer", padding: "3px 12px", fontWeight: 700 } }, "Got it"))), fams.length === 0 && sFams.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 14px", borderRadius: 8, background: "#4ade8008", border: "1px solid #4ade8033", marginBottom: 8, animation: "breathe 2s ease-in-out infinite" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#4ade80", fontFamily: "'Manrope',sans-serif", fontWeight: 700 } }, "\u{1F4A1} Buy a ward! Wards boost your score every hand, all run long.")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#c084fcbb", letterSpacing: 2, marginBottom: 6 } }, "\u{1F6E1}\uFE0F WARDS FOR SALE"), sFams.length > 0 ? /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 } }, sFams.map((f, i) => {
      const wp = famPrice(f);
      ...
      } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 16 } }, f.icon), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: can ? "#fbbf24" : "#555", fontWeight: 700 } }, wp, "\u{1F41F}", f._starter ? " \u2605" : "")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: isHtWard ? "#c084fc" : "#e8e6e3cc", marginTop: 4 } }, f.name), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888", fontFamily: "'Manrope',sans-serif", lineHeight: 1.4, marginTop: 2 } }, f.desc), isHtWard && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#c084fc66", fontFamily: "'Manrope',sans-serif", marginTop: 3, letterSpacing: 1 } }, "HAND TYPE BONUS"));
    })) : !ul.fams && !isFirstRun ? /* @__PURE__ */ React.createElement("div", { style: { color: "#555", fontSize: 10, fontFamily: "'Manrope',sans-serif", fontStyle: "italic", textAlign: "center", padding: 12, marginBottom: 12 } }, "\u{1F512} Complete a run to unlock Wards") : /* @__PURE__ */ React.createElement("div", { style: { color: "#555", fontSize: 10, fontFamily: "'Manrope',sans-serif", textAlign: "center", padding: 12, marginBottom: 12 } }, "No wards available"), fams.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 12 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888", letterSpacing: 2, marginBottom: 4 } }, "EQUIPPED (", fams.length, "/", MF, ")"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, flexWrap: "wrap" } }, fams.map((f) => /* @__PURE__ */ React.createElement("div", { key: f.id, onClick: () => toast(f.icon, `${f.name}: ${f.desc}`, "#fbbf24"), style: { display: "flex", alignItems: "center", gap: 4, padding: "4px 8px", borderRadius: 6, background: "#ffffff06", border: "1px solid #ffffff0a", cursor: "pointer" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14 } }, f.icon), /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#aaa", fontFamily: "'Manrope',sans-serif" } }, f.name), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#666", fontFamily: "'Manrope',sans-serif" } }, f.desc)), /* @__PURE__ */ React.createElement("button", { onClick: (e) => {
      e.stopPropagation();
      sellFam(f);
    }, style: { fontSize: 10, color: "#ef4444aa", background: "none", border: "none", cursor: "pointer", padding: 0, fontFamily: "'Manrope',sans-serif" } }, "\u2715"))))), runCount >= 1 && sScrolls.length > 0 && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fbbf24bb", letterSpacing: 2, marginBottom: 6 } }, "\u{1F4DC} SCROLLS"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 } }, sScrolls.map((s, i) => {
      ...
      return /* @__PURE__ */ React.createElement("div", { key: s.name, onClick: () => can && buyScroll(i), style: {
        ...
        opacity: can ? 1 : 0.4,
        transition: "all .2s"
      } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 700, color: "#fbbf24" } }, s.name), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: can ? "#fbbf24" : "#555", fontWeight: 700 } }, s.price, "\u{1F41F}")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888", fontFamily: "'Manrope',sans-serif", marginTop: 3 } }, "Lv", s.lv, " ", "\u2192", " Lv", s.nextLv, s.nextBase && /* @__PURE__ */ React.createElement("span", { style: { color: "#4ade80" } }, " (", s.nextBase.c, "C \xD7 ", s.nextBase.m, "M)")));
      ...
      return /* @__PURE__ */ React.createElement("div", { key: h.name, style: { padding: "2px 6px", borderRadius: 4, background: lv > 1 ? "#fbbf2408" : "transparent", border: lv > 1 ? "1px solid #fbbf2422" : "1px solid #ffffff08", fontSize: 10, fontFamily: "'Manrope',sans-serif", color: lv > 1 ? "#fbbf24" : "#555" } }, h.name, lv > 1 ? ` Lv${lv}` : "", " ", /* @__PURE__ */ React.createElement("span", { style: { color: lv > 1 ? "#4ade80" : "#444" } }, sc.c, "\xD7", sc.m));
      ...
      return /* @__PURE__ */ React.createElement("div", { key: breed, style: { padding: "6px 10px", borderRadius: 8, background: dev.count > 0 ? color + "08" : "transparent", border: `1px solid ${dev.count > 0 ? color + "22" : "#ffffff08"}` } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 3 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color, fontWeight: 700, fontFamily: "'Manrope',sans-serif" } }, icon, " ", breed), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: color + "88", fontFamily: "'Manrope',sans-serif" } }, dev.count, " played")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 2, marginBottom: 4 } }, (DEVOTION_MILESTONES[breed] || []).map((m, i) => {
      ...
      })), dev.unlocked.map((m, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { fontSize: 10, color: color + "99", fontFamily: "'Manrope',sans-serif" } }, "\u2713 " + m.name + ": " + m.desc)), dev.next && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", fontFamily: "'Manrope',sans-serif" } }, "\u25CB " + dev.next.at + " plays: " + dev.next.name + " \u2014 " + dev.next.desc));
    })))), shopTab === "colony" && /* @__PURE__ */ React.createElement("div", { style: { width: "100%", animation: "fadeIn .3s ease-out" } }, (() => {
      const ds = getDeckStats();
      return /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 12px", borderRadius: 8, background: "#ffffff06", border: "1px solid #ffffff0a", fontFamily: "'Manrope',sans-serif", fontSize: 10, display: "flex", gap: 6, flexWrap: "wrap", color: "#888", marginBottom: 8, lineHeight: 1.6 } }, /* @__PURE__ */ React.createElement("span", { style: { fontWeight: 700, color: "#e8e6e3" } }, uAll.length, " cats"), /* @__PURE__ */ React.createElement("span", { style: { color: "#555" } }, "\xB7"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4 } }, Object.entries(ds.bc).sort(([, a], [, b]) => b - a).map(([br, ct]) => /* @__PURE__ */ React.createElement("span", { key: br, style: { color: BREEDS[br]?.color || "#888" } }, BREEDS[br]?.icon, ct))), /* @__PURE__ */ React.createElement("span", { style: { color: "#555" } }, "\xB7"), /* @__PURE__ */ React.createElement("span", null, "Avg Power ", ds.avgPow), /* @__PURE__ */ React.createElement("span", { style: { color: "#555" } }, "\xB7"), /* @__PURE__ */ React.createElement("span", { style: { color: "#60a5fa" } }, "\u2642 ", ds.gc.M, " male"), /* @__PURE__ */ React.createElement("span", { style: { color: "#f472b6" } }, "\u2640 ", ds.gc.F, " female"), ds.scarred > 0 && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("span", { style: { color: "#555" } }, "\xB7"), /* @__PURE__ */ React.createElement("span", { style: { color: "#ef4444" } }, "\u{1FA79} ", ds.scarred, " scarred")), ds.bonded > 0 && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("span", { style: { color: "#555" } }, "\xB7"), /* @__PURE__ */ React.createElement("span", { style: { color: "#4ade80" } }, "\u{1F495} ", ds.bonded, " bonded")));
    })(), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: sellMode ? "#c084fc" : "#888", fontWeight: sellMode ? 700 : 400 } }, sellMode ? /* @__PURE__ */ React.createElement("span", null, "\u{1F54A}\uFE0F THE LETTING GO ", /* @__PURE__ */ React.createElement("span", { style: { color: sellsLeft > 0 ? "#4ade80" : "#ef4444" } }, "(", sellsLeft, " left)")) : /* @__PURE__ */ React.createElement("span", null, "YOUR COLONY")), /* @__PURE__ */ React.createElement("button", { onClick: () => setSellMode(!sellMode), style: { fontSize: 10, padding: "4px 12px", borderRadius: 6, border: `1px solid ${sellMode ? "#c084fc44" : "#fbbf2444"}`, background: sellMode ? "#c084fc11" : "transparent", color: sellMode ? "#c084fc" : "#fbbf24", cursor: "pointer", fontFamily: "'Manrope',sans-serif" } }, sellMode ? "Done" : "Release...")), sellMode && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#c084fcbb", fontFamily: "'Manrope',sans-serif", marginBottom: 6, fontStyle: "italic" } }, "Each departure leaves something behind. Tap a cat to release them."), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", maxHeight: 300, overflowY: "auto", paddingBottom: 4 } }, uAll.map((c) => {
      const pg = getPartingGifts(c);
      const canSell = sellMode && sellsLeft > 0 && uAll.length > 8 && (pg.goldVal >= 0 || gold >= Math.abs(pg.goldVal));
      return /* @__PURE__ */ React.createElement("div", { key: c.id, style: { position: "relative" } }, /* @__PURE__ */ React.createElement(CC, { cat: c, sm: true, onClick: sellMode && canSell ? () => sellCat(c) : void 0, dis: sellMode && !canSell, onTraitClick: (ct) => setTraitTip(ct) }), sellMode && canSell && /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", top: 2, right: 2, fontSize: 10, color: "#c084fc", fontWeight: 700, background: "#000000dd", borderRadius: 3, padding: "1px 3px", cursor: "pointer", maxWidth: 72, lineHeight: 1.3 } }, pg.goldVal > 0 && /* @__PURE__ */ React.createElement("span", { style: { color: "#4ade80" } }, "+", pg.goldVal, "\u{1F41F} "), pg.gifts.map((g, gi) => /* @__PURE__ */ React.createElement("span", { key: gi, style: { color: "#c084fc" } }, g, " ")), pg.goldVal === 0 && pg.gifts.length === 0 && /* @__PURE__ */ React.createElement("span", { style: { color: "#888" } }, "free")));
    })))), /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 100, background: "linear-gradient(180deg,transparent,#0a0a1a 8px,#0a0a1a)", padding: "8px 16px 12px", display: "flex", justifyContent: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { maxWidth: 700, width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 2 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "center" } }, /* @__PURE__ */ React.createElement("span", { onClick: () => toast("\u{1F41F}", `Rations: ${gold}\u{1F41F}. Spend in the Market on cats, wards, scrolls. Earn more by scoring higher. Save 5+ for interest.`, "#fbbf24"), style: { color: "#fbbf24", fontSize: 14, fontWeight: 700, cursor: "help" } }, "\u{1F41F}", gold), runCount >= 1 && interestPreview > 0 && /* @__PURE__ */ React.createElement("span", { onClick: () => toast("\u{1F4C8}", `Interest: +${interestPreview}\u{1F41F} next round (floor of rations\xF75, max 5). Save rations to earn more.`, "#4ade80"), style: { color: "#4ade80", fontSize: 10, fontFamily: "'Manrope',sans-serif", cursor: "help" } }, "+", interestPreview, " stores", interestPreview >= 5 ? " MAX" : ""), /* @__PURE__ */ React.createElement("span", { onClick: () => toast("\u2726", `Stardust: ${meta?.dust || 0}\u2726. Meta-currency from Hearth cats. Spend on permanent upgrades between runs.`, "#c084fc"), style: { color: "#c084fc", fontSize: 10, cursor: "help" } }, "\u2726", meta?.dust || 0)), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontFamily: "'Manrope',sans-serif", color: "#888" } }, nt ? /* @__PURE__ */ React.createElement("span", { onClick: () => toast("\u{1F3AF}", `Target: ${nt.target.toLocaleString()}. Score this much total across all hands to clear ${nt.blindName}. Boss blinds injure cats on failed hands.`, "#fbbf24"), style: { cursor: "help" } }, nt.blind === 2 ? /* @__PURE__ */ React.createElement("span", { style: { color: "#ef4444", fontWeight: 700 } }, boss?.icon || "\u{1F441}\uFE0F", " ", nt.blindName, " approaches") : /* @__PURE__ */ React.createElement("span", { style: { color: "#fbbf24" } }, nt.blindName, " falls"), " \xB7 Night ", nt.ante, " \xB7 ", /* @__PURE__ */ React.createElement("span", { style: { color: "#e8e6e3", fontWeight: 700 } }, nt.target.toLocaleString()), " to survive") : /* @__PURE__ */ React.createElement("span", { style: { color: "#4ade80", fontWeight: 700 } }, "Dawn approaches..."))), blind >= 2 ? /* @__PURE__ */ React.createElement("button", { onClick: () => {
      setDen([]);
      setPh("denSelect");
  ...
  return /* @__PURE__ */ React.createElement("div", { style: { ...W, animation: fFlash === "down" ? "shake .3s ease" : "none" } }, /* @__PURE__ */ React.createElement("div", { style: BG }), /* @__PURE__ */ React.createElement("style", null, CSS), isNL && /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", inset: 0, border: "2px solid #fef08a44", pointerEvents: "none", zIndex: 50, animation: "glow 2s ease-in-out infinite" } }), reshuf && /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", top: "40%", left: "50%", transform: "translate(-50%,-50%)", zIndex: 200, animation: "flash .8s ease-out forwards", fontSize: 13, color: "#fbbf24", letterSpacing: 4, fontWeight: 700, whiteSpace: "nowrap" } }, "\u267B RESHUFFLE"), toasts.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", top: 12, right: 12, zIndex: 250, display: "flex", flexDirection: "column", gap: 6, pointerEvents: "none", maxWidth: 280 } }, toasts.map((t) => /* @__PURE__ */ React.createElement("div", { key: t.id, style: { display: "flex", gap: 8, alignItems: "center", padding: t.big ? "12px 18px" : "8px 14px", borderRadius: t.big ? 10 : 8, background: "#1a1a2eee", border: `1.5px solid ${t.color}${t.big ? "66" : "44"}`, boxShadow: `0 4px 16px #00000066,0 0 ${t.big ? 16 : 8}px ${t.color}${t.big ? "44" : "22"}`, animation: "slideIn .3s ease-out", fontFamily: "'Manrope',sans-serif" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: t.big ? 22 : 16, flexShrink: 0 } }, t.icon), /* @__PURE__ */ React.createElement("span", { style: { fontSize: t.big ? 14 : 12, color: t.color, fontWeight: t.big ? 700 : 600, lineHeight: 1.3 } }, t.text)))), autoPlay && ph === "playing" && (() => {
    ...
    } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#fbbf2488", letterSpacing: 4, marginBottom: 6 } }, tour.title), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: "#e8e6e3", lineHeight: 1.6 } }, tour.msg), /* @__PURE__ */ React.createElement("button", { onClick: () => setAutoPlay((a) => a ? { ...a, step: a.step + 1 } : null), style: { marginTop: 12, fontSize: 12, background: "linear-gradient(135deg,#fbbf24,#f59e0b)", border: "none", borderRadius: 6, color: "#0a0a1a", cursor: "pointer", padding: "8px 20px", fontWeight: 700, fontFamily: "'Manrope',sans-serif", letterSpacing: 1 } }, step === -1 ? "Show me \u2192" : "Next \u2192"), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => {
    ...
    if (step === 0 && !autoRef.current) {
      autoRef.current = setTimeout(() => {
        ...
        function selectNext() {
          ...
          Audio.cardSelect();
          if (cat) toast(BREEDS[cat.breed]?.icon || "\u{1F431}", `${cat.name.split(" ")[0]} selected (${cat.breed})`, BREEDS[cat.breed]?.color || "#fbbf24", 1500);
    ...
    return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", inset: 0, zIndex: 130, pointerEvents: "auto", background: "transparent" }, onClick: (e) => e.stopPropagation() }), /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", bottom: mob ? 70 : 60, left: "50%", transform: "translateX(-50%)", zIndex: 150, padding: "16px 28px", borderRadius: 14, background: "#0a0a1aee", border: "1.5px solid #4ade8044", maxWidth: 360, animation: "fadeIn .6s ease-out", textAlign: "center", fontFamily: "'Manrope',sans-serif", boxShadow: "0 8px 32px #00000088" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#4ade8066", letterSpacing: 3, marginBottom: 4 } }, "\u{1F440} WATCHING THE DEMO"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 15, color: "#4ade80", fontWeight: 700, marginBottom: 6 } }, step === 0 ? "Selecting same-season cats..." : step <= 2 ? `Selecting cats... (${Math.min(sel.size, autoPlay.idxs.length)}/${autoPlay.idxs.length})` : "Playing the hand..."), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#4ade80aa", lineHeight: 1.5 } }, step <= 2 ? "Same season = stronger hand. Watch which cards light up." : "Now watch how chips \xD7 mult builds the score..."), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff66", marginTop: 6, fontStyle: "italic" } }, "Sit back \u2014 you'll play the next hand yourself."), /* @__PURE__ */ React.createElement("button", { onClick: () => {
      if (autoRef.current) clearTimeout(autoRef.current);
      if (stRef.current) clearTimeout(stRef.current);
      setSel(/* @__PURE__ */ new Set());
      setAutoPlay(null);
    ...
    if (guide.step === 0) {
      ...
      sub = "Watch how chips \xD7 mult builds your score.";
    ...
    return /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", bottom: guide.step >= 2 ? 280 : 220, left: "50%", transform: "translateX(-50%)", zIndex: 150, padding: "10px 18px", borderRadius: 10, background: "#0a0a1aee", border: "1px solid #fbbf2444", maxWidth: 300, animation: "fadeIn .6s ease-out", textAlign: "center", fontFamily: "'Manrope',sans-serif", boxShadow: "0 8px 32px #00000088" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: "#fbbf24", fontWeight: 700 } }, msg), sub && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf24aa", marginTop: 2 } }, sub), /* @__PURE__ */ React.createElement("button", { onClick: () => {
      setGuide(null);
      setSeen((s) => ({ ...s, guided: true }));
    }, style: { marginTop: 4, fontSize: 10, background: "none", border: "none", color: "#ffffff55", cursor: "pointer", padding: "2px 8px" } }, "dismiss"));
  })(), /* @__PURE__ */ React.createElement("div", { style: { width: "100%", maxWidth: 700, padding: mob ? "6px 12px" : "8px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", zIndex: 1, borderBottom: `1px solid ${isBoss ? "#ef444422" : "#ffffff0a"}` } }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#888", letterSpacing: 2 } }, "NIGHT ", ante, "/", MX, !mob && /* @__PURE__ */ React.createElement("span", { style: { color: "#555" } }, " Round ", blind + 1, "/3")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: mob ? 13 : 14, fontWeight: 700, color: isBoss ? "#ef4444" : "#fbbf24" } }, blindN[blind]), !mob && /* @__PURE__ */ React.createElement(ProgressMap, { ante, blind, mx: MX })), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#888", letterSpacing: 2 } }, "SCORE"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: mob ? 16 : 18, fontWeight: 900 } }, /* @__PURE__ */ React.createElement("span", { style: { color: rScore >= tgt ? "#4ade80" : "#e8e6e3" } }, rScore.toLocaleString()), /* @__PURE__ */ React.createElement("span", { style: { color: "#555", fontSize: 12 } }, " / ", tgt.toLocaleString())), !mob && rScore < tgt && hLeft > 0 && (() => {
    const nph = Math.ceil((tgt - rScore) / hLeft);
    return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: nph > 45e3 ? "#ef4444" : nph > 24e3 ? "#fb923c" : "#aaa", fontFamily: "'Manrope',sans-serif", fontWeight: 700, animation: nph > 45e3 ? "fpp 2s ease infinite" : "none" } }, "\u{1F3AF} ", nph.toLocaleString(), "/hand");
  })()), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "right" } }, /* @__PURE__ */ React.createElement("div", { style: { color: "#fbbf24", fontWeight: 700, fontSize: 13 } }, "\u{1F41F}", gold), runCount >= 1 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#4ade80", fontFamily: "'Manrope',sans-serif" } }, Math.min(5, Math.floor(gold / 5)) > 0 ? `+${Math.min(5, Math.floor(gold / 5))} stores` : "")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, alignItems: "center" } }, /* @__PURE__ */ React.createElement("button", { onClick: (e) => {
    e.stopPropagation();
    toggleMute();
    ...
    const nph = Math.ceil((tgt - rScore) / hLeft);
    return /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", padding: "3px 0", zIndex: 1, maxWidth: 700, width: "100%" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, color: nph > 45e3 ? "#ef4444" : nph > 24e3 ? "#fb923c" : "#aaa", fontFamily: "'Manrope',sans-serif", fontWeight: 800, letterSpacing: 1 } }, "\u{1F3AF} ", nph.toLocaleString(), " per hand"));
  ...
  }, style: { fontSize: 10, color: "#fbbf2466", fontFamily: "'Manrope',sans-serif", cursor: "pointer", animation: "breathe 2s ease-in-out infinite", marginRight: 2 } }, "\u2139\uFE0F"), runCount >= 1 && Object.keys(DEVOTION_MILESTONES).map((breed) => {
    const dev = getDevotionLevel(breed, devotion);
    ...
    fontFamily: "'Manrope',sans-serif"
  } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14 } }, n.icon), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: n.color, fontWeight: 700 } }, n.text))), /* @__PURE__ */ React.createElement("button", { onClick: () => setDenNews([]), style: { background: "none", border: "none", color: "#555", fontSize: 10, cursor: "pointer", padding: "2px 4px" } }, "\u2715")), /* @__PURE__ */ React.createElement("div", { style: { width: "100%", maxWidth: 700, padding: "0 16px", zIndex: 1 } }, /* @__PURE__ */ React.createElement("div", { style: { height: 3, background: "#1a1a2e", borderRadius: 2, overflow: "hidden" } }, /* @__PURE__ */ React.createElement("div", { style: { height: "100%", width: `${Math.min(100, rScore / tgt * 100)}%`, background: isNL ? "linear-gradient(90deg,#b85c2c,#f59e0b,#fef08a,#ffffffcc)" : "linear-gradient(90deg,#fbbf24,#4ade80)", borderRadius: 2, transition: "width .5s ease-out" } }))), ph === "scoring" && sRes && /* @__PURE__ */ React.createElement("div", { onClick: scoringDone ? advanceFromScoring : skipScoring, style: {
    cursor: "pointer",
    position: "fixed",
    inset: 0,
    background: "#000000cc",
  ...
  } }, scoringFlash && /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", inset: 0, background: scoringFlash, opacity: 0.08, pointerEvents: "none", transition: "opacity .15s", zIndex: 101 } }), multPop && multPop.mode === "xmult" && (() => {
    ...
    return /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", top: "12%", left: 0, right: 0, zIndex: 102, pointerEvents: "none", textAlign: "center", animation: "multFlash .7s ease-out forwards" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 18, color: srcColor, fontFamily: "'Manrope',sans-serif", letterSpacing: 3, fontWeight: 700, marginBottom: 8, textShadow: `0 0 20px ${srcColor}44` } }, multPop.label), /* @__PURE__ */ React.createElement("div", { style: {
    ...
    return /* @__PURE__ */ React.createElement("div", { style: {
      ...
      } }, displayPrimary, displayCombo ? /* @__PURE__ */ React.createElement("span", { style: { color: "#c084fc" } }, " + ", displayCombo) : ""), htLvMatch && !done && /* @__PURE__ */ React.createElement("span", { style: { fontSize: isReveal ? 13 : 11, fontWeight: 700, color: "#fbbf24", background: "#fbbf2418", padding: isReveal ? "3px 9px" : "2px 7px", borderRadius: 6, fontFamily: "'Manrope',sans-serif", letterSpacing: 1, border: "1px solid #fbbf2433", animation: isReveal ? "fadeIn .8s ease-out" : "none" } }, "LV", htLvMatch[1])), !isReveal && /* @__PURE__ */ React.createElement("div", { style: { marginBottom: done ? 2 : 4 } }), anyDisc && done && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#c084fc", fontWeight: 700, letterSpacing: 2, fontFamily: "'Manrope',sans-serif", animation: "fadeIn .5s ease-out", marginBottom: 4 } }, "\u2728 SECRET COMBO DISCOVERED \u2728"), done && !anyDisc && (() => {
        const eo = htObj?.echo;
        return eo ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff66", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", letterSpacing: 2, animation: "fadeIn 1s ease-out", marginBottom: 2 } }, eo) : null;
      })());
    })(), (() => {
      const isReveal = sStep === -1;
      ...
      return /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10, alignItems: "center", animation: isFirstStep ? "fadeIn .4s ease-out" : "none" } }, /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", transition: "transform .2s", transform: (isChipStep || isFirstStep) && !done ? "scale(1.2)" : "scale(1)" } }, /* @__PURE__ */ React.createElement("span", { style: { color: "#3b82f6", fontWeight: 900, fontSize: done ? 26 : 22, transition: "font-size .2s", textShadow: (isChipStep || isFirstStep) && !done ? "0 0 12px #3b82f644" : "none", animation: isFirstStep ? "countUp .4s ease-out" : "none" } }, displayChips), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#3b82f666", fontFamily: "'Manrope',sans-serif", letterSpacing: 2 } }, "CHIPS")), /* @__PURE__ */ React.createElement("span", { style: { color: "#ffffff55", fontSize: 16, fontFamily: "'Manrope',sans-serif" } }, "\xD7"), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", transition: "transform .2s", transform: (isMultStep || isFirstStep) && !done ? "scale(1.2)" : "scale(1)" } }, /* @__PURE__ */ React.createElement("span", { style: { color: "#ef4444", fontWeight: 900, fontSize: done ? 26 : 22, transition: "font-size .2s", textShadow: (isMultStep || isFirstStep) && !done ? "0 0 12px #ef444444" : "none", animation: isFirstStep ? "countUp .4s ease-out" : "none" } }, displayMult), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ef444466", fontFamily: "'Manrope',sans-serif", letterSpacing: 2 } }, "MULT")), /* @__PURE__ */ React.createElement("span", { style: { color: "#ffffff55", fontSize: 16, fontFamily: "'Manrope',sans-serif" } }, "="), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center" } }, /* @__PURE__ */ React.createElement("span", { style: {
        fontWeight: 900,
        fontSize: done ? 38 : 28,
        color: counterColor,
        ...
        transition: "all .15s",
        fontFamily: "'Manrope',sans-serif"
      } }, displayTotal.toLocaleString()), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: counterColor + "44", fontFamily: "'Manrope',sans-serif", letterSpacing: 2 } }, "SCORE")));
    })(), !done && s?.reason && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: stepColor + "cc", fontFamily: "'Manrope',sans-serif", letterSpacing: 0.5, marginTop: 1, animation: "fadeIn .15s ease-out", maxWidth: 280, textAlign: "center", lineHeight: 1.3 } }, s.reason), !done && flavor && !s?.reason && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: counterColor + "bb", fontFamily: "'Manrope',sans-serif", fontStyle: "italic", letterSpacing: 1.5, marginTop: 2, animation: "fadeIn .15s ease-out", maxWidth: 260, textAlign: "center", lineHeight: 1.3 } }, flavor), !done && annotation && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#4ade80", fontFamily: "'Manrope',sans-serif", marginTop: 4, padding: "3px 12px", borderRadius: 8, background: "#4ade800a", border: "1px solid #4ade8022", animation: "fadeIn .3s ease-out", maxWidth: 280, textAlign: "center", lineHeight: 1.4, letterSpacing: 0.5 } }, "\u{1F4A1} ", annotation), s && !done && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 2, textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: 1 } }, s.type === "hand" || s.type === "combo" ? /* @__PURE__ */ React.createElement(React.Fragment, null, (() => {
      ...
      return /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 4, animation: shakeAnim } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } }, /* @__PURE__ */ React.createElement("div", { style: {
        ...
        return lvMatch ? /* @__PURE__ */ React.createElement("span", { style: { fontSize: rarity >= 4 ? 14 : 12, fontWeight: 700, color: htColor, background: `${htColor}18`, padding: "2px 8px", borderRadius: 6, fontFamily: "'Manrope',sans-serif", letterSpacing: 1, border: `1px solid ${htColor}33` } }, "LV", lvMatch[1]) : null;
      })()), s.chips || s.mult ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: numSize, fontWeight: 900, display: "flex", gap: 12, animation: `scorePopFade .5s ease-out .1s both` } }, s.chips > 0 && /* @__PURE__ */ React.createElement("span", { style: { color: "#3b82f6" } }, "+", s.chips, /* @__PURE__ */ React.createElement("span", { style: { fontSize: Math.max(10, numSize - 8), opacity: 0.7, letterSpacing: 1, marginLeft: 2 } }, "CHIPS")), s.mult > 0 && /* @__PURE__ */ React.createElement("span", { style: { color: "#ef4444" } }, "+", s.mult, /* @__PURE__ */ React.createElement("span", { style: { fontSize: Math.max(10, numSize - 8), opacity: 0.7, letterSpacing: 1, marginLeft: 2 } }, "MULT"))) : null);
      ...
      fontFamily: hasX ? "'Manrope',sans-serif" : "'Manrope',sans-serif",
    ...
    })(), s.type !== "hand" && s.type !== "combo" && !hasX && (s.chips || s.mult) ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontFamily: "'Manrope',sans-serif", fontWeight: 700, display: "flex", gap: 6 } }, s.chips > 0 && /* @__PURE__ */ React.createElement("span", { style: { color: "#3b82f6" } }, "+", s.chips), s.mult > 0 && /* @__PURE__ */ React.createElement("span", { style: { color: "#ef4444" } }, "+", s.mult), s.mult < 0 && /* @__PURE__ */ React.createElement("span", { style: { color: "#ef4444" } }, s.mult)) : null)), done && /* @__PURE__ */ React.createElement("div", { style: { width: 200, height: 6, background: "#ffffff0a", borderRadius: 3, overflow: "hidden", marginTop: 6 } }, /* @__PURE__ */ React.createElement("div", { style: {
      height: "100%",
      width: `${pct}%`,
      borderRadius: 3,
    ...
    } })), done && pct >= 100 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: "#4ade80", fontFamily: "'Manrope',sans-serif", letterSpacing: 4, fontWeight: 900, animation: "comboBurst .6s ease-out", marginTop: 4, textShadow: "0 0 16px #4ade8066" } }, "TARGET PASSED \u2726"), done && pct < 100 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#ef4444", fontFamily: "'Manrope',sans-serif", letterSpacing: 3, fontWeight: 900, animation: "fpp 1.5s ease infinite", marginTop: 4 } }, "BELOW THRESHOLD"), !done && nearMiss && pct < 100 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ef4444aa", fontFamily: "'Manrope',sans-serif", letterSpacing: 2, animation: "fpp 1.5s ease infinite", marginTop: 2 } }, "CLOSE..."), done && tier?.label && /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", animation: "tierReveal .5s ease-out", marginTop: 4 } }, /* @__PURE__ */ React.createElement("div", { style: {
      fontSize: 20,
      fontWeight: 900,
      letterSpacing: 8,
      color: tier.color,
      ...
      fontFamily: "'Manrope',sans-serif"
    } }, tier.label), tier.sub && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: tier.color, opacity: 0.7, letterSpacing: 3, fontFamily: "'Manrope',sans-serif", marginTop: 2 } }, tier.sub), tier.nar && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#aaa", fontStyle: "italic", fontFamily: "'Manrope',sans-serif", marginTop: 3, opacity: 0.7 } }, tier.nar)), done && newBest && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: "#fbbf24", letterSpacing: 3, animation: "newBestPop .5s ease-out", fontFamily: "'Manrope',sans-serif", background: "#fbbf2418", padding: "3px 12px", borderRadius: 20, border: "1px solid #fbbf2433", marginTop: 4 } }, "NEW BEST ", newBest.toUpperCase()), done && _fc && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#4ade80", fontFamily: "'Manrope',sans-serif", marginTop: 6, padding: "8px 16px", borderRadius: 8, background: "#4ade800a", border: "1px solid #4ade8022", animation: "fadeIn .6s ease-out .3s both", maxWidth: 320, textAlign: "center", lineHeight: 1.6 } }, /* @__PURE__ */ React.createElement("div", { style: { fontWeight: 700, marginBottom: 4 } }, "Chips \xD7 Mult = Score"), /* @__PURE__ */ React.createElement("div", { style: { color: "#4ade80bb", fontSize: 10 } }, "Each cat's ", /* @__PURE__ */ React.createElement("span", { style: { color: "#3b82f6" } }, "Power adds Chips"), ". Traits and bonds add ", /* @__PURE__ */ React.createElement("span", { style: { color: "#ef4444" } }, "Mult"), ". More same-season cats = stronger hand type = bigger base numbers. Stack everything.")));
  })(), /* @__PURE__ */ React.createElement("div", { style: { height: scoringDone ? 8 : 16 } }), !scoringDone && sStep >= 2 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff55", fontFamily: "'Manrope',sans-serif", letterSpacing: 2, animation: "fadeIn 1.5s ease-out" } }, "TAP TO SKIP \u23ED"), scoringDone && /* @__PURE__ */ React.createElement("button", { onClick: (e) => {
    e.stopPropagation();
    advanceFromScoring();
  }, style: { background: "linear-gradient(135deg,#fbbf24,#f59e0b)", color: "#0a0a1a", border: "none", borderRadius: 10, padding: "10px 32px", fontSize: 13, fontWeight: 700, cursor: "pointer", letterSpacing: 1, fontFamily: "'Manrope',sans-serif", boxShadow: "0 0 24px #fbbf2444", animation: "fadeIn .3s ease-out" } }, "Continue"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: mob ? 4 : 8, justifyContent: "center", alignItems: "flex-start", padding: "4px 8px", flexWrap: "wrap", maxWidth: "100%", opacity: sStep < 0 ? 0 : 1, transition: "opacity .3s", animation: sStep === 0 ? "fadeIn .4s ease-out" : "none" } }, scoringCats.map((cat, ci) => {
    ...
    return /* @__PURE__ */ React.createElement("div", { key: cat.id, style: {
      display: "flex",
      flexDirection: "column",
      alignItems: "center",
      gap: 2,
      ...
      flexShrink: 0
    } }, /* @__PURE__ */ React.createElement("div", { style: { position: "relative" } }, /* @__PURE__ */ React.createElement(CC, { cat, sel: isActive || isInvolved, hl: hasFired && !isActive && !isInvolved, sm: scoringCats.length > 4, cw: vw < 500 ? 56 : void 0 }), (isActive || isInvolved) && /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", inset: -3, borderRadius: 14, border: `2px solid ${hlColor}`, boxShadow: `0 0 ${isActive ? 20 : 12}px ${hlColor}66,0 0 ${isActive ? 40 : 20}px ${hlColor}22`, pointerEvents: "none", animation: isActive ? "glow 1s ease infinite" : "none" } })), /* @__PURE__ */ React.createElement("div", { style: { minHeight: 20, display: "flex", flexDirection: "column", alignItems: "center", gap: 0 } }, hasFired && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, alignItems: "center", animation: isActive ? "countUp .3s ease-out" : "none" } }, totalC !== 0 && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: "#3b82f6", fontWeight: 700, fontFamily: "'Manrope',sans-serif" } }, totalC > 0 ? "+" : "", totalC), totalM !== 0 && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: "#ef4444", fontWeight: 800, fontFamily: "'Manrope',sans-serif" } }, totalM > 0 ? "+" : "", totalM)), xVals.map((x, xi) => /* @__PURE__ */ React.createElement("span", { key: xi, style: { fontSize: 14, color: "#fbbf24", fontWeight: 900, fontFamily: "'Manrope',sans-serif", animation: "scorePop .3s ease", textShadow: "0 0 10px #fbbf2466" } }, "\xD7", x))), isActive && activeStep?.reason && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ffffff66", fontFamily: "'Manrope',sans-serif", textAlign: "center", maxWidth: 100, lineHeight: 1.3, animation: "fadeIn .2s ease-out" } }, activeStep.reason));
    ...
    return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf24", textAlign: "center", fontFamily: "'Manrope',sans-serif", lineHeight: 1.4 } }, "\u{1F3F7}\uFE0F ", /* @__PURE__ */ React.createElement("b", null, '"', traitTip.epithet, '"'), epDef?.desc ? /* @__PURE__ */ React.createElement("span", { style: { color: "#fbbf24aa" } }, " \u2014 ", epDef.desc) : "");
  })(), traitTip.bondedTo && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#f472b6", textAlign: "center", fontFamily: "'Manrope',sans-serif" } }, "\u{1F495} Bonded. \xD71.5 mult", traitTip.epithetKey === "bonded" ? " (+2 from epithet)" : "", ", \xD71.25 for second pair"), (traitTip.grudgedWith || []).length > 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fb923c", textAlign: "center", fontFamily: "'Manrope',sans-serif" } }, "\u26A1 ", (traitTip.grudgedWith || []).length, " Grudge", (traitTip.grudgedWith || []).length > 1 ? "s" : "", ". 75% tension / 25% prove per pair"), traitTip.epithetKey === "grudgeResolved" && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#4ade80", textAlign: "center", fontFamily: "'Manrope',sans-serif" } }, "\u{1F54A}\uFE0F Immune to future grudges"), traitTip.scarred && !traitTip.injured && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf24", textAlign: "center", fontFamily: "'Manrope',sans-serif" } }, "\u2694 Battle-Hardened. \xD71.25 mult"), traitTip.injured && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#ef4444", textAlign: "center", fontFamily: "'Manrope',sans-serif" } }, "\u{1FA79} Injured. Half power, less mult (heals in ", traitTip.injuryTimer || 1, ")"), traitTip._hearthChild && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf24aa", textAlign: "center", fontFamily: "'Manrope',sans-serif" } }, "\u{1F525} Hearth Legend. Cannot be saved back.")), (traitTip.story || []).length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 10, padding: "8px 12px", borderRadius: 8, background: "#ffffff04", border: "1px solid #ffffff08" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#666", letterSpacing: 2, marginBottom: 4 } }, "STORY"), (traitTip.story || []).slice(-3).map((s, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { fontSize: 11, color: "#999", fontFamily: "'Manrope',sans-serif", lineHeight: 1.5 } }, s))), traitTip.stats && traitTip.stats.tp > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, display: "flex", gap: 10, justifyContent: "center", fontSize: 10, fontFamily: "'Manrope',sans-serif", color: "#666" } }, /* @__PURE__ */ React.createElement("span", null, "\u25B6 ", traitTip.stats.tp, " plays"), traitTip.stats.bs > 0 && /* @__PURE__ */ React.createElement("span", null, "\u{1F3C6} Best: ", traitTip.stats.bs.toLocaleString()), (() => {
    const xp = getCatXP(traitTip.stats.tp, !!getMB().xp);
    return xp ? /* @__PURE__ */ React.createElement("span", { style: { color: xp.color, fontWeight: 700 } }, xp.icon, " ", xp.label) : null;
  })()))), clutch && /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", background: "radial-gradient(circle,#fbbf2422,transparent 60%)", pointerEvents: "none" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 32, fontWeight: 900, color: "#fbbf24", letterSpacing: 8, textShadow: "0 0 40px #fbbf24cc", animation: "clutchBurst .6s ease-out", fontFamily: "'Manrope',sans-serif" } }, "THEY SHOULD HAVE STAYED DOWN")), /* @__PURE__ */ React.createElement("div", { style: { minHeight: 36, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 1, gap: 2 } }, preview ? (() => {
    if (blind === 2 && bossTraits.some((bt) => bt.fx.noStrength)) {
      return /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 700, color: "#fbbf24", letterSpacing: 2 } }, preview.type.name), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#ef4444bb", fontStyle: "italic", fontFamily: "'Manrope',sans-serif" } }, "\u{1F441}\uFE0F Watchful. it sees your hand"));
    }
    ...
    return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 15, fontWeight: 700, color: "#fbbf24", letterSpacing: 2 } }, preview.type.name), preview.combo && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, fontWeight: 700, color: "#c084fc", letterSpacing: 1 } }, "+ ", (meta?.stats?.dh || []).includes(preview.combo.name) ? preview.combo.name : "????"), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: "#888", fontFamily: "'Manrope',sans-serif" } }, baseC, "\xD7", baseM), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14, fontWeight: 900, color: tier.c, letterSpacing: 1, fontFamily: "'Manrope',sans-serif", textShadow: `0 0 8px ${tier.c}44` } }, tier.w)), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { width: 100, height: 4, background: "#1a1a2e", borderRadius: 3, overflow: "hidden", border: "1px solid #ffffff08" } }, /* @__PURE__ */ React.createElement("div", { style: { height: "100%", width: `${tier.p}%`, borderRadius: 3, background: `linear-gradient(90deg,${tier.c}66,${tier.c})`, transition: "width .3s" } })), need > 0 && hLeft > 0 && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14, fontWeight: 700, color: ratio >= 1 ? "#4ade80" : "#ef4444", fontFamily: "'Manrope',sans-serif", letterSpacing: 0.5 } }, "\u{1F3AF} ", Math.ceil(pacePerHand).toLocaleString(), "/hand")), sel.size > 1 && /* @__PURE__ */ React.createElement("button", { onClick: () => setSel(/* @__PURE__ */ new Set()), style: { background: "none", border: "1px solid #ffffff12", borderRadius: 4, color: "#555", fontSize: 10, cursor: "pointer", padding: "2px 8px", fontFamily: "'Manrope',sans-serif" } }, "Clear"), hLeft <= 1 && need > 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ef4444", fontWeight: 700, fontFamily: "'Manrope',sans-serif", animation: "fpp 1.5s ease infinite" } }, "LAST HAND"), (() => {
      const warnings = [];
      ...
      return warnings.length > 0 ? /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 1 } }, warnings.map((w, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { fontSize: 10, color: w.color, fontFamily: "'Manrope',sans-serif" } }, w.icon, " ", w.text))) : null;
    })());
  })() : /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { color: "#555", fontSize: 10 } }, "Select up to 5 cats to play"))), sel.size >= 1 && (() => {
    ...
    return /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, padding: "4px 16px", zIndex: 1, maxWidth: 700, width: "100%", justifyContent: "center", alignItems: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 3, alignItems: "flex-end" } }, ordered.map((cat, pos) => {
      if (!cat) return null;
      const b = BREEDS[cat.breed];
      return /* @__PURE__ */ React.createElement("div", { key: cat.id, style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 0 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#fbbf24", fontFamily: "'Manrope',sans-serif", fontWeight: 700 } }, pos + 1), /* @__PURE__ */ React.createElement("div", { style: { width: 24, height: 28, borderRadius: 4, background: b.bg, border: `1px solid ${b.color}44`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontSize: 10 } }, /* @__PURE__ */ React.createElement("span", null, b.icon), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: b.color, fontWeight: 700 } }, cat.power)));
    })), benchTraited.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 2, alignItems: "center", padding: "2px 6px", borderRadius: 4, background: "#ffffff06", border: "1px solid #ffffff0a" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#888", fontFamily: "'Manrope',sans-serif" } }, "\u{1FA91}"), benchTraited.slice(0, 3).map((c) => /* @__PURE__ */ React.createElement("span", { key: c.id, style: { fontSize: 10 }, title: `${c.name.split(" ")[0]}: bench bonus` }, (c.trait || {}).icon || "\xB7"))));
  })(), (() => {
    ...
    hand.forEach((cat) => {
      if (cat.bondedTo) {
        const mate = [...hand, ...draw, ...disc].find((h) => h.id === cat.bondedTo);
        cat._mateName = mate ? mate.name.split(" ")[0][0] : "";
      } else {
        cat._mateName = "";
      }
    });
    ...
    hand._guideHL = _gHL;
    return null;
  })(), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: mob ? 2 : 3, padding: "0 4px", zIndex: 1, justifyContent: "center", flexWrap: "nowrap", maxWidth: mob ? vw - 8 : 840, width: "100%", overflowX: "visible" } }, (() => {
    ...
    return sorted.map(({ c, i }) => {
      ...
      return /* @__PURE__ */ React.createElement("div", { key: c.id, style: { position: "relative", flexShrink: 0, animation: isGuideHL ? "guidePulse 1.5s ease-in-out infinite" : "none" } }, /* @__PURE__ */ React.createElement(CC, { cat: c, sel: sel.has(i), onClick: () => toggleS(i), dis: ph !== "playing" || !!autoPlay, fog: cfx.fog && !sel.has(i), chemHint: !sel.has(i) ? getHint(c) : null, hl: isRelated || isGuideHL, onTraitClick: (ct) => setTraitTip(ct), sm: mob, cw: _cw || void 0 }), isRelated && /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", top: -10, left: "50%", transform: "translateX(-50%)", fontSize: 11, fontWeight: 700, fontFamily: "'Manrope',sans-serif", background: relType === "mate" ? "#f472b622" : "#4ade8022", padding: "1px 6px", borderRadius: 4, border: `1px solid ${relType === "mate" ? "#f472b644" : "#4ade8044"}`, whiteSpace: "nowrap", animation: "countUp .3s ease-out", zIndex: 10, color: relType === "mate" ? "#f472b6" : "#4ade80" } }, relType === "mate" ? "\u{1F495} mate" : "\u{1F46A} kin"), isGuideHL && /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", top: -10, left: "50%", transform: "translateX(-50%)", fontSize: 11, fontWeight: 700, fontFamily: "'Manrope',sans-serif", background: "#fbbf2422", padding: "1px 6px", borderRadius: 4, border: "1px solid #fbbf2444", whiteSpace: "nowrap", animation: "guidePulse 1.5s ease-in-out infinite", zIndex: 10, color: "#fbbf24" } }, "\u{1F446} select"));
    });
    ...
    }, style: { textAlign: "center", padding: "2px 0", zIndex: 1, animation: "fadeIn .15s ease-out", cursor: "help" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 700, color: "#fbbf24", fontFamily: "'Manrope',sans-serif", letterSpacing: 2 } }, evald.name, htLv > 1 ? ` Lv${htLv}` : ""), runCount >= 1 && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: "#3b82f6", fontFamily: "'Manrope',sans-serif", fontWeight: 700, marginLeft: 8 } }, baseC, "C"), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: "#ffffff55", margin: "0 2px" } }, "\xD7"), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: "#ef4444", fontFamily: "'Manrope',sans-serif", fontWeight: 700 } }, baseM, "M")), bonds.length > 0 && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#4ade80", marginLeft: 6 } }, "\u{1F495}\xD71.5"), grudges.length > 0 && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#fb923c", marginLeft: 6 } }, "\u26A1\u22122M"), evald.combo && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#c084fc", marginLeft: 6 } }, "+\u26A1", evald.combo.name));
  })(), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, padding: "8px", zIndex: 10, alignItems: "center", position: mob ? "sticky" : "static", bottom: mob ? 0 : "auto", background: mob ? "#0a0a1aee" : "transparent", borderTop: mob ? "1px solid #ffffff0a" : "none", justifyContent: "center" } }, ph === "playing" && hLeft <= 0 && rScore < tgt ? /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center" } }, /* @__PURE__ */ React.createElement("button", { onClick: () => endRun(false, rScore), style: { ...BTN("linear-gradient(135deg,#ef4444,#dc2626)", "#fff"), padding: mob ? "12px 24px" : "10px 20px", fontSize: 14, animation: "fpp 1.5s ease infinite" } }, "Colony Fell"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#ef4444bb", marginTop: 4, fontFamily: "'Manrope',sans-serif" } }, "No hands remaining")) : /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center" } }, runCount >= 1 ? (() => {
    const cost = recruitCost();
    const canRecruit = ph === "playing" && !autoPlay && gold >= cost && (draw.length > 0 || disc.length > 0);
    return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("button", { onClick: recruitCat, disabled: !canRecruit, style: { ...BTN(canRecruit ? "#1a2e1a" : "#111", canRecruit ? "#4ade80" : "#444", canRecruit), border: `1px solid ${canRecruit ? "#4ade8044" : "#222"}`, minWidth: mob ? 56 : 50, padding: mob ? "10px 10px" : "8px 10px", fontSize: 10 } }, "+1 Cat"), /* @__PURE__ */ React.createElement("div", { onClick: () => toast("\u{1F431}", "Recruit: draw an extra cat into your hand. Cost doubles each time (1\u21922\u21924\u21928\u{1F41F}). More cats = better hands + bench bonuses.", "#4ade80"), style: { fontSize: 10, color: cost === 0 ? "#4ade80" : gold >= cost ? "#fbbf24" : "#ef4444", marginTop: 2, fontFamily: "'Manrope',sans-serif", cursor: "help" } }, cost === 0 ? "Free!" : cost + "\u{1F41F}"));
  })() : null), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center" } }, /* @__PURE__ */ React.createElement("button", { onClick: discardH, disabled: !sel.size || sel.size > MAX_DISCARD || dLeft <= 0 || ph !== "playing" || cfx.noDisc, style: { ...BTN(sel.size && sel.size <= MAX_DISCARD && dLeft > 0 && ph === "playing" && !cfx.noDisc ? "#1a1a2e" : "#111", sel.size && sel.size <= MAX_DISCARD && dLeft > 0 && ph === "playing" && !cfx.noDisc ? "#ef4444" : "#444", sel.size > 0 && sel.size <= MAX_DISCARD && dLeft > 0 && ph === "playing" && !cfx.noDisc), border: `1px solid ${sel.size && sel.size <= MAX_DISCARD && dLeft > 0 && !cfx.noDisc ? "#ef444444" : "#222"}`, minWidth: mob ? 56 : 60, padding: mob ? "10px 10px" : "8px 14px" } }, "Discard", cfx.noDisc ? " \u{1F6AB}" : ""), /* @__PURE__ */ React.createElement("div", { onClick: () => toast("\u267B\uFE0F", `Discard: swap up to ${MAX_DISCARD} selected cats for new draws. Free!`, "#ef4444"), style: { fontSize: 10, color: cfx.noDisc ? "#ef4444bb" : sel.size > MAX_DISCARD ? "#ef4444" : dLeft <= 0 ? "#ef4444" : "#888", marginTop: 2, fontFamily: "'Manrope',sans-serif", cursor: "help" } }, cfx.noDisc ? "Disabled" : sel.size > MAX_DISCARD ? `Max ${MAX_DISCARD}` : `${dLeft} left`), sel.size > 0 && dLeft > 0 && !cfx.noDisc && ph === "playing" && (() => {
    ...
    selCats2.forEach((c) => {
      ...
      else if (catHas(c, "Loyal")) hints.push({ icon: "\u{1FAC2}", text: "+1M all", color: "#f472b6" });
      else if (catHas(c, "Scavenger")) hints.push({ icon: "\u{1F33E}", text: "+2\u{1F41F}", color: "#4ade80" });
    });
    return hints.length > 0 ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#4ade80", fontFamily: "'Manrope',sans-serif", marginTop: 1 } }, hints.slice(0, 2).map((h, i) => /* @__PURE__ */ React.createElement("span", { key: i, style: { color: h.color } }, h.icon, h.text, " "))) : null;
  })()), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center" } }, /* @__PURE__ */ React.createElement("button", { onClick: playH, disabled: !sel.size || hLeft <= 0 || ph !== "playing" || !!autoPlay, style: { ...BTN(sel.size && ph === "playing" && !autoPlay ? "linear-gradient(135deg,#fbbf24,#f59e0b)" : "#222", sel.size && ph === "playing" && !autoPlay ? "#0a0a1a" : "#555", sel.size > 0 && ph === "playing" && !autoPlay), minWidth: mob ? 80 : 70, padding: mob ? "12px 18px" : "8px 16px", fontSize: mob ? 15 : 14, animation: hLeft === 1 && rScore < tgt ? "fpp 1.2s ease infinite" : "none", boxShadow: hLeft === 1 && rScore < tgt ? "0 0 20px #ef444488" : "none" } }, "Play", hLeft === 1 && rScore < tgt ? " \u26A0" : ""), /* @__PURE__ */ React.createElement("div", { onClick: () => toast("\u{1F0CF}", `Hands: ${hLeft} remaining this round. Each hand plays up to 5 cats. Score \u2265 threshold to clear.`, "#3b82f6"), style: { fontSize: 10, color: hLeft <= 1 && rScore < tgt ? "#ef4444" : "#888", marginTop: 2, fontFamily: "'Manrope',sans-serif", fontWeight: hLeft <= 1 && rScore < tgt ? 900 : 400, animation: hLeft <= 1 && rScore < tgt ? "fpp 1s ease infinite" : "none", cursor: "help" } }, hLeft <= 1 && rScore < tgt ? "\u26A0 FINAL" : hLeft === 0 ? "Done" : `Hands: ${hLeft}`)), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center" } }, /* @__PURE__ */ React.createElement("button", { onClick: () => setDeckView(true), style: { ...BTN("#1a1a2e", "#888"), border: "1px solid #ffffff12", padding: mob ? "10px 10px" : "8px 10px", fontSize: 10, minWidth: mob ? 44 : 45 } }, "Deck"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", marginTop: 2, fontFamily: "'Manrope',sans-serif", cursor: "pointer" }, onClick: () => setHandSort((s) => s === "season" ? "power" : "season") }, handSort === "season" ? "\u2600 szn" : "\u26A1 pwr")))), meta && meta.stats.w < 3 && ph === "playing" && !autoPlay && (() => {
    ...
    return /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", padding: "4px 16px", maxWidth: 400, margin: "0 auto", animation: "fadeIn 1s ease-out" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: isVeryNew ? 12 : 10, color: isVeryNew ? "#fbbf24aa" : "#ffffff44", fontFamily: "'Manrope',sans-serif", lineHeight: 1.5, padding: isVeryNew ? "4px 12px" : "0", borderRadius: 8, background: isVeryNew ? "#fbbf2408" : "transparent", border: isVeryNew ? "1px solid #fbbf2415" : "none" } }, isVeryNew ? "\u{1F3AF} " : "", PLAY_HINTS[hIdx]));
  })(), /* @__PURE__ */ React.createElement("div", { style: { maxWidth: 700, width: "100%", padding: "2px 16px", zIndex: 1, marginTop: "auto" } }, /* @__PURE__ */ React.createElement("details", { style: { cursor: "pointer" } }, /* @__PURE__ */ React.createElement("summary", { style: { fontSize: 10, color: "#555", letterSpacing: 2 } }, "REFERENCE"), /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(130px,1fr))", gap: 2, marginTop: 4 } }, HT.slice(1).map((h) => {
    ...
    return /* @__PURE__ */ React.createElement("div", { key: h.name, style: { fontSize: 10, fontFamily: "'Manrope',sans-serif", color: show ? "#555" : "#333" } }, show ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("span", { style: { color: discovered ? "#c084fc" : "#888", fontWeight: 600 } }, h.name, discovered ? " \u2728" : ""), " ", /* @__PURE__ */ React.createElement("span", { style: { color: "#3b82f6" } }, h.base.c), "x", /* @__PURE__ */ React.createElement("span", { style: { color: "#ef4444" } }, h.base.m), " ", h.ex, best ? /* @__PURE__ */ React.createElement("span", { style: { color: "#fbbf24bb" } }, " best:", best.toLocaleString()) : "") : /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("span", { style: { color: "#555", fontWeight: 600 } }, "????"), " ", /* @__PURE__ */ React.createElement("span", { style: { fontStyle: "italic", color: "#444444bb" } }, "Trait combo")));
  }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", letterSpacing: 1, marginTop: 4, borderTop: "1px solid #ffffff08", paddingTop: 3 } }, "POWER COMBOS ", /* @__PURE__ */ React.createElement("span", { style: { color: "#555", fontStyle: "italic" } }, "(stack on season hands)")), POWER_COMBOS.map((p) => {
    const best = handBests[p.name];
    const discovered = (meta?.stats?.dh || []).includes(p.name);
    return /* @__PURE__ */ React.createElement("div", { key: p.name, style: { fontSize: 10, fontFamily: "'Manrope',sans-serif", color: discovered ? "#555" : "#333" } }, discovered ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("span", { style: { color: "#c084fc", fontWeight: 600 } }, p.name, " \u2728"), " +", /* @__PURE__ */ React.createElement("span", { style: { color: "#3b82f6" } }, p.bonus.c), "C +", /* @__PURE__ */ React.createElement("span", { style: { color: "#ef4444" } }, p.bonus.m), "M ", p.ex, best ? /* @__PURE__ */ React.createElement("span", { style: { color: "#fbbf24bb" } }, " best:", best.toLocaleString()) : "") : /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("span", { style: { color: "#555", fontWeight: 600 } }, "????"), " ", /* @__PURE__ */ React.createElement("span", { style: { fontStyle: "italic", color: "#444444bb" } }, p.ex.includes("consecutive") ? "Consecutive combo" : "Matching combo")));
    ...
    const tl = traitTierLabel(t);
    return /* @__PURE__ */ React.createElement("div", { key: t.name, style: { fontSize: 10, fontFamily: "'Manrope',sans-serif", color: tl.color } }, t.icon, " ", /* @__PURE__ */ React.createElement("span", { style: { fontWeight: 600 } }, t.name), " ", /* @__PURE__ */ React.createElement("span", { style: { opacity: 0.6, fontSize: 10 } }, "(", tl.label, ")"), " ", t.desc);
  }))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, paddingTop: 4, fontSize: 10, fontFamily: "'Manrope',sans-serif", color: "#666", flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement("span", null, "\u2694 Scarred: \xD71.25 mult"), /* @__PURE__ */ React.createElement("span", null, "\u{1FA79} Injured: Half power, \u22122 mult (heals in 2 rounds)"), /* @__PURE__ */ React.createElement("span", null, "\u{1F525} Nerve: \xD71.0 to \xD72.2 (every clear +1, boss +hands remaining)")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, paddingBottom: 4, fontSize: 10, fontFamily: "'Manrope',sans-serif", color: "#666", flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement("span", null, "\u{1F495} Bonded pair: \xD7", getMB().bondBoost ? "1.75" : "1.5", " mult (from den shelter)"), /* @__PURE__ */ React.createElement("span", null, "\u26A1 Grudge: Always \u2212", (getMB().grudgeWisdom || 0) > 0 ? "1" : "2", " mult when grudged cats play together"), /* @__PURE__ */ React.createElement("span", null, "\u{1FA91} Bench: Unplayed hand cats give passive bonuses (traits or power as chips)")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, paddingBottom: 4, fontSize: 10, fontFamily: "'Manrope',sans-serif", color: "#666", flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement("span", null, "\u{1F4E3} Recruit: Pay \u{1F41F} to draw extra cats (1\u21922\u21924\u21928\u{1F41F}). More cards = better combos + bench"), /* @__PURE__ */ React.createElement("span", null, "\u{1F3F7}\uFE0F Epithets: Cats earn titles from events (scars, bonds, boss clears). Shown in Hearth")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, paddingBottom: 6, fontSize: 10, fontFamily: "'Manrope',sans-serif", color: "#666", flexWrap: "wrap" } }, Object.keys(DEVOTION_MILESTONES).map((k) => /* @__PURE__ */ React.createElement("span", { key: k, style: { color: BREEDS[k]?.color || "#888" } }, BREEDS[k]?.icon, " ", k, ": Play ", k, " cats to unlock devotion bonuses"))))), showAbandon && /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", top: 8, left: 8, zIndex: 200, display: "flex", gap: 4, alignItems: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#ffffff55", fontFamily: "'Manrope',sans-serif", marginRight: 2 } }, "v0.65"), abandonConfirm ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("button", { onClick: () => {
    setAbandonConfirm(false);
    setPh("title");
    setTab("play");
  }, style: { background: "#ef444433", border: "1px solid #ef4444", borderRadius: 6, fontSize: 10, cursor: "pointer", padding: "4px 10px", color: "#ef4444", fontFamily: "'Manrope',sans-serif", animation: "fadeIn .2s ease-out" } }, "Abandon Run"), /* @__PURE__ */ React.createElement("button", { onClick: () => setAbandonConfirm(false), style: { background: "none", border: "1px solid #ffffff22", borderRadius: 6, fontSize: 10, cursor: "pointer", padding: "4px 8px", color: "#888", fontFamily: "'Manrope',sans-serif" } }, "\u2715")) : /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 3 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => setAbandonConfirm(true), style: { background: "#ffffff08", border: "1px solid #ffffff22", borderRadius: 8, fontSize: 11, cursor: "pointer", padding: "6px 10px", color: "#aaa", fontFamily: "'Manrope',sans-serif", display: "flex", alignItems: "center", gap: 4 }, title: "Abandon run" }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14 } }, "\u2630"), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, letterSpacing: 1 } }, "MENU")), /* @__PURE__ */ React.createElement("button", { onClick: () => setShowLog((l) => !l), style: { background: showLog ? "#fbbf2412" : "#ffffff08", border: `1px solid ${showLog ? "#fbbf2444" : "#ffffff22"}`, borderRadius: 8, fontSize: 11, cursor: "pointer", padding: "6px 10px", color: showLog ? "#fbbf24" : "#aaa", fontFamily: "'Manrope',sans-serif", display: "flex", alignItems: "center", gap: 4 }, title: "Run log" }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14 } }, "\u{1F4CB}"), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, letterSpacing: 1 } }, "LOG")))), showLog && /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", top: 0, left: 0, bottom: 0, width: Math.min(320, vw - 40), zIndex: 250, background: "#0d1117f8", borderRight: "1px solid #ffffff15", overflowY: "auto", padding: "40px 12px 20px", animation: "slideIn .2s ease-out", fontFamily: "'Manrope',sans-serif" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#fbbf24", letterSpacing: 3, fontWeight: 700 } }, "\u{1F4CB} RUN LOG"), /* @__PURE__ */ React.createElement("button", { onClick: () => setShowLog(false), style: { background: "none", border: "none", color: "#555", fontSize: 14, cursor: "pointer" } }, "\u2715")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", marginBottom: 8 } }, "Night ", ante, " \xB7 Round ", blind + 1, "/3 \xB7 ", runLog.length, " events"), runLog.length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#555", fontStyle: "italic" } }, "Nothing has happened yet.") : /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 3 } }, [...runLog].reverse().map((e, i) => {
    ...
    return /* @__PURE__ */ React.createElement("div", { key: i, style: { fontSize: 10, color, lineHeight: 1.4, padding: "2px 0", borderBottom: "1px solid #ffffff06" } }, /* @__PURE__ */ React.createElement("span", { style: { opacity: 0.5 } }, icon), " ", text, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#555", marginLeft: 4 } }, "N", e.ante, ".", e.blind + 1));
  }))), toasts.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", top: 12, right: 12, zIndex: 300, display: "flex", flexDirection: "column", gap: 6, pointerEvents: "none", maxWidth: 280 } }, toasts.map((t) => /* @__PURE__ */ React.createElement("div", { key: t.id, style: { display: "flex", gap: 8, alignItems: "center", padding: t.big ? "12px 18px" : "8px 14px", borderRadius: t.big ? 10 : 8, background: "#1a1a2eee", border: `1.5px solid ${t.color}${t.big ? "66" : "44"}`, boxShadow: `0 4px 16px #00000066,0 0 ${t.big ? 16 : 8}px ${t.color}${t.big ? "44" : "22"}`, animation: "slideIn .3s ease-out", fontFamily: "'Manrope',sans-serif" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: t.big ? 22 : 16, flexShrink: 0 } }, t.icon), /* @__PURE__ */ React.createElement("span", { style: { fontSize: t.big ? 14 : 12, color: t.color, fontWeight: t.big ? 700 : 600, lineHeight: 1.3 } }, t.text)))));
}
