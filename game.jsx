...
// ═══════════════════════════════════════════════════════════════
// NINTH LIFE v53 - THE FORGE
// "Draw. Recruit. Play. Remember."
//
...
const TRAITS=[
  ...
  {name:"Forager",icon:"🌾",desc:"+1 mult per ration held (max +5). Discard: +2 rations",tier:"common"},
...
const HT=[
  ...
  {name:"Full Den",base:{c:130,m:6},ex:"3 of one season + 2 of another",echo:"home"},  // ★ v54: 120×5→130×6. Was over-nerfed, bosses unbeatable.
...
const NERVE=[
  ...
  {name:"Tense",     xM:1.1, color:"#9a8672",  glow:"#9a867233",    desc:""},                     // 4
  {name:"Focused",   xM:1.1, color:"#9a8672",  glow:"#9a867244",    desc:""},                     // 5
...
]; // ★ v54: 10→18 levels. Slower climb, more granularity. Only god-tier hits max.
const NERVE_MAX=17;
...
const UPGRADES=[
  ...
  {id:"u_bench",name:"Deep Bench",icon:"🪑",desc:"Benched cats give double passive bonus",cost:90,b:{doubleBench:1},max:1,tier:3},
  ...
  {id:"u_trait_luck",name:"The Colony's Memory",icon:"🧬",desc:"Drafted cats 25% more likely to have rare+ traits",cost:140,b:{traitLuck:1},max:1,tier:4},
  ...
  {id:"u_mythic",name:"Mythic Bloodline",icon:"🌟",desc:"10% chance drafted cats get mythic trait",cost:200,b:{mythicChance:0.1},max:1,tier:4},
];
...
const ACHIEVEMENTS=[
  ...
  // Tier 2 — Mastery
  {id:"all_breeds",name:"All Four Seasons",desc:"Save each season to Hearth",icon:"🐾",check:s=>{const br=new Set((s.disc||[]).map(d=>d.split("-")[0]));return["Autumn","Summer","Winter","Spring"].every(b=>br.has(b));},reward:"Season icons glow on title"},
...
// ★ v52: CHAPTER TITLES — stat-derived, no save state needed
...
function getEpigraph(meta){
  ...
  // ★ v52: Career-aware epigraphs — the game knows your history
...
function calcScore(cats,fams,fLvl,cfx={},ctx={}){
  ...
  let resonance=false;
  if(combo){
    // ★ v54: RESONANCE — if all combo cats share a season, bonus gets ×1.5
    ...
    const comboBoostUp=ctx.comboBoost||0; // ★ v54: Power Resonance upgrade (+50%)
  ...
  toS.forEach((c,si)=>{
    ...
    if(!isKit&&hasMirrored&&!c._re&&c._ci===maxPowIdx)basePow=Math.max(2,Math.floor(basePow/2));
  ...
  const benchMultiplier=(ctx.doubleBench||0)>0?2:1; // ★ v54: Deep Bench upgrade doubles bench bonuses
  if(bench.length>0){
    let bc=0,bm=0;
    bench.forEach(c=>{
      ...
      else if(catHas(c,"Forager")){bG+=1*benchMultiplier;}
...
function getTarget(a,b,firstRun,isLong){const scale=isLong?1.45:1.65;const base=(firstRun?[1200,2400,5000]:[2000,4000,8500])[b]*Math.pow(scale,a-1);const bossDiscount=b===2?(a===1?0.8:a===2&&firstRun?0.85:1):1;return Math.round(base*bossDiscount);}
...
// ★ v52: SAVE SYSTEM — 3 slots, backup on every save, field-level validation
const SLOT_COUNT=3;
...
function calcTotalHearthDust(cats,dustBonus=0,heatMult=1){
  ...
  // ★ v52: Hearth tier bonus — Circle +15%, Constellation +30%
...
let _goldBorders=false; // ★ v52: Gold card border cosmetic (NINTH LIFE achievement)
...
function CC({cat:_cat,sel,onClick,sm,cw:_cw,dis,hl,fog,chemHint,denMode,onTraitClick}){
  ...
  const tierGlow=isMythicTier?",0 0 12px #c084fc44,0 0 20px #c084fc22":isLegendaryTier?",0 0 10px #f59e0b33":isRareTier?",0 0 8px #4ade8022":"";
  const goldBorder=_goldBorders&&!isPlain; // ★ v52: Gold border cosmetic for traited cats
  let rankLabel=null;
  ...
  return(
    <div onClick={dis?undefined:onClick} title={cat.name+" - "+b.name+", P"+cat.power} style={{
      ...
      {(cat.injured||cat.scarred||cat.bondedTo||(cat.grudgedWith||[]).length>0)&&
      <div style={{position:"absolute",top:sm?2:3,left:sm?2:3,zIndex:5,display:"flex",flexDirection:"column",gap:1}}>
        {cat.injured&&<div style={{background:"#0d1117ee",border:"1px solid "+nd,borderRadius:3,
          padding:"1px 3px",fontSize:sm?7:9,color:neon,lineHeight:1}}>{"\u271A"}</div>}
        {!cat.injured&&cat.scarred&&<div style={{background:"#0d1117ee",border:"1px solid "+nd,borderRadius:3,
          padding:"1px 3px",fontSize:sm?7:9,color:neon,lineHeight:1}}>{"\u2694"}</div>}
        {cat.bondedTo&&<div style={{background:"#0d1117ee",border:"1px solid "+nd,borderRadius:3,
          padding:"1px 3px",fontSize:sm?7:9,color:neon,lineHeight:1}}>{"\u2661"}</div>}
        {(cat.grudgedWith||[]).length>0&&<div style={{background:"#0d1117ee",border:"1px solid "+nd,borderRadius:3,
          padding:"1px 3px",fontSize:sm?7:9,color:neon,lineHeight:1}}>{"\u26A1"}</div>}
      </div>}
      ...
      <div style={{position:"absolute",bottom:0,left:0,right:0,zIndex:4}}>
        ...
        <div style={{background:"#0d1117",padding:sm?"1px 6px 4px":"2px 8px 5px",textAlign:"center"}}>
          <div style={{fontSize:xs?(fn.length>5?5:7):(sm?(fn.length>7?7:fn.length>5?8:10):(fn.length>8?10:fn.length>6?11:13)),
            ...
            whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",lineHeight:1.2}}>{fn}<span style={{fontSize:xs?5:(sm?7:9),fontWeight:400,opacity:.6,marginLeft:2,color:cat.sex==="M"?"#60a5fa":"#f472b6"}}>{cat.sex==="M"?"♂":"♀"}</span></div>
        </div>
      </div>
...
function FM({level,prev}){
  ...
  return(<div style={{width:"100%",maxWidth:700,padding:"0 16px"}}>
    ...
    <div style={{height:8,background:"#1a1a2e",borderRadius:4,overflow:"hidden",border:"1px solid #ffffff08"}}><div style={{height:"100%",width:`${pct}%`,borderRadius:4,background:mx?"linear-gradient(90deg,#b85c2c,#f59e0b,#fef08a,#ffffffcc)":`linear-gradient(90deg,#9a8672,${fv.color})`,transition:"width .5s cubic-bezier(.4,0,.2,1)",boxShadow:level>5?`0 0 8px ${fv.color}44`:"none"}}/></div>
...
function NinthLife(){
  ...
  const[colonyName,setColonyName]=useState(""); // ★ v52: Colony naming (unlocked via achievement)
  ...
  const[babyNamingQueue,setBabyNamingQueue]=useState([]); // ★ v54: Queue of den babies to name
  const pendingRenames=useRef({}); // ★ v54: {catId: newFullName} applied in nextBlind
  ...
  function toast(icon,text,color="#fbbf24",dur=1500){const id=++toastRef.current;setToasts(t=>[...t,{id,icon,text,color,big:dur>2000}]);setTimeout(()=>setToasts(t=>t.filter(x=>x.id!==id)),dur);}
  ...
  const[slotSummaries,setSlotSummaries]=useState(null); // ★ v52: 3-slot save system
  ...
  useEffect(()=>{
    (async()=>{
      ...
      setMeta(d);
      _goldBorders=(d.achv||[]).includes("legend_score"); // ★ v52: Gold border cosmetic
      const r=await loadRun();
      setSavedRun(r);
    })();
  },[]);
  useEffect(()=>{Audio.muted=muted;},[muted]);
  // ★ v54 FIX: Safety net REMOVED. It was firing prematurely due to React batching.
  ...
  useEffect(()=>{
    if(ph==="playing"&&hLeft>0&&hand.length===0&&draw.length===0&&disc.length===0){
      ...
      },1000); // ★ v54: Extended delay from 300ms to 1000ms to survive React batching
  ...
  function startAutoPlay(){
    ...
    // ★ Start with UI tour (step -3 to -1), then auto-play (step 0+)
  ...
  // ★ v54: NERVE — every clear contributes. Dusk +1, Mid +1, Boss +handsRemaining (no min).
  ...
  function startGame(st){
    ...
    // ★ v54: Dynamic stray count. Balanced: floor(strayCount/4) per season, rest random.
    ...
    do{
      ...
      // ★ v54: Mythic Bloodline upgrade allows mythic traits in draft (10% chance each)
      allDraft.forEach(c=>{
        if(mb.mythicChance>0&&Math.random()<mb.mythicChance){c.trait=pk(MYTHIC_TRAITS);c.name=gN(c.breed,c.trait);}
        else if((c.trait||PLAIN).tier==="mythic"){c.trait=pk(LEGENDARY_TRAITS);c.name=gN(c.breed,c.trait);}
      });
      safety++;
    ...
    // ★ v54: Ancient Scrolls upgrade — start with a random hand type leveled up
    if(mb.startScroll>0){
  ...
  function pickDraft(i){
    ...
    // ★ v53: Always allow naming drafted cats
    setNamingCat(cat);
  ...
  function advanceFromScoring(){
    ...
    // ★ v52: Career hand type play counter
  ...
  function playH(){
    ...
    // ★ v53: beatingPace — used for strength meter display (no longer affects nerve)
    ...
    function animStep(){
      ...
      if(step<tot){
        ...
        if(s){
          ...
          else if(s.type==="combo"){Audio.comboHit();setScoreShake(1);setTimeout(()=>setScoreShake(0),200);setScoringFlash("#fbbf24");setTimeout(()=>setScoringFlash(null),120);}
  ...
  function discardH(){
    ...
    d.forEach(cat=>{
      ...
      else if(catHas(cat,"Forager")){goldDelta+=2;toast("🌾","Forager discarded: +2🐟","#4ade80");}
    });
    ...
    // ★ v53: Discards are FREE — no nerve cost. Hand crafting should feel empowering.
    Audio.cardSelect();
  }

  // ★ v53: RECRUIT — pay rations to draw extra cats into hand (scaling cost)
  ...
  const recruitCost=()=>{
    ...
    const discount=getMB().recruitDiscount||0; // ★ v54: Keen Eye reduces cost
  ...
  function showOF(fs,tgt,uh){
    ...
    if(interest===0&&excessGold===0&&blind<2&&!isFirstRun){
      setOData(null);
      genShop();setPh("shop"); // always go to shop first (shop→event→next)
      return;
    }
    setOData({excess,uh,gR,fs,tgt,interest,excessGold});setPh("overflow");
  }
  ...
  function endRun(won,finalScore){
    ...
    if(meta){(async()=>{try{
      ...
      // ★ v52: Show newly earned achievements with their rewards
  ...
  async function saveCatM(cat){if(!meta||savingRef.current)return;
    ...
    const ser={breed:cat.breed,power:cat.power,sex:cat.sex||"M",
      ...
      // ★ v52: Auto-generated story for Hearth display
      story:(()=>{
        const fn=cat.name.split(" ")[0];const tp=cat.stats?.tp||0;const bs=cat.stats?.bs||0;
  ...
  function nextBlind(){
    ...
    if(nb===0){
      ...
      // ★ v54: No night decay. Nerve only goes up (clear) or down (fail).
      if(na>ante){
        // v10: Ante transition
    ...
    const all=shuf([...hand,...draw,...disc].map(c=>{
      // ★ v54: Apply any pending renames from baby/draft naming
  ...
  function fireEvent(){
    ...
    let pool=[...eventSource].filter(e=>{
      ...
      // ★ v52: Cross-run arc checks — requires meta stats
  ...
  function chooseEvent(idx){
    ...
    // ★ v52: Cross-run chronicle flag — persists in meta.stats across runs
    if(fx.chronicleSet&&meta){
      ...
      // ★ v52: Meta toast — let the player know this choice echoes
      const arcToasts={
    ...
    if(fx.wagerGold){
      ...
      if(Math.random()<0.55){// won. triple it back
        ...
        // ★ v54: Nerve only goes up. No penalty.
      }
    }
      ...
      ()=>(<>
        ...
        <div style={{display:"flex",gap:16,alignItems:"center",justifyContent:"center",animation:"fadeIn 1s ease-out",marginTop:4}}>
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"10px 14px",borderRadius:10,background:"#1a1a2e",border:`2px solid ${BREEDS.Autumn.color}66`,gap:3,minWidth:72}}>
            ...
            <div style={{fontSize:11,color:BREEDS.Autumn.color,fontWeight:700}}>Copper</div>
    ...
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:12,padding:20,maxWidth:420}}>
        ...
        <button id="nameConfirm" onClick={()=>{
          ...
          // ★ v54: Store rename for nextBlind to apply (avoids stale closure)
  ...
  if(ph==="colonyFormed"&&colonyData){
    ...
    return(<div style={W} onClick={enterNight}><div style={BG}/><style>{CSS}</style>
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:14,padding:20,maxWidth:600}}>
        ...
        {/* ★ v53: Colony naming — always available */}
        ...
        <div style={{fontSize:10,color:"#ffffff22",fontFamily:"system-ui",marginTop:4}}>tap anywhere to continue</div>
      </div>
    </div>);
  }
    ...
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      ...
      {showSlots&&slotSummaries&&<div style={{position:"fixed",top:40,right:10,zIndex:300,background:"#0d1117ee",border:"1px solid #ffffff15",borderRadius:12,padding:14,width:220,animation:"fadeIn .2s ease-out",fontFamily:"system-ui"}}>
        ...
        {slotSummaries.map((s,i)=>{const n=i+1;const isActive=n===activeSlot;return(
          <div key={n} onClick={async()=>{if(!isActive){await setActiveSlot(n);const d=await loadS(n);setMeta(d);const r=await loadRun();setSavedRun(r);setShowSlots(false);}}} style={{padding:"8px 10px",borderRadius:8,marginBottom:4,cursor:isActive?"default":"pointer",
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              {!s.empty&&<span style={{fontSize:9,color:"#555"}}>{s.wins}W / {s.runs}R</span>}
            </div>
            {s.empty?<div style={{fontSize:10,color:"#444",fontStyle:"italic"}}>Empty</div>
        ...
        <div style={{borderTop:"1px solid #ffffff0a",marginTop:6,paddingTop:6,display:"flex",gap:4,flexWrap:"wrap"}}>
          <button onClick={async()=>{const json=await exportSlot(activeSlot);navigator.clipboard?.writeText(json);toast("📋","Save copied to clipboard","#4ade80");}} style={{fontSize:9,background:"none",border:"1px solid #ffffff15",borderRadius:4,color:"#4ade80",cursor:"pointer",padding:"3px 8px"}}>Export Slot {activeSlot}</button>
          <button onClick={()=>setShowImport(!showImport)} style={{fontSize:9,background:"none",border:"1px solid #ffffff15",borderRadius:4,color:"#fb923c",cursor:"pointer",padding:"3px 8px"}}>Import</button>
        </div>
        {showImport&&<div style={{marginTop:6}}>
          <textarea value={importText} onChange={e=>setImportText(e.target.value)} placeholder="Paste save JSON..." onClick={e=>e.stopPropagation()} style={{width:"100%",height:50,background:"#0a0a1a",border:"1px solid #ffffff15",borderRadius:4,color:"#e8e6e3",fontSize:9,fontFamily:"monospace",padding:4,resize:"none"}}/>
          <button onClick={async()=>{if(!importText.trim())return;const d=await importSlot(importText,activeSlot);if(d){setMeta(d);toast("✅","Save imported","#4ade80");setShowImport(false);setImportText("");setShowSlots(false);}else{toast("❌","Invalid save data","#ef4444");}}} style={{fontSize:9,background:"#fb923c",border:"none",borderRadius:4,color:"#0a0a1a",cursor:"pointer",padding:"3px 10px",marginTop:3,fontWeight:700}}>Import to Slot {activeSlot}</button>
        </div>}
      </div>}
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:12,padding:20,textAlign:"center",maxWidth:600}}>
        ...
        <div style={{fontSize:9,color:"#ffffff15",fontFamily:"system-ui",letterSpacing:2,marginTop:-6}}>v0.54</div>
        ...
        {/* ★ v52: Achievement constellation — earned as glow dots, unearned as dim */}
        {meta&&meta.stats.r>=1&&<div style={{display:"flex",gap:3,flexWrap:"wrap",justifyContent:"center",maxWidth:200,animation:"fadeIn 2s ease-out"}}>
          {ACHIEVEMENTS.map(a=>{const earned=(meta.achv||[]).includes(a.id);return(
            <div key={a.id} onClick={()=>toast(a.icon,`${a.name}: ${a.desc}${earned?" ✓ "+a.reward:""}`,earned?"#fbbf24":"#555")} style={{width:8,height:8,borderRadius:"50%",cursor:"pointer",
              background:earned?"#fbbf24":"#ffffff0a",
              boxShadow:earned?`0 0 6px #fbbf2466`:"none",
              border:earned?"1px solid #fbbf2466":"1px solid #ffffff12",
        ...
        {safeTab==="play"&&<>
          ...
          <button onClick={()=>{try{Audio.init();if(!meta?.stats?.r){setGuide({step:0,msg:""});startGame(starter);setPh("coldOpen");}else{startGame(starter);}}catch(e){console.error("Start error:",e);}}} style={{...BTN("linear-gradient(135deg,#fbbf24,#f59e0b)","#0a0a1a"),padding:"14px 48px",fontSize:18,letterSpacing:3,textTransform:"uppercase",boxShadow:"0 0 30px #fbbf2444"}}>{savedRun?"New Colony":"Enter the Night"}</button>
        ...
        {safeTab==="✦ upgrades"&&meta&&<div style={{width:"100%",display:"flex",flexDirection:"column",gap:6}}>
          {(()=>{
            ...
            return tiers.map(t=>{
              ...
              return(<div key={t}>
                <div style={{fontSize:10,color:unlocked?tierColor[t]:tierColor[t]+"66",letterSpacing:2,fontWeight:700,marginTop:t>1?8:0,marginBottom:4,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span>{unlocked?"":"🔒 "}{tierName[t].toUpperCase()}</span>
                  {!unlocked&&<span style={{fontSize:9,color:"#555",fontWeight:400}}>Unlocks at {tierReq[t]} upgrades ({totalLevels}/{tierReq[t]})</span>}
                </div>
          ...
          {meta.cats.length>0?(()=>{
            ...
            return(<>
              <div style={{textAlign:"center",padding:"8px 0"}}>
                {/* ★ v52: Hearth tier display */}
                {(()=>{const n=meta.cats.length;const tier=n>=15?{name:"THE CONSTELLATION",icon:"⭐",color:"#fef08a",bonus:"+30% stardust"}:n>=6?{name:"THE CIRCLE",icon:"🔥",color:"#fb923c",bonus:"+15% stardust"}:{name:"THE EMBER",icon:"🕯️",color:"#9a8672",bonus:""};
                  return <div style={{fontSize:9,color:tier.color+"88",letterSpacing:3,fontFamily:"system-ui",marginBottom:4}}>{tier.icon} {tier.name}{tier.bonus?` · ${tier.bonus}`:""}</div>;
                })()}
              ...
              {activeCats.length>0&&<>
                ...
                {/* ★ v52: Hearth whisper — a random cat speaks */}
...
Saved from Night ${c.fromAnte||"?"}`} style={{
  ...
  if(ph==="bossIntro"){const b=boss||BOSSES[0];
    ...
    // ★ v52: Boss W/L record + mastery title
  ...
  if(ph==="event"&&colEvent){
    ...
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:0,padding:20,maxWidth:480}}>
        ...
        {evt.tag&&<div style={{fontSize:9,color:evtGlow+"66",fontFamily:"system-ui",letterSpacing:4,padding:"3px 12px",borderRadius:12,border:`1px solid ${evtGlow}15`,marginBottom:16,animation:"fadeIn 1s ease-out",textTransform:"uppercase"}}>{evt.tag}</div>}
        ...
        {/* ★ v52: Hearth whisper — if a saved cat shares a breed with a target, echo their memory */}
    ...
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:10,padding:20}}>
      ...
      {blind>=2?(()=>{
        ...
        :<div style={{display:"flex",gap:8,marginTop:6}}>
          <button onClick={()=>{setOData(null);setSkipShop(false);genShop();setPh("shop");}} style={{...BTN(isFirstRun&&!seen.shop?"linear-gradient(135deg,#4ade80,#22d3ee)":"linear-gradient(135deg,#fbbf24,#f59e0b)",isFirstRun&&!seen.shop?"#0a0a1a":"#0a0a1a"),padding:isFirstRun&&!seen.shop?"12px 36px":"10px 32px",fontSize:isFirstRun&&!seen.shop?16:14,animation:isFirstRun&&!seen.shop?"breathe 2s ease-in-out infinite":"none",boxShadow:isFirstRun&&!seen.shop?"0 0 20px #4ade8066":"none"}}>{isFirstRun&&!seen.shop?"Enter the Market ✦":"Visit the Market"}</button>
          <button onClick={()=>{const skipG=3+ante;setOData(null);setGold(g=>g+skipG);if(blind<=1){fireEvent();}else{nextBlind();}}} style={{...BTN("#1a1a2e","#fbbf24"),padding:"10px 16px",fontSize:11,border:"1px solid #fbbf2444"}}>Skip Market +{3+ante}🐟</button>
        </div>}
    </div></div>);}
  ...
  if(ph==="gameOver"||ph==="victory"){const won=ph==="victory";
    ...
    // ★ v52: Compute run stats for cascade
    ...
    return(<div style={W}><div style={BG}/><style>{CSS}</style>
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",zIndex:1,gap:12,padding:20,maxWidth:550}}>
        ...
        {/* ★ v52: Stats cascade — each stat ticks in staggered */}
        {won&&statItems.length>0&&<div style={{display:"flex",gap:6,flexWrap:"wrap",justifyContent:"center",maxWidth:400,marginTop:4}}>
          {statItems.map((st,i)=>(<div key={i} style={{textAlign:"center",padding:"6px 12px",borderRadius:8,background:`${st.color}08`,border:`1px solid ${st.color}22`,animation:`scorePop .4s ease-out ${0.8+i*0.3}s both`,minWidth:80}}>
            <div style={{fontSize:8,color:`${st.color}88`,letterSpacing:2,fontFamily:"system-ui"}}>{st.label}</div>
        ...
        {/* ★ v52: First win — dramatic unlock reveal with stardust shower */}
        ...
        {denRes.slice(0,denStep+1).map((r,i)=>{
          ...
          <div key={i} style={{width:"100%",padding:mob?"8px 10px":"10px 14px",borderRadius:10,
            ...
            {r.type==="fight"&&<div>
              ...
              {/* ★ v52: Den echo — Hearth cat scarred in a past colony surfaces */}
              {meta&&meta.cats.length>0&&(()=>{const scarred=meta.cats.filter(c=>c.scarred);if(!scarred.length||Math.random()>0.2)return null;const sc=pk(scarred);return <div style={{fontSize:10,color:"#c084fc33",fontStyle:"italic",fontFamily:"system-ui",marginTop:4}}>The den remembers. {sc.name?.split(" ")[0]} was scarred here, {meta.stats.r>1?"colonies":"a colony"} ago.</div>;})()}
            </div>}
            {r.type==="phoenix"&&<div>
          ...
          return(<div style={{display:"flex",flexDirection:"column",gap:6,maxWidth:420,marginTop:8}}>
            ...
            {sections.map((sec,si)=>(
              <div key={sec.key} style={{padding:"6px 12px",borderRadius:8,background:"#ffffff04",borderLeft:`3px solid ${sec.border}`,display:"flex",flexDirection:"column",gap:2,animation:`scorePop .4s ease-out ${.2+si*.15}s both`}}>
                ...
                {sec.items.map((l,i)=><div key={i} style={{fontSize:vw<500?13:11,fontFamily:"system-ui",color:l.color,fontWeight:l.bold?800:600,lineHeight:1.4}}>{l.icon} {l.text}{l.tip&&<span style={{fontSize:vw<500?10:9,color:"#ffffff77",fontWeight:400,marginLeft:4}}>— {l.tip}</span>}</div>)}
              </div>
            ))}
            ...
            {/* ★ v54: Baby naming moved to dedicated naming screen after den */}
        {denDone&&<button onClick={()=>{
          // ★ v54: Queue babies for naming via the same naming screen used for drafts
          const babies=[];
          ...
          if(babies.length>0){
            // Queue babies for naming — reuse the draft naming screen
        ...
        <div style={{maxWidth:700,width:"100%",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
          <div style={{display:"flex",flexDirection:"column",gap:2}}>
            ...
            <div style={{fontSize:10,fontFamily:"system-ui",color:"#888"}}>
              {nt?<span>{nt.blind===2?<span style={{color:"#ef4444",fontWeight:700}}>{boss?.icon||"👁️"} {nt.blindName} approaches</span>:<span style={{color:"#fbbf24"}}>{nt.blindName} falls</span>} · Night {nt.ante} · <span style={{color:"#e8e6e3",fontWeight:700}}>{nt.target.toLocaleString()}</span> to survive</span>
  ...
  return(
    <div style={{...W,animation:fFlash==="down"?"shake .3s ease":"none"}}>
      ...
      {autoPlay&&ph==="playing"&&(()=>{
        ...
        const tourSteps=[
          {step:-3,title:"YOUR FIRST NIGHT",msg:"Let me show you how this works. Watch closely — I'll play the first hand for you.",highlight:null,pos:"center"},
          {step:-2,title:"SCORE & TARGET",msg:"Score enough each round to survive. Fall below the threshold and the dark forgets you were here.",highlight:"score",pos:"top"},
          {step:-1,title:"YOUR COLONY",msg:"These are your cats. Cards of the same season score better together. Watch what happens when I play matching cats.",highlight:"hand",pos:"bottom"},
        ];
        ...
        if(isUiTour&&tour)return(<>
          ...
          <div style={{position:"fixed",top:tour.pos==="center"?"35%":tour.pos==="top"?80:"auto",bottom:tour.pos==="bottom"?240:"auto",left:"50%",transform:"translateX(-50%)",zIndex:142,
            ...
            <div style={{marginTop:8}}><button onClick={()=>{if(autoRef.current)clearTimeout(autoRef.current);if(stRef.current)clearTimeout(stRef.current);setSel(new Set());setAutoPlay(null);setGuide({step:3,msg:""});}} style={{fontSize:10,background:"none",border:"none",color:"#ffffff33",cursor:"pointer",padding:"3px 10px"}}>Skip tutorial</button></div>
          </div>
        </>);
        ...
        if(step===0&&!autoRef.current){
          autoRef.current=setTimeout(()=>{
            ...
            function selectNext(){
              ...
              setSel(prev=>{const ns=new Set(prev);ns.add(autoPlay.idxs[ci]);return ns;});
              Audio.cardSelect();
        ...
        return(<div style={{position:"fixed",bottom:60,left:"50%",transform:"translateX(-50%)",zIndex:150,padding:"14px 24px",borderRadius:12,background:"#0a0a1aee",border:"1px solid #4ade8044",maxWidth:340,animation:"fadeIn .6s ease-out",textAlign:"center",fontFamily:"system-ui",boxShadow:"0 8px 32px #00000088"}}>
          <div style={{fontSize:14,color:"#4ade80",fontWeight:700,marginBottom:4}}>{
            step===0?"Selecting same-season cats..."
            :step<=2?"Selecting cats..."
            :"Playing the hand..."
          }</div>
          <div style={{fontSize:11,color:"#4ade80aa",lineHeight:1.5}}>{
            step<=2?"More cats of the same season = stronger hand."
      ...
      {guide&&!autoPlay&&ante===1&&blind===0&&ph==="playing"&&(()=>{
        ...
        if(guide.step===0){
          ...
          sub=postAuto?`${hLeft} hands left. Tap cats that share a season, then hit Play.`:need>0?(hLeft<=2?"Make this count.":"Pick a different group, or discard to draw new ones."):"The bigger you score beyond the threshold, the more bonus rations and Nerve you earn.";
        }
      ...
      <div style={{width:"100%",maxWidth:700,padding:mob?"6px 12px":"8px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",zIndex:1,borderBottom:`1px solid ${isBoss?"#ef444422":"#ffffff0a"}`}}>
        ...
        <div style={{textAlign:"center"}}><div style={{fontSize:11,color:"#888",letterSpacing:2}}>SCORE</div><div style={{fontSize:mob?16:18,fontWeight:900}}><span style={{color:rScore>=tgt?"#4ade80":"#e8e6e3"}}>{rScore.toLocaleString()}</span><span style={{color:"#444",fontSize:12}}> / {tgt.toLocaleString()}</span></div>
        ...
        <div style={{display:"flex",gap:4,alignItems:"center"}}>
          <button onClick={e=>{e.stopPropagation();setAbandonConfirm(c=>!c);}} style={{background:abandonConfirm?"#ef444422":"none",border:`1px solid ${abandonConfirm?"#ef4444":"#ffffff12"}`,borderRadius:4,fontSize:10,cursor:"pointer",opacity:abandonConfirm?1:.4,padding:"2px 6px",color:abandonConfirm?"#ef4444":"#888",fontFamily:"system-ui"}} title="Abandon run">{abandonConfirm?"Confirm?":"✕"}</button>
          {abandonConfirm&&<button onClick={e=>{e.stopPropagation();setAbandonConfirm(false);setPh("title");setTab("play");}} style={{background:"#ef444433",border:"1px solid #ef4444",borderRadius:4,fontSize:10,cursor:"pointer",padding:"2px 8px",color:"#ef4444",fontFamily:"system-ui",animation:"fadeIn .2s ease-out"}}>Abandon</button>}
      ...
      {/* ★ v52: Mobile needed-per-hand breakout — most important strategic number gets its own line */}
      ...
      {mob?<div style={{display:"flex",gap:4,padding:"2px 12px",zIndex:1,maxWidth:700,width:"100%",justifyContent:"center",alignItems:"center",flexWrap:"nowrap"}}>
        {/* ★ v52: One-time tap nudge */}
        {!seen.mobBar&&(fams.length>0||Object.values(devotion).some(v=>v>0))&&<span onClick={()=>{setSeen(s=>({...s,mobBar:true}));toast("ℹ️","Tap any icon for details","#fbbf24");}} style={{fontSize:9,color:"#fbbf2466",fontFamily:"system-ui",cursor:"pointer",animation:"breathe 2s ease-in-out infinite",marginRight:2}}>ℹ️</span>}
      ...
      <div style={{display:"flex",gap:3,padding:"2px 16px",zIndex:1,maxWidth:700,width:"100%",justifyContent:"center",flexWrap:"wrap"}}>
        {Object.keys(DEVOTION_MILESTONES).map(breed=>{
          ...
          return(<div key={breed} title={dev.next?`${breed}: ${dev.count}/${dev.next.at} → ${dev.next.name}: ${dev.next.desc}`:`${breed}: ALL UNLOCKED`} style={{display:"flex",alignItems:"center",gap:2,fontSize:9,color:color+"88",fontFamily:"system-ui"}}>
            ...
            {lastUnlocked&&<span style={{fontSize:8,color:color+"66"}}>✓{dev.unlocked.length}</span>}
          </div>);
        })}
      </div>
      ...
      {denNews.length>0&&ph==="playing"&&<div style={{display:"flex",gap:4,padding:"4px 16px",zIndex:1,maxWidth:700,width:"100%",justifyContent:"center",flexWrap:"wrap"}}>
        ...
        <button onClick={()=>setDenNews([])} style={{background:"none",border:"none",color:"#444",fontSize:10,cursor:"pointer",padding:"2px 4px"}}>✕</button>
      </div>}
      ...
      {ph==="scoring"&&sRes&&(
        <div onClick={scoringDone?advanceFromScoring:skipScoring} style={{cursor:"pointer",position:"fixed",inset:0,background:"#000000cc",zIndex:100,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:0,
          ...
          {(()=>{
            ...
            // ★ v52: Scoring annotations for early runs (first 2 wins) — teach the math
            ...
            return(<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:0,
              ...
              {(()=>{
                ...
                <span style={{color:"#ffffff22",fontSize:16,fontFamily:"'Cinzel',serif"}}>{"×"}</span>
              ...
              {!done&&nearMiss&&pct<100&&<div style={{fontSize:9,color:"#ef4444aa",fontFamily:"system-ui",letterSpacing:2,animation:"fpp 1.5s ease infinite",marginTop:2}}>CLOSE...</div>}
      ...
      <div style={{minHeight:36,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",zIndex:1,gap:2}}>
        {preview?(()=>{
          ...
          // ★ v54: Resonance check for preview
        ...
        return(
          <div style={{display:"flex",gap:6,padding:"4px 16px",zIndex:1,maxWidth:700,width:"100%",justifyContent:"center",alignItems:"center"}}>
            ...
            {benchTraited.length>0&&<div style={{display:"flex",gap:2,alignItems:"center",padding:"2px 6px",borderRadius:4,background:"#ffffff06",border:"1px solid #ffffff0a"}}>
              <span style={{fontSize:9,color:"#888",fontFamily:"system-ui"}}>🪑</span>
      ...
      {/* ★ v52: Hand type preview — shows what hand the selected cats form + base C×M */}
      {sel.size>=1&&ph==="playing"&&!autoPlay&&(()=>{
        ...
        return(<div style={{textAlign:"center",padding:"2px 0",zIndex:1,animation:"fadeIn .15s ease-out"}}>
          ...
          <span style={{fontSize:11,color:"#3b82f6",fontFamily:"system-ui",fontWeight:700,marginLeft:8}}>{baseC}C</span>
          <span style={{fontSize:11,color:"#ffffff22",margin:"0 2px"}}>×</span>
      ...
      <div style={{display:"flex",gap:8,padding:"8px",zIndex:10,alignItems:"center",position:mob?"sticky":"static",bottom:mob?0:"auto",background:mob?"#0a0a1aee":"transparent",borderTop:mob?"1px solid #ffffff0a":"none",justifyContent:"center"}}>
        ...
        {/* ★ v53: RECRUIT — pay rations to draw extra cat into hand */}
        <div style={{textAlign:"center"}}>
          {(()=>{const cost=recruitCost();const canRecruit=ph==="playing"&&!autoPlay&&gold>=cost&&(draw.length>0||disc.length>0);
            return(<>
              ...
              <div style={{fontSize:10,color:cost===0?"#4ade80":gold>=cost?"#fbbf24":"#ef4444",marginTop:2,fontFamily:"system-ui"}}>{cost===0?"Free!":cost+"🐟"}</div>
            </>);
          })()}
        </div>
        <div style={{textAlign:"center"}}>
          ...
          <div style={{fontSize:10,color:cfx.noDisc?"#ef4444bb":dLeft<=0?"#ef4444":"#888",marginTop:2,fontFamily:"system-ui"}}>{cfx.noDisc?"Disabled":`${dLeft} left`}</div>
          {sel.size>0&&dLeft>0&&!cfx.noDisc&&ph==="playing"&&(()=>{
            ...
            return hints.length>0?<div style={{fontSize:9,color:"#4ade80",fontFamily:"system-ui",marginTop:1}}>
        ...
        <div style={{textAlign:"center"}}>
          ...
          <div style={{fontSize:10,color:hLeft<=1&&rScore<tgt?"#ef4444":"#888",marginTop:2,fontFamily:"system-ui",fontWeight:hLeft<=1&&rScore<tgt?900:400,animation:hLeft<=1&&rScore<tgt?"fpp 1s ease infinite":"none"}}>{hLeft<=1&&rScore<tgt?"⚠ FINAL":hLeft===0?"Done":`Hands: ${hLeft}`}</div>
        </div>
        {/* Deck view + Sort */}
        <div style={{textAlign:"center"}}>
          ...
          <div style={{fontSize:9,color:"#555",marginTop:2,fontFamily:"system-ui",cursor:"pointer"}} onClick={()=>setHandSort(s=>s==="season"?"power":"season")}>{handSort==="season"?"☀ szn":"⚡ pwr"}</div>
        </div>
        </>)}
      </div>

      {/* ★ v52: Persistent gameplay hint — shows for first 3 wins, core rule always first */}
